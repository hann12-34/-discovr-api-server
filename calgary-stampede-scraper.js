...
// Calgary Stampede Events Scraper - NEW CLEAN VERSION
const puppeteer = require('puppeteer');

class CalgaryStampedeEventsScraper {
    constructor() {
        this.venueName = 'Calgary Stampede';
        this.venueUrl = 'https://www.calgarystampede.com/events';
        this.city = 'Calgary';
        this.venue = {
            name: this.venueName,
    ...
    async scrape() {
        ...
        try {
            ...
            const events = await page.evaluate(() => {
                const eventElements = document.querySelectorAll('.event, .event-item, .event-card, [class*="event"]');
                const results = [];
            ...
            const formattedEvents = events.map(event => {
                ...
                if (event.title.toLowerCase().includes('rodeo')) {
                    category = 'Rodeo';
                } else if (event.title.toLowerCase().includes('concert') || event.title.toLowerCase().includes('music')) {
                    category = 'Music';
                } else if (event.title.toLowerCase().includes('show')) {
                    category = 'Entertainment';
                }
                
                return {
                    id: `stampede-${event.title.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
                    title: event.title,
                    description: event.description || `${event.title} at Calgary Stampede`,
                    ...
                    image: 'https://www.calgarystampede.com/images/logo.png',
                    isFeatured: true
                };
            });
...
module.exports = CalgaryStampedeEventsScraper;
