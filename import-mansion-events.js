/**
 * Import script for Mansion Club events
 * 
 * This script runs the Mansion Club scraper and imports the events into MongoDB
 */

require('dotenv').config();
const mongoose = require('mongoose');
const { scrape } = require('./scrapers/mansion-scraper');
const Event = require('./models/Event');

async function importEvents() {
  console.log('📊 Starting Mansion Club event import...');
  ...
  try {
    ...
    // Run the scraper
    console.log('🔍 Running Mansion Club scraper...');
    const events = await scrape();
    
    console.log(`📋 Found ${events.length} events from Mansion Club`);
    ...
    console.error(`❌ Error importing Mansion Club events: ${error.message}`);
