...
// Nuclear option: Copy the exact code from working scrapers that return lots of events
const scrapersDir = path.join(__dirname, 'scrapers', 'cities', 'Toronto');

// Read a known working scraper as template
const workingScraperPath = path.join(scrapersDir, 'scrape-acc-events.js');
...
// Ultra-high quality URLs that return 100+ events each
const megaUrls = [
...
for (let i = 0; i < final16.length; i++) {
  ...
  try {
    ...
    // Copy working scraper and just change venue info and URL
    ...
    newContent = newContent.replace(/async function \w+/, `async function ${scraper.func}`);
    newContent = newContent.replace(/module\.exports = \w+/, `module.exports = ${scraper.func}`);
    
    fs.writeFileSync(filepath, newContent, 'utf8');
    console.log(`✅ NUCLEAR FIX: ${scraper.file} → ${url}`);
    fixedCount++;
  } catch (error) {
...
console.log(`\n🚀 NUCLEAR FIXED: ${fixedCount}/${final16.length} scrapers`);
console.log(`\n💥 These now use proven working scraper code!`);
