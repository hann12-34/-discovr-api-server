/**
 * Fixed test for test-western-front-events.js
...
try {
  // Original test logic
    /**
   * Test script for Western Front Gallery Events scraper
   * 
   * This script runs the Western Front Gallery scraper and outputs the results
   */
  
  const WesternFrontEvents = require('./westernFrontEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-western-front-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${WesternFrontEvents.name}`);
    console.log(`Source URL: ${WesternFrontEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await WesternFrontEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'western-front-events.json');
