/**
 * Fixed test for test-space-centre-events.js
...
try {
  ...
  const SpaceCentreEvents = require('./spaceCentreEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-space-centre-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${SpaceCentreEvents.name}`);
    console.log(`Source URL: ${SpaceCentreEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'space-centre-events.json');
