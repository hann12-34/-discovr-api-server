/**
 * Fixed test for test-jazz-fest-events.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver International Jazz Festival Events scraper
   * 
   * This script runs the Vancouver Jazz Festival Events scraper and outputs the results
   */
  
  const VancouverJazzFestEvents = require('./vancouverJazzFestEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-jazz-fest-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${VancouverJazzFestEvents.name}`);
    console.log(`Source URL: ${VancouverJazzFestEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await VancouverJazzFestEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'jazz-fest-events.json');
