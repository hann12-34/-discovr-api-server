/**
 * Test script for Vancouver International Dance Festival Events scraper
 * 
 * This script runs the VIDF Events scraper and outputs the results
 */

const DanceFestivalEvents = require('./danceFestivalEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-dance-festival-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${DanceFestivalEvents.name}`);
  console.log(`Source URL: ${DanceFestivalEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await DanceFestivalEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'dance-festival-events.json');
