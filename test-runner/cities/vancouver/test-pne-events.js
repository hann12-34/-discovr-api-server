/**
 * Fixed test for test-pne-events.js
...
try {
  // Original test logic
    /**
   * Test script for Pacific National Exhibition (PNE) Events scraper
   * 
   * This script runs the PNE Events scraper and outputs the results
   */
  
  const PNEEvents = require('./pneEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-pne-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${PNEEvents.name}`);
    console.log(`Source URL: ${PNEEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await PNEEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'pne-events.json');
