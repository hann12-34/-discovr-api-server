/**
 * Test script for Chan Centre for the Performing Arts Events scraper
 * 
 * This script runs the Chan Centre Events scraper and outputs the results
 */

const ChanCentreEvents = require('./chanCentreEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-chan-centre-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${ChanCentreEvents.name}`);
  console.log(`Source URL: ${ChanCentreEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await ChanCentreEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'chan-centre-events.json');
