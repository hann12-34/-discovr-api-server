/**
 * Test script for Contemporary Art Gallery Events scraper
 * 
 * This script runs the Contemporary Art Gallery scraper and outputs the results
 */

const ContemporaryArtGalleryEvents = require('./contemporaryArtGalleryEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-contemporary-art-gallery.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${ContemporaryArtGalleryEvents.name}`);
  console.log(`Source URL: ${ContemporaryArtGalleryEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await ContemporaryArtGalleryEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'contemporary-art-gallery-events.json');
