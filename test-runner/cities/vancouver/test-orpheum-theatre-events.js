/**
 * Fixed test for test-orpheum-theatre-events.js
...
try {
  // Original test logic
    /**
   * Test script for Orpheum Theatre Events scraper
   * 
   * This script runs the Orpheum Theatre Events scraper and outputs the results
   */
  
  const OrpheumTheatreEvents = require('./orpheumTheatreEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-orpheum-theatre-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${OrpheumTheatreEvents.name}`);
    console.log(`Source URL: ${OrpheumTheatreEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'orpheum-theatre-events.json');
