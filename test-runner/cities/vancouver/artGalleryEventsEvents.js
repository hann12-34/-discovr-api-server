/**
 * Improved scraper for Art Gallery Events
...
async function scrape() {
  console.log('🔍 Scraping events from Art Gallery Events...');
  
  try {
    const sourceUrl = 'https://www.artgalleryevents.com/events/';
    ...
    fs.writeFileSync('art-gallery-events-debug.html', response.data);
    console.log(`Saved HTML to art-gallery-events-debug.html`);
        ...
        elements.each((i, el) => {
          try {
            ...
            events.push({
              ...
              categories: guessCategories('Art Gallery Events', title)
            });
          } catch (error) {
    ...
    console.log(`Found ${events.length} events from Art Gallery Events`);
