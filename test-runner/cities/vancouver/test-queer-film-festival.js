/**
 * Fixed test for test-queer-film-festival.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver Queer Film Festival Events scraper
   * 
   * This script runs the Vancouver Queer Film Festival scraper and outputs the results
   */
  ...
  // Add debug logging
  console.log(`Testing test-queer-film-festival.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${QueerFilmFestivalEvents.name}`);
    console.log(`Source URL: ${QueerFilmFestivalEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await QueerFilmFestivalEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'queer-film-festival-events.json');
