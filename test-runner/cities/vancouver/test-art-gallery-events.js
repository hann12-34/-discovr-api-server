/**
 * Fixed test for test-art-gallery-events.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver Art Gallery Events scraper
   * 
   * This script runs the Art Gallery Events scraper and outputs the results
   */
  
  const ArtGalleryEvents = require('./artGalleryEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-art-gallery-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${ArtGalleryEvents.name}`);
    console.log(`Source URL: ${ArtGalleryEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await ArtGalleryEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'art-gallery-events.json');
