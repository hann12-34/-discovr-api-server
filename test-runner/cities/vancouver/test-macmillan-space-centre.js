/**
 * Fixed test for test-macmillan-space-centre.js
...
try {
  // Original test logic
    /**
   * Test script for H.R. MacMillan Space Centre Events scraper
   * 
   * This script runs the H.R. MacMillan Space Centre Events scraper and outputs the results
   */
  
  const MacMillanSpaceCentreEvents = require('./macMillanSpaceCentreEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-macmillan-space-centre.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${MacMillanSpaceCentreEvents.name}`);
    console.log(`Source URL: ${MacMillanSpaceCentreEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await MacMillanSpaceCentreEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'macmillan-space-centre-events.json');
