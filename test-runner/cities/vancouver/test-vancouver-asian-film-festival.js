/**
 * Fixed test for test-vancouver-asian-film-festival.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver Asian Film Festival Events scraper
   * 
   * This script runs the Vancouver Asian Film Festival scraper and outputs the results
   */
  ...
  // Add debug logging
  console.log(`Testing test-vancouver-asian-film-festival.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${VancouverAsianFilmFestivalEvents.name}`);
    console.log(`Source URL: ${VancouverAsianFilmFestivalEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await VancouverAsianFilmFestivalEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'vancouver-asian-film-festival-events.json');
