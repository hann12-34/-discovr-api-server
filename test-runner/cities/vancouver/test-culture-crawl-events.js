/**
 * Test script for Eastside Culture Crawl Events scraper
 * 
 * This script runs the Eastside Culture Crawl Events scraper and outputs the results
 */

const CultureCrawlEvents = require('./cultureCrawlEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-culture-crawl-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${CultureCrawlEvents.name}`);
  console.log(`Source URL: ${CultureCrawlEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await CultureCrawlEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'culture-crawl-events.json');
