/**
 * Test script for Vancouver Cherry Blossom Festival Events scraper
 * 
 * This script runs the Vancouver Cherry Blossom Festival Events scraper and outputs the results
 */

const CherryBlossomFestEvents = require('./cherryBlossomFestEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-cherry-blossom-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${CherryBlossomFestEvents.name}`);
  console.log(`Source URL: ${CherryBlossomFestEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await CherryBlossomFestEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'cherry-blossom-events.json');
