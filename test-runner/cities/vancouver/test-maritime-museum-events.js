/**
 * Fixed test for test-maritime-museum-events.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver Maritime Museum Events scraper
   * 
   * This script runs the Vancouver Maritime Museum Events scraper and outputs the results
   */
  
  const MaritimeMuseumEvents = require('./maritimeMuseumEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-maritime-museum-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${MaritimeMuseumEvents.name}`);
    console.log(`Source URL: ${MaritimeMuseumEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'maritime-museum-events.json');
