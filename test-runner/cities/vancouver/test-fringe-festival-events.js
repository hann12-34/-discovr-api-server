/**
 * Test script for Vancouver Fringe Festival Events scraper
 * 
 * This script runs the Vancouver Fringe Festival Events scraper and outputs the results
 */

const FringeFestivalEvents = require('./fringeFestivalEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-fringe-festival-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${FringeFestivalEvents.name}`);
  console.log(`Source URL: ${FringeFestivalEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await FringeFestivalEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'fringe-festival-events.json');
