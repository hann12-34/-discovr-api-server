...
const GranvilleIslandEvents = require('./granvilleIslandEvents');
const fs = require('fs');
const path = require('path');

// Add debug logging
console.log(`Testing test-granville-island-events.js...`);
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${GranvilleIslandEvents.name}`);
  console.log(`Source URL: ${GranvilleIslandEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await GranvilleIslandEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'granville-island-events.json');
