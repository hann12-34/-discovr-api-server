/**
 * Fixed test for test-vogue-theatre-events.js
...
try {
  // Original test logic
    /**
   * Test script for Vogue Theatre Events scraper
   * 
   * This script runs the Vogue Theatre Events scraper and outputs the results
   */
  
  const VogueTheatreEvents = require('./vogueTheatreEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-vogue-theatre-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${VogueTheatreEvents.name}`);
    console.log(`Source URL: ${VogueTheatreEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await VogueTheatreEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'vogue-theatre-events.json');
