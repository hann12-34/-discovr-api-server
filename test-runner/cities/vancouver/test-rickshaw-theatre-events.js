/**
 * Fixed test for test-rickshaw-theatre-events.js
...
try {
  // Original test logic
    /**
   * Test script for Rickshaw Theatre Events scraper
   * 
   * This script runs the Rickshaw Theatre Events scraper and outputs the results
   */
  
  const RickshawTheatreEvents = require('./rickshawTheatreEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-rickshaw-theatre-events.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${RickshawTheatreEvents.name}`);
    console.log(`Source URL: ${RickshawTheatreEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await RickshawTheatreEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'rickshaw-theatre-events.json');
