/**
 * Fixed test for test-vancouver-film-festival.js
...
try {
  // Original test logic
    /**
   * Test script for Vancouver International Film Festival Events scraper
   * 
   * This script runs the Vancouver International Film Festival Events scraper and outputs the results
   */
  
  const VancouverFilmFestivalEvents = require('./vancouverFilmFestivalEvents');
  const fs = require('fs');
  const path = require('path');
  
  // Add debug logging
  console.log(`Testing test-vancouver-film-festival.js...`);
  ...
  async function testScraper() {
    console.log(`🔍 Testing scraper: ${VancouverFilmFestivalEvents.name}`);
    console.log(`Source URL: ${VancouverFilmFestivalEvents.url}`);
    console.log('--------------------------------------------');
    ...
    try {
      // Run the scraper
      const events = await VancouverFilmFestivalEvents.scrape();
      ...
      if (events.length === 0) {
        ...
        const outputFile = path.join(outputDir, 'vancouver-film-festival-events.json');
