 ...
 * 
 * This script runs the MOA Events scraper and outputs the results
 */

const AnthropologyMuseumEvents = require('./scrapers/cities/vancouver/anthropologyMuseumEvents');
const fs = require('fs');
...
async function testScraper() {
  console.log(`🔍 Testing scraper: ${AnthropologyMuseumEvents.name}`);
  console.log(`Source URL: ${AnthropologyMuseumEvents.url}`);
  console.log('--------------------------------------------');
  ...
  try {
    // Run the scraper
    const events = await AnthropologyMuseumEvents.scrape();
    ...
    if (events.length === 0) {
      ...
      const outputFile = path.join(outputDir, 'anthropology-museum-events.json');
