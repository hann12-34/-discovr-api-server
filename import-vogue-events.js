/**
 * Import script for The Vogue Theatre events
 * 
 * This script runs the Vogue Theatre scraper and imports the events into MongoDB
 */

require('dotenv').config();
const mongoose = require('mongoose');
const { scrape } = require('./scrapers/vogue-scraper');
const Event = require('./models/Event');

async function importEvents() {
  console.log('📊 Starting The Vogue Theatre event import...');
  ...
  try {
    ...
    // Run the scraper
    console.log('🔍 Running The Vogue Theatre scraper...');
    const events = await scrape();
    
    console.log(`📋 Found ${events.length} events from The Vogue Theatre`);
    ...
    console.error(`❌ Error importing Vogue Theatre events: ${error.message}`);
