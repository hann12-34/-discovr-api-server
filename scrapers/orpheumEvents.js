...
class OrpheumEvents {
  ...
  async extractEvents(page) {
    try {
      ...
      // Save debug files
      await this.takeScreenshot(page, 'orpheum-events-pre-extract.png');
      await this.saveHtml(page, 'orpheum-events-pre-extract.html');
      ...
      const hasEvents = await page.evaluate(() => {
        ...
        const vsoSelectors = [
          // VCT event cards from the screenshots the user provided
          '.vct-event-card',
          '.event-card',
          '.event-item', 
        ...
        const extractedEvents = await page.evaluate((selector) => {
          return Array.from(document.querySelectorAll(selector)).map(el => {
            try {
              ...
              const title = el.querySelector('h2, h3, h4, .title, .event-title')?.innerText || 'Unknown Event';
...
module.exports = new OrpheumEvents();
