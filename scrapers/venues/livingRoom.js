/**
 * Scraper for The Living Room venue website
 * Extracts event information from https://www.the-livingroom.ca/whats-on
...
function parseEventDate(dateString) {
  try {
    ...
    console.warn(`Could not parse The Living Room date: ${dateStr}`);
    return null;
  } catch (error) {
    console.error(`Error parsing The Living Room date: ${dateString}`, error);
    return null;
  }
}
...
/**
 * Scrape The Living Room website for events
 * @returns {Promise<Array>} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: 'Living Room' });
  logger.info("Starting The Living Room scraper...");
  const events = [];
  const venueUrl = "https://www.the-livingroom.ca/whats-on";
  ...
  try {
    ...
    if (eventElements.length === 0) {
      eventElements = $("[data-category='event'], .gig, .show, [class*='event'], .performances .item");
    }
    
    logger.info(`Found ${eventElements.length} potential events at The Living Room`);
    
    // No fallback events - if no events found, return empty array
    if (eventElements.length === 0) {
      logger.warn('No events found on The Living Room website');
      return [];
    }
    ...
    eventElements.each((i, element) => {
      try {
        ...
        const event = {
          ...
          imageURL,
          location: "1230 Hamilton St, Vancouver, BC",
          type: "Event",
          category: "Entertainment",
          season,
          status: "active",
          description: `${title} at The Living Room in Vancouver.`
        };
        
        events.push(event);
      } catch (error) {
        console.error("Error processing The Living Room event:", error);
      }
    });
    
    logger.info(`Successfully scraped ${events.length} events from The Living Room`);
    return events;
  } catch (error) {
    logger.error({ error }, `Error scraping The Living Room: ${error.message}`);
...
module.exports = {
  ...
  urls: ["https://www.the-livingroom.ca/whats-on"],
  scrape
};
