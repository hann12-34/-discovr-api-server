/**
 * Morris and Helen Belkin Art Gallery Scraper (UBC)
 * URL: https://belkin.ubc.ca/
 */
...
// Define venue info
const name = 'Morris and Helen Belkin Art Gallery';
const url = 'https://belkin.ubc.ca/';
const exhibitionsUrl = 'https://belkin.ubc.ca/exhibitions/';
const eventsUrl = 'https://belkin.ubc.ca/events/';
const venueAddress = '1825 Main Mall';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6T 1Z2';
...
/**
 * Scrape events from Belkin Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Morris and Helen Belkin Art Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition-item, .exhibition, article, .grid-item, .post, .card, .entry').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Morris and Helen Belkin Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event-item, .event, article, .grid-item, .post, .card, .entry').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Morris and Helen Belkin Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Morris and Helen Belkin Art Gallery. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Morris and Helen Belkin Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Morris and Helen Belkin Art Gallery');
