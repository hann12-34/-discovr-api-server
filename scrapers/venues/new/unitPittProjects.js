/**
 * Unit/Pitt Projects Scraper
 * URL: https://unitpitt.ca/
 */
...
// Define venue info
const name = 'Unit/Pitt Projects';
const url = 'https://unitpitt.ca/';
const exhibitionsUrl = 'https://unitpitt.ca/exhibitions/';
const eventsUrl = 'https://unitpitt.ca/events/';
const venueAddress = '236 East Pender Street';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6A 1T7';
...
/**
 * Scrape events from Unit/Pitt Projects
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Unit/Pitt Projects scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, article, .post, .entry, .exhibition-item, .wp-post-container, .grid-item, .project').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Unit/Pitt Projects. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, article, .post, .entry, .event-item, .wp-post-container, .grid-item, .project').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Unit/Pitt Projects. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .featured-project').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Unit/Pitt Projects. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Unit/Pitt Projects, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Unit/Pitt Projects');
