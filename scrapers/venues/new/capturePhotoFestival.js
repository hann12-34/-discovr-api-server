...
async function scrape() {
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, article, .post, .entry, .exhibition-item, .wp-block-post, .grid-item, .event-card, .card').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract location/venue info if available
          let location = $(element).find('.location, .venue, .place').text().trim();
          
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .entry-content').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at ${location || 'Capture Photography Festival'}. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, article, .post, .entry, .event-item, .wp-block-post, .grid-item, .event-card, .card').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract location/venue info if available
          let location = events$(element).find('.location, .venue, .place').text().trim();
          
          // Extract description
          let description = events$(element).find('.description, .excerpt, .summary, p, .entry-content').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at ${location || 'Capture Photography Festival'}. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .featured-project, .home-section').each((i, element) => {
        try {
          ...
          // Try to find a location for this event
          let location = main$(element).find('.location, .venue, .place').text().trim();
          ...
          events.push({
            title,
            date: parseDate(dateText) || 'Current festival',
            url: fullUrl,
            venue: location || name,
            address: venueAddress,
            city: venueCity,
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            // Get better venue information
            const betterLocation = detail$('.location, .venue, .place').text().trim();
            if (betterLocation && betterLocation.length > 3) {
              events[i].venue = betterLocation || events[i].venue;
            }
