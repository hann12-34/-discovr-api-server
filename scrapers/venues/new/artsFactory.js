/**
 * The Arts Factory Scraper
 * URL: https://www.theartsfactory.ca/
 */
...
// Define venue info
const name = 'The Arts Factory';
const url = 'https://www.theartsfactory.ca/';
const eventsUrl = 'https://www.theartsfactory.ca/events/';
const venueAddress = '281 Industrial Avenue';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6A 2P2';
...
/**
 * Scrape events from The Arts Factory
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting The Arts Factory scraper');
  ...
  try {
    ...
    if (eventsResponse.data) {
      ...
      $('.event, article, .post, .event-item, .wp-block-post, .sqs-block-content, .summary-item, .summary-content').each((i, element) => {
        try {
          // Extract title
          const title = $(element).find('h1, h2, h3, h4, .title, .entry-title, .summary-title').first().text().trim();
          if (!title) return;
          ...
          if (!description || description.length < 10) {
            description = `${title} at The Arts Factory. Visit ${fullUrl} for more information.`;
          }
          ...
          let imageUrl = $(element).find('img').attr('src') || 
                         ...
                         $(element).find('.summary-thumbnail').attr('data-image');
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-event, .sqs-block-content, .promotion, .homepage-section').each((i, element) => {
        try {
          ...
          const fullUrl = contentUrl && contentUrl.startsWith('http') ? 
            ...
            contentUrl ? 
              (contentUrl.startsWith('/') ? 
                `${url}${contentUrl.substring(1)}` : 
                `${url}${contentUrl}`) : 
              url;
          
          let dateText = main$(element).find('.date, time, .event-date, .meta, .dates').text().trim();
          
          let description = main$(element).find('p, .description, .content, .summary').text().trim();
          if (!description) {
            description = `${title} at The Arts Factory. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .banner-image img, .sqs-block-image img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for The Arts Factory, returning empty array');
    } else {
      logger.info(`Found ${events.length} events and featured content`);
    }
    
    return events;
  } catch (error) {
    logger.error({ error: error.message }, 'Error scraping The Arts Factory');
