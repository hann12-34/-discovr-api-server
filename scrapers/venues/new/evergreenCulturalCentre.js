/**
 * Evergreen Cultural Centre Scraper
 * URL: https://www.evergreenculturalcentre.ca/
 */
...
// Define venue info
const name = 'Evergreen Cultural Centre';
const url = 'https://www.evergreenculturalcentre.ca/';
const eventsUrl = 'https://www.evergreenculturalcentre.ca/events/';
const exhibitionsUrl = 'https://www.evergreenculturalcentre.ca/exhibitions/';
const venueAddress = '1205 Pinetree Way';
const venueCity = 'Coquitlam';
const venueRegion = 'BC';
const venuePostalCode = 'V3B 7Y3';
...
/**
 * Scrape events from Evergreen Cultural Centre
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Evergreen Cultural Centre scraper');
  ...
  try {
    ...
    // Scrape performances and events
    ...
    if (eventsResponse.data) {
      ...
      $('.event, .event-item, .event-preview, .performance, article, .show').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .show-date').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Evergreen Cultural Centre. Visit ${fullUrl} for more information.`;
          }
    ...
    if (exhibitionsResponse.data) {
      ...
      exhibitions$('.exhibition, .exhibition-item, article, .art-show, .gallery-item').each((i, element) => {
        try {
          // Extract title
          const title = exhibitions$(element).find('h1, h2, h3, h4, .title, .exhibition-title').first().text().trim();
          if (!title) return;
          ...
          // Extract URL
          let exhibitUrl = exhibitions$(element).find('a').attr('href');
          ...
          // Extract date
          let dateText = exhibitions$(element).find('.date, time, .event-date, .meta, .exhibition-date').text().trim();
          if (!dateText) {
            const text = exhibitions$(element).text();
            const dateMatch = text.match(/([A-Za-z]+\s+\d{1,2}(?:,?\s*\d{4})?(?:\s*[-–—]\s*[A-Za-z]+\s+\d{1,2},?\s*\d{4})?)/i);
            if (dateMatch) dateText = dateMatch[0];
          }
          ...
          // Extract description
          let description = exhibitions$(element).find('.description, .excerpt, .summary, p, .content').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at Evergreen Cultural Centre. Visit ${fullUrl} for more information.`;
          }
          
          // Extract image
          let imageUrl = exhibitions$(element).find('img').attr('src') || 
                        exhibitions$(element).find('img').attr('data-src') ||
                        exhibitions$(element).find('img').attr('data-lazy-src') ||
                        exhibitions$(element).find('.image, .featured-image').attr('style');
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-event, .featured-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Evergreen Cultural Centre. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, .single-content, .event-description').text().trim();
            ...
            const betterDate = detail$('.date, time, .event-date, .meta, .show-dates').first().text().trim();
    ...
    if (events.length === 0) {
      logger.info('No events were found for Evergreen Cultural Centre, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Evergreen Cultural Centre');
