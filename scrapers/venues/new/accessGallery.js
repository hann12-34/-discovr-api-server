/**
 * Access Gallery Scraper
 * URL: https://accessgallery.ca/
 */
...
// Define venue info
const name = 'Access Gallery';
const url = 'https://accessgallery.ca/';
const exhibitionsUrl = 'https://accessgallery.ca/exhibitions/';
const eventsUrl = 'https://accessgallery.ca/events/';
const venueAddress = '222 East Georgia Street';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6A 1Z7';
...
/**
 * Scrape events from Access Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Access Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, article, .post, .entry, .exhibition-item, .wp-block-post, .grid-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Access Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, article, .post, .entry, .event-item, .wp-block-post, .grid-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Access Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .home-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Access Gallery. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Access Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Access Gallery');
