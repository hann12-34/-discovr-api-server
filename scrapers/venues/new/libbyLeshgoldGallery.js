/**
 * Libby Leshgold Gallery Scraper (Emily Carr University)
 * URL: https://libby.ecuad.ca/
 */
...
// Define venue info
const name = 'Libby Leshgold Gallery';
const url = 'https://libby.ecuad.ca/';
const exhibitionsUrl = 'https://libby.ecuad.ca/exhibitions/';
const eventsUrl = 'https://libby.ecuad.ca/events/';
const venueAddress = '520 East 1st Avenue';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V5T 0H2';
...
/**
 * Scrape events from Libby Leshgold Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Libby Leshgold Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, article, .post, .entry, .exhibition-item, .entry-content-asset, .grid-list article').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Libby Leshgold Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, article, .post, .entry, .event-item, .entry-content-asset, .grid-list article').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Libby Leshgold Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .exhibition-item').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Libby Leshgold Gallery. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Libby Leshgold Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Libby Leshgold Gallery');
