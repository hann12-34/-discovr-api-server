/**
 * Deer Lake Art Gallery Scraper
 * URL: https://burnabyartscouncil.org/deer-lake-gallery/
 */
...
// Define venue info
const name = 'Deer Lake Art Gallery';
const url = 'https://burnabyartscouncil.org/';
const galleryUrl = 'https://burnabyartscouncil.org/deer-lake-gallery/';
const exhibitionsUrl = 'https://burnabyartscouncil.org/exhibitions/';
const eventsUrl = 'https://burnabyartscouncil.org/events/';
const venueAddress = '6584 Deer Lake Avenue';
const venueCity = 'Burnaby';
const venueRegion = 'BC';
const venuePostalCode = 'V5G 3T7';
...
/**
 * Scrape events from Deer Lake Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Deer Lake Art Gallery scraper');
  ...
  try {
    ...
    if (galleryResponse.data) {
      ...
      const galleryContent = gallery$('.page-content, .entry-content, .content').text().trim();
      const galleryImage = gallery$('.featured-image img, .wp-post-image, .gallery-image img').attr('src');
      
      // Look for any upcoming exhibitions mentioned on the gallery page
      gallery$('h2, h3, h4, .title').each((i, element) => {
        try {
          const headerText = gallery$(element).text().trim();
          if (headerText.toLowerCase().includes('exhibition') || 
              headerText.toLowerCase().includes('gallery') || 
              headerText.toLowerCase().includes('showcase')) {
            
            const section = gallery$(element).nextUntil('h2, h3, h4').text().trim();
            if (section.length > 50) {
              const titleMatch = headerText.match(/[^:]+/);
              const title = titleMatch ? titleMatch[0].trim() : 'Current Exhibition';
              
              // Check for dates in the section
              const dateMatch = section.match(/([A-Za-z]+\s+\d{1,2}(?:,?\s*\d{4})?(?:\s*[-–—]\s*[A-Za-z]+\s+\d{1,2},?\s*\d{4})?)/i);
              const dateText = dateMatch ? dateMatch[0] : '';
              
              events.push({
                title,
                date: parseDate(dateText) || 'Current exhibition',
                url: galleryUrl,
                venue: name,
                ...
                description: section.substring(0, 300) + (section.length > 300 ? '...' : ''),
                image: galleryImage && galleryImage.startsWith('http') ? 
                  galleryImage : 
                  galleryImage ? 
                    `${url}${galleryImage.startsWith('/') ? galleryImage.substring(1) : galleryImage}` : 
                    null
              });
              
              logger.info({ event: { title } }, 'Found gallery content');
            }
          }
        } catch (error) {
          logger.error({ error: error.message }, 'Error processing gallery section');
        }
      });
    }
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .exhibition-item, .exhibition-card, .event-list > div, .event-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Deer Lake Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .event-card, .event-list > div').each((i, element) => {
        try {
          // Extract title
          const title = events$(element).find('h1, h2, h3, h4, .entry-title, .title').first().text().trim();
          if (!title) return;
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Deer Lake Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Deer Lake Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Deer Lake Art Gallery');
