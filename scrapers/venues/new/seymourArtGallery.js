/**
 * Seymour Art Gallery Scraper
 * URL: https://seymourartgallery.com/
 */
...
// Define venue info
const name = 'Seymour Art Gallery';
const url = 'https://seymourartgallery.com/';
const exhibitionsUrl = 'https://seymourartgallery.com/exhibitions/';
const eventsUrl = 'https://seymourartgallery.com/events/';
const venueAddress = '4360 Gallant Ave';
const venueCity = 'North Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V7G 1L2';
...
/**
 * Scrape events from Seymour Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Seymour Art Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .gallery-item').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .entry-date').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Seymour Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .entry-date').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Seymour Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Seymour Art Gallery. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDate = detail$('.date, time, .event-date, .meta, .entry-date').first().text().trim();
    ...
    if (events.length === 0) {
      logger.info('No events were found for Seymour Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Seymour Art Gallery');
