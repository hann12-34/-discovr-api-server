/**
 * Griffin Art Projects Scraper
 * URL: https://www.griffinartprojects.ca/
 */
...
// Define venue info
const name = 'Griffin Art Projects';
const url = 'https://www.griffinartprojects.ca/';
const exhibitionsUrl = 'https://www.griffinartprojects.ca/exhibitions';
const eventsUrl = 'https://www.griffinartprojects.ca/events';
const venueAddress = '1174 Welch Street';
const venueCity = 'North Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V7P 2R5';
...
/**
 * Scrape events from Griffin Art Projects
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Griffin Art Projects scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .grid-item, .post, article, .entry, .exhibition-item, .exhibition-card, .event-item, .summary-item').each((i, element) => {
        try {
          // Extract title
          const title = $(element).find('h1, h2, h3, h4, .entry-title, .title, .summary-title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .exhibition-date, .dates, .summary-metadata-item--date').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .entry-content, .summary-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at Griffin Art Projects. Visit ${fullUrl} for more information.`;
          }
          
          // Extract image
          let imageUrl = $(element).find('img').attr('src') || 
                       $(element).find('img').attr('data-src') ||
                       $(element).find('img').attr('data-image') ||
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .event-card, .summary-item, .grid-item').each((i, element) => {
        try {
          // Extract title
          const title = events$(element).find('h1, h2, h3, h4, .entry-title, .title, .summary-title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .dates, .summary-metadata-item--date').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = events$(element).find('.description, .excerpt, .summary, p, .entry-content, .summary-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at Griffin Art Projects. Visit ${fullUrl} for more information.`;
          }
          
          // Extract image
          let imageUrl = events$(element).find('img').attr('src') || 
                        events$(element).find('img').attr('data-src') ||
                        events$(element).find('img').attr('data-image') ||
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .block, .promoted').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Griffin Art Projects. Visit website for details.`;
          }
          
          let imageUrl = main$(element).find('img').attr('src') || 
                         main$(element).find('img').attr('data-image') ||
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, .single-content, .sqs-block-content').text().trim();
            ...
            const betterDate = detail$('.date, time, .event-date, .meta, .dates, .summary-metadata-item--date').first().text().trim();
            if (betterDate && betterDate.length > 5) {
              events[i].date = parseDate(betterDate) || events[i].date;
            }
            
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .wp-post-image, .sqs-main-image img').attr('src') || 
                               detail$('.featured-image img, .main-image img, .wp-post-image, .sqs-main-image img').attr('data-src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Griffin Art Projects, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Griffin Art Projects');
