/**
 * Arts Council of New Westminster Scraper
 * URL: https://artscouncilnewwest.org/
 */
...
// Define venue info
const name = 'Arts Council of New Westminster';
const url = 'https://artscouncilnewwest.org/';
const exhibitionsUrl = 'https://artscouncilnewwest.org/gallery/exhibitions/';
const eventsUrl = 'https://artscouncilnewwest.org/events/';
const venueAddress = '713 Columbia Street';
const venueCity = 'New Westminster';
const venueRegion = 'BC';
const venuePostalCode = 'V3M 1B2';
...
/**
 * Scrape events from Arts Council of New Westminster
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Arts Council of New Westminster scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .gallery-item, .wp-block-post').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Arts Council of New Westminster. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .wp-block-post').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Arts Council of New Westminster. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .wp-block-cover').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Arts Council of New Westminster. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Arts Council of New Westminster, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Arts Council of New Westminster');
