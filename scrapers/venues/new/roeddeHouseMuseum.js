/**
 * Roedde House Museum Scraper
 * URL: https://roeddehouse.org/
 */
...
// Define venue info
const name = 'Roedde House Museum';
const url = 'https://roeddehouse.org/';
const eventsUrl = 'https://roeddehouse.org/visit/events/';
const exhibitsUrl = 'https://roeddehouse.org/visit/exhibits/';
const venueAddress = '1415 Barclay Street';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6G 1J6';
...
/**
 * Scrape events from Roedde House Museum
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Roedde House Museum scraper');
  ...
  try {
    ...
    if (eventsResponse.data) {
      ...
      // Process event listings - try multiple selectors as site structure might vary
      $('.event, .event-item, article, .post, .card, .entry, .wp-block-column, .col-md-6').each((i, element) => {
        try {
          // Extract title
          const title = $(element).find('h2, h3, h4, .title, .event-title, strong').first().text().trim();
          ...
          if (!description || description.length < 10) {
            description = `${title} at Roedde House Museum. Visit ${fullUrl} for more information.`;
          }
      ...
      if (events.length === 0) {
        ...
        $('section, .section, .content, .entry-content, .page-content').each((i, section) => {
          ...
          if (heading.includes('event') || heading.includes('program') || 
            ...
            $(section).find('p strong, li strong, h4, .event-title').each((j, item) => {
              try {
                const title = $(item).text().trim();
                if (!title || title.length < 3) return;
                
                // Check paragraphs around this item for date and description
                const parentEl = $(item).parent();
                let dateText = '';
                let description = '';
                
                if (parentEl.text().match(/\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]* \d{1,2}(?:st|nd|rd|th)?(?:,? \d{4})?/i)) {
                  dateText = parentEl.text().match(/\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]* \d{1,2}(?:st|nd|rd|th)?(?:,? \d{4})?/i)[0];
                }
                
                // Get surrounding paragraphs for description
                const nextEl = parentEl.next('p');
                if (nextEl.length) {
                  description = nextEl.text().trim();
                } else {
                  description = parentEl.text().replace(title, '').trim();
                }
    ...
    // Check main page for upcoming events
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .event-preview, .upcoming').each((i, element) => {
        try {
          const title = main$(element).find('h2, h3, h1, .title, strong').first().text().trim();
          if (!title || title.length < 3) return;
          ...
          if (!description) {
            description = `${title} at Roedde House Museum. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p').text().trim();
            ...
            // Get better image
            const betterImage = detail$('.featured-image img, .wp-post-image, .hero img').attr('src');
            if (betterImage) {
    ...
    // Check for exhibits/permanent collections
    logger.info('Checking for museum exhibits and collections');
    const exhibitsResponse = await axios.get(exhibitsUrl, axiosConfig).catch(() => 
      axios.get(url + 'collections/', axiosConfig).catch(() => ({ data: '' }))
    );
    ...
    if (exhibitsResponse.data) {
      ...
      exhib$('.collection, .exhibit, .exhibition, article, .post, .entry, .wp-block-column').each((i, element) => {
        try {
          // Extract title
          const title = exhib$(element).find('h2, h3, h4, .title, strong').first().text().trim();
          if (!title || title.length < 3) return;
          ...
          // Extract description
          let description = exhib$(element).find('p, .description, .excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} collection at Roedde House Museum. Visit the museum to view this historic collection.`;
          }
          ...
          const exhibit = {
            title,
            date: 'Permanent Collection', // Most museum displays are permanent
            url: fullUrl,
            venue: name,
            address: venueAddress,
            city: venueCity,
          ...
          logger.info({ event: { title } }, 'Found museum collection/exhibit');
          events.push(exhibit);
        } catch (exhibitError) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Roedde House Museum, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Roedde House Museum');
