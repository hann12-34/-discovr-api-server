/**
 * Nanaimo Art Gallery Scraper
 * URL: https://nanaimoartgallery.ca/
 */
...
// Define venue info
const name = 'Nanaimo Art Gallery';
const url = 'https://nanaimoartgallery.ca/';
const exhibitionsUrl = 'https://nanaimoartgallery.ca/exhibitions/';
const eventsUrl = 'https://nanaimoartgallery.ca/events/';
const venueAddress = '150 Commercial Street';
const venueCity = 'Nanaimo';
const venueRegion = 'BC';
const venuePostalCode = 'V9R 5G6';
...
/**
 * Scrape events from Nanaimo Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Nanaimo Art Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .exhibition-item, .exhibition-card').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Nanaimo Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .event-card').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Nanaimo Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Nanaimo Art Gallery. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Nanaimo Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Nanaimo Art Gallery');
