/**
 * Kelowna Art Gallery Scraper
 * URL: https://kelownaartgallery.com/
 */
...
// Define venue info
const name = 'Kelowna Art Gallery';
const url = 'https://kelownaartgallery.com/';
const exhibitionsUrl = 'https://kelownaartgallery.com/exhibitions/';
const eventsUrl = 'https://kelownaartgallery.com/events/';
const venueAddress = '1315 Water Street';
const venueCity = 'Kelowna';
const venueRegion = 'BC';
const venuePostalCode = 'V1Y 9R3';
...
/**
 * Scrape events from Kelowna Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Kelowna Art Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .item-exhibition, .exhibition-wrap').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .entry-content, .exhibition-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at Kelowna Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .item-event, .event-wrap').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .event-time, .dates').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = events$(element).find('.description, .excerpt, .summary, p, .entry-content, .event-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at Kelowna Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .featured-event').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Kelowna Art Gallery. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, .single-content, .event-content, .exhibition-content').text().trim();
    ...
    if (events.length === 0) {
      logger.info('No events were found for Kelowna Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Kelowna Art Gallery');
