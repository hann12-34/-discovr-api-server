...
// Define venue info
const name = 'Grunt Gallery';
const url = 'https://grunt.ca/';
const eventsUrl = 'https://grunt.ca/events/';
const exhibitionsUrl = 'https://grunt.ca/exhibitions/';
const venueAddress = '116-350 E 2nd Ave';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V5T 4R8';
...
/**
 * Scrape events from Grunt Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Grunt Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .card, .grid-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Grunt Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .card, .event-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Grunt Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .carousel, .home-featured').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Grunt Gallery. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Grunt Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Grunt Gallery');
