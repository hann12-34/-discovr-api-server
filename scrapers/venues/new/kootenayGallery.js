/**
 * Kootenay Gallery of Art Scraper
...
// Define venue info
const name = 'Kootenay Gallery of Art';
const url = 'https://www.kootenaygallery.com/';
const exhibitionsUrl = 'https://www.kootenaygallery.com/exhibitions';
const eventsUrl = 'https://www.kootenaygallery.com/events';
const venueAddress = '120 Heritage Way';
const venueCity = 'Castlegar';
const venueRegion = 'BC';
const venuePostalCode = 'V1N 4M5';
...
/**
 * Scrape events from Kootenay Gallery of Art
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Kootenay Gallery of Art scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition-item, .summary-item, .blog-item, article, .grid-item').each((i, element) => {
        try {
          // Extract title
          const title = $(element).find('h1, h2, h3, h4, .title, .summary-title, .exhibition-title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .summary-metadata-item--date').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .content, .summary-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at Kootenay Gallery of Art. Visit website for details.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event-item, .summary-item, .blog-item, article, .grid-item').each((i, element) => {
        try {
          // Extract title
          const title = events$(element).find('h1, h2, h3, h4, .title, .summary-title, .event-title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .summary-metadata-item--date').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = events$(element).find('.description, .excerpt, .summary, p, .content, .summary-excerpt').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at Kootenay Gallery of Art. Visit website for details.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .block, .promoted').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `Featured content at Kootenay Gallery of Art. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, main p, .sqs-block-content').text().trim();
            ...
            const betterDate = detail$('.date, time, .event-date, .meta, .summary-metadata-item--date').first().text().trim();
            if (betterDate && betterDate.length > 5) {
              events[i].date = parseDate(betterDate) || events[i].date;
            }
            
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .banner-image img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Kootenay Gallery of Art, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Kootenay Gallery of Art');
