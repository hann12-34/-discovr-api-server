/**
 * Museum of North Vancouver (MONOVA) Scraper
 * URL: https://monova.ca/
 */
...
// Define venue info
const name = 'Museum of North Vancouver';
const url = 'https://monova.ca/';
const eventsUrl = 'https://monova.ca/events/';
const exhibitionsUrl = 'https://monova.ca/exhibitions/';
const venueAddress = '115 West Esplanade';
const venueCity = 'North Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V7M 0G7';
...
/**
 * Scrape events from the Museum of North Vancouver
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Museum of North Vancouver scraper');
  ...
  try {
    ...
    if (eventsResponse.data) {
      ...
      $('.event-item, .event, article, .eventlist-event, .card').each((i, element) => {
        try {
          // Extract title
          const title = $(element).find('h1, h2, h3, h4, .title, .event-title, .eventlist-title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .eventlist-meta-date, .meta').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, .eventlist-excerpt, p:not(.date)').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at the Museum of North Vancouver. Visit ${fullUrl} for more information.`;
          }
      ...
      if (events.length === 0) {
        ...
        // Try to find events in sections with event-related text
        $('section, .content, .block').each((i, element) => {
          if ($(element).text().toLowerCase().includes('event') || 
              $(element).text().toLowerCase().includes('program') || 
              $(element).text().toLowerCase().includes('workshop')) {
            
            try {
              const title = $(element).find('h1, h2, h3, h4, h5').first().text().trim();
              if (!title || title.length < 3 || title.toLowerCase() === 'events') return;
              ...
              if (!description || description.length < 10) {
                description = `${title} at the Museum of North Vancouver. Visit website for more details.`;
              }
    ...
    if (exhibitionsResponse.data) {
      ...
      exhib$('.exhibition, .exhibit-item, article, .card, .post, .exhibition-item').each((i, element) => {
        try {
          // Extract title
          const title = exhib$(element).find('h1, h2, h3, h4, .title').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = exhib$(element).find('.date, .meta, .exhibition-date, .time-range').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at the Museum of North Vancouver. Visit ${fullUrl} for more information.`;
          }
          
          // Extract image
          let imageUrl = exhib$(element).find('img').attr('src') || 
                       exhib$(element).find('img').attr('data-src') ||
                       exhib$(element).find('.image, .featured-image').attr('style');
      ...
      // If no exhibitions found with primary selectors, try alternative selectors
      if (events.length === 0) {
        ...
        exhib$('.content-block, section, .block').each((i, element) => {
          if (exhib$(element).text().toLowerCase().includes('exhibit') || 
              exhib$(element).text().toLowerCase().includes('collection') || 
              exhib$(element).text().toLowerCase().includes('display')) {
            
            try {
              const title = exhib$(element).find('h1, h2, h3, h4, h5').first().text().trim();
              ...
              let contentUrl = exhib$(element).find('a').attr('href');
              const fullUrl = contentUrl && contentUrl.startsWith('http') ? 
                contentUrl : 
                contentUrl ? 
                  `${url}${contentUrl.startsWith('/') ? contentUrl.substring(1) : contentUrl}` : 
              ...
              let description = exhib$(element).find('p').text().trim();
              if (!description || description.length < 10) {
                description = `${title} exhibition at the Museum of North Vancouver. Visit website for details.`;
              }
              ...
              events.push({
                title,
                date: 'Permanent exhibit', // Most museum exhibits are permanent unless specified
                url: fullUrl,
                venue: name,
                address: venueAddress,
                city: venueCity,
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .carousel').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at the Museum of North Vancouver. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Museum of North Vancouver, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Museum of North Vancouver');
