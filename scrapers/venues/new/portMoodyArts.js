/**
 * Port Moody Arts Centre Scraper
 * URL: https://pomoarts.ca/
 */
...
// Define venue info
const name = 'Port Moody Arts Centre';
const url = 'https://pomoarts.ca/';
const exhibitionsUrl = 'https://pomoarts.ca/exhibitions';
const eventsUrl = 'https://pomoarts.ca/events';
const venueAddress = '2425 St Johns Street';
const venueCity = 'Port Moody';
const venueRegion = 'BC';
const venuePostalCode = 'V3H 2B2';
...
/**
 * Scrape events from Port Moody Arts Centre
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Port Moody Arts Centre scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .exhibition-item, .exhibition-card, .event-item, .exhibition-block').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Port Moody Arts Centre. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .event-card, .event-block').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Port Moody Arts Centre. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition, .showcase').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Port Moody Arts Centre. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Port Moody Arts Centre, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Port Moody Arts Centre');
