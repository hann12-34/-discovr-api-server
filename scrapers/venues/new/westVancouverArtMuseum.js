/**
 * West Vancouver Art Museum Scraper
...
// Define venue info
const name = 'West Vancouver Art Museum';
const url = 'https://westvancouverartmuseum.ca/';
const exhibitionsUrl = 'https://westvancouverartmuseum.ca/exhibition/';
const eventsUrl = 'https://westvancouverartmuseum.ca/events/';
const venueAddress = '680 17th Street';
const venueCity = 'West Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V7V 3T2';
...
/**
 * Scrape events from West Vancouver Art Museum
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting West Vancouver Art Museum scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition-block, .exhibition, article, .post, .card, .grid-item').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = $(element).find('.date, .exhibition-date, time, .exhibition-meta, .meta').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, p, .content').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at West Vancouver Art Museum. Visit ${fullUrl} for more information.`;
          }
      ...
      if (events.length === 0) {
        ...
        $('section, .section, .content, .entry-content, .page-content').each((i, section) => {
          ...
          if (heading.includes('exhibition') || heading.includes('current') || 
              heading.includes('showing') || heading.includes('featured')) {
            
            $(section).find('article, .card, .item, .exhibition-item').each((j, item) => {
              try {
                // Extract title
                const title = $(item).find('h3, h4, .title, strong').first().text().trim();
                if (!title) return;
                ...
                events.push({
                  ...
                  description: description ? 
                    ...
                    `${title} exhibition at West Vancouver Art Museum.`,
                  image: imageUrl
                });
    ...
    if (eventsResponse.data) {
      ...
      events$('.events-block, .event, .event-item, article, .card').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract description
          let description = events$(element).find('.description, .excerpt, p, .content').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at West Vancouver Art Museum. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .current-exhibition, .home-feature').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at West Vancouver Art Museum. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDate = detail$('.date, time, .exhibition-date, .meta').first().text().trim();
            if (betterDate && betterDate.length > 5) {
              events[i].date = parseDate(betterDate);
            }
            
            // Get better image
            const betterImage = detail$('.featured-image img, .wp-post-image, .featured img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for West Vancouver Art Museum, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping West Vancouver Art Museum');
