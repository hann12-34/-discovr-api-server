/**
 * Gordon Smith Gallery of Canadian Art Scraper
 * URL: https://gordonsmithgallery.ca/
 */
...
// Define venue info
const name = 'Gordon Smith Gallery of Canadian Art';
const url = 'https://gordonsmithgallery.ca/';
const exhibitionsUrl = 'https://gordonsmithgallery.ca/exhibitions/';
const eventsUrl = 'https://gordonsmithgallery.ca/events/';
const venueAddress = '2121 Lonsdale Ave';
const venueCity = 'North Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V7M 2K6';
...
/**
 * Scrape events from Gordon Smith Gallery of Canadian Art
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Gordon Smith Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .exhibition-block, .listing, .card').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .exhibition-dates').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `Exhibition at Gordon Smith Gallery. Visit website for details.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .event-block, .listing, .card').each((i, element) => {
        try {
          // Extract title
          const title = events$(element).find('h1, h2, h3, h4, .title, .heading').first().text().trim();
          if (!title) return;
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .event-dates').text().trim();
          if (!dateText) {
          ...
          if (!description || description.length < 10) {
            description = `Event at Gordon Smith Gallery. Visit website for details.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          const title = main$(element).find('h1, h2, h3, h4, .title, .heading').first().text().trim();
          ...
          if (!description) {
            description = `Featured content at Gordon Smith Gallery. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .gallery-image img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Gordon Smith Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Gordon Smith Gallery');
