...
// Define venue info
const name = 'SUM Gallery';
const url = 'https://sumgallery.ca/';
const exhibitionsUrl = 'https://sumgallery.ca/exhibitions/';
const eventsUrl = 'https://sumgallery.ca/events/';
const venueAddress = '425-268 Keefer St';
...
/**
 * Scrape events from SUM Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting SUM Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .grid-item, .exhibition-item, .post-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at SUM Gallery. Visit website for details.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .grid-item, .event-item, .post-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at SUM Gallery. Visit website for details.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `Featured content at SUM Gallery. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .single-image img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for SUM Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping SUM Gallery');
