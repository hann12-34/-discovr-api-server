/**
 * Western Front Gallery Scraper
 * URL: https://front.bc.ca
 */
...
// Define venue info
const name = 'Western Front';
const url = 'https://front.bc.ca';
const eventsUrl = 'https://front.bc.ca/program/';
const venueAddress = '303 E 8th Ave';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V5T 1S1';
...
/**
 * Scrape events from Western Front
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Western Front scraper');
  ...
  try {
    ...
    if (eventsResponse.data) {
      ...
      $('.event-item, .event, article, .grid-item, .program-item').each((i, element) => {
        try {
          // Extract title
          ...
          const fullUrl = eventUrl && eventUrl.startsWith('http') ? 
            eventUrl : 
            eventUrl ? 
              `${url}${eventUrl.startsWith('/') ? '' : '/'}${eventUrl}` : 
              eventsUrl;
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .content, .text').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at Western Front. Visit ${fullUrl} for more information.`;
          }
          ...
          if (imageUrl && !imageUrl.startsWith('http')) {
            imageUrl = imageUrl.startsWith('//') ? 
              `https:${imageUrl}` : 
              `${url}${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
          }
      ...
      if (events.length === 0) {
        ...
        $('.program-item, .post, .entry, .current').each((i, element) => {
          try {
            ...
            let description = $(element).find('p, .description, .content').text().trim();
            if (!description) {
              description = `${title} at Western Front. Visit website for details.`;
            }
            
            let imageUrl = $(element).find('img').attr('src');
            if (imageUrl && !imageUrl.startsWith('http')) {
              imageUrl = `${url}${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
            }
    ...
    // Scrape main page for featured events
    logger.info('Fetching main page for featured events');
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .carousel-item, .featured-event').each((i, element) => {
        try {
          ...
          events.push({
            title,
            date: parseDate(dateText) || 'Featured event',
            url: fullUrl,
            venue: name,
            address: venueAddress,
            city: venueCity,
          ...
          logger.error({ error: featureError.message }, 'Error processing featured event');
        }
      });
    }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, main p, .details').text().trim();
            ...
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .event-image img').attr('src');
            if (betterImage) {
              events[i].image = betterImage.startsWith('http') ? 
                betterImage : 
                `${url}${betterImage.startsWith('/') ? '' : '/'}${betterImage}`;
            }
          }
        } catch (detailError) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Western Front, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Western Front');
