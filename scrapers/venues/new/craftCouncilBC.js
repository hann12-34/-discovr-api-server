/**
 * Craft Council of British Columbia Scraper
 * URL: https://craftcouncilbc.ca/
 */
...
// Define venue info
const name = 'Craft Council of British Columbia';
const url = 'https://craftcouncilbc.ca/';
const exhibitionsUrl = 'https://craftcouncilbc.ca/exhibitions/';
const eventsUrl = 'https://craftcouncilbc.ca/events/';
const venueAddress = '1386 Cartwright Street';
const venueCity = 'Vancouver';
const venueRegion = 'BC';
const venuePostalCode = 'V6H 3R8';
...
/**
 * Scrape events from Craft Council of British Columbia
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Craft Council of British Columbia scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .exhibition-item, .type-exhibition').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} exhibition at Craft Council of British Columbia. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .type-event').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description || description.length < 10) {
            description = `${title} event at Craft Council of British Columbia. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Craft Council of British Columbia. Visit website for details.`;
          }
    ...
    if (events.length === 0) {
      logger.info('No events were found for Craft Council of British Columbia, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Craft Council of British Columbia');
