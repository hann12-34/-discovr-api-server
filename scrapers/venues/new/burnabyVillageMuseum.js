/**
 * Burnaby Village Museum Scraper
 * URL: https://www.burnabyvillagemuseum.ca/
 */
...
// Define venue info
const name = 'Burnaby Village Museum';
const url = 'https://www.burnabyvillagemuseum.ca/';
const eventsUrl = 'https://www.burnabyvillagemuseum.ca/events';
const venueAddress = '6501 Deer Lake Avenue';
const venueCity = 'Burnaby';
const venueRegion = 'BC';
const venuePostalCode = 'V5G 3T6';
const venueCountry = 'Canada';

/**
 * Scrape events from Burnaby Village Museum
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Burnaby Village Museum scraper');
  ...
  try {
    ...
    if (eventsResponse.data) {
      ...
      // Process all event listings
      $('.event, .event-listing, article, .event-item').each((i, element) => {
        try {
          // Extract title
          ...
          if (!description) {
            description = `${title} at Burnaby Village Museum. Visit website for details.`;
          }
      ...
      if (events.length === 0) {
        ...
        $('section, .section, .content-block').each((i, section) => {
          ...
          if (sectionTitle.includes('event') || sectionTitle.includes('program') || 
            ...
            $(section).find('.item, .card, article').each((j, item) => {
              try {
    ...
    // Check for seasonal events on the main page
    logger.info('Checking main page for seasonal events');
    const mainResponse = await axios.get(url, axiosConfig).catch(() => ({ data: '' }));
    
    if (mainResponse.data) {
      const main$ = cheerio.load(mainResponse.data);
      
      // Look for featured or seasonal events
      main$('.featured, .highlight, .seasonal, .banner, .hero').each((i, element) => {
        try {
          ...
          logger.info({ event: { title } }, 'Found seasonal event from main page');
        } catch (featureError) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Burnaby Village Museum, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Burnaby Village Museum');
