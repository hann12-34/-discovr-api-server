...
// Define venue info
const name = 'Kamloops Art Gallery';
const url = 'https://kag.bc.ca/';
const exhibitionsUrl = 'https://kag.bc.ca/exhibitions/';
const eventsUrl = 'https://kag.bc.ca/programs-and-events/';
const venueAddress = '101-465 Victoria Street';
const venueCity = 'Kamloops';
const venueRegion = 'BC';
const venuePostalCode = 'V2C 2A9';
...
/**
 * Scrape events from Kamloops Art Gallery
 * @returns {Array} Array of event objects
 */
async function scrape() {
  const logger = scrapeLogger.child({ scraper: name });
  logger.info('Starting Kamloops Art Gallery scraper');
  ...
  try {
    ...
    if (exhibitionsResponse.data) {
      ...
      $('.exhibition, .post, article, .entry, .exhibition-item, .exhibition-card, .exhibition-list-item').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = $(element).find('.date, time, .event-date, .meta, .exhibition-date, .dates, .exhibition-dates').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = $(element).find('.description, .excerpt, .summary, p, .entry-content, .exhibition-description').text().trim();
          if (!description || description.length < 10) {
            description = `${title} exhibition at Kamloops Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (eventsResponse.data) {
      ...
      events$('.event, .post, article, .entry, .event-item, .event-card, .event-list-item').each((i, element) => {
        try {
          // Extract title
          ...
          // Extract date
          let dateText = events$(element).find('.date, time, .event-date, .meta, .dates, .event-dates').text().trim();
          if (!dateText) {
          ...
          // Extract description
          let description = events$(element).find('.description, .excerpt, .summary, p, .entry-content, .event-description').text().trim();
          if (!description || description.length < 10) {
            description = `${title} event at Kamloops Art Gallery. Visit ${fullUrl} for more information.`;
          }
    ...
    if (mainResponse.data) {
      ...
      main$('.featured, .highlight, .banner, .hero, .slider, .featured-exhibition, .current-exhibition').each((i, element) => {
        try {
          ...
          if (!description) {
            description = `${title} at Kamloops Art Gallery. Visit website for details.`;
          }
      ...
      for (let i = 0; i < Math.min(events.length, 5); i++) {
        try {
          ...
          if (detailResponse.data) {
            ...
            const betterDescription = detail$('.entry-content, .content, article p, .description, .single-content, .event-description, .exhibition-description').text().trim();
            ...
            const betterDate = detail$('.date, time, .event-date, .meta, .dates, .exhibition-dates').first().text().trim();
            if (betterDate && betterDate.length > 5) {
              events[i].date = parseDate(betterDate) || events[i].date;
            }
            
            // Get better image
            const betterImage = detail$('.featured-image img, .main-image img, .wp-post-image, .exhibition-image img').attr('src');
            if (betterImage) {
    ...
    if (events.length === 0) {
      logger.info('No events were found for Kamloops Art Gallery, returning empty array');
    } else {
    ...
    logger.error({ error: error.message }, 'Error scraping Kamloops Art Gallery');
