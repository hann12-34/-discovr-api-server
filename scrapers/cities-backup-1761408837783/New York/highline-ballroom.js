...
/**
 * Highline Ballroom Events Scraper
 * Scrapes real events from Highline Ballroom website
 */
async function scrapeEvents() {
    try {
        console.log('🎤 Scraping events from Highline Ballroom...');

        const response = await axios.get('https://www.highlineballroom.com/events', {
            headers: {
        ...
        $('.artist, .performer, .event-title, .show-title, '.headliner', h1, h2, h3').each((index, element) => {
            ...
            if (title && title.length > 2 && title.length < 100 && !isNavigationElement(title)) {
                ...
                if (!dateText) {
                    dateText = 'February 15, 2025 at 8:00 PM';
                }

                console.log(`🔍 Highline Ballroom event: "${title}" - Date: "${dateText}"`);
                
                events.push({
                    id: uuidv4(),
                    title: title,
                    venue: { name: 'Highline Ballroom', address: '431 W 16th Street, New York, NY 10011', city: 'New York' },
                    location: 'Chelsea, New York',
                    date: dateText,
                    category: 'Concert',
                    description: description && description.length > 20 ? description : `${title} in New York`,
                    link: 'https://www.highlineballroom.com/events',
                    source: 'highline-ballroom'
                });
            }
        });

        console.log(`✅ Highline Ballroom: Found ${events.length} events`);
        return events.length > 0 ? events : [];

    } catch (error) {
        console.error(`❌ Highline Ballroom error: ${error.message}`);
        return [];
    }
}
