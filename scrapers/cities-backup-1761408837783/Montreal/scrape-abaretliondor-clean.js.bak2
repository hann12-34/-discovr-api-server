...
class CabaretLionDOrEvents {
    constructor() {
        ...
        this.city = 'Montreal';
        this.isEnabled = false; // Disabled due to constructor error - scraper needs refactoring
        this.province = 'QC';
            ...
            const events = await page.evaluate(() => {
                ...
                eventElements.forEach((element, index) => {
                    try {
                        ...
                        eventsList.push({
                            title,
                            description: description && description.length > 20 ? description : `${title} - ${title} at Cabaret Lion d Or`,
                            dateText,
                            price,
                            url: eventUrl
                        });
...
async function scrapeEvents() {
  const scraper = new CabaretLionDOrEvents();
