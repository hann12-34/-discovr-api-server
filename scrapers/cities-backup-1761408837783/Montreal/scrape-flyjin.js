...
/**
 * Fly Jin Events Scraper
 * Scrapes events from Fly Jin restaurant in Montreal
 */
class FlyJinEvents {
    constructor() {
        this.name = 'Fly Jin';
        this.eventsUrl = 'https://flyjin.ca/events';
        this.source = 'fly-jin';
        this.city = 'Montreal';
            ...
            $('.event, .event-item, .event-card, .show, .party, .listing, .card').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Fly Jin`,
                        date: this.parseDate(dateText) || new Date(),
                        venue: {
                            name: 'Fly Jin',
                            address: '438 Avenue du Mont-Royal E, Montreal, QC H2J 1W2',
                            city: this.city,
...
async function scrapeEvents() {
  const scraper = new FlyJinEvents();
