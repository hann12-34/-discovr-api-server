...
/**
 * Barraca Events Scraper
 * Scrapes events from Barraca in Montreal
 */
class BarracaEvents {
    constructor() {
        this.name = 'Barraca';
            ...
            $('.event, .event-item, .event-card, .show, .party, .listing, .card').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Barraca`,
                        date: this.parseDate(dateText) || new Date(),
                        venue: {
                            name: 'Barraca',
                            address: '1134 Avenue du Mont-Royal E, Montreal, QC',
                            city: this.city,
