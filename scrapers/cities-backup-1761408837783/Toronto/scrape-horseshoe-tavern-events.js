...
const EVENTS_URL = 'https://horseshoetavern.com';
const VENUE_NAME = 'The Horseshoe Tavern';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🐎 Scraping The Horseshoe Tavern events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .gig, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content, .details').first().text().trim() || 'Live music at The Horseshoe Tavern, Toronto\'s historic music venue since 1947.';
            ...
            const event = {
                ...
                venue: {
                    name: VENUE_NAME,
                    address: '370 Queen St W, Toronto, ON, Canada'
                },
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'rock', 'indie', 'concerts']),
                price: price,
                tags: ['live music', 'rock', 'indie', 'concerts', 'historic venue', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Horseshoe Tavern`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Horseshoe Tavern events: ${error.message}`);
        return [];
    }
}
