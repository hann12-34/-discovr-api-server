...
const EVENTS_URL = 'https://therex.ca';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎷 Scraping The Rex Hotel events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .gig, .performance').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content, .details').first().text().trim() || 'Live jazz and blues at The Rex Hotel, Toronto\'s premier jazz venue.';
            ...
            const event = {
                ...
                venue: {
                    name: VENUE_NAME,
                    address: '194 Queen St W, Toronto, ON, Canada'
                },
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: extractCategories(title, description, ['jazz', 'blues', 'live music', 'concerts']),
                price: price,
                tags: ['jazz', 'blues', 'live music', 'concerts', 'premier venue', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Rex Hotel`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Rex Hotel events: ${error.message}`);
        return [];
    }
}
