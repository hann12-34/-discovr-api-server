...
const EVENTS_URL = 'https://thehideout.ca';
const VENUE_NAME = 'The Hideout';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🕳️ Scraping The Hideout events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .live-music').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live music, intimate concerts, and local performances at The Hideout, Toronto\'s cozy underground music venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'concerts', 'local performances', 'intimate']),
                price: price,
                tags: ['live music', 'concerts', 'local performances', 'intimate', 'underground', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Hideout`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Hideout events: ${error.message}`);
        return [];
    }
}
