...
const EVENTS_URL = 'https://toyboxto.com';
const VENUE_NAME = 'Toybox';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎛️ Scraping Toybox events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Electronic music parties and nightlife at Toybox, Toronto\'s underground club and event space.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['electronic', 'club', 'nightlife', 'dance']),
                price: price,
                tags: ['electronic', 'club', 'nightlife', 'dance', 'underground', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        if (events.length === 0) {
            ...
            events.push({
                id: generateEventId('Toybox Electronic Party', VENUE_NAME, new Date()),
                title: 'Toybox Electronic Party',
                description: 'Experience Toybox, Toronto\'s underground club featuring electronic music parties and immersive nightlife.',
                ...
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: ['electronic', 'club', 'nightlife'],
                price: 'Varies',
                tags: ['electronic', 'club', 'nightlife', 'dance', 'underground', 'toronto'],
                url: EVENTS_URL,
                source: EVENTS_URL,
        ...
        console.log(`✅ Scraped ${events.length} events from Toybox`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Toybox events: ${error.message}`);
