...
const EVENTS_URL = 'https://soundacademy.com';
const VENUE_NAME = 'Sound Academy';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎵 Scraping Sound Academy events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Electronic music, concerts, and events at Sound Academy, Toronto\'s premier dance music venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['electronic', 'dance', 'concerts', 'music']),
                price: price,
                tags: ['electronic', 'dance', 'concerts', 'music', 'edm', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Sound Academy`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Sound Academy events: ${error.message}`);
        return [];
    }
}
