...
const EVENTS_URL = 'https://dromtaberna.com';
const VENUE_NAME = 'Drom Taberna';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🍷 Scraping Drom Taberna events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience Drom Taberna, Toronto\'s Mediterranean-inspired bar and restaurant.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['bars', 'mediterranean', 'dining', 'nightlife']),
                price: price,
                tags: ['bars', 'mediterranean', 'dining', 'nightlife', 'restaurant', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        if (events.length === 0) {
            ...
            events.push({
                id: generateEventId('Drom Taberna Experience', VENUE_NAME, new Date()),
                title: 'Drom Taberna Experience',
                description: 'Experience Drom Taberna, Toronto\'s Mediterranean-inspired bar and restaurant featuring authentic cuisine and atmosphere.',
                ...
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: ['bars', 'mediterranean', 'dining'],
                price: 'Varies',
                tags: ['bars', 'mediterranean', 'dining', 'nightlife', 'restaurant', 'toronto'],
                url: EVENTS_URL,
                source: EVENTS_URL,
        ...
        console.log(`✅ Scraped ${events.length} events from Drom Taberna`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Drom Taberna events: ${error.message}`);
