...
const EVENTS_URL = 'https://civillibertiesbar.com';
const VENUE_NAME = 'Civil Liberties';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`⚖️ Scraping Civil Liberties events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .news-item').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience Civil Liberties - Toronto\'s sophisticated cocktail bar.';
            ...
            const event = {
                ...
                price: price,
                tags: ['cocktails', 'bar', 'nightlife', 'sophisticated', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Civil Liberties`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Civil Liberties events: ${error.message}`);
        return [];
    }
}
