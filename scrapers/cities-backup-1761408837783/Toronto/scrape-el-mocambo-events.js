...
const EVENTS_URL = 'https://elmocambo.com';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎤 Scraping El Mocambo events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .gig, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content, .details').first().text().trim() || 'Live music at El Mocambo, Toronto\'s legendary music venue with decades of rock history.';
            ...
            const event = {
                ...
                venue: {
                    name: VENUE_NAME,
                    address: '464 Spadina Ave, Toronto, ON, Canada'
                },
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'rock', 'blues', 'concerts']),
                price: price,
                tags: ['live music', 'rock', 'blues', 'concerts', 'legendary venue', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from El Mocambo`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping El Mocambo events: ${error.message}`);
        return [];
    }
}
