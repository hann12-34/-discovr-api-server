...
const EVENTS_URL = 'https://comedybar.ca';
const VENUE_NAME = 'Comedy Bar';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`😂 Scraping Comedy Bar events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .comedy').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Comedy shows and stand-up performances at Comedy Bar, Toronto\'s premier comedy venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['comedy', 'stand-up', 'entertainment', 'live performance']),
                price: price,
                tags: ['comedy', 'stand-up', 'entertainment', 'live performance', 'humor', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Comedy Bar`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Comedy Bar events: ${error.message}`);
        return [];
    }
}
