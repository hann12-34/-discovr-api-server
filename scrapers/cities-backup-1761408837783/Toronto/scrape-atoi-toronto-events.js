...
const EVENTS_URL = 'https://atoitoronto.com';
const VENUE_NAME = 'À Toi';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🍷 Scraping À Toi events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .news-item').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience À Toi - Toronto\'s sophisticated cocktail bar and dining destination.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['cocktails', 'bars', 'nightlife', 'dining']),
                price: price,
                tags: ['cocktails', 'bar', 'nightlife', 'dining', 'french', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from À Toi`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping À Toi events: ${error.message}`);
