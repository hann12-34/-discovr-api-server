...
const EVENTS_URL = 'https://thegreathall.ca';
const VENUE_NAME = 'The Great Hall';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🏛️ Scraping The Great Hall events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live music, events, and performances at The Great Hall, Toronto\'s historic entertainment venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'concerts', 'entertainment', 'historic']),
                price: price,
                tags: ['live music', 'concerts', 'entertainment', 'historic', 'venue', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Great Hall`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Great Hall events: ${error.message}`);
        return [];
    }
}
