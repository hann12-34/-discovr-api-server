...
const EVENTS_URL = 'https://staffonly.ca';
const VENUE_NAME = 'Staff Only';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`👨‍💼 Scraping Staff Only events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Exclusive cocktail experiences at Staff Only, Toronto\'s upscale speakeasy-style bar.';
            ...
            const event = {
                ...
                price: price,
                tags: ['cocktails', 'speakeasy', 'bars', 'nightlife', 'upscale', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Staff Only`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Staff Only events: ${error.message}`);
        return [];
    }
}
