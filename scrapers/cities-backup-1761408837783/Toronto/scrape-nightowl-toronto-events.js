...
const EVENTS_URL = 'https://nightowltoronto.com';
const VENUE_NAME = 'Nightowl Toronto';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🦉 Scraping Nightowl Toronto events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience Nightowl Toronto, the city\'s premier late-night entertainment venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['nightlife', 'bars', 'entertainment', 'late night']),
                price: price,
                tags: ['nightlife', 'bars', 'entertainment', 'late night', 'party', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Nightowl Toronto`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Nightowl Toronto events: ${error.message}`);
