...
const EVENTS_URL = 'https://spintoronto.com';
const VENUE_NAME = 'Spin Toronto';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🏓 Scraping Spin Toronto events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .ping-pong').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Ping pong, dining, and social events at Spin Toronto, the ultimate table tennis social club.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['ping pong', 'sports', 'social', 'dining']),
                price: price,
                tags: ['ping pong', 'sports', 'social', 'dining', 'club', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Spin Toronto`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Spin Toronto events: ${error.message}`);
        return [];
    }
}
