...
const EVENTS_URL = 'https://embracetoronto.com';
const VENUE_NAME = 'Embrace';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`💎 Scraping Embrace events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Upscale nightlife and events at Embrace, Toronto\'s luxury nightclub and entertainment venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['nightlife', 'luxury', 'club', 'upscale']),
                price: price,
                tags: ['nightlife', 'luxury', 'club', 'upscale', 'entertainment', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Embrace`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Embrace events: ${error.message}`);
        return [];
    }
}
