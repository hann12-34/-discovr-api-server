...
const EVENTS_URL = 'https://projectgigglewater.com';
const VENUE_NAME = 'Project Gigglewater';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🥂 Scraping Project Gigglewater events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Creative cocktail experiences at Project Gigglewater, Toronto\'s innovative cocktail laboratory.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['cocktails', 'bars', 'nightlife', 'innovative']),
                price: price,
                tags: ['cocktails', 'bars', 'nightlife', 'innovative', 'craft', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Project Gigglewater`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Project Gigglewater events: ${error.message}`);
        return [];
    }
}
