...
const EVENTS_URL = 'https://gladstonehotel.com';
const VENUE_NAME = 'Gladstone Hotel';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🏨 Scraping Gladstone Hotel events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .art').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Art exhibitions, live music, and cultural events at Gladstone Hotel, Toronto\'s iconic arts hotel.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['art', 'exhibitions', 'live music', 'cultural']),
                price: price,
                tags: ['art', 'exhibitions', 'live music', 'cultural', 'hotel', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Gladstone Hotel`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Gladstone Hotel events: ${error.message}`);
        return [];
    }
}
