...
const EVENTS_URL = 'https://lula.ca';
const VENUE_NAME = 'Lula Lounge';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`💃 Scraping Lula Lounge events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .latin').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Latin music, salsa dancing, and live performances at Lula Lounge, Toronto\'s premier Latin music venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['latin music', 'salsa', 'dance', 'live music']),
                price: price,
                tags: ['latin music', 'salsa', 'dance', 'live music', 'cultural', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Lula Lounge`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Lula Lounge events: ${error.message}`);
