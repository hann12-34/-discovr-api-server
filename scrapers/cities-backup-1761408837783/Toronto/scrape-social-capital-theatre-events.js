...
const EVENTS_URL = 'https://socialcapitaltheatre.ca';
const VENUE_NAME = 'Social Capital Theatre';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎭 Scraping Social Capital Theatre events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .theatre').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experimental and contemporary theatre at Social Capital Theatre, Toronto\'s innovative performance space.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['theatre', 'experimental', 'contemporary', 'performance']),
                price: price,
                tags: ['theatre', 'experimental', 'contemporary', 'performance', 'innovative', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Social Capital Theatre`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Social Capital Theatre events: ${error.message}`);
        return [];
    }
}
