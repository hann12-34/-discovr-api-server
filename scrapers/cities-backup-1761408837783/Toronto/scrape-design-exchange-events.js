...
const EVENTS_URL = 'https://dx.org';
const VENUE_NAME = 'Design Exchange';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎨 Scraping Design Exchange events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .exhibition, .workshop, .program').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Design and innovation events at the Design Exchange, Toronto\'s premier design museum.';
            ...
            const event = {
                ...
                venue: {
                    name: VENUE_NAME,
                    address: '234 Bay St, Toronto, ON, Canada'
                },
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: extractCategories(title, description, ['design', 'museums', 'art', 'exhibitions']),
                price: price,
                tags: ['design', 'museums', 'art', 'exhibitions', 'innovation', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Design Exchange`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Design Exchange events: ${error.message}`);
        return [];
    }
}
