...
const EVENTS_URL = 'https://reservoirlounge.com';
const VENUE_NAME = 'Reservoir Lounge';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎷 Scraping Reservoir Lounge events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .jazz').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Jazz, blues, and live music at Reservoir Lounge, Toronto\'s intimate music venue and cocktail bar.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['jazz', 'blues', 'live music', 'cocktails']),
                price: price,
                tags: ['jazz', 'blues', 'live music', 'cocktails', 'intimate', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Reservoir Lounge`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Reservoir Lounge events: ${error.message}`);
        return [];
    }
}
