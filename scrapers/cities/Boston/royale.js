/**
 * Royale Nightclub Boston Events Scraper
...
async function scrapeRoyale(city = 'Boston') {
  console.log('🎵 Scraping Royale Nightclub...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Royale Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Royale Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Royale Nightclub error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRoyale;
