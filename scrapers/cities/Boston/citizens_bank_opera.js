/**
 * Citizens Bank Opera House Boston Events Scraper
 * URL: https://www.citicenter.org/plan-your-visit/venues/citizens-bank-opera-house
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCitizensOpera(city = 'Boston') {
  console.log('🎭 Scraping Citizens Opera...');
  ...
  try {
    ...
    await page.goto('https://www.citicenter.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Citizens Opera'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Citizens Opera events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Citizens Opera error:', error.message);
    return [];
  }
}

module.exports = scrapeCitizensOpera;
