/**
 * Wilbur Theatre Boston Events Scraper
 * URL: https://www.thewilbur.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWilburTheatre(city = 'Boston') {
  console.log('🎭 Scraping Wilbur Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.thewilbur.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Arts',
        source: 'Wilbur Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Wilbur Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Wilbur Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeWilburTheatre;
