/**
 * Royale Boston - Boston Events Scraper
...
async function scrapeRoyaleBoston(city = 'Boston') {
  console.log('🎭 Scraping Royale Boston...');
  try {
    const resp = await axios.get('https://royaleboston.com/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Royale Boston', address: addr || '279 Tremont St, Boston, MA 02116', city: 'Boston' },
            latitude: 42.3503,
            longitude: -71.0651,
            city: 'Boston',
            category: 'Events',
            source: 'Royale Boston'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://royaleboston.com/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://royaleboston.com/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Royale Boston', address: '279 Tremont St, Boston, MA 02116', city: 'Boston' },
            latitude: 42.3503,
            longitude: -71.0651,
            city: 'Boston',
            category: 'Events',
    ...
    console.log('  ✅ Royale Boston:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Royale Boston error:', error.message);
    return [];
  }
}
