/**
 * Wally's Cafe Updated Boston Events Scraper
 * URL: https://www.wallyscafe.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWallyUpdated(city = 'Boston') {
  console.log('🎷 Scraping Wally Updated...');
  ...
  try {
    ...
    await page.goto('https://www.wallyscafe.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Wally Updated'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Wally Updated events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Wally Updated error:', error.message);
    return [];
  }
}

module.exports = scrapeWallyUpdated;
