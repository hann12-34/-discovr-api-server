/**
 * Nick's Comedy Stop Boston Events Scraper
 * URL: https://www.nickscomedystop.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNicksComedy(city = 'Boston') {
  console.log('😂 Scraping Nick\'s Comedy...');
  ...
  try {
    ...
    await page.goto('https://www.nickscomedystop.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Nick\'s Comedy'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Nick's Comedy events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nick\'s Comedy error:', error.message);
    return [];
  }
}

module.exports = scrapeNicksComedy;
