/**
 * Middle East Final Boston Events Scraper
 * URL: https://mideastoffers.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMiddleEastFinal(city = 'Boston') {
  console.log('🎸 Scraping Middle East Final...');
  ...
  try {
    ...
    await page.goto('https://mideastoffers.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Middle East Final'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Middle East Final events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Middle East Final error:', error.message);
    return [];
  }
}

module.exports = scrapeMiddleEastFinal;
