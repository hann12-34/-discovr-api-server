/**
 * Comedy Studio Boston Events Scraper
 * URL: https://thecomedystudio.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeComedyStudio(city = 'Boston') {
  console.log('😂 Scraping Comedy Studio...');
  ...
  try {
    ...
    await page.goto('https://thecomedystudio.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Comedy Studio'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Comedy Studio events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Comedy Studio error:', error.message);
    return [];
  }
}

module.exports = scrapeComedyStudio;
