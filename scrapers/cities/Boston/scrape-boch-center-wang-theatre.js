/**
 * Boch Center Wang Theatre - Boston Events Scraper
 * URL: https://www.bochcenter.org/events
 */
...
async function scrapeBochCenterWangTheatre(city = 'Boston') {
  console.log('🎭 Scraping Boch Center Wang Theatre...');
  try {
    const resp = await axios.get('https://www.bochcenter.org/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Boch Center Wang Theatre', address: addr || '270 Tremont St, Boston, MA 02116', city: 'Boston' },
            latitude: 42.3506,
            longitude: -71.0648,
            city: 'Boston',
            category: 'Events',
            source: 'Boch Center Wang Theatre'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.bochcenter.org/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.bochcenter.org/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Boch Center Wang Theatre', address: '270 Tremont St, Boston, MA 02116', city: 'Boston' },
            latitude: 42.3506,
            longitude: -71.0648,
            city: 'Boston',
            category: 'Events',
    ...
    console.log('  ✅ Boch Center Wang Theatre:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Boch Center Wang Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeBochCenterWangTheatre;
