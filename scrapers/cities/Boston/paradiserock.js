/**
 * Paradise Rock Club Boston Events Scraper
...
async function scrapeParadiserock(city = 'Boston') {
  console.log('🎵 Scraping Paradise Rock Club...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Paradise Rock Club'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Paradise Rock Club events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Paradise Rock Club error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeParadiserock;
