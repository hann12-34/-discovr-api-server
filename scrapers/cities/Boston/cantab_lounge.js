/**
 * Cantab Lounge Boston Events Scraper
 * URL: https://cantab-lounge.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCantabLounge(city = 'Boston') {
  console.log('🎸 Scraping Cantab Lounge...');
  ...
  try {
    ...
    await page.goto('https://cantab-lounge.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Cantab Lounge'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cantab Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cantab Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeCantabLounge;
