/**
 * Phoenix Landing Boston Events Scraper
 * URL: https://phoenixlandingbar.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePhoenixLanding(city = 'Boston') {
  console.log('🔥 Scraping Phoenix Landing...');
  ...
  try {
    ...
    await page.goto('https://phoenixlandingbar.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Phoenix Landing'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Phoenix Landing events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Phoenix Landing error:', error.message);
    return [];
  }
}

module.exports = scrapePhoenixLanding;
