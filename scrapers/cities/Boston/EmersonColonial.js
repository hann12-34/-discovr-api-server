/**
 * Emerson Colonial Theatre Boston Events Scraper
 * URL: https://artsemerson.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEmersonColonial(city = 'Boston') {
  console.log('🎭 Scraping Emerson Colonial...');
  ...
  try {
    ...
    await page.goto('https://artsemerson.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Emerson Colonial'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Emerson Colonial events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Emerson Colonial error:', error.message);
    return [];
  }
}

module.exports = scrapeEmersonColonial;
