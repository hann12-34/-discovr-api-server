/**
 * Middlesex Lounge Boston Events Scraper
 * Popular nightclub in Boston
 * URL: https://middlesexlounge.us/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMiddlesexLoungeBoston(city = 'Boston') {
  console.log('🎵 Scraping Middlesex Lounge Boston...');
  ...
  try {
    ...
    await page.goto('https://middlesexlounge.us/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Middlesex Lounge'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Middlesex Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Middlesex Lounge error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMiddlesexLoungeBoston;
