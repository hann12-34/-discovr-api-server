/**
 * Colonial Theatre Boston Events Scraper
 * URL: https://www.colonialbostontheatre.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeColonialTheatre(city = 'Boston') {
  console.log('🎭 Scraping Colonial Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.colonialbostontheatre.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Colonial Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Colonial Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Colonial Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeColonialTheatre;
