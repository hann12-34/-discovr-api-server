/**
 * Agganis Arena Boston Events Scraper
 * URL: https://www.agganisarena.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAgganisArena(city = 'Boston') {
  console.log('🏟️ Scraping Agganis Arena...');
  ...
  try {
    ...
    await page.goto('https://www.agganisarena.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Agganis Arena'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Agganis Arena events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Agganis Arena error:', error.message);
    return [];
  }
}

module.exports = scrapeAgganisArena;
