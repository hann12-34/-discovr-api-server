...
async function scrapeParadise(city = 'Boston') {
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Paradise Rock Club`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Paradise events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Paradise error:', error.message);
    return [];
  }
}

module.exports = scrapeParadise;
