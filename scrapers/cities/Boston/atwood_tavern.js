/**
 * Atwood's Tavern Boston Events Scraper
 * URL: https://www.atwoodstavern.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAtwoodTavern(city = 'Boston') {
  console.log('🍺 Scraping Atwood Tavern...');
  ...
  try {
    ...
    await page.goto('https://www.atwoodstavern.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Atwood Tavern'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Atwood Tavern events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Atwood Tavern error:', error.message);
    return [];
  }
}

module.exports = scrapeAtwoodTavern;
