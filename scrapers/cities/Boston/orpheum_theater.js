/**
 * Orpheum Theatre Boston Events Scraper
 * URL: https://www.orpheum-theater.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOrpheumBoston(city = 'Boston') {
  console.log('🎭 Scraping Orpheum Boston...');
  ...
  try {
    ...
    await page.goto('https://www.orpheum-theater.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Orpheum Boston'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Orpheum Boston events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Orpheum Boston error:', error.message);
    return [];
  }
}

module.exports = scrapeOrpheumBoston;
