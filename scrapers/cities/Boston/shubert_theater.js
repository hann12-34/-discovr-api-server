/**
 * Shubert Theatre Boston Events Scraper
 * URL: https://bochcenter.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeShubertBoston(city = 'Boston') {
  console.log('🎭 Scraping Shubert Theatre...');
  ...
  try {
    ...
    await page.goto('https://bochcenter.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Shubert Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Shubert Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Shubert Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeShubertBoston;
