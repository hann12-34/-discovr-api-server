/**
 * Paradise Rock Club Final Boston Events Scraper
 * URL: https://www.crossroadspresents.com/paradise-rock-club
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeParadiseFinal(city = 'Boston') {
  console.log('🎸 Scraping Paradise Final...');
  ...
  try {
    ...
    await page.goto('https://www.crossroadspresents.com/paradise-rock-club', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Boston',
        category: 'Nightlife',
        source: 'Paradise Final'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Paradise Final events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Paradise Final error:', error.message);
    return [];
  }
}

module.exports = scrapeParadiseFinal;
