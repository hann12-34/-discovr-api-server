...
/**
 * Calgary Folk Music Festival Events Scraper
 * Scrapes events from Calgary Folk Music Festival
 * URL: https://calgaryfolkfest.com
 */
class CalgaryFolkFestivalEvents {
    constructor() {
        this.baseUrl = 'https://calgaryfolkfest.com';
        this.eventsUrl = 'https://calgaryfolkfest.com/events';
        this.source = 'Calgary Folk Music Festival';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Calgary Folk Festival
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Calgary Folk Music Festival',
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .artist-name, .performer, .artist, h1, h2, h3, h4, a[href*="artist"]').first().text()
        );
        ...
        if (titleLower.includes('folk')) {
            ...
            category = 'Workshop';
        } else if (titleLower.includes('jam') || titleLower.includes('session')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Calgary Folk Music Festival`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with folk/music content
                eventElements = $('[class*="event"], [class*="show"], [class*="artist"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('folk') || text.includes('music') || text.includes('artist') ||
                           text.includes('performance') || text.includes('event') || text.includes('concert');
                };
            }
...
module.exports = CalgaryFolkFestivalEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-calgary-folk-festival.js Toronto');
    process.exit(1);
  }
        const scraper = new CalgaryFolkFestivalEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CALGARY FOLK FESTIVAL TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CalgaryFolkFestivalEvents');
    }
};