...
/**
 * Sled Island Music Festival Events Scraper
 * Scrapes events from Sled Island Music & Arts Festival
 * URL: https://www.sledisland.com
 */
class SledIslandEvents {
    constructor() {
        this.baseUrl = 'https://sledisland.com';
        this.eventsUrl = 'https://sledisland.com/festival';
        this.source = 'Sled Island Music Festival';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Sled Island venues (multiple venues)
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Sled Island Festival Venue',
            address: 'Various venues across Calgary',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .artist, .band, h1, h2, h3, h4, a[href*="show"]').first().text()
        );

        if (!title) return null;

        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date, .show-date, .day').first().text();
        ...
        // Most Sled Island events are music
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Sled Island Music Festival`,
            date: eventDate,
...
module.exports = SledIslandEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-sled-island.js Toronto');
    process.exit(1);
  }
        const scraper = new SledIslandEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('SLED ISLAND FESTIVAL TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in SledIslandEvents');
    }
};