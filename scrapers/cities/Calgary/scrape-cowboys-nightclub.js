...
/**
 * Cowboys Nightclub Events Scraper
 * Scrapes events from Cowboys Nightclub
 * URL: https://cowboysmusichall.com
 */
class CowboysNightclubEvents {
    constructor() {
        this.baseUrl = 'https://cowboysmusichall.com';
        this.eventsUrl = 'https://cowboysmusichall.com/events-tickets';
    ...
    /**
     * Get default coordinates for Cowboys Nightclub
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Cowboys Nightclub',
            address: 'Calgary, AB',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .party-title, .night-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('country')) {
            category = 'Country Night';
        } else if (titleLower.includes('line dancing')) {
        ...
        } else if (titleLower.includes('theme')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Cowboys Nightclub`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with nightlife content
                eventElements = $('[class*="event"], [class*="party"], [class*="night"], [class*="dance"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('party') || text.includes('night') || text.includes('dance') ||
                           text.includes('dj') || text.includes('live') || text.includes('show') ||
                           text.includes('country') || text.includes('music') || text.includes('entertainment');
                };
            }
...
module.exports = CowboysNightclubEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-cowboys-nightclub.js Toronto');
    process.exit(1);
  }
        const scraper = new CowboysNightclubEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('COWBOYS NIGHTCLUB TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CowboysNightclubEvents');
    }
};