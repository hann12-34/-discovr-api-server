...
/**
 * GlobalFest Events Scraper
 * Scrapes events from GlobalFest Calgary
 * URL: https://globalfest.ca
 */
class GlobalFestEvents {
    constructor() {
        this.baseUrl = 'https://globalfest.ca';
        this.eventsUrl = 'https://globalfest.ca/events';
    ...
    /**
     * Get default coordinates for GlobalFest
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .fireworks-title, .performance-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('fireworks')) {
            category = 'Fireworks';
        } else if (titleLower.includes('festival')) {
            category = 'Festival';
        } else if (titleLower.includes('cultural')) {
            category = 'Cultural';
        } else if (titleLower.includes('international')) {
            ...
            category = 'Food';
        } else if (titleLower.includes('arts')) {
            category = 'Arts';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at GlobalFest`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with festival content
                eventElements = $('[class*="event"], [class*="show"], [class*="performance"], [class*="fireworks"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('festival') || text.includes('fireworks') || text.includes('performance') ||
                           text.includes('cultural') || text.includes('celebration') || text.includes('show') ||
                           text.includes('international') || text.includes('global');
                };
            }
...
module.exports = GlobalFestEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-globalfest.js Toronto');
    process.exit(1);
  }
        const scraper = new GlobalFestEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('GLOBALFEST TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in GlobalFestEvents');
    }
};