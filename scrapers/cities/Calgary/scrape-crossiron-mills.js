...
/**
 * CrossIron Mills Events Scraper
 * Scrapes events from CrossIron Mills
 * URL: https://www.crossironmills.com
 */
class CrossIronMillsEvents {
    constructor() {
        this.baseUrl = 'https://www.crossironmills.com';
        this.eventsUrl = 'https://www.crossironmills.com/en/events';
    ...
    /**
     * Get default coordinates for CrossIron Mills
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .mall, .store').first();
        ...
        return {
            name: venueName || 'CrossIron Mills',
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .promotion-title, .sale-title, .activity-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );

        if (!title) return null;

        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date, .promotion-date, .sale-date').first().text();
        ...
        // Extract price
        const priceText = $event.find('.price, .cost, .ticket-price, .admission, .discount').text();
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at CrossIron Mills`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with shopping content
                eventElements = $('[class*="event"], [class*="promotion"], [class*="sale"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('sale') || text.includes('promotion') || text.includes('event') ||
                           text.includes('shopping') || text.includes('offer') || text.includes('contest');
                };
            }
...
module.exports = CrossIronMillsEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-crossiron-mills.js Toronto');
    process.exit(1);
  }
        const scraper = new CrossIronMillsEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CROSSIRON MILLS TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CrossIronMillsEvents');
    }
};