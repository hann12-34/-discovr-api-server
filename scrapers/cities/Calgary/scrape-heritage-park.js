...
/**
 * Heritage Park Historical Village Events Scraper
 * Scrapes events from Heritage Park Historical Village
 * URL: https://www.heritagepark.ca
 */
class HeritageParkEvents {
    constructor() {
        this.baseUrl = 'https://www.heritagepark.ca';
        this.eventsUrl = 'https://www.heritagepark.ca/events';
        this.source = 'Heritage Park Historical Village';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Heritage Park
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place').first();
        ...
        return {
            name: venueName || 'Heritage Park Historical Village',
    ...
    extractEventDetails($, eventElement) {
        ...
        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date').first().text();
        ...
        if (titleLower.includes('family')) {
            ...
            category = 'Festival';
        } else if (titleLower.includes('educational') || titleLower.includes('school')) {
            category = 'Educational';
        } else if (titleLower.includes('heritage') || titleLower.includes('historical')) {
            category = 'Historical';
        } else if (titleLower.includes('train')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Heritage Park Historical Village`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with heritage-related content
                eventElements = $('[class*="event"], [class*="activity"], [class*="program"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('event') || text.includes('program') || text.includes('activity') ||
                           text.includes('heritage') || text.includes('historical') || text.includes('train');
                };
            }
...
module.exports = HeritageParkEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-heritage-park.js Toronto');
    process.exit(1);
  }
        const scraper = new HeritageParkEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('HERITAGE PARK TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in HeritageParkEvents');
    }
};