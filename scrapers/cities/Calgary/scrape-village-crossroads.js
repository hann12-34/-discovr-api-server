...
/**
 * Village at Crossroads Events Scraper
 * Scrapes events from Village at Crossroads
 * URL: https://villageatcrossroads.com
 */
class VillageCrossroadsEvents {
    constructor() {
        this.baseUrl = 'https://villageatcrossroads.com';
        this.eventsUrl = 'https://villageatcrossroads.com/events';
        this.source = 'Village at Crossroads';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Village at Crossroads
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Village at Crossroads',
            address: 'Calgary, AB',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Village at Crossroads`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with shopping/community content
                eventElements = $('[class*="event"], [class*="activity"], [class*="promotion"], [class*="sale"]').filter(function() {
                    ...
                    return text.includes('event') || text.includes('activity') || text.includes('sale') ||
                           text.includes('shopping') || text.includes('community') || text.includes('family') ||
                           text.includes('market') || text.includes('festival') || text.includes('celebration');
                };
            }
...
module.exports = VillageCrossroadsEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-village-crossroads.js Toronto');
    process.exit(1);
  }
        const scraper = new VillageCrossroadsEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('VILLAGE AT CROSSROADS TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in VillageCrossroadsEvents');
    }
};