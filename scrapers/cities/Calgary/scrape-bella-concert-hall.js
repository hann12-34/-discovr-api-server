...
/**
 * Bella Concert Hall Events Scraper
 * Scrapes events from Bella Concert Hall at Mount Royal University
 * URL: https://mru.ca/bella-concert-hall
 */
class BellaConcertHallEvents {
    constructor() {
        this.baseUrl = 'https://mru.ca';
        this.eventsUrl = 'https://mru.ca/bella-concert-hall/performances';
    ...
    /**
     * Get default coordinates for Bella Concert Hall
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Bella Concert Hall',
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .concert-title, .performer, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('classical') || titleLower.includes('orchestra') || titleLower.includes('symphony')) {
            ...
            category = 'Jazz';
        } else if (titleLower.includes('choir') || titleLower.includes('choral')) {
            category = 'Choral';
        } else if (titleLower.includes('student') || titleLower.includes('recital')) {
        ...
        try {
            ...
            if (eventElements.length === 0) {
                ...
                eventElements = $('[class*="event"], [class*="show"], [class*="concert"]').filter(function() {
                    ...
                    return text.includes('concert') || text.includes('performance') || text.includes('recital') ||
                           text.includes('music') || text.includes('event') || text.includes('show');
                };
            }
...
module.exports = BellaConcertHallEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-bella-concert-hall.js Toronto');
    process.exit(1);
  }
        const scraper = new BellaConcertHallEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('BELLA CONCERT HALL TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in BellaConcertHallEvents');
    }
};