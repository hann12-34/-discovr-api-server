...
/**
 * Edworthy Park Events Scraper
 * Scrapes events from Edworthy Park
 * URL: https://www.calgary.ca/csps/parks/locations/edworthy-park.html
 */
class EdworthyParkEvents {
    constructor() {
        this.baseUrl = 'https://www.calgary.ca';
        this.eventsUrl = 'https://www.calgary.ca/csps/parks/locations/edworthy-park.html';
    ...
    /**
     * Get default coordinates for Edworthy Park
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Edworthy Park',
            address: '5050 Spruce Dr SW, Calgary, AB T3C 3X9',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .park-title, .activity-title, .trail-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );

        if (!title) return null;

        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date, .park-date').first().text();
        ...
        if (titleLower.includes('hiking')) {
            category = 'Hiking';
        } else if (titleLower.includes('trail')) {
            category = 'Trail';
        } else if (titleLower.includes('walking')) {
            category = 'Walking';
        } else if (titleLower.includes('running')) {
            category = 'Running';
        } else if (titleLower.includes('jogging')) {
            category = 'Jogging';
        } else if (titleLower.includes('biking')) {
            category = 'Biking';
        } else if (titleLower.includes('cycling')) {
            category = 'Cycling';
        } else if (titleLower.includes('mountain bike')) {
            ...
            category = 'Picnic';
        } else if (titleLower.includes('barbecue') || titleLower.includes('bbq')) {
            category = 'BBQ';
            ...
            category = 'Sports';
        } else if (titleLower.includes('disc golf')) {
            category = 'Disc Golf';
        } else if (titleLower.includes('frisbee')) {
            category = 'Frisbee';
        } else if (titleLower.includes('playground')) {
            category = 'Playground';
        } else if (titleLower.includes('river')) {
            category = 'River';
        } else if (titleLower.includes('bow river')) {
            category = 'Bow River';
            ...
            category = 'Kids';
        } else if (titleLower.includes('dog')) {
            category = 'Dog Park';
        } else if (titleLower.includes('pet')) {
            category = 'Pet';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Edworthy Park`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with park/outdoor content
                eventElements = $('[class*="event"], [class*="activity"], [class*="park"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('park') || text.includes('outdoor') || text.includes('trail') ||
                           text.includes('hiking') || text.includes('biking') || text.includes('nature');
                };
            }
...
module.exports = EdworthyParkEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-edworthy-park.js Toronto');
    process.exit(1);
  }
        const scraper = new EdworthyParkEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('EDWORTHY PARK TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in EdworthyParkEvents');
    }
};