...
/**
 * Grey Eagle Resort and Casino Events Scraper
 * Scrapes events from Grey Eagle Resort and Casino
 * URL: https://www.greyeaglecasino.ca
 */
class GreyEagleCasinoEvents {
    constructor() {
        this.baseUrl = 'https://www.greyeaglecasino.ca';
        this.eventsUrl = 'https://www.greyeaglecasino.ca/entertainment/events';
        this.source = 'Grey Eagle Resort and Casino';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Grey Eagle Resort and Casino
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Grey Eagle Resort and Casino',
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .artist, .performer, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('concert') || titleLower.includes('music') || titleLower.includes('band')) {
            ...
            category = 'Comedy';
        } else if (titleLower.includes('casino') || titleLower.includes('poker') || titleLower.includes('gaming')) {
            category = 'Gaming';
        } else if (titleLower.includes('dining') || titleLower.includes('restaurant')) {
            category = 'Food & Drink';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Grey Eagle Resort and Casino`,
            date: eventDate,
            ...
            if (eventElements.length === 0) {
                ...
                eventElements = $('[class*="event"], [class*="show"], [class*="performance"]').filter(function() {
                    ...
                    return text.includes('show') || text.includes('performance') || text.includes('concert') ||
                           text.includes('music') || text.includes('event') || text.includes('entertainment');
                };
            }
...
module.exports = GreyEagleCasinoEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-grey-eagle-casino.js Toronto');
    process.exit(1);
  }
        const scraper = new GreyEagleCasinoEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('GREY EAGLE CASINO TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in GreyEagleCasinoEvents');
    }
};