...
class CalgaryZooEvents {
    constructor() {
        ...
        this.eventsUrl = 'https://www.calgaryzoo.com/events';
    ...
    /**
     * Get default coordinates for Calgary Zoo
    ...
    extractEventDetails($, eventElement) {
        ...
        if (titleLower.includes('zoo') || titleLower.includes('animal')) {
            category = 'Wildlife';
        } else if (titleLower.includes('educational') || titleLower.includes('learn')) {
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with zoo-related content
                eventElements = $('[class*="event"], [class*="activity"], [class*="program"]').filter(function() {
                    ...
                    return text.includes('event') || text.includes('program') || text.includes('activity') ||
                           text.includes('exhibit') || text.includes('zoo') || text.includes('animal');
                };
            }
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-calgary-zoo.js Toronto');
    process.exit(1);
  }
        const scraper = new CalgaryZooEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CALGARY ZOO TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CalgaryZooEvents');
    }
};