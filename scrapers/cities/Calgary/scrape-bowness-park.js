...
/**
 * Bowness Park Events Scraper
 * Scrapes events from Bowness Park
 * URL: https://www.calgary.ca/csps/parks/locations/bowness-park.html
 */
class BownessParkEvents {
    constructor() {
        this.baseUrl = 'https://www.calgary.ca';
        this.eventsUrl = 'https://www.calgary.ca/csps/parks/locations/bowness-park.html';
        this.source = 'Bowness Park';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Bowness Park
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .park, .lagoon').first();
        ...
        return {
            name: venueName || 'Bowness Park',
            address: '8900 48 Ave NW, Calgary, AB T3B 2A7',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .park-title, .activity-title, .seasonal-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('skating')) {
            category = 'Skating';
        } else if (titleLower.includes('ice')) {
            category = 'Ice';
        } else if (titleLower.includes('winter')) {
            category = 'Winter';
        } else if (titleLower.includes('summer')) {
            category = 'Summer';
        } else if (titleLower.includes('lagoon')) {
            category = 'Lagoon';
        } else if (titleLower.includes('boat')) {
            category = 'Boating';
        } else if (titleLower.includes('paddle')) {
            category = 'Paddling';
        } else if (titleLower.includes('canoe')) {
            category = 'Canoeing';
        } else if (titleLower.includes('kayak')) {
            category = 'Kayaking';
            ...
            category = 'Sports';
        } else if (titleLower.includes('volleyball')) {
            category = 'Volleyball';
            ...
            category = 'Canada Day';
        } else if (titleLower.includes('stampede')) {
            category = 'Stampede';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Bowness Park`,
            date: eventDate,
        ...
        try {
            ...
            if (eventElements.length === 0) {
                ...
                eventElements = $('[class*="event"], [class*="activity"], [class*="park"]').filter(function() {
                    ...
                    return text.includes('park') || text.includes('outdoor') || text.includes('skating') ||
                           text.includes('lagoon') || text.includes('activity') || text.includes('seasonal');
                };
            }
...
module.exports = BownessParkEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-bowness-park.js Toronto');
    process.exit(1);
  }
        const scraper = new BownessParkEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('BOWNESS PARK TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in BownessParkEvents');
    }
};