...
/**
 * WinSport Events Scraper
 * Scrapes events from WinSport (Canada Olympic Park)
 * URL: https://winsport.ca
 */
class WinSportEvents {
    constructor() {
        this.baseUrl = 'https://winsport.ca';
        this.eventsUrl = 'https://winsport.ca/visit/events';
    ...
    /**
     * Get default coordinates for WinSport
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .facility').first();
        ...
        return {
            name: venueName || 'WinSport - Canada Olympic Park',
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .program-title, .activity-title, .course-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );

        if (!title) return null;

        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date, .program-date, .course-date').first().text();
        ...
        // Extract price
        const priceText = $event.find('.price, .cost, .fee, .registration-fee').text();
        ...
        if (titleLower.includes('ski') || titleLower.includes('skiing')) {
            category = 'Skiing';
        } else if (titleLower.includes('snowboard')) {
            category = 'Snowboarding';
        } else if (titleLower.includes('bobsled')) {
            category = 'Bobsled';
        } else if (titleLower.includes('luge')) {
            category = 'Luge';
        } else if (titleLower.includes('hockey')) {
            category = 'Hockey';
        } else if (titleLower.includes('mountain bike') || titleLower.includes('mtb')) {
            category = 'Mountain Biking';
        } else if (titleLower.includes('zipline')) {
            category = 'Zipline';
        } else if (titleLower.includes('camp')) {
            category = 'Camp';
        } else if (titleLower.includes('lesson')) {
            category = 'Lesson';
        } else if (titleLower.includes('clinic')) {
            category = 'Clinic';
        } else if (titleLower.includes('race') || titleLower.includes('competition')) {
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with sport content
                eventElements = $('[class*="event"], [class*="program"], [class*="activity"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('ski') || text.includes('program') || text.includes('lesson') ||
                           text.includes('camp') || text.includes('event') || text.includes('sport');
                };
            }
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-winsport.js Toronto');
    process.exit(1);
  }
        const scraper = new WinSportEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('WINSPORT TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in WinSportEvents');
    }
};