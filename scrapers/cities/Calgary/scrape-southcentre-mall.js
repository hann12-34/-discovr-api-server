...
/**
 * Southcentre Mall Events Scraper
 * Scrapes events from Southcentre Mall
 * URL: https://southcentremall.com
 */
class SouthcentreMallEvents {
    constructor() {
        this.baseUrl = 'https://southcentremall.com';
        this.eventsUrl = 'https://southcentremall.com/events';
        this.source = 'Southcentre Mall';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Southcentre Mall
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .store').first();
        ...
        return {
            name: venueName || 'Southcentre Mall',
            address: 'Calgary, AB',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .activity-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        if (titleLower.includes('shopping')) {
            ...
            category = 'Kids Event';
        } else if (titleLower.includes('santa')) {
            category = 'Santa Event';
        } else if (titleLower.includes('easter')) {
            ...
            category = 'Contest';
        } else if (titleLower.includes('giveaway')) {
            category = 'Giveaway';
        } else if (titleLower.includes('promotion')) {
            category = 'Promotion';
        } else if (titleLower.includes('launch')) {
            category = 'Store Launch';
        } else if (titleLower.includes('opening')) {
            category = 'Store Opening';
        } else if (titleLower.includes('beauty')) {
            category = 'Beauty Event';
        } else if (titleLower.includes('makeup')) {
            category = 'Makeup Event';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Southcentre Mall`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with mall/shopping content
                eventElements = $('[class*="event"], [class*="activity"], [class*="promotion"], [class*="sale"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('event') || text.includes('activity') || text.includes('sale') ||
                           text.includes('shopping') || text.includes('family') || text.includes('promotion') ||
                           text.includes('contest') || text.includes('launch') || text.includes('fashion');
                };
            }
...
module.exports = SouthcentreMallEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-southcentre-mall.js Toronto');
    process.exit(1);
  }
        const scraper = new SouthcentreMallEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('SOUTHCENTRE MALL TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in SouthcentreMallEvents');
    }
};