...
class CalgaryStampedeEvents {
    constructor() {
        this.baseUrl = 'https://www.calgarystampede.com';
        this.eventsUrl = 'https://www.calgarystampede.com/stampede';
    ...
    /**
     * Get default coordinates for Calgary Stampede
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: this.city, // Use city name for proper filtering (Calgary)
    ...
    extractEventDetails($, eventElement) {
        ...
        if (titleLower.includes('rodeo')) {
            ...
            category = 'Food & Drink';
        } else if (titleLower.includes('midway') || titleLower.includes('ride')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Calgary Stampede`,
            date: eventDate,
            venue: venue,
            city: this.city,
            province: this.province,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with stampede-related content
                eventElements = $('[class*="event"], [class*="show"], [class*="activity"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('show') || text.includes('rodeo') || text.includes('concert') ||
                           text.includes('music') || text.includes('event') || text.includes('stampede');
                };
            }
...
async function scrape(city) {
    const scraper = new CalgaryStampedeEvents();
    return await scraper.scrapeEvents();
}

module.exports = { CalgaryStampedeEvents, scrape };
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-calgary-stampede.js Toronto');
    process.exit(1);
  }
        const scraper = new CalgaryStampedeEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CALGARY STAMPEDE TEST RESULTS');
...
module.exports.CalgaryStampedeEvents = CalgaryStampedeEvents;