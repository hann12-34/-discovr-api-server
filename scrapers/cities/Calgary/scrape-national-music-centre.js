...
/**
 * National Music Centre Events Scraper
 * Scrapes events from National Music Centre
 * URL: https://studiobell.ca
 */
class NationalMusicCentreEvents {
    constructor() {
    ...
    /**
     * Get default coordinates for National Music Centre
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .studio').first();
        ...
        return {
            name: venueName || 'National Music Centre',
            address: 'Calgary, AB',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        if (titleLower.includes('concert')) {
            ...
            category = 'Symphony';
        } else if (titleLower.includes('orchestral')) {
            category = 'Orchestral';
        ...
        } else if (titleLower.includes('electronic')) {
        ...
        } else if (titleLower.includes('awards')) {
            category = 'Awards';
        } else if (titleLower.includes('recording')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at National Music Centre`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                eventElements = $('[class*="event"], [class*="concert"], [class*="performance"], [class*="music"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('concert') || text.includes('performance') || text.includes('music') ||
                           text.includes('show') || text.includes('recital') || text.includes('live') ||
                           text.includes('jazz') || text.includes('classical') || text.includes('workshop');
                };
            }
...
module.exports = NationalMusicCentreEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-national-music-centre.js Toronto');
    process.exit(1);
  }
        const scraper = new NationalMusicCentreEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('NATIONAL MUSIC CENTRE TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in NationalMusicCentreEvents');
    }
};