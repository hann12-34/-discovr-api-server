...
/**
 * Bow Valley College Events Scraper
 * Scrapes events from Bow Valley College
 * URL: https://www.bowvalleycollege.ca
 */
class BowValleyCollegeEvents {
    constructor() {
        this.baseUrl = 'https://www.bowvalleycollege.ca';
        this.eventsUrl = 'https://www.bowvalleycollege.ca/events';
    ...
    /**
     * Get default coordinates for Bow Valley College
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .campus, .room').first();
        ...
        return {
            name: venueName || 'Bow Valley College',
    ...
    extractEventDetails($, eventElement) {
        ...
        if (titleLower.includes('course')) {
            ...
            category = 'English';
        } else if (titleLower.includes('esl')) {
            category = 'ESL';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Bow Valley College`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with educational content
                eventElements = $('[class*="event"], [class*="course"], [class*="program"]').filter(function() {
                    ...
                    return text.includes('course') || text.includes('program') || text.includes('workshop') ||
                           text.includes('training') || text.includes('seminar') || text.includes('education');
                };
            }
...
module.exports = BowValleyCollegeEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-bow-valley-college.js Toronto');
    process.exit(1);
  }
        const scraper = new BowValleyCollegeEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('BOW VALLEY COLLEGE TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in BowValleyCollegeEvents');
    }
};