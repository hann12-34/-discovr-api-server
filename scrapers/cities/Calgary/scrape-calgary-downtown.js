/**
 * Calgary Downtown Events Scraper
 * URL: https://www.calgarydowntown.com/events
...
async function scrape(city = 'Calgary') {
  console.log('🏙️ Scraping Calgary Downtown events...');

  try {
    const response = await axios.get('https://www.calgarydowntown.com/events', {
    ...
    $('a[href*="/event"], article, [class*="event"], .card').each((i, el) => {
      const $e = $(el);
      ...
      if (/menu|filter|search|login|view all|see more/i.test(title)) return;
      ...
      if (url && !url.startsWith('http')) {
        url = 'https://www.calgarydowntown.com' + url;
      }
      ...
      if (img.length) {
        ...
        if (image && image.includes('datocms-assets.com')) {
          // Get larger version
          image = image.replace(/max-w=\d+/, 'max-w=800');
        }
      ...
      if (title && image && eventDate) {
        events.push({
          ...
          city: 'Calgary',
          category: 'Community',
          source: 'Calgary Downtown'
        });
      }
    });

    // Fetch dates from event pages
    ...
    console.log(`✅ Calgary Downtown: ${events.length} events with images`);
    return events;

  } catch (error) {
    console.error('  ⚠️ Calgary Downtown error:', error.message);
    return [];
  }
}
