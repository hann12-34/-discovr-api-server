...
/**
 * Comedy Cave Events Scraper
 * Scrapes events from Comedy Cave Calgary
 * URL: https://comedycave.ca
 */
class ComedyCaveEvents {
    constructor() {
        this.baseUrl = 'https://comedycave.ca';
        this.eventsUrl = 'https://comedycave.ca/shows';
    ...
    /**
     * Get default coordinates for Comedy Cave
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .room').first();
        ...
        return {
            name: venueName || 'Comedy Cave',
            address: '1126 Kensington Rd NW, Calgary, AB T2N 3P1',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .comedian-name, .performer, .artist, h1, h2, h3, h4, a[href*="show"]').first().text()
        );
        ...
        const description = this.cleanText(
            $event.find('.description, .summary, .excerpt, .content, p, .show-description, .bio').first().text()
        );
        ...
        if (titleLower.includes('stand-up') || titleLower.includes('standup')) {
            category = 'Stand-up Comedy';
        } else if (titleLower.includes('improv')) {
            category = 'Improv Comedy';
        } else if (titleLower.includes('open mic')) {
            category = 'Open Mic';
        } else if (titleLower.includes('showcase')) {
            category = 'Comedy Showcase';
        } else if (titleLower.includes('workshop')) {
            category = 'Comedy Workshop';
        } else if (titleLower.includes('roast')) {
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Comedy Cave`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with comedy content
                eventElements = $('[class*="event"], [class*="show"], [class*="comedy"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('comedy') || text.includes('comedian') || text.includes('show') ||
                           text.includes('performance') || text.includes('event') || text.includes('stand-up');
                };
            }
...
module.exports = ComedyCaveEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-comedy-cave.js Toronto');
    process.exit(1);
  }
        const scraper = new ComedyCaveEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('COMEDY CAVE TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in ComedyCaveEvents');
    }
};