...
/**
 * Craft Beer Market Events Scraper
 * Scrapes events from Craft Beer Market
 * URL: https://craftbeermarket.ca
 */
class CraftBeerMarketEvents {
    constructor() {
        this.baseUrl = 'https://craftbeermarket.ca';
        this.eventsUrl = 'https://craftbeermarket.ca/events';
    ...
    /**
     * Get default coordinates for Craft Beer Market
    ...
    extractVenueInfo($, eventElement) {
        const venueElement = $(eventElement).find('.venue, .location, .where, .place, .restaurant').first();
        ...
        return {
            name: venueName || 'Craft Beer Market',
            address: 'Calgary, AB',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .show-title, .beer-event-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );
        ...
        } else if (titleLower.includes('menu')) {
            category = 'Menu Event';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} at Craft Beer Market`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with restaurant/beer content
                eventElements = $('[class*="event"], [class*="beer"], [class*="tasting"], [class*="show"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('beer') || text.includes('tasting') || text.includes('craft') ||
                           text.includes('show') || text.includes('music') || text.includes('live') ||
                           text.includes('trivia') || text.includes('event') || text.includes('food');
                };
            }
...
module.exports = CraftBeerMarketEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-craft-beer-market.js Toronto');
    process.exit(1);
  }
        const scraper = new CraftBeerMarketEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CRAFT BEER MARKET TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CraftBeerMarketEvents');
    }
};