...
class EventbriteCalgaryEvents {
    ...
    extractEventDetails($, eventElement) {
        ...
        const dateText = $event.find('[data-realEvent-date, .date').first().text();
        const eventDate = this.parseDate(dateText);
        const description = this.cleanText($event.find('.event-card__description, .event-description, p').first().text());
        const price = this.cleanText($event.find('[data-realEvent-card__price, .price').text()) || 'Check website for pricing';
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-eventbrite-calgary.js Toronto');
    process.exit(1);
  }
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in EventbriteCalgaryEvents');
    }
};