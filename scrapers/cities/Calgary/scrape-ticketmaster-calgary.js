...
class TicketmasterCalgaryEvents {
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText($event.find('[data-realEvent-name, h3, h2, h1').first().text());

        if (!title) return null;

        const dateText = $event.find('[data-realEvent-date, .date, .time').first().text();
        const eventDate = this.parseDate(dateText);
        const description = this.cleanText($event.find('.event-description, .description, p').first().text());
        const price = this.cleanText($event.find('[data-realEvent-price"], .price').text()) || 'Check website for pricing';
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-ticketmaster-calgary.js Toronto');
    process.exit(1);
  }
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in TicketmasterCalgaryEvents');
    }
};