...
/**
 * Calgary Philharmonic Orchestra Events Scraper
 * Scrapes events from Calgary Philharmonic Orchestra
 * URL: https://calgaryphil.com
 */
class CalgaryPhilharmonicEvents {
    constructor() {
        this.baseUrl = 'https://calgaryphil.com';
        this.eventsUrl = 'https://calgaryphil.com/events';
        this.source = 'Calgary Philharmonic Orchestra';
        this.city = 'Calgary';
        this.province = 'AB';
    }

    /**
     * Get default coordinates for Calgary Philharmonic
    ...
    extractVenueInfo($, eventElement) {
        ...
        return {
            name: venueName || 'Jack Singer Concert Hall',
            address: 'Arts Commons, 205 8 Ave SE, Calgary, AB T2G 0K7',
            city: this.city,
    ...
    extractEventDetails($, eventElement) {
        ...
        const title = this.cleanText(
            $event.find('.title, .event-title, .concert-title, .performance-title, .program-title, h1, h2, h3, h4, a[href*="event"]').first().text()
        );

        if (!title) return null;

        // Extract date
        const dateText = $event.find('.date, .when, .time, .event-date, .concert-date, .performance-date').first().text();
        ...
        let category = 'Classical Music';
        const titleLower = title.toLowerCase();
        if (titleLower.includes('symphony')) {
            category = 'Symphony';
        } else if (titleLower.includes('concert')) {
            category = 'Concert';
        } else if (titleLower.includes('masterworks')) {
            category = 'Masterworks';
        } else if (titleLower.includes('pops')) {
            category = 'Pops';
        } else if (titleLower.includes('family')) {
            category = 'Family Concert';
        ...
        return {
            ...
            name: title,
            title: title,
            description: description || `${title} with Calgary Philharmonic Orchestra`,
            date: eventDate,
    ...
    async scrapeEvents() {
        try {
            ...
            if (eventElements.length === 0) {
                ...
                // Try finding events by looking for elements with orchestral content
                eventElements = $('[class*="event"], [class*="concert"], [class*="performance"]').filter(function() {
                    const text = $(this).text().toLowerCase();
                    return text.includes('concert') || text.includes('symphony') || text.includes('orchestra') ||
                           text.includes('performance') || text.includes('event') || text.includes('masterworks');
                };
            }
...
module.exports = CalgaryPhilharmonicEvents;
...
if (require.main === module) {
  ...
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-calgary-philharmonic.js Toronto');
    process.exit(1);
  }
        const scraper = new CalgaryPhilharmonicEvents();
        const events = await scraper.scrapeEvents();
        console.log('\n' + '='.repeat(50));
        console.log('CALGARY PHILHARMONIC TEST RESULTS');
...
module.exports = async (city) => {
    ...
    if (typeof scraper.scrape === 'function') {
        ...
        throw new Error('No scrape method found in CalgaryPhilharmonicEvents');
    }
};