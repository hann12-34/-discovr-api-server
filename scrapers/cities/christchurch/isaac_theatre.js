/**
 * Isaac Theatre Royal Christchurch Events Scraper
 * URL: https://www.isaactheatreroyal.co.nz/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeIsaacTheatre(city = 'Christchurch') {
  console.log('🎭 Scraping Isaac Theatre Royal...');
  ...
  try {
    ...
    await page.goto('https://www.isaactheatreroyal.co.nz/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '145 Gloucester St, Christchurch Central, Christchurch 8011',
          city: 'Christchurch'
        },
        latitude: -43.5315,
        longitude: 172.6370,
        city: 'Christchurch',
        category: 'Arts',
        source: 'Isaac Theatre Royal'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Isaac Theatre Royal events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Isaac Theatre Royal error:', error.message);
    return [];
  }
}

module.exports = scrapeIsaacTheatre;
