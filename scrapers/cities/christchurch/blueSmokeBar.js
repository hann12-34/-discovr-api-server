/**
 * Blue Smoke Bar Christchurch Events Scraper
 * Live music venue
 * URL: https://www.bluesmoke.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBlueSmokeBar(city = 'Christchurch') {
  console.log('🎸 Scraping Blue Smoke Bar Christchurch...');
  ...
  try {
    ...
    await page.goto('https://www.bluesmoke.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Christchurch',
        category: 'Nightlife',
        source: 'Blue Smoke Bar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Blue Smoke Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Blue Smoke Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeBlueSmokeBar;
