/**
 * World Buskers Festival Christchurch Events Scraper
 * URL: https://www.worldbuskersfestival.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWorldBuskers(city = 'Christchurch') {
  console.log('🎪 Scraping World Buskers Festival...');
  ...
  try {
    ...
    await page.goto('https://www.worldbuskersfestival.com/programme', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="show"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.worldbuskersfestival.com', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Christchurch',
        category: 'Festival',
        source: 'World Buskers'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} World Buskers events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  World Buskers error:', error.message);
    return [];
  }
}

module.exports = scrapeWorldBuskers;
