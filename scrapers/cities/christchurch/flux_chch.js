/**
 * Flux Christchurch Events Scraper
 * Popular nightclub in Christchurch
 * URL: https://fluxbar.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFluxChch(city = 'Christchurch') {
  console.log('⚡ Scraping Flux Christchurch...');
  ...
  try {
    ...
    await page.goto('https://fluxbar.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Flux',
          address: '90 Lichfield St, Christchurch 8011',
          city: 'Christchurch'
        },
        city: 'Christchurch',
        category: 'Nightlife',
        source: 'Flux Christchurch'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Flux events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Flux error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeFluxChch;
