/**
 * Christchurch NZ Official Events Scraper
 * URL: https://www.christchurchnz.com/visit/whats-on
 * Official Christchurch tourism events
 */
...
async function scrapeChristchurchNZEvents(city = 'Christchurch') {
  console.log('🏙️ Scraping Christchurch NZ Events...');
  let browser;
  ...
  try {
    ...
    await page.goto('https://www.christchurchnz.com/visit/whats-on', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(3000);

    const eventLinks = await page.evaluate(() => {
      const links = [];
      const anchors = document.querySelectorAll('a[href*="/visit/whats-on/"], a[href*="/event/"]');
      anchors.forEach(a => {
        const href = a.getAttribute('href');
        if (href && !href.endsWith('/whats-on') && !href.endsWith('/whats-on/')) {
          const fullUrl = href.startsWith('http') ? href : `https://www.christchurchnz.com${href}`;
    ...
    for (const url of eventLinks) {
      try {
        ...
        const datePatterns = [
          ...
          /(\d{1,2})\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)(?:uary|ruary|ch|il|e|y|ust|tember|ober|ember)?\s*(\d{4})?/i,
        ...
        for (const pattern of datePatterns) {
          ...
          if (match) {
            if (!isNaN(parseInt(match[1])) && match[1].length <= 2) {
              ...
              const month = months[monthKey] || months[match[1].toLowerCase()];
        ...
        events.push({
          ...
          venue: {
            name: eventData.location ? eventData.location.split(',')[0].trim() : 'Christchurch',
            address: eventData.location || 'Christchurch, New Zealand',
            city: 'Christchurch'
          },
          latitude: -43.5321,
          longitude: 172.6362,
    ...
    console.log(`  ✅ Found ${unique.length} Christchurch NZ events`);
    return unique;

  } catch (error) {
    console.error(`  ⚠️ Christchurch NZ Events error: ${error.message}`);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeChristchurchNZEvents;
