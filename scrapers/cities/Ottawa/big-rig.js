/**
 * Big Rig Brewery Ottawa Events Scraper
 * URL: https://bigrigbrewery.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBigRig(city = 'Ottawa') {
  console.log('🎵 Scraping Big Rig Brewery...');
  ...
  try {
    ...
    await page.goto('https://bigrigbrewery.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Big Rig Brewery'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Big Rig Brewery events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Big Rig Brewery error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBigRig;
