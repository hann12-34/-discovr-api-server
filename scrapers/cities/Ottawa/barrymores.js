/**
 * Barrymore's Music Hall Ottawa Events Scraper
 * Live music venue
 * URL: https://www.barrymores.on.ca/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBarrymores(city = 'Ottawa') {
  console.log('🎸 Scraping Barrymore\'s Ottawa...');
  ...
  try {
    ...
    await page.goto('https://www.barrymores.on.ca/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Barrymore's Music Hall`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Barrymore's events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Barrymore\'s error:', error.message);
    return [];
  }
}

module.exports = scrapeBarrymores;
