/**
 * Ritual Nightclub Ottawa Events Scraper
 * Nightclub venue
 * URL: https://ritualnightclub.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRitual(city = 'Ottawa') {
  console.log('🎧 Scraping Ritual Ottawa...');
  ...
  try {
    ...
    await page.goto('https://ritualnightclub.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Club night at Ritual`,
        date: isoDate,
        startDate: new Date(isoDate + 'T22:00:00'),
        url: event.url,
        imageUrl: (event.imageUrl && event.imageUrl.startsWith('http') && !event.imageUrl.includes('data:image') && !event.imageUrl.includes('placeholder')) ? event.imageUrl : null,
        venue: {
          name: 'Ritual Nightclub',
          address: '137 Besserer Street, Ottawa ON K1N 6A7',
          city: 'Ottawa'
        },
        latitude: 45.4260,
        longitude: -75.6867,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Ritual events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Ritual error:', error.message);
    return [];
  }
}

module.exports = scrapeRitual;
