/**
 * Billings Estate Museum Ottawa Events Scraper
 * URL: https://ottawa.ca/billings
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBillingsEstate(city = 'Ottawa') {
  console.log('🎵 Scraping Billings Estate Museum...');
  ...
  try {
    ...
    await page.goto('https://ottawa.ca/billings/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Billings Estate Museum'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Billings Estate Museum events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Billings Estate Museum error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBillingsEstate;
