/**
 * Beckta Dining Ottawa Events Scraper
 * URL: https://beckta.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBeckta(city = 'Ottawa') {
  console.log('🎵 Scraping Beckta Dining...');
  ...
  try {
    ...
    await page.goto('https://beckta.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Beckta Dining'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Beckta Dining events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Beckta Dining error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBeckta;
