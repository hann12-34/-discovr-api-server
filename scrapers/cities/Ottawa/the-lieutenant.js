/**
 * The Lieutenants Pump Ottawa Events Scraper
 * URL: https://lieutenantspump.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTheLieutenant(city = 'Ottawa') {
  console.log('🎵 Scraping The Lieutenants Pump...');
  ...
  try {
    ...
    await page.goto('https://lieutenantspump.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'The Lieutenants Pump'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} The Lieutenants Pump events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  The Lieutenants Pump error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTheLieutenant;
