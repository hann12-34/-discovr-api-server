/**
 * Brasseurs du Montcalm Ottawa Events Scraper
 * URL: https://brassersdumontcalm.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBrasseurMont(city = 'Ottawa') {
  console.log('🎵 Scraping Brasseurs du Montcalm...');
  ...
  try {
    ...
    await page.goto('https://brassersdumontcalm.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Brasseurs du Montcalm'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Brasseurs du Montcalm events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Brasseurs du Montcalm error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBrasseurMont;
