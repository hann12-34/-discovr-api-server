/**
 * Kanata Theatre Ottawa Events Scraper
 * URL: https://kanatatheatre.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKanataTheatre(city = 'Ottawa') {
  console.log('🎵 Scraping Kanata Theatre...');
  ...
  try {
    ...
    await page.goto('https://kanatatheatre.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Kanata Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kanata Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Kanata Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeKanataTheatre;
