/**
 * Gladstone Theatre Ottawa Events Scraper
 * URL: https://www.glebetheatre.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeGladstoneTheatre(city = 'Ottawa') {
  console.log('🎵 Scraping Gladstone Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.glebetheatre.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Gladstone Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Gladstone Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Gladstone Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeGladstoneTheatre;
