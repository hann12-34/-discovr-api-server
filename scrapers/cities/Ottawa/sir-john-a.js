/**
 * Sir John A Pub Ottawa Events Scraper
 * URL: https://sirjohna.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSirJohnA(city = 'Ottawa') {
  console.log('🎵 Scraping Sir John A Pub...');
  ...
  try {
    ...
    await page.goto('https://sirjohna.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Sir John A Pub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sir John A Pub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Sir John A Pub error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSirJohnA;
