/**
 * Ottawa Little Theatre Ottawa Events Scraper
 * URL: https://ottawalittletheatre.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOttawaLittleTheatre(city = 'Ottawa') {
  console.log('🎵 Scraping Ottawa Little Theatre...');
  ...
  try {
    ...
    await page.goto('https://ottawalittletheatre.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Ottawa Little Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ottawa Little Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Ottawa Little Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOttawaLittleTheatre;
