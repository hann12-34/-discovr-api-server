/**
 * NAC Ottawa Events - Ottawa Events Scraper
...
async function scrapeNACOttawaEvents(city = 'Ottawa') {
  console.log('🎭 Scraping NAC Ottawa Events...');
  try {
    const resp = await axios.get('https://nac-cna.ca/en/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'NAC Ottawa Events', address: addr || '1 Elgin St, Ottawa', city: 'Ottawa' },
            latitude: 45.4231,
            longitude: -75.6927,
            city: 'Ottawa',
            category: 'Events',
            source: 'NAC Ottawa Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://nac-cna.ca/en/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://nac-cna.ca/en/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'NAC Ottawa Events', address: '1 Elgin St, Ottawa', city: 'Ottawa' },
            latitude: 45.4231,
            longitude: -75.6927,
            city: 'Ottawa',
            category: 'Events',
    ...
    console.log('  ✅ NAC Ottawa Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ NAC Ottawa Events error:', error.message);
    return [];
  }
}

module.exports = scrapeNACOttawaEvents;
