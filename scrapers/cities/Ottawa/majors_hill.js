/**
 * Major's Hill Park Ottawa Events Scraper
 * URL: https://ncc-ccn.gc.ca/places/majors-hill-park
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMajorsHill(city = 'Ottawa') {
  console.log('⛰️ Scraping Majors Hill Park...');
  ...
  try {
    ...
    await page.goto('https://ncc-ccn.gc.ca/places/majors-hill-park', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Mackenzie Avenue, Ottawa ON K1N 5A3',
          city: 'Ottawa'
        },
        latitude: 45.4267,
        longitude: -75.6951,
        city: 'Ottawa',
        category: 'Festival',
        source: 'Majors Hill Park'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Majors Hill events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Majors Hill error:', error.message);
    return [];
  }
}

module.exports = scrapeMajorsHill;
