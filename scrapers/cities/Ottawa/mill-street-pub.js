/**
 * Mill Street Brew Pub Ottawa Events Scraper
 * URL: https://millstreetbrewery.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMillStreetPub(city = 'Ottawa') {
  console.log('🎵 Scraping Mill Street Brew Pub...');
  ...
  try {
    ...
    await page.goto('https://millstreetbrewery.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Mill Street Brew Pub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Mill Street Brew Pub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Mill Street Brew Pub error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMillStreetPub;
