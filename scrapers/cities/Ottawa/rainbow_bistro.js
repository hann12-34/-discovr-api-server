/**
 * Rainbow Bistro Ottawa Events Scraper
 * URL: https://www.therainbow.ca/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRainbowBistro(city = 'Ottawa') {
  console.log('🌈 Scraping Rainbow Bistro...');
  ...
  try {
    ...
    await page.goto('https://www.therainbow.ca/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Rainbow Bistro',
          address: '76 Murray Street, Ottawa ON K1N 5M5',
          city: 'Ottawa'
        },
        latitude: 45.4295,
        longitude: -75.6927,
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Rainbow Bistro'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Rainbow Bistro events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Rainbow Bistro error:', error.message);
    return [];
  }
}

module.exports = scrapeRainbowBistro;
