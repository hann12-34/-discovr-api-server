/**
 * Sock and Buskin Theatre Ottawa Events Scraper
 * URL: https://sockandbuskin.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSockBuskin(city = 'Ottawa') {
  console.log('🎵 Scraping Sock and Buskin Theatre...');
  ...
  try {
    ...
    await page.goto('https://sockandbuskin.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Sock and Buskin Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sock and Buskin Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Sock and Buskin Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSockBuskin;
