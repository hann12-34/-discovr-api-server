/**
 * Lansdowne Park Ottawa Events Scraper
...
async function scrapeLansdowne(city = 'Ottawa') {
  console.log('🎵 Scraping Lansdowne Park...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Lansdowne Park'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Lansdowne Park events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Lansdowne Park error: ${error.message}`);
    return [];
  }
}
