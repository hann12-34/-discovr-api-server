/**
 * Live On Elgin Ottawa Events Scraper
 * URL: https://www.liveonelgin.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLiveOnElgin(city = 'Ottawa') {
  console.log('🎵 Scraping Live On Elgin...');
  ...
  try {
    ...
    await page.goto('https://www.liveonelgin.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Live On Elgin'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Live On Elgin events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Live On Elgin error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeLiveOnElgin;
