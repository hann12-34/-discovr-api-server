/**
 * Rideau Carleton Raceway Ottawa Events Scraper
 * URL: https://rideaucarletonraceway.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRideauCarleton(city = 'Ottawa') {
  console.log('🎵 Scraping Rideau Carleton Raceway...');
  ...
  try {
    ...
    await page.goto('https://rideaucarletonraceway.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Rideau Carleton Raceway'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Rideau Carleton Raceway events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Rideau Carleton Raceway error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRideauCarleton;
