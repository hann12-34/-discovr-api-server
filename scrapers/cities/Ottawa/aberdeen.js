/**
 * Aberdeen Pavilion Ottawa Events Scraper
...
async function scrapeAberdeen(city = 'Ottawa') {
  console.log('🎵 Scraping Aberdeen Pavilion Ottawa...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Festivals',
        source: 'Aberdeen Pavilion'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Aberdeen Pavilion events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Aberdeen Pavilion error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeAberdeen;
