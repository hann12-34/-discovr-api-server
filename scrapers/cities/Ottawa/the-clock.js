/**
 * Clock Tower Brew Pub Ottawa Events Scraper
 * URL: https://clocktower.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTheClock(city = 'Ottawa') {
  console.log('🎵 Scraping Clock Tower Brew Pub...');
  ...
  try {
    ...
    await page.goto('https://clocktower.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Clock Tower Brew Pub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Clock Tower Brew Pub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Clock Tower Brew Pub error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTheClock;
