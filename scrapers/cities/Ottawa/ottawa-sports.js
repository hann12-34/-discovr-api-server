/**
 * Ottawa Sports Hall of Fame Ottawa Events Scraper
 * URL: https://ottawasportshalloffame.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOttawaSports(city = 'Ottawa') {
  console.log('🎵 Scraping Ottawa Sports Hall of Fame...');
  ...
  try {
    ...
    await page.goto('https://ottawasportshalloffame.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Ottawa Sports Hall of Fame',
          address: 'Ottawa, ON',
          city: 'Ottawa'
        },
        latitude: 45.4215,
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Ottawa Sports Hall of Fame'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ottawa Sports Hall of Fame events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Ottawa Sports Hall of Fame error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOttawaSports;
