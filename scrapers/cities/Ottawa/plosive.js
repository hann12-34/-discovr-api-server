/**
 * Plosive Theatre Ottawa Events Scraper
 * URL: https://plosivetheatre.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePlosive(city = 'Ottawa') {
  console.log('🎵 Scraping Plosive Theatre...');
  ...
  try {
    ...
    await page.goto('https://plosivetheatre.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Plosive Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Plosive Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Plosive Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePlosive;
