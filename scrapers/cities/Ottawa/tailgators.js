/**
 * Tailgators Ottawa Events Scraper
 * URL: https://tailgators.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTailgators(city = 'Ottawa') {
  console.log('🎵 Scraping Tailgators...');
  ...
  try {
    ...
    await page.goto('https://tailgators.ca/events/', {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Tailgators events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Tailgators error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTailgators;
