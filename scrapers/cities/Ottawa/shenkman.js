/**
 * Shenkman Arts Centre Ottawa Events Scraper
 * URL: https://ottawa.ca/shenkman
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeShenkman(city = 'Ottawa') {
  console.log('🎵 Scraping Shenkman Arts Centre...');
  ...
  try {
    ...
    await page.goto('https://ottawa.ca/shenkman/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Shenkman Arts Centre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Shenkman Arts Centre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Shenkman Arts Centre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeShenkman;
