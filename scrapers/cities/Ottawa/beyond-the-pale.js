/**
 * Beyond the Pale Brewing Ottawa Events Scraper
 * URL: https://beyondthepale.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBeyondThePale(city = 'Ottawa') {
  console.log('🎵 Scraping Beyond the Pale Brewing...');
  ...
  try {
    ...
    await page.goto('https://beyondthepale.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Beyond the Pale Brewing'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Beyond the Pale Brewing events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Beyond the Pale Brewing error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBeyondThePale;
