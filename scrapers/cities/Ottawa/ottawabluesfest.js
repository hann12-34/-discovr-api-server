/**
 * Ottawa Bluesfest Ottawa Events Scraper
...
async function scrapeOttawabluesfest(city = 'Ottawa') {
  console.log('🎵 Scraping Ottawa Bluesfest Ottawa...');
  ...
  try {
    ...
    await page.goto('https://ottawabluesfest.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Festivals',
        source: 'Ottawa Bluesfest'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ottawa Bluesfest events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Ottawa Bluesfest error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOttawabluesfest;
