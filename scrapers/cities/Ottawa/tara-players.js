/**
 * Tara Players Ottawa Events Scraper
 * URL: https://taraplayers.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTaraPlayers(city = 'Ottawa') {
  console.log('🎵 Scraping Tara Players...');
  ...
  try {
    ...
    await page.goto('https://taraplayers.ca/events/', {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Tara Players events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Tara Players error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTaraPlayers;
