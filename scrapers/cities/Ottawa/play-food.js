/**
 * Play Food and Wine Ottawa Events Scraper
 * URL: https://playfoodandwine.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePlayFood(city = 'Ottawa') {
  console.log('🎵 Scraping Play Food and Wine...');
  ...
  try {
    ...
    await page.goto('https://playfoodandwine.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Play Food and Wine'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Play Food and Wine events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Play Food and Wine error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePlayFood;
