/**
 * Meridian Theatres at Centrepointe Ottawa Events Scraper
 * URL: https://www.meridianartscentre.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMeridianTheatres(city = 'Ottawa') {
  console.log('🎵 Scraping Meridian Theatres at Centrepointe...');
  ...
  try {
    ...
    await page.goto('https://www.meridianartscentre.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Meridian Theatres at Centrepointe'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Meridian Theatres at Centrepointe events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Meridian Theatres at Centrepointe error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMeridianTheatres;
