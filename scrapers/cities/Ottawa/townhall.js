/**
 * Town Hall Public House Ottawa Events Scraper
 * URL: https://townhallpublichouse.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTownhall(city = 'Ottawa') {
  console.log('🎵 Scraping Town Hall Public House...');
  ...
  try {
    ...
    await page.goto('https://townhallpublichouse.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Town Hall Public House'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Town Hall Public House events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Town Hall Public House error: ${error.message}`);
    return [];
  }
}
