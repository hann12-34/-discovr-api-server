/**
 * The Rainbow Bistro Ottawa Events Scraper
 * URL: https://therainbow.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTheRainbow(city = 'Ottawa') {
  console.log('🎵 Scraping The Rainbow Bistro...');
  ...
  try {
    ...
    await page.goto('https://therainbow.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'The Rainbow Bistro'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} The Rainbow Bistro events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  The Rainbow Bistro error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTheRainbow;
