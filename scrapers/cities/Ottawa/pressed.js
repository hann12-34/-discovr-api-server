/**
 * Pressed Ottawa Events Scraper
 * URL: https://pressed.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePressed(city = 'Ottawa') {
  console.log('🎵 Scraping Pressed...');
  ...
  try {
    ...
    await page.goto('https://pressed.ca/events/', {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Pressed events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Pressed error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePressed;
