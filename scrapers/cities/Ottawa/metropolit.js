/**
 * Metropolitain Brasserie Ottawa Events Scraper
 * URL: https://metropolitain.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMetropolit(city = 'Ottawa') {
  console.log('🎵 Scraping Metropolitain Brasserie...');
  ...
  try {
    ...
    await page.goto('https://metropolitain.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Metropolitain Brasserie'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Metropolitain Brasserie events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Metropolitain Brasserie error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMetropolit;
