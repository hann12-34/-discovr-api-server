...
async function scrapeZaphods(city = 'Ottawa') {
  console.log('🎵 Scraping Zaphod Beeblebrox Ottawa...');
  ...
  try {
    ...
    await page.goto('https://zaphodb.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Zaphod Beeblebrox'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Zaphod Beeblebrox events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Zaphod Beeblebrox error: ${error.message}`);
    return [];
  }
}
