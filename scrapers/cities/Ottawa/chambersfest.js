/**
 * Chamberfest Ottawa Ottawa Events Scraper
...
async function scrapeChambersfest(city = 'Ottawa') {
  console.log('🎵 Scraping Chamberfest Ottawa Ottawa...');
  ...
  try {
    ...
    await page.goto('https://chamberfest.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Festivals',
        source: 'Chamberfest Ottawa'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Chamberfest Ottawa events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Chamberfest Ottawa error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeChambersfest;
