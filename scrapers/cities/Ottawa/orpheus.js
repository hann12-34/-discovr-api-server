/**
 * Orpheus Musical Theatre Ottawa Events Scraper
...
async function scrapeOrpheus(city = 'Ottawa') {
  console.log('🎵 Scraping Orpheus Musical Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.orpheus-theatre.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'Orpheus Musical Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Orpheus Musical Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Orpheus Musical Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOrpheus;
