/**
 * The Senate Tavern Ottawa Events Scraper
 * URL: https://thesenatetavern.ca
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTheSenate(city = 'Ottawa') {
  console.log('🎵 Scraping The Senate Tavern...');
  ...
  try {
    ...
    await page.goto('https://thesenatetavern.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Ottawa',
        category: 'Nightlife',
        source: 'The Senate Tavern'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} The Senate Tavern events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  The Senate Tavern error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTheSenate;
