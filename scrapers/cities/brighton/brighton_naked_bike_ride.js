/**
 * Brighton Naked Bike Ride Events Scraper
 * URL: https://www.brighton-naked-bike-ride.info
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNakedBikeRide(city = 'Brighton') {
  console.log('🚴 Scraping Naked Bike Ride...');
  ...
  try {
    ...
    await page.goto('https://www.brighton-naked-bike-ride.info', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Festival',
        source: 'Naked Bike Ride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Naked Bike Ride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Naked Bike Ride error:', error.message);
    return [];
  }
}

module.exports = scrapeNakedBikeRide;
