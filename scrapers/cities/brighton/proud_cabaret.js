/**
 * Proud Cabaret Brighton Events Scraper
 * URL: https://proud.co.uk/brighton/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeProudCabaret(city = 'Brighton') {
  console.log('🎭 Scraping Proud Cabaret...');
  ...
  try {
    ...
    await page.goto('https://proud.co.uk/brighton/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      eventElements.forEach(item => {
        try {
          ...
          if (!url || seen.has(url) || url.endsWith('/whats-on') || url.endsWith('/brighton/')) return;
          seen.add(url);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Proud Cabaret Brighton',
          address: '13 Pavilion Buildings, Brighton BN1 1EE',
          city: 'Brighton'
        },
        latitude: 50.8205,
        longitude: -0.1380,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Proud Cabaret events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Proud Cabaret error:', error.message);
    return [];
  }
}

module.exports = scrapeProudCabaret;
