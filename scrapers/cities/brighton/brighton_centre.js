/**
 * Brighton Centre Events Scraper
 * URL: https://brightoncentre.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBrightonCentre(city = 'Brighton') {
  console.log('🏛️ Scraping Brighton Centre...');
  ...
  try {
    ...
    await page.goto('https://brightoncentre.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Arts',
        source: 'Brighton Centre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Brighton Centre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Brighton Centre error:', error.message);
    return [];
  }
}

module.exports = scrapeBrightonCentre;
