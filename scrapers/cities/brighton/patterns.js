/**
 * Patterns Brighton Events Scraper
 * URL: https://patternsbrighton.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePatterns(city = 'Brighton') {
  console.log('🎵 Scraping Patterns...');
  ...
  try {
    ...
    await page.goto('https://patternsbrighton.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.event-card', '.event', 'article', '[class*="event"]', '.listing', 'a[href*="/event/"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Patterns events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Patterns error:', error.message);
    return [];
  }
}

module.exports = scrapePatterns;
