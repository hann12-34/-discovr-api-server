/**
 * Sticky Mikes Brighton Events Scraper
 * URL: https://www.stickymikes.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeeStickyMikes(city = 'Brighton') {
  console.log('🎤 Scraping Sticky Mikes...');
  ...
  try {
    ...
    await page.goto('https://www.stickymikes.com/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-item', '.event', 'article', '[class*="event"]', '[class*="show"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Sticky Mikes events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sticky Mikes error:', error.message);
    return [];
  }
}

module.exports = scrapeeStickyMikes;
