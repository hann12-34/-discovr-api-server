/**
 * Brighton Bonfire Night Events Scraper
...
async function scrapeBonfire(city = 'Brighton') {
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="bonfire"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Festival',
        source: 'Bonfire Night'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bonfire Night events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bonfire Night error:', error.message);
    return [];
  }
}
