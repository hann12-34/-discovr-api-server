/**
 * Brighton Dome Events Scraper
 * URL: https://brightondome.org/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBrightonDome(city = 'Brighton') {
  console.log('🎭 Scraping Brighton Dome...');
  ...
  try {
    ...
    await page.goto('https://brightondome.org/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.event-listing', '.event', 'article', '[class*="event"]', '[class*="show"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Brighton Dome events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Brighton Dome error:', error.message);
    return [];
  }
}

module.exports = scrapeBrightonDome;
