/**
 * O2 Academy Brighton Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academybrighton/events
 */
...
async function scrapeO2AcademyBrighton(city = 'Brighton') {
  console.log('🎸 Scraping O2 Academy Brighton...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academybrighton/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Brighton', address: '31-37 West Street, Brighton BN1 2RE', city: 'Brighton' },
        latitude: 50.8225,
        longitude: -0.1416,
        city: 'Brighton',
        category: 'Nightlife',
        source: 'O2 Academy Brighton'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Brighton events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Brighton error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyBrighton;
