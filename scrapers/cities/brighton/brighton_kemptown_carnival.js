/**
 * Kemptown Carnival Brighton Events Scraper
 * URL: https://www.kemptowncarnival.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKemptownCarnival(city = 'Brighton') {
  console.log('🎉 Scraping Kemptown Carnival...');
  ...
  try {
    ...
    await page.goto('https://www.kemptowncarnival.com', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Kemptown',
          address: 'St Georges Road, Brighton',
          city: 'Brighton'
        },
        latitude: 50.8195,
        longitude: -0.1268,
        city: 'Brighton',
        category: 'Festival',
        source: 'Kemptown Carnival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kemptown Carnival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kemptown Carnival error:', error.message);
    return [];
  }
}

module.exports = scrapeKemptownCarnival;
