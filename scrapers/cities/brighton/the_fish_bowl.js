/**
 * The Fish Bowl Brighton Events Scraper
 * URL: https://www.thefishbowlbrighton.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFishBowl(city = 'Brighton') {
  console.log('🐠 Scraping The Fish Bowl...');
  ...
  try {
    ...
    await page.goto('https://www.thefishbowlbrighton.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Fish Bowl',
          address: '10-11 Little East St, Brighton BN1 1HT',
          city: 'Brighton'
        },
        latitude: 50.8220,
        longitude: -0.1395,
        city: 'Brighton',
        category: 'Nightlife',
        source: 'The Fish Bowl'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Fish Bowl events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Fish Bowl error:', error.message);
    return [];
  }
}

module.exports = scrapeFishBowl;
