/**
 * Brighton Burning the Clocks Events Scraper
 * URL: https://www.burningtheclocks.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBurningClocks(city = 'Brighton') {
  console.log('🔥 Scraping Burning the Clocks...');
  ...
  try {
    ...
    await page.goto('https://www.burningtheclocks.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Festival',
        source: 'Burning the Clocks'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Burning the Clocks events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Burning the Clocks error:', error.message);
    return [];
  }
}

module.exports = scrapeBurningClocks;
