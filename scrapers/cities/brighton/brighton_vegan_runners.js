/**
 * Brighton Vegan Festival Events Scraper
 * URL: https://www.veganfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVeganEvents(city = 'Brighton') {
  console.log('🌱 Scraping Vegan Events...');
  ...
  try {
    ...
    await page.goto('https://www.veganfestival.co.uk', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="brighton"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Festival',
        source: 'Vegan Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Vegan events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Vegan Events error:', error.message);
    return [];
  }
}

module.exports = scrapeVeganEvents;
