/**
 * Brighton Digital Festival Events Scraper
 * URL: https://www.brightondigitalfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDigitalFest(city = 'Brighton') {
  console.log('💻 Scraping Digital Festival...');
  ...
  try {
    ...
    await page.goto('https://www.brightondigitalfestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Festival',
        source: 'Digital Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Digital Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Digital Festival error:', error.message);
    return [];
  }
}
