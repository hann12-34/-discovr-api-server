/**
 * The Prince Albert Brighton Events Scraper
 * URL: https://www.princealbert.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePrinceAlbert(city = 'Brighton') {
  console.log('👑 Scraping The Prince Albert...');
  ...
  try {
    ...
    await page.goto('https://www.princealbert.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Brighton',
        category: 'Nightlife',
        source: 'The Prince Albert'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Prince Albert events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Prince Albert error:', error.message);
    return [];
  }
}

module.exports = scrapePrinceAlbert;
