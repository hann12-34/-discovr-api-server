/**
 * Brighton Beach Summer Events Scraper
...
async function scrapeBeachEvents(city = 'Brighton') {
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="beach"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Beach events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Beach Events error:', error.message);
    return [];
  }
}
