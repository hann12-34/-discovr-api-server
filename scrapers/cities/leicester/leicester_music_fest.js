...
async function scrapeMusicFest(city = 'Leicester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Victoria Park', address: 'Victoria Park, Leicester', city: 'Leicester' }, latitude: 52.6282, longitude: -1.1268, city: 'Leicester', category: 'Festival', source: 'Music Festival' });
    }
