/**
 * Haymarket Theatre Leicester Events Scraper
 * URL: https://www.haymarketleicester.org/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHaymarket(city = 'Leicester') {
  console.log('🎭 Scraping Haymarket Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.haymarketleicester.org/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Belgrave Gate, Leicester LE1 3YQ',
          city: 'Leicester'
        },
        latitude: 52.6380,
        longitude: -1.1290,
        city: 'Leicester',
        category: 'Arts',
        source: 'Haymarket Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Haymarket Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Haymarket Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeHaymarket;
