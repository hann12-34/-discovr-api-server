/**
 * Leicester Diwali Celebrations Events Scraper
...
async function scrapeDiwaliLeicester(city = 'Leicester') {
  console.log('🪔 Scraping Diwali Leicester...');
  ...
  try {
    ...
    await page.goto('https://www.leicester.gov.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="diwali"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Belgrave Road',
          address: 'Belgrave Road, Leicester',
          city: 'Leicester'
        },
        latitude: 52.6512,
        longitude: -1.1317,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Diwali events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Diwali Leicester error:', error.message);
    return [];
  }
}

module.exports = scrapeDiwaliLeicester;
