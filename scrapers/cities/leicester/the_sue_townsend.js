/**
 * The Sue Townsend Theatre Leicester Events Scraper
 * URL: https://www.suetownsendtheatre.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSueTownsend(city = 'Leicester') {
  console.log('🎭 Scraping Sue Townsend Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.suetownsendtheatre.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Sue Townsend Theatre',
          address: 'Hazel Community Centre, Humberstone Dr, Leicester LE5 0GY',
          city: 'Leicester'
        },
        latitude: 52.6495,
        longitude: -1.1025,
        city: 'Leicester',
        category: 'Arts',
        source: 'Sue Townsend Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sue Townsend Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sue Townsend Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeSueTownsend;
