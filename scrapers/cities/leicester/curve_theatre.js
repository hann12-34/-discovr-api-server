/**
 * Curve Theatre Leicester Events Scraper
 * URL: https://www.curveonline.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCurveTheatre(city = 'Leicester') {
  console.log('🎭 Scraping Curve Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.curveonline.co.uk/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.production-item', '.event', 'article', '[class*="production"]', '[class*="event"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Leicester',
        category: 'Arts',
        source: 'Curve Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Curve Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Curve Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeCurveTheatre;
