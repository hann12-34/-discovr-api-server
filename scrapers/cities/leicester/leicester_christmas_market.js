/**
 * Leicester Christmas Market - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeChristmasMarket(city = 'Leicester') {
  ...
  try {
    ...
    await page.goto('https://www.visitleicester.info/whats-on/christmas', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T10:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Town Hall Square', address: 'Town Hall Square, Leicester', city: 'Leicester' }, latitude: 52.6369, longitude: -1.1332, city: 'Leicester', category: 'Festival', source: 'Christmas Market' });
    }
