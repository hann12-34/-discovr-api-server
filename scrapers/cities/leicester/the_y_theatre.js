/**
 * The Y Theatre Leicester Events Scraper
 * URL: https://www.ytheatre.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeYTheatre(city = 'Leicester') {
  console.log('🎪 Scraping The Y Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.ytheatre.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Y Theatre',
          address: 'East St, Leicester LE1 6EY',
          city: 'Leicester'
        },
        latitude: 52.6350,
        longitude: -1.1305,
        city: 'Leicester',
        category: 'Arts',
        source: 'The Y Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Y Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Y Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeYTheatre;
