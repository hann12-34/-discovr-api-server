/**
 * O2 Academy Leicester Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academyleicester/events
 */
...
async function scrapeO2AcademyLeicester(city = 'Leicester') {
  console.log('🎸 Scraping O2 Academy Leicester...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academyleicester/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Leicester', address: '56 Rutland Street, Leicester LE1 1RD', city: 'Leicester' },
        latitude: 52.6369,
        longitude: -1.1398,
        city: 'Leicester',
        category: 'Nightlife',
        source: 'O2 Academy Leicester'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Leicester events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Leicester error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyLeicester;
