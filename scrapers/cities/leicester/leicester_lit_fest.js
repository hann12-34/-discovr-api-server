...
async function scrapeLitFest(city = 'Leicester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Phoenix Arts Centre', address: 'Midland Street, Leicester', city: 'Leicester' }, latitude: 52.6344, longitude: -1.1373, city: 'Leicester', category: 'Festival', source: 'Literary Festival' });
    }
