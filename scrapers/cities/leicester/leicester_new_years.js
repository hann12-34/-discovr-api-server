...
async function scrapeNYELeicester(city = 'Leicester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Town Hall Square', address: 'Town Hall Square, Leicester', city: 'Leicester' }, latitude: 52.6369, longitude: -1.1332, city: 'Leicester', category: 'Festival', source: 'NYE Leicester' });
    }
