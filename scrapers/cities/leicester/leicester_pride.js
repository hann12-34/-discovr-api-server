/**
 * Leicester Pride Events Scraper
 * URL: https://www.leicesterpride.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLeicesterPride(city = 'Leicester') {
  console.log('🏳️‍🌈 Scraping Leicester Pride...');
  ...
  try {
    ...
    await page.goto('https://www.leicesterpride.com', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Victoria Park',
          address: 'Victoria Park, Leicester',
          city: 'Leicester'
        },
        latitude: 52.6282,
        longitude: -1.1268,
        city: 'Leicester',
        category: 'Festival',
        source: 'Leicester Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Leicester Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Leicester Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeLeicesterPride;
