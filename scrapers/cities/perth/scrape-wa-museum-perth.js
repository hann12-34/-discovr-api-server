/**
 * WA Museum Perth - perth Events Scraper
 * URL: https://museum.wa.gov.au/whats-on
 */
...
async function scrapeWAMuseumPerth(city = 'perth') {
  console.log('🎭 Scraping WA Museum Perth...');
  try {
    const resp = await axios.get('https://museum.wa.gov.au/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'WA Museum Perth', address: addr || 'Perth Cultural Centre, Perth WA', city: 'perth' },
            latitude: -31.9474,
            longitude: 115.8605,
            city: 'perth',
            category: 'Arts',
            source: 'WA Museum Perth'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://museum.wa.gov.au/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://museum.wa.gov.au/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'WA Museum Perth', address: 'Perth Cultural Centre, Perth WA', city: 'perth' },
            latitude: -31.9474,
            longitude: 115.8605,
            city: 'perth',
            category: 'Arts',
    ...
    console.log('  ✅ WA Museum Perth:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ WA Museum Perth error:', error.message);
    return [];
  }
}

module.exports = scrapeWAMuseumPerth;
