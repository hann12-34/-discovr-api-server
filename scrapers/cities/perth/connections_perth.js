 ...
 * Popular LGBTQ+ nightclub in Perth
 * URL: https://connectionsnightclub.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeConnectionsPerth(city = 'Perth') {
  console.log('🌈 Scraping Connections Perth...');
  ...
  try {
    ...
    await page.goto('https://connectionsnightclub.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '81 James St, Northbridge WA 6003',
          city: 'Perth'
        },
        city: 'Perth',
        category: 'Nightlife',
        source: 'Connections Perth'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Connections events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Connections error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeConnectionsPerth;
