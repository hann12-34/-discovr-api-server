/**
 * Fremantle Markets Perth Events Scraper
...
async function scrapeFremantleMarkets(city = 'Perth') {
  console.log('🛍️ Scraping Fremantle Markets...');
  ...
  try {
    ...
    await page.goto('https://www.fremantlemarkets.com.au/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.fremantlemarkets.com.au', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'South Terrace, Fremantle WA',
          city: 'Perth'
        },
        latitude: -32.0569,
        longitude: 115.7507,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Fremantle Markets events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Fremantle Markets error:', error.message);
    return [];
  }
}

module.exports = scrapeFremantleMarkets;
