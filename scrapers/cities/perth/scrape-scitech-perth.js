/**
 * Scitech Perth - perth Events Scraper
 * URL: https://www.scitech.org.au/whats-on/
 */
...
async function scrapeScitechPerth(city = 'perth') {
  console.log('🎭 Scraping Scitech Perth...');
  try {
    const resp = await axios.get('https://www.scitech.org.au/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Scitech Perth', address: addr || 'City West Centre, Perth WA', city: 'perth' },
            latitude: -31.9505,
            longitude: 115.8605,
            city: 'perth',
            category: 'Science',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.scitech.org.au/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.scitech.org.au/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Scitech Perth', address: 'City West Centre, Perth WA', city: 'perth' },
            latitude: -31.9505,
            longitude: 115.8605,
            city: 'perth',
            category: 'Science',
    ...
    console.log('  ✅ Scitech Perth:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Scitech Perth error:', error.message);
    return [];
  }
}

module.exports = scrapeScitechPerth;
