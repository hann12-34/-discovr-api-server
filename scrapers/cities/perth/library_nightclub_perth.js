/**
 * Library Nightclub Perth Events Scraper
 * URL: https://www.thelibrary.com.au
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLibraryNightclub(city = 'Perth') {
  console.log('📚 Scraping Library Nightclub Perth...');
  ...
  try {
    ...
    await page.goto('https://www.thelibrary.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Perth',
        category: 'Nightlife',
        source: 'Library Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Library Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Library Nightclub error:', error.message);
    return [];
  }
}

module.exports = scrapeLibraryNightclub;
