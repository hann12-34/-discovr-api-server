/**
 * Villa Nightclub Perth Events Scraper
 * Popular nightclub in Northbridge
 * URL: https://www.villanightclub.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVillaNightclub(city = 'Perth') {
  console.log('🏠 Scraping Villa Nightclub Perth...');
  ...
  try {
    ...
    await page.goto('https://www.villanightclub.com.au/', {
      ...
      for (const selector of selectors) {
        document.querySelectorAll(selector).forEach(el => {
          try {
            ...
            let container = el;
            if (el.tagName === 'A') {
              container = el.closest('div') || el.parentElement;
            }

            const titleEl = container?.querySelector('h1, h2, h3, h4, .title, [class*="title"]') || el;
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '187 Stirling Street, Perth, WA 6000',
          city: 'Perth'
        },
        latitude: -31.9485,
        longitude: 115.8630,
        city: 'Perth',
        category: 'Nightlife',
        source: 'Villa Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Villa Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ Villa Nightclub error:', error.message);
    return [];
  }
}

module.exports = scrapeVillaNightclub;
