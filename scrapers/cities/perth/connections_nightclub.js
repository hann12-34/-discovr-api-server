/**
 * Connections Nightclub Perth Events Scraper
 * Perth's premier LGBTQIA+ venue - longest running gay club in southern hemisphere
 * URL: https://www.connectionsnightclub.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeConnectionsNightclub(city = 'Perth') {
  console.log('🏳️‍🌈 Scraping Connections Nightclub Perth...');
  ...
  try {
    ...
    await page.goto('https://www.connectionsnightclub.com', {
    ...
    // Try to find events page or event listings
    const eventsPageUrl = await page.evaluate(() => {
      const links = Array.from(document.querySelectorAll('a[href]'));
      for (const link of links) {
        const href = link.href.toLowerCase();
        if (href.includes('event') || href.includes('whats-on') || href.includes('whatson')) {
          return link.href;
        }
      }
      return null;
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Perth',
        category: 'Nightlife',
        source: 'Connections Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Connections Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ Connections Nightclub error:', error.message);
    return [];
  }
}

module.exports = scrapeConnectionsNightclub;
