...
async function scrapeRottnestSwim(city = 'Perth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Cottesloe Beach', address: 'Marine Parade, Cottesloe WA', city: 'Perth' }, latitude: -32.0010, longitude: 115.7574, city: 'Perth', category: 'Festival', source: 'Rottnest Swim' });
    }
