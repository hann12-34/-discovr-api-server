/**
 * Air Nightclub Perth Events Scraper
 * URL: https://www.airnightclub.com.au
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAirNightclub(city = 'Perth') {
  console.log('✈️ Scraping Air Nightclub...');
  ...
  try {
    ...
    await page.goto('https://www.airnightclub.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Air Nightclub',
          address: '139 James Street, Northbridge',
          city: 'Perth'
        },
        latitude: -31.9468,
        longitude: 115.8580,
        city: 'Perth',
        category: 'Nightlife',
        source: 'Air Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Air Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Air Nightclub error:', error.message);
    return [];
  }
}

module.exports = scrapeAirNightclub;
