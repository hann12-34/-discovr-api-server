...
async function scrapeScarborough(city = 'Perth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Scarborough Beach', address: 'Scarborough Beach, Perth', city: 'Perth' }, latitude: -31.8944, longitude: 115.7622, city: 'Perth', category: 'Festival', source: 'Scarborough' });
    }
