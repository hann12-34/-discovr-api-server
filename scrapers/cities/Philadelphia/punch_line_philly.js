/**
 * Punch Line Philadelphia Comedy Events Scraper
 * URL: https://philadelphia.punchlinephilly.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePunchLinePhilly(city = 'Philadelphia') {
  console.log('😂 Scraping Punch Line Philly...');
  ...
  try {
    ...
    await page.goto('https://philadelphia.punchlinephilly.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Philadelphia',
        category: 'Nightlife',
        source: 'Punch Line Philly'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Punch Line events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Punch Line error:', error.message);
    return [];
  }
}

module.exports = scrapePunchLinePhilly;
