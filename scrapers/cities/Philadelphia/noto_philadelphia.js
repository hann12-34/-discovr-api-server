/**
 * Noto Philadelphia Events Scraper
 * Popular nightclub in Philadelphia
 * URL: https://notophilly.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNotoPhiladelphia(city = 'Philadelphia') {
  console.log('🎵 Scraping Noto Philadelphia...');
  ...
  try {
    ...
    await page.goto('https://notophilly.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Noto',
          address: '1209 Vine St, Philadelphia, PA 19107',
          city: 'Philadelphia'
        },
        city: 'Philadelphia',
        category: 'Nightlife',
        source: 'Noto Philadelphia'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Noto events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Noto error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeNotoPhiladelphia;
