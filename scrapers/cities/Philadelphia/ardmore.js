/**
 * Ardmore Music Hall Philadelphia Events Scraper
 * Live music venue in Ardmore
 * URL: https://ardmoremusichall.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeArdmore(city = 'Philadelphia') {
  console.log('🎸 Scraping Ardmore Music Hall Philadelphia...');
  ...
  try {
    ...
    await page.goto('https://ardmoremusichall.com/events/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.tribe-events-calendar-list__event, article, .event').forEach(el => {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Philadelphia',
        category: 'Nightlife',
        source: 'Ardmore Music Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Ardmore events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Ardmore error:', error.message);
    return [];
  }
}

module.exports = scrapeArdmore;
