/**
 * Keswick Theatre Philadelphia Events Scraper
 * URL: https://keswicktheatre.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKeswick(city = 'Philadelphia') {
  console.log('🎭 Scraping Keswick Theatre...');
  ...
  try {
    ...
    await page.goto('https://keswicktheatre.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '291 N Keswick Ave, Glenside, PA 19038',
          city: 'Philadelphia'
        },
        latitude: 40.1026,
        longitude: -75.1527,
        city: 'Philadelphia',
        category: 'Concert',
        source: 'Keswick Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Keswick events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Keswick error:', error.message);
    return [];
  }
}

module.exports = scrapeKeswick;
