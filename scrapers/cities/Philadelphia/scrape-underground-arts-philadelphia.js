/**
 * Underground Arts Philadelphia - Philadelphia Events Scraper
 * URL: https://www.undergroundarts.org/events
 */
...
async function scrapeUndergroundArtsPhiladelphia(city = 'Philadelphia') {
  console.log('🎭 Scraping Underground Arts Philadelphia...');
  try {
    const resp = await axios.get('https://www.undergroundarts.org/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Underground Arts Philadelphia', address: addr || '1200 Callowhill St, Philadelphia, PA', city: 'Philadelphia' },
            latitude: 39.9601,
            longitude: -75.1574,
            city: 'Philadelphia',
            category: 'Music',
            source: 'Underground Arts Philadelphia'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.undergroundarts.org/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.undergroundarts.org/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Underground Arts Philadelphia', address: '1200 Callowhill St, Philadelphia, PA', city: 'Philadelphia' },
            latitude: 39.9601,
            longitude: -75.1574,
            city: 'Philadelphia',
            category: 'Music',
    ...
    console.log('  ✅ Underground Arts Philadelphia:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Underground Arts Philadelphia error:', error.message);
    return [];
  }
}

module.exports = scrapeUndergroundArtsPhiladelphia;
