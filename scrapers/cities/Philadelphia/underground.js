/**
 * Underground Arts Philadelphia Events Scraper
 * Live music and event venue
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Underground Arts`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Underground Arts events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Underground Arts error:', error.message);
    return [];
  }
}
