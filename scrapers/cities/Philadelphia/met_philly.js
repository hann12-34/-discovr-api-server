/**
 * The Met Philadelphia Events Scraper
 * Historic concert hall
 * URL: https://www.themetphilly.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMetPhilly(city = 'Philadelphia') {
  console.log('🎵 Scraping The Met Philadelphia...');
  ...
  try {
    ...
    await page.goto('https://www.themetphilly.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Philadelphia',
        category: 'Concert',
        source: 'The Met Philadelphia'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Met Philly events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Met Philly error:', error.message);
    return [];
  }
}

module.exports = scrapeMetPhilly;
