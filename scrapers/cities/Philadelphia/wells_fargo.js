/**
 * Wells Fargo Center Philadelphia Events Scraper
 * Major arena venue
 * URL: https://www.wellsfargocenterphilly.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWellsFargo(city = 'Philadelphia') {
  console.log('🏟️ Scraping Wells Fargo Center Philadelphia...');
  ...
  try {
    ...
    await page.goto('https://www.wellsfargocenterphilly.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Philadelphia',
        category: 'Concert',
        source: 'Wells Fargo Center'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Wells Fargo Center events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Wells Fargo Center error:', error.message);
    return [];
  }
}
