...
async function scrapeMadeInAmerica(city = 'Philadelphia') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Made In America Festival', address: 'Benjamin Franklin Parkway, Philadelphia PA 19130', city: 'Philadelphia' }, latitude: 39.9656, longitude: -75.1731, city: 'Philadelphia', category: 'Festival', source: 'Made In America' });
    }
