/**
 * Dobbs on South Philadelphia Events Scraper
 * URL: https://www.dobbsonsouth.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDobbs(city = 'Philadelphia') {
  console.log('🎸 Scraping Dobbs on South Philadelphia...');
  ...
  try {
    ...
    await page.goto('https://www.dobbsonsouth.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Dobbs on South',
          address: '304 South St, Philadelphia, PA 19147',
          city: 'Philadelphia'
        },
        latitude: 39.9420,
        longitude: -75.1508,
        city: 'Philadelphia',
        category: 'Nightlife',
        source: 'Dobbs on South'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Dobbs events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Dobbs error:', error.message);
    return [];
  }
}

module.exports = scrapeDobbs;
