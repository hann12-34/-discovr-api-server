/**
 * PhilaMOCA Philadelphia - Philadelphia Events Scraper
 * URL: https://www.philamoca.org/events
 */
...
async function scrapePhilaMOCAPhiladelphia(city = 'Philadelphia') {
  console.log('🎭 Scraping PhilaMOCA Philadelphia...');
  try {
    const resp = await axios.get('https://www.philamoca.org/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'PhilaMOCA Philadelphia', address: addr || '531 N 12th St, Philadelphia, PA', city: 'Philadelphia' },
            latitude: 39.9613,
            longitude: -75.1578,
            city: 'Philadelphia',
            category: 'Arts',
            source: 'PhilaMOCA Philadelphia'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.philamoca.org/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.philamoca.org/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'PhilaMOCA Philadelphia', address: '531 N 12th St, Philadelphia, PA', city: 'Philadelphia' },
            latitude: 39.9613,
            longitude: -75.1578,
            city: 'Philadelphia',
            category: 'Arts',
    ...
    console.log('  ✅ PhilaMOCA Philadelphia:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ PhilaMOCA Philadelphia error:', error.message);
    return [];
  }
}

module.exports = scrapePhilaMOCAPhiladelphia;
