...
async function scrapePhillyFolkFest(city = 'Philadelphia') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Philadelphia Folk Festival', address: 'Old Pool Farm, Schwenksville PA 19473', city: 'Philadelphia' }, latitude: 40.2559, longitude: -75.4645, city: 'Philadelphia', category: 'Festival', source: 'Philly Folk Festival' });
    }
