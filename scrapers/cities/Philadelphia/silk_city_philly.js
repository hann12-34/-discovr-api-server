/**
 * Silk City Philly Events Scraper
 * Popular venue in Philadelphia
 * URL: https://silkcityphilly.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSilkCityPhilly(city = 'Philadelphia') {
  console.log('🎵 Scraping Silk City Philly...');
  ...
  try {
    ...
    await page.goto('https://silkcityphilly.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Philadelphia',
        category: 'Nightlife',
        source: 'Silk City Philly'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Silk City events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Silk City error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSilkCityPhilly;
