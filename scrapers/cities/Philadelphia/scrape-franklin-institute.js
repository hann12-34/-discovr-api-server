/**
 * Franklin Institute - Philadelphia Events Scraper
 * URL: https://www.fi.edu/en/events
 */
...
async function scrapeFranklinInstitute(city = 'Philadelphia') {
  console.log('🎭 Scraping Franklin Institute...');
  try {
    const resp = await axios.get('https://www.fi.edu/en/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Franklin Institute', address: addr || '222 N 20th St, Philadelphia, PA 19103', city: 'Philadelphia' },
            latitude: 39.9582,
            longitude: -75.1731,
            city: 'Philadelphia',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.fi.edu/en/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.fi.edu/en/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Franklin Institute', address: '222 N 20th St, Philadelphia, PA 19103', city: 'Philadelphia' },
            latitude: 39.9582,
            longitude: -75.1731,
            city: 'Philadelphia',
            category: 'Events',
    ...
    console.log('  ✅ Franklin Institute:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Franklin Institute error:', error.message);
    return [];
  }
}

module.exports = scrapeFranklinInstitute;
