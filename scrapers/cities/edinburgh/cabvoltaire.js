/**
 * Cabaret Voltaire Edinburgh Events Scraper
...
async function scrapeCabaretVoltaire(city = 'Edinburgh') {
  console.log('🎧 Scraping Cabaret Voltaire Edinburgh...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edinburgh',
        category: 'Nightlife',
        source: 'Cabaret Voltaire'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Cabaret Voltaire events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cabaret Voltaire error:', error.message);
    return [];
  }
}

module.exports = scrapeCabaretVoltaire;
