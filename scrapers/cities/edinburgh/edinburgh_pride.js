/**
 * Edinburgh Pride Events Scraper
 * URL: https://www.edinburghpride.org
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEdPride(city = 'Edinburgh') {
  console.log('🏳️‍🌈 Scraping Edinburgh Pride...');
  ...
  try {
    ...
    await page.goto('https://www.edinburghpride.org', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edinburgh',
        category: 'Festival',
        source: 'Edinburgh Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Edinburgh Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Edinburgh Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeEdPride;
