...
async function scrapeMurrayfield(city = 'Edinburgh') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Murrayfield Stadium', address: 'Roseburn Street, Edinburgh', city: 'Edinburgh' }, latitude: 55.9422, longitude: -3.2408, city: 'Edinburgh', category: 'Festival', source: 'Murrayfield' });
    }
