/**
 * Why Not Edinburgh Events Scraper
 * Popular nightclub in Edinburgh
 * URL: https://whynotclub.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWhyNotEdinburgh(city = 'Edinburgh') {
  console.log('🎉 Scraping Why Not Edinburgh...');
  ...
  try {
    ...
    await page.goto('https://whynotclub.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edinburgh',
        category: 'Nightlife',
        source: 'Why Not Edinburgh'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Why Not events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Why Not error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeWhyNotEdinburgh;
