/**
 * Capital Theatres Edinburgh - edinburgh Events Scraper
 * URL: https://www.capitaltheatres.com/whats-on
 */
...
async function scrapeCapitalTheatresEdinburgh(city = 'edinburgh') {
  console.log('🎭 Scraping Capital Theatres Edinburgh...');
  try {
    const resp = await axios.get('https://www.capitaltheatres.com/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Capital Theatres Edinburgh', address: addr || 'Edinburgh, UK', city: 'edinburgh' },
            latitude: 55.9475,
            longitude: -3.2003,
            city: 'edinburgh',
            category: 'Theatre',
            source: 'Capital Theatres Edinburgh'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.capitaltheatres.com/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.capitaltheatres.com/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Capital Theatres Edinburgh', address: 'Edinburgh, UK', city: 'edinburgh' },
            latitude: 55.9475,
            longitude: -3.2003,
            city: 'edinburgh',
            category: 'Theatre',
    ...
    console.log('  ✅ Capital Theatres Edinburgh:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Capital Theatres Edinburgh error:', error.message);
    return [];
  }
}

module.exports = scrapeCapitalTheatresEdinburgh;
