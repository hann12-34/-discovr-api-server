/**
 * Edinburgh International Book Festival Events Scraper
...
async function scrapeEdBookFest(city = 'Edinburgh') {
  console.log('📚 Scraping Edinburgh Book Festival...');
  ...
  try {
    ...
    await page.goto('https://www.edbookfest.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edinburgh',
        category: 'Festival',
        source: 'Book Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Book Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Book Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeEdBookFest;
