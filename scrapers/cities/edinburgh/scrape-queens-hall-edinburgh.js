/**
 * Queens Hall Edinburgh - edinburgh Events Scraper
...
async function scrapeQueensHallEdinburgh(city = 'edinburgh') {
  console.log('🎭 Scraping Queens Hall Edinburgh...');
  try {
    const resp = await axios.get('https://www.thequeenshall.net/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Queens Hall Edinburgh', address: addr || '85-89 Clerk St, Edinburgh EH8 9JG', city: 'edinburgh' },
            latitude: 55.9405,
            longitude: -3.1832,
            city: 'edinburgh',
            category: 'Events',
            source: 'Queens Hall Edinburgh'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.thequeenshall.net/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.thequeenshall.net/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Queens Hall Edinburgh', address: '85-89 Clerk St, Edinburgh EH8 9JG', city: 'edinburgh' },
            latitude: 55.9405,
            longitude: -3.1832,
            city: 'edinburgh',
            category: 'Events',
    ...
    console.log('  ✅ Queens Hall Edinburgh:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Queens Hall Edinburgh error:', error.message);
    return [];
  }
}

module.exports = scrapeQueensHallEdinburgh;
