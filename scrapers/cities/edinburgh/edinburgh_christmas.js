/**
 * Edinburgh's Christmas Events Scraper
 * URL: https://www.edinburghschristmas.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEdChristmas(city = 'Edinburgh') {
  console.log('🎄 Scraping Edinburgh Christmas...');
  ...
  try {
    ...
    await page.goto('https://www.edinburghschristmas.com', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="attraction"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edinburgh',
        category: 'Festival',
        source: 'Edinburgh Christmas'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Edinburgh Christmas events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Edinburgh Christmas error:', error.message);
    return [];
  }
}

module.exports = scrapeEdChristmas;
