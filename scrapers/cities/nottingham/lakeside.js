/**
 * Lakeside Arts Nottingham Events Scraper
 * URL: https://lakesidearts.org.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLakeside(city = 'Nottingham') {
  console.log('🎨 Scraping Lakeside Arts...');
  ...
  try {
    ...
    await page.goto('https://lakesidearts.org.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Nottingham',
        category: 'Arts',
        source: 'Lakeside Arts'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Lakeside Arts events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Lakeside Arts error:', error.message);
    return [];
  }
}

module.exports = scrapeLakeside;
