/**
 * JamCafe Nottingham Events Scraper
 * URL: https://jamcafe.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeJamCafe(city = 'Nottingham') {
  console.log('🎵 Scraping JamCafe...');
  ...
  try {
    ...
    await page.goto('https://jamcafe.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'JamCafe',
          address: '7-9 Heathcoat St, Nottingham NG1 3AF',
          city: 'Nottingham'
        },
        latitude: 52.9570,
        longitude: -1.1565,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} JamCafe events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  JamCafe error:', error.message);
    return [];
  }
}

module.exports = scrapeJamCafe;
