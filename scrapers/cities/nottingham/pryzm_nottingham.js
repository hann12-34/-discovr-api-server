/**
 * Pryzm Nottingham Events Scraper
 * Popular nightclub in Nottingham
 * URL: https://pryzm.co.uk/nottingham
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePryzmNottingham(city = 'Nottingham') {
  console.log('🎉 Scraping Pryzm Nottingham...');
  ...
  try {
    ...
    await page.goto('https://pryzm.co.uk/nottingham', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Pryzm',
          address: 'Lower Parliament St, Nottingham NG1 3BB',
          city: 'Nottingham'
        },
        city: 'Nottingham',
        category: 'Nightlife',
        source: 'Pryzm Nottingham'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Pryzm events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Pryzm error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePryzmNottingham;
