/**
 * Nottingham Mela Events Scraper
 * URL: https://www.nottinghammela.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNottinghamMela(city = 'Nottingham') {
  console.log('🎉 Scraping Nottingham Mela...');
  ...
  try {
    ...
    await page.goto('https://www.nottinghammela.com', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Forest Recreation Ground, Nottingham',
          city: 'Nottingham'
        },
        latitude: 52.9685,
        longitude: -1.1625,
        city: 'Nottingham',
        category: 'Festival',
        source: 'Nottingham Mela'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Nottingham Mela events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nottingham Mela error:', error.message);
    return [];
  }
}

module.exports = scrapeNottinghamMela;
