/**
 * Trent Bridge Nottingham Events Scraper
 * URL: https://www.trentbridge.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTrentBridge(city = 'Nottingham') {
  console.log('🏏 Scraping Trent Bridge...');
  ...
  try {
    ...
    await page.goto('https://www.trentbridge.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.fixture', '.event', 'article', '[class*="event"]', '[class*="match"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Trent Bridge',
          address: 'Bridgford Rd, West Bridgford, Nottingham NG2 6AG',
          city: 'Nottingham'
        },
        latitude: 52.9375,
        longitude: -1.1320,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Trent Bridge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Trent Bridge error:', error.message);
    return [];
  }
}

module.exports = scrapeTrentBridge;
