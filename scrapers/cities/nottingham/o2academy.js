/**
 * O2 Academy Nottingham Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academynottingham/events
 */
...
async function scrapeO2AcademyNottingham(city = 'Nottingham') {
  console.log('🎸 Scraping O2 Academy Nottingham...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academynottingham/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Nottingham', address: 'Goldsmith Street, Nottingham NG1 5LB', city: 'Nottingham' },
        latitude: 52.9548,
        longitude: -1.1581,
        city: 'Nottingham',
        category: 'Nightlife',
        source: 'O2 Academy Nottingham'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Nottingham events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Nottingham error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyNottingham;
