/**
 * Royal Concert Hall Nottingham Events Scraper
 * URL: https://www.trch.co.uk/royal-concert-hall/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoyalConcertHall(city = 'Nottingham') {
  console.log('🎻 Scraping Royal Concert Hall...');
  ...
  try {
    ...
    await page.goto('https://www.trch.co.uk/royal-concert-hall/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.concert-item', '.event', 'article', '[class*="concert"]', '[class*="event"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Nottingham',
        category: 'Arts',
        source: 'Royal Concert Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Royal Concert Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Royal Concert Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeRoyalConcertHall;
