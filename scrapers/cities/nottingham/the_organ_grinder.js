/**
 * The Organ Grinder Nottingham Events Scraper
 * URL: https://www.theorgan.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOrganGrinder(city = 'Nottingham') {
  console.log('🎹 Scraping The Organ Grinder...');
  ...
  try {
    ...
    await page.goto('https://www.theorgan.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '21 Alfreton Rd, Nottingham NG7 3JE',
          city: 'Nottingham'
        },
        latitude: 52.9600,
        longitude: -1.1645,
        city: 'Nottingham',
        category: 'Nightlife',
        source: 'The Organ Grinder'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Organ Grinder events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Organ Grinder error:', error.message);
    return [];
  }
}

module.exports = scrapeOrganGrinder;
