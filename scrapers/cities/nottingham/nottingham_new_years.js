/**
 * Nottingham New Year's Eve - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNYENottingham(city = 'Nottingham') {
  console.log('🎆 Scraping New Year\'s Eve Nottingham...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T20:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Old Market Square', address: 'Old Market Square, Nottingham', city: 'Nottingham' }, latitude: 52.9540, longitude: -1.1497, city: 'Nottingham', category: 'Festival', source: 'NYE Nottingham' });
    }
    console.log(`  ✅ Found ${results.length} NYE Nottingham events`);
    return results;
  } catch (err) {
    console.error('NYE Nottingham error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeNYENottingham;
