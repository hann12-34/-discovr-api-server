...
async function scrapeJazzJam(city = 'Nottingham') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Various Jazz Venues', address: 'Nottingham City Centre', city: 'Nottingham' }, latitude: 52.9548, longitude: -1.1581, city: 'Nottingham', category: 'Nightlife', source: 'Jazz Jam' });
    }
    console.log(`  ✅ Found ${results.length} Jazz Jam events`);
    return results;
  } catch (err) {
