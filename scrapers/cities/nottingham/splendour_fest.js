...
async function scrapeSplendour(city = 'Nottingham') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Wollaton Park', address: 'Wollaton Park, Nottingham', city: 'Nottingham' }, latitude: 52.9415, longitude: -1.2087, city: 'Nottingham', category: 'Festival', source: 'Splendour' });
    }
