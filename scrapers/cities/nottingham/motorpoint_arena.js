/**
 * Motorpoint Arena Nottingham Events Scraper
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Nottingham',
        category: 'Arts',
        source: 'Motorpoint Arena'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Motorpoint Arena events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Motorpoint Arena error:', error.message);
    return [];
  }
}
