...
async function scrapeMotorpointArena(city = 'Nottingham') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Motorpoint Arena', address: 'Bolero Square, Nottingham', city: 'Nottingham' }, latitude: 52.9599, longitude: -1.1394, city: 'Nottingham', category: 'Festival', source: 'Motorpoint Arena' });
    }
