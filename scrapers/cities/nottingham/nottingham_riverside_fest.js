/**
 * Nottingham Riverside Festival Events Scraper
 * URL: https://www.riversidefestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRiversideFest(city = 'Nottingham') {
  ...
  try {
    ...
    await page.goto('https://www.riversidefestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Embankment',
          address: 'Trent Embankment, Nottingham',
          city: 'Nottingham'
        },
        latitude: 52.9430,
        longitude: -1.1362,
        city: 'Nottingham',
        category: 'Festival',
        source: 'Riverside Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Riverside Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Riverside Festival error:', error.message);
    return [];
  }
}
