/**
 * Nottingham Pride Events Scraper
 * URL: https://www.nottinghampride.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNottinghamPride(city = 'Nottingham') {
  console.log('🏳️‍🌈 Scraping Nottingham Pride...');
  ...
  try {
    ...
    await page.goto('https://www.nottinghampride.com', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Arboretum',
          address: 'The Arboretum, Nottingham',
          city: 'Nottingham'
        },
        latitude: 52.9629,
        longitude: -1.1592,
        city: 'Nottingham',
        category: 'Festival',
        source: 'Nottingham Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Nottingham Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nottingham Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeNottinghamPride;
