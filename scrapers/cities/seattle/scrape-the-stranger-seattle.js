/**
 * The Stranger Seattle - Seattle Events Scraper
...
async function scrapeTheStrangerSeattle(city = 'Seattle') {
  console.log('🎭 Scraping The Stranger Seattle...');
  try {
    const resp = await axios.get('https://www.thestranger.com/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'The Stranger Seattle', address: addr || 'Seattle, WA', city: 'Seattle' },
            latitude: 47.6062,
            longitude: -122.3321,
            city: 'Seattle',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.thestranger.com/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.thestranger.com/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'The Stranger Seattle', address: 'Seattle, WA', city: 'Seattle' },
            latitude: 47.6062,
            longitude: -122.3321,
            city: 'Seattle',
            category: 'Events',
    ...
    console.log('  ✅ The Stranger Seattle:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ The Stranger Seattle error:', error.message);
    return [];
  }
}

module.exports = scrapeTheStrangerSeattle;
