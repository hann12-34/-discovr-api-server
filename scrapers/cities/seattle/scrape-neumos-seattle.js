/**
 * Neumos Seattle - Seattle Events Scraper
...
async function scrapeNeumosSeattle(city = 'Seattle') {
  console.log('🎭 Scraping Neumos Seattle...');
  try {
    const resp = await axios.get('https://www.neumos.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Neumos Seattle', address: addr || '925 E Pike St, Seattle, WA 98122', city: 'Seattle' },
            latitude: 47.6143,
            longitude: -122.3183,
            city: 'Seattle',
            category: 'Events',
            source: 'Neumos Seattle'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.neumos.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.neumos.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Neumos Seattle', address: '925 E Pike St, Seattle, WA 98122', city: 'Seattle' },
            latitude: 47.6143,
            longitude: -122.3183,
            city: 'Seattle',
            category: 'Events',
    ...
    console.log('  ✅ Neumos Seattle:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Neumos Seattle error:', error.message);
    return [];
  }
}

module.exports = scrapeNeumosSeattle;
