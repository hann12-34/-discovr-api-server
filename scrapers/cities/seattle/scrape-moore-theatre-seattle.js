/**
 * Moore Theatre Seattle - Seattle Events Scraper
 * URL: https://www.stgpresents.org/moore
 */
...
async function scrapeMooreTheatreSeattle(city = 'Seattle') {
  console.log('🎭 Scraping Moore Theatre Seattle...');
  try {
    const resp = await axios.get('https://www.stgpresents.org/moore', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Moore Theatre Seattle', address: addr || '1932 2nd Ave, Seattle, WA 98101', city: 'Seattle' },
            latitude: 47.6118,
            longitude: -122.341,
            city: 'Seattle',
            category: 'Events',
            source: 'Moore Theatre Seattle'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.stgpresents.org/moore');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.stgpresents.org/moore').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Moore Theatre Seattle', address: '1932 2nd Ave, Seattle, WA 98101', city: 'Seattle' },
            latitude: 47.6118,
            longitude: -122.341,
            city: 'Seattle',
            category: 'Events',
    ...
    console.log('  ✅ Moore Theatre Seattle:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Moore Theatre Seattle error:', error.message);
    return [];
  }
}

module.exports = scrapeMooreTheatreSeattle;
