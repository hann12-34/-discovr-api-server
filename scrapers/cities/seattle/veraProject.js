/**
 * The Vera Project Seattle Scraper - REAL Puppeteer
 * All-ages nonprofit music venue
...
async function scrapeVeraProject(city = 'Seattle') {
  console.log('🎤 Scraping The Vera Project Seattle...');
  ...
  try {
    ...
    await page.goto('https://theveraproject.org/calendar/', {
    ...
    const events = await page.evaluate(() => {
      ...
      const items = document.querySelectorAll('.event, .show, article, [class*="event"], .tribe-events-event');
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} Vera Project events`);
    ...
    const formattedEvents = events.map(event => ({
      ...
      venue: { name: 'The Vera Project', address: '305 Harrison St, Seattle, WA', city: 'Seattle' },
      latitude: 47.6221,
      longitude: -122.3489,
      city: 'Seattle',
      category: 'Festival',
    ...
    console.error('  ⚠️  Vera Project error:', error.message);
    return [];
  }
}

module.exports = scrapeVeraProject;
