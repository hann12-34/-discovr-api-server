/**
 * House of Blues San Diego Events Scraper v2
 * URL: https://www.houseofblues.com/sandiego/concert-events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHOBSD2(city = 'San Diego') {
  ...
  try {
    ...
    await page.goto('https://www.houseofblues.com/sandiego/concert-events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="event"], article, .card, a[href*="event"], .concert-card').forEach(el => {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Nightlife',
        source: 'House of Blues San Diego'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} HOB San Diego events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  HOB San Diego error:', error.message);
    return [];
  }
}

module.exports = scrapeHOBSD2;
