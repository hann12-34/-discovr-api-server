/**
 * Bang Bang San Diego Events Scraper
 * Nightclub and sushi restaurant in Gaslamp
 * URL: https://www.bangbangsd.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBangBang(city = 'San Diego') {
  console.log('🎉 Scraping Bang Bang San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.bangbangsd.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, .event, article, .collection-item').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Bang Bang events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bang Bang error:', error.message);
    return [];
  }
}

module.exports = scrapeBangBang;
