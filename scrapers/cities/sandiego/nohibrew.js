/**
 * North Park Beer Co San Diego Events Scraper
 * URL: https://northparkbeerco.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNorthParkBeer(city = 'San Diego') {
  console.log('🍺 Scraping North Park Beer Co San Diego...');
  ...
  try {
    ...
    await page.goto('https://northparkbeerco.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Nightlife',
        source: 'North Park Beer Co'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} North Park Beer Co events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  North Park Beer Co error:', error.message);
    return [];
  }
}

module.exports = scrapeNorthParkBeer;
