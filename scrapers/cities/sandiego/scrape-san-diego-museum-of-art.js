/**
 * San Diego Museum of Art - sandiego Events Scraper
 * URL: https://www.sdmart.org/events/
 */
...
async function scrapeSanDiegoMuseumofArt(city = 'sandiego') {
  console.log('🎭 Scraping San Diego Museum of Art...');
  try {
    const resp = await axios.get('https://www.sdmart.org/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'San Diego Museum of Art', address: addr || '1450 El Prado, San Diego, CA 92101', city: 'sandiego' },
            latitude: 32.7323,
            longitude: -117.1509,
            city: 'sandiego',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.sdmart.org/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.sdmart.org/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'San Diego Museum of Art', address: '1450 El Prado, San Diego, CA 92101', city: 'sandiego' },
            latitude: 32.7323,
            longitude: -117.1509,
            city: 'sandiego',
            category: 'Events',
    ...
    console.log('  ✅ San Diego Museum of Art:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ San Diego Museum of Art error:', error.message);
    return [];
  }
}

module.exports = scrapeSanDiegoMuseumofArt;
