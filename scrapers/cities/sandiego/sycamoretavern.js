/**
 * Sycamore Den San Diego Events Scraper
 * Popular cocktail bar with events in Normal Heights
 * URL: https://www.sycamoreden.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSycamoreDen(city = 'San Diego') {
  console.log('🍸 Scraping Sycamore Den San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.sycamoreden.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.eventlist-event, .summary-item, article').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Sycamore Den events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sycamore Den error:', error.message);
    return [];
  }
}

module.exports = scrapeSycamoreDen;
