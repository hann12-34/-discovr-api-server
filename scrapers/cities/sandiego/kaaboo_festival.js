...
async function scrapeKaaboo(city = 'San Diego') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Del Mar Fairgrounds', address: '2260 Jimmy Durante Boulevard, Del Mar CA 92014', city: 'San Diego' }, latitude: 32.9781, longitude: -117.2570, city: 'San Diego', category: 'Festival', source: 'KAABOO' });
    }
