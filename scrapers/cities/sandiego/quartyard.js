/**
 * Quartyard San Diego Events Scraper
 * Outdoor event space in East Village
 * URL: https://www.quartyardsd.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeQuartyard(city = 'San Diego') {
  console.log('🎉 Scraping Quartyard San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.quartyardsd.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.eventlist-event, .summary-item, .collection-item, article').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Quartyard events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Quartyard error:', error.message);
    return [];
  }
}

module.exports = scrapeQuartyard;
