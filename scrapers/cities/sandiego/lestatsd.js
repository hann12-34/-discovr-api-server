/**
 * Lestat's West San Diego Events Scraper
 * URL: https://lestats.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLestatWest(city = 'San Diego') {
  console.log('🎸 Scraping Lestat\'s West San Diego...');
  ...
  try {
    ...
    await page.goto('https://lestats.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Nightlife',
        source: 'Lestat\'s West'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Lestat's West events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Lestat\'s West error:', error.message);
    return [];
  }
}

module.exports = scrapeLestatWest;
