...
async function scrapeSDBeerweck(city = 'San Diego') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Various San Diego Locations', address: 'San Diego CA 92101', city: 'San Diego' }, latitude: 32.7157, longitude: -117.1611, city: 'San Diego', category: 'Festival', source: 'SD Beer Week' });
    }
