...
async function scrapeSDPride(city = 'San Diego') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Balboa Park', address: '1549 El Prado, San Diego CA 92101', city: 'San Diego' }, latitude: 32.7321, longitude: -117.1473, city: 'San Diego', category: 'Festival', source: 'SD Pride' });
    }
