/**
 * San Diego Symphony Events Scraper
 * URL: https://www.sandiegosymphony.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSDSymphony(city = 'San Diego') {
  console.log('🎻 Scraping SD Symphony...');
  ...
  try {
    ...
    await page.goto('https://www.sandiegosymphony.org/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('div.event, article, a[href*="event"], a[href*="concert"]').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} SD Symphony events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  SD Symphony error:', error.message);
    return [];
  }
}

module.exports = scrapeSDSymphony;
