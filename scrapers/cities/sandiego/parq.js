/**
 * Parq San Diego Events Scraper
 * Upscale nightclub in Gaslamp Quarter
 * URL: https://parqsd.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeParq(city = 'San Diego') {
  console.log('🎉 Scraping Parq San Diego...');
  ...
  try {
    ...
    await page.goto('https://parqsd.com/events/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, .event, article, .item').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Parq events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Parq error:', error.message);
    return [];
  }
}

module.exports = scrapeParq;
