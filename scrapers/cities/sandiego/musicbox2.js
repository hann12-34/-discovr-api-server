/**
 * Music Box San Diego Events Scraper v2
...
async function scrapeMusicBox2(city = 'San Diego') {
  console.log('🎸 Scraping Music Box San Diego v2...');
  ...
  try {
    ...
    await page.goto('https://musicboxsd.com/shows/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="event"], article, .card, a[href*="event"], .show, .listing').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Music Box v2 events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Music Box v2 error:', error.message);
    return [];
  }
}

module.exports = scrapeMusicBox2;
