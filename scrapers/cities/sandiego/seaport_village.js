/**
 * Seaport Village San Diego Events Scraper
 * URL: https://www.seaportvillage.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSeaportVillage(city = 'San Diego') {
  console.log('🚢 Scraping Seaport Village...');
  ...
  try {
    ...
    await page.goto('https://www.seaportvillage.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Festival',
        source: 'Seaport Village'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Seaport Village events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Seaport Village error:', error.message);
    return [];
  }
}

module.exports = scrapeSeaportVillage;
