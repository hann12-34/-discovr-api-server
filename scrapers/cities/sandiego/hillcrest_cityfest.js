...
async function scrapeHillcrestFest(city = 'San Diego') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Hillcrest', address: 'University Avenue, San Diego CA 92103', city: 'San Diego' }, latitude: 32.7488, longitude: -117.1637, city: 'San Diego', category: 'Festival', source: 'Hillcrest CityFest' });
    }
