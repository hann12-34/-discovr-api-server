/**
 * Petco Park San Diego Concerts Scraper
 * URL: https://www.petcoparkevents.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePetcoPark(city = 'San Diego') {
  console.log('⚾ Scraping Petco Park...');
  ...
  try {
    ...
    await page.goto('https://www.mlb.com/padres/ballpark/concerts-and-events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], div.event, article').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Petco Park events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Petco Park error:', error.message);
    return [];
  }
}

module.exports = scrapePetcoPark;
