/**
 * CRSSD Festival San Diego Events Scraper
...
async function scrapeCRSSD(city = 'San Diego') {
  console.log('🎵 Scraping CRSSD Festival...');
  ...
  try {
    ...
    await page.goto('https://crssdfest.com', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, article, .event-item, .show, a[href*="event"]').forEach(el => {
        ...
        const href = linkEl?.href || 'https://crssdfest.com';
    ...
    console.log(`  ✅ Found ${formattedEvents.length} CRSSD events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  CRSSD error:', error.message);
    return [];
  }
}

module.exports = scrapeCRSSD;
