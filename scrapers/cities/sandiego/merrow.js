/**
 * The Merrow San Diego Events Scraper
 * Live music venue in Hillcrest
 * URL: https://www.themerrow.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMerrow(city = 'San Diego') {
  console.log('🎸 Scraping The Merrow San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.themerrow.com/calendar', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event, .show, article, .calendar-item').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Merrow events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Merrow error:', error.message);
    return [];
  }
}

module.exports = scrapeMerrow;
