/**
 * Spreckels Theatre San Diego Events Scraper
 * URL: https://spreckels.net/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSpreckel(city = 'San Diego') {
  console.log('🎭 Scraping Spreckels Theatre...');
  ...
  try {
    ...
    await page.goto('https://spreckels.net/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Spreckels Theatre',
          address: '121 Broadway, San Diego CA 92101',
          city: 'San Diego'
        },
        latitude: 32.7153,
        longitude: -117.1625,
        city: 'San Diego',
        category: 'Festival',
        source: 'Spreckels Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Spreckels events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Spreckels error:', error.message);
    return [];
  }
}

module.exports = scrapeSpreckel;
