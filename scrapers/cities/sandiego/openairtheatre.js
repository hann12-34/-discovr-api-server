/**
 * Cal Coast Credit Union Open Air Theatre Events Scraper
 * URL: https://www.openairtheatre.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOpenAir(city = 'San Diego') {
  console.log('🎵 Scraping Open Air Theatre San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.openairtheatre.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Concert',
        source: 'Open Air Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Open Air Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Open Air Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeOpenAir;
