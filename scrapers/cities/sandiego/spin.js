/**
 * Spin Nightclub San Diego Events Scraper
 * Dance club in Hillcrest
 * URL: https://www.spinnightclub.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSpin(city = 'San Diego') {
  console.log('🎉 Scraping Spin Nightclub San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.spinnightclub.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event, .event-card, article, .listing').forEach(el => {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Diego',
        category: 'Nightlife',
        source: 'Spin Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Spin events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Spin error:', error.message);
    return [];
  }
}

module.exports = scrapeSpin;
