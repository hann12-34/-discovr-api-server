/**
 * Music Box San Diego Enhanced Events Scraper
 * URL: https://musicboxsd.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMusicBoxSD(city = 'San Diego') {
  console.log('🎵 Scraping Music Box SD Enhanced...');
  ...
  try {
    ...
    await page.goto('https://musicboxsd.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('div.event, article, .show-listing, a[href*="event"]').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Music Box SD events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Music Box SD error:', error.message);
    return [];
  }
}

module.exports = scrapeMusicBoxSD;
