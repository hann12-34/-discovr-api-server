...
async function scrapeWorldBeat(city = 'San Diego') {
  console.log('🎵 Scraping WorldBeat Center San Diego...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="event"], article, .card, a[href*="event"]').forEach(el => {
        ...
        // CRITICAL: Skip if no URL - we MUST have a URL per rules
    ...
    console.log(`  ✅ Found ${formattedEvents.length} WorldBeat events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  WorldBeat error:', error.message);
    return [];
  }
}
