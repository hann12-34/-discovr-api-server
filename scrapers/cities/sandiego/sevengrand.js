/**
 * Seven Grand San Diego Events Scraper
 * Whiskey bar with live music in North Park
 * URL: https://www.sevengrandbars.com/san-diego
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSevenGrand(city = 'San Diego') {
  console.log('🥃 Scraping Seven Grand San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.sevengrandbars.com/san-diego', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event, .eventlist-event, article, .collection-item').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Seven Grand events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Seven Grand error:', error.message);
    return [];
  }
}

module.exports = scrapeSevenGrand;
