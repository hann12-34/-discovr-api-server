...
async function scrapePacificBeachfest(city = 'San Diego') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Pacific Beach', address: 'Garnet Avenue, San Diego CA 92109', city: 'San Diego' }, latitude: 32.7965, longitude: -117.2295, city: 'San Diego', category: 'Festival', source: 'Pacific BeachFest' });
    }
