/**
 * Moonshine Flats San Diego Events Scraper
 * URL: https://www.moonshineflats.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMoonshineFlats(city = 'San Diego') {
  console.log('🤠 Scraping Moonshine Flats San Diego...');
  ...
  try {
    ...
    await page.goto('https://www.moonshineflats.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Moonshine Flats',
          address: '344 7th Ave, San Diego, CA 92101',
          city: 'San Diego'
        },
        latitude: 32.7129,
        longitude: -117.1596,
        city: 'San Diego',
        category: 'Nightlife',
        source: 'Moonshine Flats'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Moonshine Flats events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Moonshine Flats error:', error.message);
    return [];
  }
}

module.exports = scrapeMoonshineFlats;
