...
const BASE_URL = 'https://www.dirtymartini.com';
...
async function scrapeDirtyMartiniEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Dirty Martini events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Dirty Martini URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Dirty Martini URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Dirty Martini page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Dirty Martini event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Dirty Martini in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Dirty Martini events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Dirty Martini event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Dirty Martini event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Dirty Martini event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Dirty Martini events`);
    return events;
  } catch (error) {
    console.error('Error scraping Dirty Martini events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeDirtyMartiniEventsClean  };
