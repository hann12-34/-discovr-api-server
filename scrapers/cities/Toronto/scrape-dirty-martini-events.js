...
const DIRTY_MARTINI_URL = 'http://dirtymartini.ca/web/#events';
const DIRTY_MARTINI_VENUE = {
  name: 'Dirty Martini',
  address: 'Toronto, ON',
  website: 'http://dirtymartini.ca/',
...
async function scrapeDirtyMartiniEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Dirty Martini website...');
    
    // Fetch HTML content
    const response = await axios.get('http://dirtymartini.ca/web/', {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card, #events').each((i, el) => {
      try {
        ...
        // Extract event URL
        const eventUrl = $el.find('a').attr('href') || DIRTY_MARTINI_URL;
        const fullEventUrl = eventUrl.startsWith('http') ? eventUrl : `http://dirtymartini.ca${eventUrl}`;
        
        // Extract image
        const imageUrl = $el.find('img').attr('src') || '';
        const fullImageUrl = imageUrl && !imageUrl.startsWith('http') ? 
                            `http://dirtymartini.ca${imageUrl}` : imageUrl;
    ...
    for (const event of events) {
      ...
      const eventId = generateEventId(DIRTY_MARTINI_VENUE.name, event.title, startDate.toISOString());
      const categories = ['Nightlife', 'Cocktails', 'Lounge', 'Bar'];
      ...
      const eventDoc = {
        ...
        startDate,
        endDate,
        venue: DIRTY_MARTINI_VENUE,
        categories: categories,
        price: price || "Contact venue",
        officialWebsite: DIRTY_MARTINI_VENUE.website,
        location: DIRTY_MARTINI_VENUE.location,
        sourceURL: DIRTY_MARTINI_VENUE.website,
    ...
    console.error(`❌ Error scraping Dirty Martini events: ${error.message}`);
  }
...
async function scrapeDirtyMartiniEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeDirtyMartiniEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeDirtyMartiniEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeDirtyMartiniEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Dirty Martini scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Dirty Martini scraper:', error);
      process.exit(1);
    });
}
