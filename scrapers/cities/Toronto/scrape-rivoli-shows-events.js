...
const BASE_URL = 'https://www.rivolishows.com';
...
async function scrapeRivoliShowsEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rivoli Shows events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rivoli Shows URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rivoli Shows URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rivoli Shows page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rivoli Shows event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rivoli Shows in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rivoli Shows events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rivoli Shows event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rivoli Shows event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rivoli Shows event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Rivoli Shows events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeRivoliShowsEventsClean  };
