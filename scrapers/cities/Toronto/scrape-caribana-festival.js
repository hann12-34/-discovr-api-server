...
const BASE_URL = 'https://www.caribanafestival.com';
...
async function scrapeCaribanaFestivalEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Caribana Festival events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Caribana Festival URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Caribana Festival URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Caribana Festival page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Caribana Festival event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Caribana Festival in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Caribana Festival events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Caribana Festival-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Caribana Festival event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Caribana Festival event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Caribana Festival event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Caribana Festival events`);
    return events;
  } catch (error) {
    console.error('Error scraping Caribana Festival events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeCaribanaFestivalEventsClean  };
