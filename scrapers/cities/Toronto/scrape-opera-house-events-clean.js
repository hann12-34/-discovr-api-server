...
const BASE_URL = 'https://www.theoperahousetoronto.com';
...
async function scrapeOperaHouseEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Opera House events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Opera House URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Opera House URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Opera House page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Opera House event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Opera House in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Opera House events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Opera House event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Opera House event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Opera House event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Opera House events`);
    return events;
  } catch (error) {
    console.error('Error scraping Opera House events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeOperaHouseEventsClean  };
