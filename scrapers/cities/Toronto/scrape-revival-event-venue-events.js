...
const REVIVAL_URL = 'https://www.revivaleventvenue.ca/events/';
const REVIVAL_VENUE = {
  name: 'Revival Event Venue',
  address: '783 Dundas St W, Toronto, ON M6J 1T8',
  city: city,
  province: 'Ontario',
  country: 'Canada',
  website: 'https://www.revivaleventvenue.ca/',
...
async function scrapeRevivalEventVenueEvents(eventsCollection) {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-revival-event-venue-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;

  try {
    console.log('🔍 Fetching events from Revival Event Venue website...');

    // Fetch HTML content
    const response = await axios.get(REVIVAL_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card, .sqs-block-content').each((i, el) => {
      try {
        ...
        // Extract event URL
        const eventUrl = $el.find('a').attr('href') || REVIVAL_URL;
        const fullEventUrl = eventUrl.startsWith('http') ? eventUrl : `https://www.revivaleventvenue.ca${eventUrl}`;

        // Extract image
        const imageUrl = $el.find('img').attr('src') || '';
        const fullImageUrl = imageUrl && !imageUrl.startsWith('http') ?
                            `https://www.revivaleventvenue.ca${imageUrl}` : imageUrl;
    ...
    for (const event of events) {
      ...
      const eventId = generateEventId(REVIVAL_VENUE.name, event.title, startDate.toISOString());
      const categories = ['Nightlife', 'Entertainment', 'Music'];
      ...
      const eventDoc = {
        ...
        startDate,
        endDate,
        venue: { ...REVIVAL_VENUE, city },
        categories: categories,
        price: price || "Contact venue",
        officialWebsite: REVIVAL_VENUE.website,
        location: REVIVAL_VENUE.location,
        sourceURL: REVIVAL_VENUE.website,
        lastUpdated: new Date(),
        source: 'Revival Event Venue',
        url: event.eventUrl,
    ...
    console.error(`❌ Error scraping Revival Event Venue events: ${error.message}`);
  }
...
async function scrapeRevivalEventVenueEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeRevivalEventVenueEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeRevivalEventVenueEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeRevivalEventVenueEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Revival Event Venue scraper completed. Added ${addedEvents} new events.`);
    }
    .catch(error => {
      console.error('❌ Error running Revival Event Venue scraper:', error);
      process.exit(1);
    };
}
