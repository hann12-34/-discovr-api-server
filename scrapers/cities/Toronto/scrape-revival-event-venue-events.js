...
const BASE_URL = 'https://www.revivaleventvenue.com';
...
async function scrapeRevivalEventVenueEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Revival Event Venue events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Revival Event Venue URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Revival Event Venue URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Revival Event Venue page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Revival Event Venue event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Revival Event Venue in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Revival Event Venue events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Revival Event Venue-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Revival Event Venue event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Revival Event Venue event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Revival Event Venue event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Revival Event Venue events`);
    return events;
  } catch (error) {
    console.error('Error scraping Revival Event Venue events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRevivalEventVenueEventsClean  };
