...
const BASE_URL = 'https://www.ontarioplace.com';
...
async function scrapeOntarioplaceEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Ontarioplace events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Ontarioplace URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Ontarioplace URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Ontarioplace page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Ontarioplace event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Ontarioplace in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Ontarioplace events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Ontarioplace event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Ontarioplace event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Ontarioplace event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Ontarioplace events`);
    return events;
  } catch (error) {
    console.error('Error scraping Ontarioplace events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeOntarioplaceEventsClean  };
