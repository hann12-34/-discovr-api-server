...
const BASE_URL = 'https://www.ajaxcommunity.com';
...
async function scrapeAjaxCommunityEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Ajax Community events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Ajax Community URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Ajax Community URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Ajax Community page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Ajax Community event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Ajax Community in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Ajax Community events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Ajax Community event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Ajax Community event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Ajax Community event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Ajax Community events`);
    return events;
  } catch (error) {
    console.error('Error scraping Ajax Community events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeAjaxCommunityEventsClean  };
