...
const BASE_URL = 'https://www.steamwhistleenhanced.com';
...
async function scrapeSteamWhistleEnhancedEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Steam Whistle Enhanced events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Steam Whistle Enhanced URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Steam Whistle Enhanced URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Steam Whistle Enhanced page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Steam Whistle Enhanced event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Steam Whistle Enhanced in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Steam Whistle Enhanced events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Steam Whistle Enhanced-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Steam Whistle Enhanced event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Steam Whistle Enhanced event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Steam Whistle Enhanced event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Steam Whistle Enhanced events`);
    return events;
  } catch (error) {
    console.error('Error scraping Steam Whistle Enhanced events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeSteamWhistleEnhancedEventsClean  };
