...
const BASE_URL = 'https://www.bloorwestvillage.com';
...
async function scrapeBloorwestvillageEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Bloorwestvillage events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Bloorwestvillage URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Bloorwestvillage URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Bloorwestvillage page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Bloorwestvillage event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Bloorwestvillage in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Bloorwestvillage events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Bloorwestvillage-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Bloorwestvillage event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Bloorwestvillage event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Bloorwestvillage event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Bloorwestvillage events`);
    return events;
  } catch (error) {
    console.error('Error scraping Bloorwestvillage events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeBloorwestvillageEventsClean  };
