...
async function scrapeFactoryTheatreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Factory Theatre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Factory Theatre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Factory Theatre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Factory Theatre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Factory Theatre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Factory Theatre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Factory Theatre events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Factory Theatre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Factory Theatre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Factory Theatre event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeFactoryTheatreEventsClean  };
