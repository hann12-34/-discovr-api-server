...
const BASE_URL = 'https://www.roundhousepark.com';
...
async function scrapeRoundhouseParkEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Roundhouse Park events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Roundhouse Park URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Roundhouse Park URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Roundhouse Park page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Roundhouse Park event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Roundhouse Park in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Roundhouse Park events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Roundhouse Park-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Roundhouse Park event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Roundhouse Park event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Roundhouse Park event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeRoundhouseParkEventsClean  };
