...
const BASE_URL = 'https://www.waterfronttoronto.com';
...
async function scrapeWaterfrontTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Waterfront Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Waterfront Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Waterfront Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Waterfront Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Waterfront Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Waterfront Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Waterfront Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Waterfront Toronto-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Waterfront Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Waterfront Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Waterfront Toronto event "${event.title}":`, error);
      }
    }
    ...
    console.error('Error scraping Waterfront Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeWaterfrontTorontoEventsClean  };
