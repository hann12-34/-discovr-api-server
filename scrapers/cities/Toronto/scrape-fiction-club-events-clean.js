...
const BASE_URL = 'http://www.fictionclub.ca';
...
async function scrapeFictionClubEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Fiction Club events (clean version)...');

    // Anti-bot delay
    await delay(Math.floor(Math.random() * 2000) + 1000);

    const urlsToTry = [
      'http://www.fictionclub.ca/',
      'https://www.fictionclub.ca/',
      'http://www.fictionclub.ca/events/',
      'https://www.fictionclub.ca/events/',
      'http://www.fictionclub.ca/shows/'
    ];
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Fiction Club URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Fiction Club URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Fiction Club page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Fiction Club event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Fiction Club in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Fiction Club events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Fiction Club event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Fiction Club event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Fiction Club event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Fiction Club events`);
    return events;
  } catch (error) {
    console.error('Error scraping Fiction Club events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeFictionClubEventsClean  };
