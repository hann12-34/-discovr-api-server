/**
 * Toybox Toronto Events Scraper
 * Based on events from https://toyboxtoronto.com/events/
 */
...
// Constants
const TOYBOX_URL = 'https://toyboxtoronto.com/';
const TOYBOX_VENUE = {
  name: 'Toybox Toronto',
...
function generateEventId(title, startDate) {
  const dataToHash = `${TOYBOX_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function extractCategories(title, description, eventType = '') {
  ...
  if (text.match(/\b(alternative|indie|underground)\b/)) {
...
function normalizeUrl(url, baseUrl = 'https://toyboxtoronto.com') {
  if (!url) return '';
...
async function scrapeToyboxTorontoEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Toybox Toronto website...');
    
    // Fetch HTML content
    const response = await axios.get(TOYBOX_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card').each((i, el) => {
      try {
        ...
        const description = element.find('p, .description, .details, .event-description').first().text().trim() || 
                           'Experience unique nightlife at Toybox Toronto with creative atmosphere and diverse entertainment in Adelaide West.';
    ...
    console.error('❌ Error scraping Toybox Toronto events:', error);
  }
...
async function scrapeToyboxTorontoEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeToyboxTorontoEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeToyboxTorontoEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeToyboxTorontoEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Toybox Toronto scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Toybox Toronto scraper:', error);
      process.exit(1);
    });
}
