...
const BASE_URL = 'https://www.toyboxtoronto.com';
...
async function scrapeToyboxTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toybox Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toybox Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toybox Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toybox Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toybox Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toybox Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toybox Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toybox Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toybox Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toybox Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Toybox Toronto events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeToyboxTorontoEventsClean  };
