...
const BASE_URL = 'https://www.burlingtonperformingarts.com';
...
const getGardinerVenue = (city) => ({
  name: 'Burlington Performing Arts',
...
async function scrapeBurlingtonPerformingArtsEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Burlington Performing Arts events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Burlington Performing Arts URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Burlington Performing Arts URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Burlington Performing Arts page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Burlington Performing Arts event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Burlington Performing Arts in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Burlington Performing Arts events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Burlington Performing Arts-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Burlington Performing Arts event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Burlington Performing Arts event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Burlington Performing Arts event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Burlington Performing Arts events`);
    return events;
  } catch (error) {
    console.error('Error scraping Burlington Performing Arts events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeBurlingtonPerformingArtsEventsClean  };
