...
const BASE_URL = 'https://www.secondcity.com/shows/toronto';
...
async function scrapeSecondCityEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Second City events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Second City URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Second City URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Second City page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Second City event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Second City in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Second City events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Second City event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Second City event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Second City event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeSecondCityEventsClean  };
