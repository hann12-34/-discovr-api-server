...
const BASE_URL = 'https://www.thephoenixconcerttheatre.com';
...
async function scrapePhoenixConcertTheatreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Phoenix Concert Theatre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Phoenix Concert Theatre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Phoenix Concert Theatre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Phoenix Concert Theatre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Phoenix Concert Theatre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Phoenix Concert Theatre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Phoenix Concert Theatre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Phoenix Concert Theatre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Phoenix Concert Theatre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Phoenix Concert Theatre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Phoenix Concert Theatre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Phoenix Concert Theatre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Phoenix Concert Theatre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapePhoenixConcertTheatreEventsClean  };
