...
const BASE_URL = 'https://www.heritagetrust.on.ca';
...
async function scrapeElginWinterGardenEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Elgin Winter Garden URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Elgin Winter Garden URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Elgin Winter Garden page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Elgin Winter Garden event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Elgin Winter Garden events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Elgin Winter Garden-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Elgin Winter Garden event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Elgin Winter Garden event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Elgin Winter Garden event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Elgin Winter Garden events`);
    return events;
  } catch (error) {
    console.error('Error scraping Elgin Winter Garden events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeElginWinterGardenEventsClean  };
