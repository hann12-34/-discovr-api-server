...
const BASE_URL = 'https://www.rosetheatre.com';
...
async function scrapeRoseTheatreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rose Theatre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rose Theatre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rose Theatre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rose Theatre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rose Theatre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rose Theatre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rose Theatre events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rose Theatre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rose Theatre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rose Theatre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Rose Theatre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Rose Theatre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRoseTheatreEventsClean  };
