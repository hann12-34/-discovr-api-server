...
const BASE_URL = 'https://www.unionville.com';
...
async function scrapeUnionvilleEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Unionville events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Unionville URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Unionville URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Unionville page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Unionville event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Unionville in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Unionville events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Unionville event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Unionville event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Unionville event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Unionville events`);
    return events;
  } catch (error) {
    console.error('Error scraping Unionville events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeUnionvilleEventsClean  };
