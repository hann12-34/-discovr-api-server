...
const BASE_URL = 'https://www.vertigotoronto.com';
...
// Enhanced filtering for Vertigo nightclub content
const isValidEvent = (title) => {
  if (!title || title.length < 5) return false;
  
  const skipPatterns = [
    /^(home|about|contact|menu|search|login|register|subscribe|follow|visit|hours|directions|book|table)$/i,
    /^(vertigo|toronto|nightclub|club|bar|drink|bottle|service|party|dance|music|dj|entertainment)$/i,
...
const hasEventCharacteristics = (title, description, dateText, eventUrl) => {
  ...
  const eventIndicators = [
    ...
    /saturday|friday|weekend|tonight|tomorrow|evening|late|after|midnight|club|dance/i,
    /\d{4}|\d{1,2}\/\d{1,2}|january|february|march|april|may|june|july|august|september|october|november|december/i,
    /guest|artist|performer|headliner|feature|resident|opening|closing|set|stage|floor|rooftop/i
  ];
...
const getVertigoVenue = (city) => ({
  name: 'Vertigo Toronto',
  address: '184 Front St E, Toronto, ON M5A 4N3',
  city: 'Toronto',
  state: 'ON',
  zip: 'M5A 4N3',
  latitude: 43.6491,
...
async function scrapeVertigoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Vertigo Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Vertigo Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Vertigo Toronto URLs failed, cannot proceed');
      return [];
    }

    const $ = cheerio.load(response.data);
    const candidateEvents = [];
    const venue = getVertigoVenue(city);

    console.log(`📊 Vertigo Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Vertigo Toronto event: "${title}"`);
        ...
        qualityScore += /party|dj|live|special|vip|guest|rooftop/.test(title.toLowerCase()) ? 1 : 0;
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at Vertigo Toronto rooftop nightclub.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Vertigo Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          description: event.description || `Experience ${event.title} at Vertigo Toronto rooftop nightclub.`,
          startDate: startDate || new Date(),
          endDate: endDate || startDate || new Date(),
          venue: venue,
          price: extractPrice('Cover charge applies') || 'Contact venue',
          categories: extractCategories('Nightlife, Music, Dancing, Rooftop, Toronto'),
          source: 'Vertigo Toronto-Toronto',
          city: 'Toronto',
          featured: false,
          tags: ['nightclub', 'rooftop', 'music', 'dancing', 'toronto'],
          createdAt: new Date(),
          updatedAt: new Date()
        };
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Vertigo Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Vertigo Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Vertigo Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Vertigo Toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping Vertigo Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
