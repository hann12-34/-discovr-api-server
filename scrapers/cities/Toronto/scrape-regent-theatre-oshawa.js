...
const BASE_URL = 'https://www.regenttheatreoshawa.com';
...
async function scrapeRegentTheatreOshawaEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Regent Theatre Oshawa events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Regent Theatre Oshawa URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Regent Theatre Oshawa URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Regent Theatre Oshawa page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Regent Theatre Oshawa event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Regent Theatre Oshawa in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Regent Theatre Oshawa events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Regent Theatre Oshawa-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Regent Theatre Oshawa event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Regent Theatre Oshawa event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Regent Theatre Oshawa event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Regent Theatre Oshawa events`);
    return events;
  } catch (error) {
    console.error('Error scraping Regent Theatre Oshawa events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRegentTheatreOshawaEventsClean  };
