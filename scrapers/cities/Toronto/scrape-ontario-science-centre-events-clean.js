...
const BASE_URL = 'https://ontariosciencecentre.ca';
...
async function scrapeOntarioScienceCentreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Ontario Science Centre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Ontario Science Centre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Ontario Science Centre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Ontario Science Centre page loaded from ${workingUrl}, analyzing content...`);

    // Enhanced selectors for Science Centre content
    const eventSelectors = [
      '.event, .program, .exhibition, .activity',
      '.event-item, .program-item, .activity-item',
      '.calendar-event, .upcoming-event',
      '.card, .tile, .content-item',
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        const titleSelectors = ['h1', 'h2', 'h3', 'h4', '.title', '.event-title', '.program-title', '.activity-title', '.headline'];
        let title = '';
        ...
        console.log(`📝 Found qualified Ontario Science Centre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Ontario Science Centre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Ontario Science Centre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Ontario Science Centre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Ontario Science Centre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Ontario Science Centre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Ontario Science Centre event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeOntarioScienceCentreEventsClean  };
