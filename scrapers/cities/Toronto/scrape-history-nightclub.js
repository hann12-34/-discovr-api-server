/**
 * History Toronto Scraper
 * Popular nightlife venue
 * URL: https://historyto.com
 */
...
async function scrape(city = 'Toronto') {
  console.log('🎭 Scraping History nightclub events...');
  
  try {
    const response = await axios.get('https://historyto.com/events', {
      headers: {
    ...
    $('.event, .show, article, [class*="event"], [class*="show"], .card').each((i, el) => {
      ...
      if (eventUrl && !eventUrl.startsWith('http')) {
        eventUrl = eventUrl.startsWith('/') 
          ? 'https://historyto.com' + eventUrl
          : 'https://historyto.com/' + eventUrl;
      }
      if (!eventUrl) {
        eventUrl = 'https://historyto.com/events';
      }
    ...
    console.error('  ⚠️  History error:', error.message);
    return [];
  }
}
