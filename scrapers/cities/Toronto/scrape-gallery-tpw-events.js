...
const BASE_URL = 'https://www.gallerytpw.com';
...
async function scrapeGalleryTpwEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Gallery Tpw events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Gallery Tpw URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Gallery Tpw URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Gallery Tpw page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Gallery Tpw event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Gallery Tpw in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Gallery Tpw events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Gallery Tpw event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Gallery Tpw event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Gallery Tpw event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeGalleryTpwEventsClean  };
