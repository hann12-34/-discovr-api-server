...
async function scrapeCasaLomaEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Casa Loma events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Casa Loma URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Casa Loma URLs failed, cannot proceed');
      return [];
    }

    const $ = cheerio.load(response.data);
    const candidateEvents = [];
    const venue = getCasaLomaVenue(city);

    console.log(`📊 Casa Loma page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Casa Loma event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Casa Loma in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Casa Loma events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Casa Loma event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Casa Loma event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Casa Loma event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Casa Loma events`);
    return events;
  } catch (error) {
    console.error('Error scraping Casa Loma events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeCasaLomaEventsClean  };
