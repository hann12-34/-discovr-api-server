...
async function scrapeQueenBooksEvents() {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-queen-books-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          venue: { ...RegExp.venue: { ...RegExp.venue: QUEEN_BOOKS_VENUE,, city }, city },,
...
module.exports = scrapeQueenBooksEvents;