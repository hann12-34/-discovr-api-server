...
const BASE_URL = 'https://www.club54.com';
...
async function scrapeClub54EventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Club54 events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Club54 URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Club54 URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Club54 page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Club54 event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Club54 in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Club54 events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Club54 event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Club54 event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Club54 event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Club54 events`);
    return events;
  } catch (error) {
    console.error('Error scraping Club54 events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeClub54EventsClean  };
