...
const BASE_URL = 'https://www.markhamtheatre.com';
...
async function scrapeMarkhamTheatreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Markham Theatre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Markham Theatre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Markham Theatre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Markham Theatre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Markham Theatre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Markham Theatre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Markham Theatre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Markham Theatre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Markham Theatre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Markham Theatre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Markham Theatre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Markham Theatre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Markham Theatre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeMarkhamTheatreEventsClean  };
