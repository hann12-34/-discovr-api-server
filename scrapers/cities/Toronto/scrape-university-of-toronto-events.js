...
const BASE_URL = 'https://www.universityoftoronto.com';
...
async function scrapeUniversityOfTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping University Of Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying University Of Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All University Of Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 University Of Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified University Of Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the University Of Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality University Of Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added University Of Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate University Of Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing University Of Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new University Of Toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping University Of Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeUniversityOfTorontoEventsClean  };
