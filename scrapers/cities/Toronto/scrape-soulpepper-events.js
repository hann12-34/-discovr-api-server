...
const BASE_URL = 'https://soulpepper.ca';
...
async function scrapeSoulpepperEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Soulpepper events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Soulpepper URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Soulpepper URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Soulpepper page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Soulpepper event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Soulpepper in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Soulpepper events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Soulpepper event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Soulpepper event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Soulpepper event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Soulpepper events`);
    return events;
  } catch (error) {
    console.error('Error scraping Soulpepper events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeSoulpepperEventsClean  };
