...
class CenturyScraper extends AbstractScraper {
    constructor(city) {
        super();
        this.city = city;
        this.source = 'Century';
    ...
    _parseDate(dateText, timeText = '') {
        if (!dateText) return null;
        try {
            dateText = dateText.trim().replace(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\s*/i, '');
            const fullDateStr = `${dateText}, ${new Date().getFullYear()} ${timeText}`.trim();
            const date = new Date(fullDateStr);
            return isNaN(date.getTime()) ? null : date;
        } catch (e) {
            this.log(`Error parsing date: ${dateText}`);
            return null;
        }
    }
