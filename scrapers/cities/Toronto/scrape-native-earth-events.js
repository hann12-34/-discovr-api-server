...
const BASE_URL = 'https://www.nativeearth.com';
...
async function scrapeNativeEarthEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Native Earth events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Native Earth URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Native Earth URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Native Earth page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Native Earth event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Native Earth in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Native Earth events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Native Earth event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Native Earth event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Native Earth event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeNativeEarthEventsClean  };
