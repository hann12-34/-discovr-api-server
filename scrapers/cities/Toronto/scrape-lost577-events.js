...
const BASE_URL = 'https://www.lost577.com';
...
async function scrapeLost577EventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Lost577 URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Lost577 URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Lost577 page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Lost577 event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Lost577 events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Lost577 event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Lost577 event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Lost577 event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Lost577 events`);
    return events;
  } catch (error) {
    console.error('Error scraping Lost577 events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeLost577EventsClean  };
