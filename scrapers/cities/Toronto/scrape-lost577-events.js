/**
 * Lost 577 Events Scraper
 * Based on events from https://www.lost577.com/events
 */
...
// Constants
const LOST577_URL = 'https://www.lost577.com/events';
const LOST577_VENUE = {
  name: 'Lost 577',
...
function generateEventId(title, startDate) {
  const dataToHash = `${LOST577_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function normalizeUrl(url, baseUrl = 'https://www.lost577.com') {
  if (!url) return '';
...
async function scrapeLost577Events(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Lost 577 website...');
    
    // Fetch HTML content
    const response = await axios.get(LOST577_URL, {
      timeout: 15000,
      headers: {
    ...
    console.error('❌ Error scraping Lost 577 events:', error);
  }
...
if (require.main === module) {
  scrapeLostEvents()
    .then(addedEvents => {
      console.log(`✅ Lost 577 scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Lost 577 scraper:', error);
      process.exit(1);
    });
}
