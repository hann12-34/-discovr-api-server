...
const BASE_URL = 'https://steamwhistle.ca';
...
async function scrapeSteamWhistleEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Steam Whistle events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Steam Whistle URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Steam Whistle URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Steam Whistle page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Steam Whistle event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Steam Whistle in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Steam Whistle events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Steam Whistle event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Steam Whistle event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Steam Whistle event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Steam Whistle events`);
    return events;
  } catch (error) {
    console.error('Error scraping Steam Whistle events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeSteamWhistleEventsClean  };
