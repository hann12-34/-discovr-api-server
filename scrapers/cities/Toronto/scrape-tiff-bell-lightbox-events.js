...
const BASE_URL = 'https://www.tiffbelllightbox.com';
...
async function scrapeTiffBellLightboxEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Tiff Bell Lightbox events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Tiff Bell Lightbox URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Tiff Bell Lightbox URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Tiff Bell Lightbox page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Tiff Bell Lightbox event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Tiff Bell Lightbox in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Tiff Bell Lightbox events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Tiff Bell Lightbox event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Tiff Bell Lightbox event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Tiff Bell Lightbox event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeTiffBellLightboxEventsClean  };
