...
const BASE_URL = 'https://www.pickeringcasino.com';
...
async function scrapePickeringCasinoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Pickering Casino events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Pickering Casino URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Pickering Casino URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Pickering Casino page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Pickering Casino event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Pickering Casino in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Pickering Casino events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Pickering Casino-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Pickering Casino event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Pickering Casino event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Pickering Casino event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Pickering Casino events`);
    return events;
  } catch (error) {
    console.error('Error scraping Pickering Casino events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapePickeringCasinoEventsClean  };
