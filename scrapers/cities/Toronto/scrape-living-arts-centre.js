...
const BASE_URL = 'https://www.livingartscentre.com';
...
async function scrapeLivingArtsCentreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Living Arts Centre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Living Arts Centre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Living Arts Centre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Living Arts Centre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Living Arts Centre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Living Arts Centre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Living Arts Centre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Living Arts Centre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Living Arts Centre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Living Arts Centre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Living Arts Centre event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeLivingArtsCentreEventsClean  };
