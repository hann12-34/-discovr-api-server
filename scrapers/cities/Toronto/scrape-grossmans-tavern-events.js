...
async function scrapeGrossmansTavernEvents() {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-grossmans-tavern-events.js Toronto');
    process.exit(1);
  }
  let browser;
  let addedEvents = 0;
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          endDate: endDate,
          venue: {
                name: city, ...GROSSMANS_VENUE, city
            },
...
module.exports = scrapeGrossmansTavernEvents;