...
const BASE_URL = 'https://www.grossmanstavern.com';
...
async function scrapeGrossmansTavernEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Grossmans Tavern events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Grossmans Tavern URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Grossmans Tavern URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Grossmans Tavern page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Grossmans Tavern event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Grossmans Tavern in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Grossmans Tavern events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Grossmans Tavern-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Grossmans Tavern event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Grossmans Tavern event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Grossmans Tavern event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Grossmans Tavern events`);
    return events;
  } catch (error) {
    console.error('Error scraping Grossmans Tavern events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeGrossmansTavernEventsClean  };
