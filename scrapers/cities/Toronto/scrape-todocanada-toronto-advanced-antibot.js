...
const BASE_URL = 'https://www.todocanadatorontoadvancedantibot.com';
...
const getGardinerVenue = (city) => ({
  name: 'Todocanada Toronto Advanced Antibot',
...
async function scrapeTodocanadaTorontoAdvancedAntibotEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Todocanada Toronto Advanced Antibot events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Todocanada Toronto Advanced Antibot URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Todocanada Toronto Advanced Antibot URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Todocanada Toronto Advanced Antibot page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Todocanada Toronto Advanced Antibot event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Todocanada Toronto Advanced Antibot in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Todocanada Toronto Advanced Antibot events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Todocanada Toronto Advanced Antibot-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Todocanada Toronto Advanced Antibot event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Todocanada Toronto Advanced Antibot event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Todocanada Toronto Advanced Antibot event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Todocanada Toronto Advanced Antibot events`);
    return events;
  } catch (error) {
    console.error('Error scraping Todocanada Toronto Advanced Antibot events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTodocanadaTorontoAdvancedAntibotEventsClean  };
