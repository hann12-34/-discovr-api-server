...
const BASE_URL = 'https://www.toronto.ca/explore-enjoy/parks-recreation/parks-trails/';
...
async function scrapeHighparkEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Highpark events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Highpark URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Highpark URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Highpark page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Highpark event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Highpark in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Highpark events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Highpark event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Highpark event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Highpark event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Highpark events`);
    return events;
  } catch (error) {
    console.error('Error scraping Highpark events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeHighparkEventsClean  };
