...
class OntarioScienceCentreEventsScraper extends AbstractScraper {
    ...
    _parseDate(daeventDateText) {
        ...
        if (daeventDateText.includes(' - ')) {
            const parts = daeventDateText.split(' - ');
        ...
        if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
            this.log(`Could not parse date: "${daeventDateText}". Using current date as fallback.`);
...
module.exports = async (city) => {
  const scraper = new OntarioScienceCentreEventsScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.OntarioScienceCentreEventsScraper = OntarioScienceCentreEventsScraper;