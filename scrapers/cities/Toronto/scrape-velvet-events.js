...
const BASE_URL = 'https://www.velvet.com';
...
async function scrapeVelvetEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Velvet events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Velvet URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Velvet URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Velvet page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Velvet event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Velvet in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Velvet events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Velvet event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Velvet event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Velvet event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Velvet events`);
    return events;
  } catch (error) {
    console.error('Error scraping Velvet events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeVelvetEventsClean  };
