...
const BASE_URL = 'https://www.bloodbrothers.com';
...
async function scrapeBloodBrothersEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Blood Brothers events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Blood Brothers URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Blood Brothers URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Blood Brothers page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Blood Brothers event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Blood Brothers in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Blood Brothers events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Blood Brothers event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Blood Brothers event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Blood Brothers event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Blood Brothers events`);
    return events;
  } catch (error) {
    console.error('Error scraping Blood Brothers events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeBloodBrothersEventsClean  };
