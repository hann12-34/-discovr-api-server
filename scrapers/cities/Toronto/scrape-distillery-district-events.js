...
async function scrapeDistilleryDistrictEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Distillery District events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Distillery District URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Distillery District URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Distillery District page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Distillery District event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Distillery District in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Distillery District events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Distillery District event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Distillery District event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Distillery District event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeDistilleryDistrictEventsClean  };
