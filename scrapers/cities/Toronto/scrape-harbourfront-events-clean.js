...
async function scrapeHarbourfrontEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Harbourfront events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Harbourfront URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Harbourfront URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Harbourfront page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Harbourfront event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Harbourfront in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Harbourfront events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Harbourfront event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Harbourfront event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Harbourfront event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Harbourfront events`);
    return events;
  } catch (error) {
    console.error('Error scraping Harbourfront events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeHarbourfrontEventsClean  };
