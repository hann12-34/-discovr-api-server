...
const BASE_URL = 'https://www.6ixlounge.com';
...
async function scrape6ixLoungeEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping 6ix Lounge events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying 6ix Lounge URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All 6ix Lounge URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 6ix Lounge page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified 6ix Lounge event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the 6ix Lounge in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality 6ix Lounge events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added 6ix Lounge event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate 6ix Lounge event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing 6ix Lounge event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new 6ix Lounge events`);
    return events;
  } catch (error) {
    console.error('Error scraping 6ix Lounge events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrape6ixLoungeEventsClean  };
