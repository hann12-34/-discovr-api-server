...
const SIXIX_LOUNGE_URL = 'https://the6ixlounge.com/#events';
const SIXIX_LOUNGE_VENUE = {
  name: 'The 6ix Lounge',
  address: 'Toronto, ON',
  website: 'https://the6ixlounge.com/',
...
async function scrape6ixLoungeEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from The 6ix Lounge website...');
    
    // Fetch HTML content
    const response = await axios.get('https://the6ixlounge.com/', {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card, #events').each((i, el) => {
      try {
        const $el = $(el);
        ...
        // Extract event URL
        const eventUrl = $el.find('a').attr('href') || SIXIX_LOUNGE_URL;
        const fullEventUrl = eventUrl.startsWith('http') ? eventUrl : `https://the6ixlounge.com${eventUrl}`;
        
        // Extract image
        const imageUrl = $el.find('img').attr('src') || '';
        const fullImageUrl = imageUrl && !imageUrl.startsWith('http') ? 
                            `https://the6ixlounge.com${imageUrl}` : imageUrl;
    ...
    for (const event of events) {
      ...
      const eventId = generateEventId(SIXIX_LOUNGE_VENUE.name, event.title, startDate.toISOString());
      const categories = ['Nightlife', 'Lounge', 'VIP', 'Entertainment'];
      ...
      const eventDoc = {
        ...
        startDate,
        endDate,
        venue: SIXIX_LOUNGE_VENUE,
        categories: categories,
        price: price || "Contact venue",
        officialWebsite: SIXIX_LOUNGE_VENUE.website,
        location: SIXIX_LOUNGE_VENUE.location,
        sourceURL: SIXIX_LOUNGE_VENUE.website,
        lastUpdated: new Date(),
        source: 'The 6ix Lounge',
        url: event.eventUrl,
    ...
    console.error(`❌ Error scraping The 6ix Lounge events: ${error.message}`);
  }
...
async function scrape6ixLoungeEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrape6ixLoungeEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrape6ixLoungeEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrape6ixLoungeEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ The 6ix Lounge scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running The 6ix Lounge scraper:', error);
      process.exit(1);
    });
}
