...
const BASE_URL = 'https://www.popboxmrkt.com';
...
async function scrapePopboxMrktEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Popbox Mrkt events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Popbox Mrkt URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Popbox Mrkt URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Popbox Mrkt page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Popbox Mrkt event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Popbox Mrkt in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Popbox Mrkt events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Popbox Mrkt event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Popbox Mrkt event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Popbox Mrkt event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapePopboxMrktEventsClean  };
