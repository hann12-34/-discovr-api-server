/**
 * DPRTMNT Events Scraper
 * Based on events from https://dprtmnt.com/events/
 */
...
// Constants
const DPRTMNT_URL = 'https://dprtmnt.com/events/';
const DPRTMNT_VENUE = {
  name: 'DPRTMNT',
...
function generateEventId(title, startDate) {
  const dataToHash = `${DPRTMNT_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function normalizeUrl(url, baseUrl = 'https://dprtmnt.com') {
  if (!url) return '';
...
async function scrapeDprtmntEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from DPRTMNT website...');
    
    // Fetch HTML content
    const response = await axios.get(DPRTMNT_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card').each((i, el) => {
      try {
        ...
        const description = element.find('p, .description, .details, .event-description').first().text().trim() || 
                           'Experience Toronto\'s most exclusive nightlife destination at DPRTMNT with premium entertainment and luxury atmosphere.';
    ...
    console.error('❌ Error scraping DPRTMNT events:', error);
  }
...
async function scrapeDprtmntEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeDprtmntEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeDprtmntEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeDprtmntEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ DPRTMNT scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running DPRTMNT scraper:', error);
      process.exit(1);
    });
}
