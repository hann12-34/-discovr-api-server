...
const BASE_URL = 'https://www.dprtmnt.com';
...
async function scrapeDprtmntEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Dprtmnt events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Dprtmnt URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Dprtmnt URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Dprtmnt page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Dprtmnt event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Dprtmnt in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Dprtmnt events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Dprtmnt event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Dprtmnt event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Dprtmnt event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Dprtmnt events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeDprtmntEventsClean  };
