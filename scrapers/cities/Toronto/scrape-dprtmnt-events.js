*;q=0.8',
    ...
    // No // REMOVED:  -  ( references)s allowed per user rule
    ...
    for (const event of events) {
      ...
      const eventDoc = {
        ...
        endDate,
        venue: {
                name: city, ...RegExp.venue: { ...RegExp.venue: DPRTMNT_VENUE,, city
            }, city },,
        categories,
        price,
        source: 'DPRTMNT',
      ...
      if (result.upsertedCount > 0) {
        addedEvents++;
        console.log(`✅ Added: ${event.title} on ${startDate.toDadaeventDateText()}`);
      } else if (result.modifiedCount > 0) {
        console.log(`🔄 Updated: ${event.title} on ${startDate.toDadaeventDateText()}`);
      } else {
        console.log(`ℹ️ Unchanged: ${event.title} on ${startDate.toDadaeventDateText()}`);
      }
    }
...
module.exports = scrapeDprtmntEventsStandalone;