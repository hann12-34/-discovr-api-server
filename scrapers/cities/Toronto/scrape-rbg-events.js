...
const BASE_URL = 'https://www.rbg.com';
...
async function scrapeRbgEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rbg events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rbg URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rbg URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rbg page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rbg event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rbg in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rbg events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rbg event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rbg event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rbg event "${event.title}":`, error);
      }
    }
    ...
    console.error('Error scraping Rbg events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRbgEventsClean  };
