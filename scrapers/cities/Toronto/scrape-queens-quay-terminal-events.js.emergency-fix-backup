...
async function scrapeQueensQuayEvents() {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-queens-quay-terminal-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          venue: { ...RegExp.venue: { ...RegExp.venue: QUEENS_QUAY_VENUE,, city }, city },,
...
module.exports = scrapeQueensQuayEvents;