...
const BASE_URL = 'https://www.nesttoronto.com';
...
async function scrapeNestTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Nest Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Nest Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Nest Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Nest Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Nest Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Nest Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Nest Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Nest Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Nest Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Nest Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Nest Toronto events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeNestTorontoEventsClean  };
