/**
 * Nest Toronto Events Scraper
 * Based on events from https://www.nesttoronto.ca/
 */
...
// Constants
const NEST_URL = 'https://www.nesttoronto.ca/';
const NEST_VENUE = {
  name: 'Nest Toronto',
...
function generateEventId(title, startDate) {
  const dataToHash = `${NEST_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function extractCategories(title, description, eventType = '') {
  ...
  if (text.match(/\b(intimate|cozy|underground|alternative)\b/)) {
...
function normalizeUrl(url, baseUrl = 'https://www.nesttoronto.ca') {
  if (!url) return '';
...
async function scrapeNestTorontoEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Nest Toronto website...');
    
    // Fetch HTML content
    const response = await axios.get(NEST_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card').each((i, el) => {
      try {
        ...
        const description = element.find('p, .description, .details, .event-description').first().text().trim() || 
                           'Experience intimate nightlife at Nest Toronto with cozy atmosphere and great music in the heart of College Street.';
    ...
    console.error('❌ Error scraping Nest Toronto events:', error);
  }
...
async function scrapeNestTorontoEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeNestTorontoEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeNestTorontoEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeNestTorontoEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Nest Toronto scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Nest Toronto scraper:', error);
      process.exit(1);
    });
}
