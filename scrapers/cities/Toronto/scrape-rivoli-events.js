...
const BASE_URL = 'https://www.rivoli.com';
...
async function scrapeRivoliEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rivoli events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rivoli URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rivoli URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rivoli page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rivoli event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rivoli in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rivoli events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rivoli event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rivoli event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rivoli event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeRivoliEventsClean  };
