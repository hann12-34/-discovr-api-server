...
const BASE_URL = 'https://www.torontoproductionready.com';
...
async function scrapeTorontoProductionReadyEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Production Ready events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Production Ready URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Production Ready URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Production Ready page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Production Ready event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Production Ready in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Production Ready events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Toronto Production Ready-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Production Ready event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Production Ready event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Production Ready event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Toronto Production Ready events`);
    return events;
  } catch (error) {
    console.error('Error scraping Toronto Production Ready events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTorontoProductionReadyEventsClean  };
