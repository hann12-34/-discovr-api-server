...
async function scrapeFutureNightlifeEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Future Nightlife events (clean version)...');

    // Anti-bot delay
    await delay(Math.floor(Math.random() * 2000) + 1000);

    const urlsToTry = [
      'https://futurenightlife.com/#upcoming-events',
      'https://futurenightlife.com/events/',
      'https://futurenightlife.com/calendar/',
      'https://futurenightlife.com/shows/',
      'https://futurenightlife.com/',
      `${BASE_URL}/`
    ];
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Future Nightlife event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Future Nightlife in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Future Nightlife events`);
...
module.exports = { scrapeEvents: scrapeFutureNightlifeEventsClean  };
