...
const BASE_URL = 'https://danforthmusichal.com';
...
async function scrapeDanforthMusicHallEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Danforth Music Hall events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Danforth Music Hall URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Danforth Music Hall URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Danforth Music Hall page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Danforth Music Hall event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Danforth Music Hall in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Danforth Music Hall events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Danforth Music Hall event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Danforth Music Hall event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Danforth Music Hall event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Danforth Music Hall events`);
    return events;
  } catch (error) {
    console.error('Error scraping Danforth Music Hall events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeDanforthMusicHallEventsClean  };
