...
const BASE_URL = 'https://www.ontariosciencecentre.com';
...
async function scrapeOntariosciencecentreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Ontariosciencecentre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Ontariosciencecentre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Ontariosciencecentre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Ontariosciencecentre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Ontariosciencecentre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Ontariosciencecentre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Ontariosciencecentre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Ontariosciencecentre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Ontariosciencecentre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Ontariosciencecentre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Ontariosciencecentre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Ontariosciencecentre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Ontariosciencecentre events:', error);
    throw error;
  } finally {
    await client.close();
  }
