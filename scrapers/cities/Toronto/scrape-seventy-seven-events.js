...
const BASE_URL = 'https://www.seventyseven.com';
...
async function scrapeSeventySevenEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Seventy Seven events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Seventy Seven URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Seventy Seven URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Seventy Seven page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Seventy Seven event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Seventy Seven in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Seventy Seven events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Seventy Seven event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Seventy Seven event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Seventy Seven event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Seventy Seven events`);
    return events;
  } catch (error) {
    console.error('Error scraping Seventy Seven events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeSeventySevenEventsClean  };
