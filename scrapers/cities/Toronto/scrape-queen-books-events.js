...
const BASE_URL = 'https://www.queenbooks.com';
...
async function scrapeQueenBooksEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Queen Books events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Queen Books URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Queen Books URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Queen Books page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Queen Books event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Queen Books in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Queen Books events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Queen Books event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Queen Books event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Queen Books event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeQueenBooksEventsClean  };
