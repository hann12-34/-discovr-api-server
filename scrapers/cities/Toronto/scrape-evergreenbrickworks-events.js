...
const BASE_URL = 'https://www.evergreenbrickworks.com';
...
async function scrapeEvergreenbrickworksEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Evergreenbrickworks events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Evergreenbrickworks URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Evergreenbrickworks URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Evergreenbrickworks page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Evergreenbrickworks event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Evergreenbrickworks in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Evergreenbrickworks events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Evergreenbrickworks-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Evergreenbrickworks event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Evergreenbrickworks event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Evergreenbrickworks event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Evergreenbrickworks events`);
    return events;
  } catch (error) {
    console.error('Error scraping Evergreenbrickworks events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeEvergreenbrickworksEventsClean  };
