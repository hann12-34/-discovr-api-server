...
async function scrapeXclubEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Xclub URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Xclub URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Xclub page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Xclub event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Xclub events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Xclub event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Xclub event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Xclub event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Xclub events`);
    return events;
  } catch (error) {
    console.error('Error scraping Xclub events:', error);
    throw error;
  } finally {
    await client.close();
  }
