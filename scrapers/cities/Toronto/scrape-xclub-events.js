...
const XCLUB_URL = 'https://www.thexclub.net/events.php';
const XCLUB_VENUE = {
  name: 'The X Club',
  address: 'Toronto, ON',
  website: 'https://www.thexclub.net/',
...
async function scrapeXClubEvents(eventsCollection) {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-xclub-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;

  try {
    console.log('🔍 Fetching events from The X Club website...');

    // Fetch HTML content
    const response = await axios.get(XCLUB_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card, tr, .event-row').each((i, el) => {
      try {
        const $el = $(el);

        // Extract title
        let title = $el.find('h1, h2, h3, h4, .title, .event-title, .name, td').first().text().trim();
        if (!title) {
        ...
        // Extract date
        const dateText = $el.find('.date, .event-date, .time, .when, td').text().trim() ||
                        $el.text().match(/\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2}(?:,?\s+\d{4}?\b/i)?.[0] ||
                        '';

        // Extract description
        const description = $el.find('p, .description, .event-description, .summary, td').text().trim() ||
                          $el.text().replace(title, '').trim().substring(0, 200);

        // Extract event URL
        const eventUrl = $el.find('a').attr('href') || XCLUB_URL;
        const fullEventUrl = eventUrl.startsWith('http') ? eventUrl : `https://www.thexclub.net/${eventUrl}`;

        // Extract image
        const imageUrl = $el.find('img').attr('src') || '';
        const fullImageUrl = imageUrl && !imageUrl.startsWith('http') ?
                            `https://www.thexclub.net/${imageUrl}` : imageUrl;
    ...
    for (const event of events) {
      ...
      const eventId = generateEventId(XCLUB_VENUE.name, event.title, startDate.toISOString());
      const categories = ['Nightlife', 'Alternative', 'Music', 'Underground'];
      ...
      const eventDoc = {
        ...
        officialWebsite: XCLUB_VENUE.website,
        location: XCLUB_VENUE.location,
        sourceURL: XCLUB_VENUE.website,
    ...
    console.error(`❌ Error scraping The X Club events: ${error.message}`);
  }
...
async function scrapeXClubEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeXClubEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeXClubEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeXClubEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ The X Club scraper completed. Added ${addedEvents} new events.`);
    }
    .catch(error => {
      console.error('❌ Error running The X Club scraper:', error);
      process.exit(1);
    };
}
