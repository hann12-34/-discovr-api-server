...
const BASE_URL = 'https://www.yorkuniversity.com';
...
async function scrapeYorkUniversityEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping York University events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying York University URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All York University URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 York University page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified York University event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the York University in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality York University events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added York University event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate York University event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing York University event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new York University events`);
    return events;
  } catch (error) {
    console.error('Error scraping York University events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeYorkUniversityEventsClean  };
