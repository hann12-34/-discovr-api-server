...
const BASE_URL = 'https://www.pandemonium.com';
...
async function scrapePandemoniumEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Pandemonium events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Pandemonium URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Pandemonium URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Pandemonium page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Pandemonium event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Pandemonium in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Pandemonium events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Pandemonium event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Pandemonium event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Pandemonium event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapePandemoniumEventsClean  };
