/**
 * 44 Toronto Events Scraper
 * Based on events from https://www.44toronto.com/events
 */
...
// Constants
const TORONTO44_URL = 'https://www.44toronto.com/events';
const TORONTO44_VENUE = {
  name: '44 Toronto',
...
function generateEventId(title, startDate) {
  const dataToHash = `${TORONTO44_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function extractCategories(title, description, eventType = '') {
  ...
  if (text.match(/\b(upscale|luxury|premium|high-end)\b/)) {
    categories.push('Upscale');
  }
...
function normalizeUrl(url, baseUrl = 'https://www.44toronto.com') {
  if (!url) return '';
...
async function scrape44TorontoEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from 44 Toronto website...');
    
    // Fetch HTML content
    const response = await axios.get(TORONTO44_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card').each((i, el) => {
      try {
        ...
        const description = element.find('p, .description, .details, .event-description').first().text().trim() || 
                           'Experience premium nightlife at 44 Toronto with world-class entertainment and sophisticated atmosphere.';
        ...
        if (title && title.length > 3) {
          events.push({
            ...
            description,
            imageUrl,
            eventUrl: eventUrl || TORONTO44_URL
          });
        }
      } catch (error) {
    ...
    console.error('❌ Error scraping 44 Toronto events:', error);
  }
...
async function scrape44TorontoEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrape44TorontoEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrape44TorontoEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrape44TorontoEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ 44 Toronto scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running 44 Toronto scraper:', error);
      process.exit(1);
    });
}
