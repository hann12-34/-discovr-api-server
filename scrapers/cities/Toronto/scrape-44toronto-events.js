...
const BASE_URL = 'https://www.44toronto.com';
...
async function scrape44torontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping 44toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying 44toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All 44toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 44toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified 44toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the 44toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality 44toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added 44toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate 44toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing 44toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new 44toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping 44toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrape44torontoEventsClean  };
