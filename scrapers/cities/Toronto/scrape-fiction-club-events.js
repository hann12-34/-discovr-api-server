/**
 * Fiction Club Events Scraper
 * Based on events from http://www.fictionclub.ca/index.html#fridays-tile
 */
...
// Constants
const FICTION_CLUB_URL = 'http://www.fictionclub.ca/index.html';
const FICTION_CLUB_VENUE = {
  name: 'Fiction Club',
  address: '1 Yonge St, Toronto, ON',
  city: 'Toronto',
  region: 'Ontario',
  country: 'Canada',
  url: 'http://www.fictionclub.ca',
};
...
function generateEventId(title, startDate) {
  const dataToHash = `${FICTION_CLUB_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function parseDateAndTime(dateText, timeText = '') {
  ...
  try {
    ...
    // Set end date (assume 3 hours duration for club events)
...
function extractCategories(title, description, eventType = '') {
  ...
  if (text.match(/\b(dj|dance|club|party|nightlife|music|electronic|house|techno|hip hop|r&b)\b/)) {
    categories.push('Nightlife');
  }
  if (text.match(/\b(live music|concert|band|performance|show)\b/)) {
    categories.push('Music');
  }
  if (text.match(/\b(friday|saturday|weekend|night)\b/)) {
    categories.push('Weekend Events');
  }
  if (text.match(/\b(special|vip|guest|celebrity)\b/)) {
...
function normalizeUrl(url, baseUrl = 'http://www.fictionclub.ca') {
  if (!url) return '';
...
async function scrapeFictionClubEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Fiction Club website...');
    
    // Fetch HTML content
    const response = await axios.get(FICTION_CLUB_URL, {
      timeout: 15000,
      headers: {
...
if (require.main === module) {
  scrapeFictionEvents()
    .then(addedEvents => {
      console.log(`✅ Fiction Club scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Fiction Club scraper:', error);
      process.exit(1);
    });
}
