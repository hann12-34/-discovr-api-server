...
const BASE_URL = 'https://www.kensingtonmarket.com';
...
async function scrapeKensingtonMarketEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Kensington Market events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Kensington Market URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Kensington Market URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Kensington Market page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Kensington Market event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Kensington Market in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Kensington Market events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Kensington Market-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Kensington Market event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Kensington Market event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Kensington Market event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Kensington Market events`);
    return events;
  } catch (error) {
    console.error('Error scraping Kensington Market events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeKensingtonMarketEventsClean  };
