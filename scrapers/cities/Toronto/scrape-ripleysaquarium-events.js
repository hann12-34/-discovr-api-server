...
async function scrapeRipleysaquariumEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Ripleysaquarium URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Ripleysaquarium URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Ripleysaquarium page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Ripleysaquarium event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Ripleysaquarium events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Ripleysaquarium-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Ripleysaquarium event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Ripleysaquarium event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Ripleysaquarium event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Ripleysaquarium events`);
    return events;
  } catch (error) {
    console.error('Error scraping Ripleysaquarium events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRipleysaquariumEventsClean  };
