...
const BASE_URL = 'https://www.lulalounge.com';
...
async function scrapeLulaLoungeEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Lula Lounge events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Lula Lounge URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Lula Lounge URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Lula Lounge page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Lula Lounge event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Lula Lounge in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Lula Lounge events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Lula Lounge event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Lula Lounge event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Lula Lounge event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Lula Lounge events`);
    return events;
  } catch (error) {
    console.error('Error scraping Lula Lounge events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeLulaLoungeEventsClean  };
