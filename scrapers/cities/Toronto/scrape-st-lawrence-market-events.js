...
const BASE_URL = 'https://www.stlawrencemarket.com';
...
async function scrapeStLawrenceMarketEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping St Lawrence Market events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying St Lawrence Market URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All St Lawrence Market URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 St Lawrence Market page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified St Lawrence Market event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the St Lawrence Market in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality St Lawrence Market events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'St Lawrence Market-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added St Lawrence Market event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate St Lawrence Market event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing St Lawrence Market event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeStLawrenceMarketEventsClean  };
