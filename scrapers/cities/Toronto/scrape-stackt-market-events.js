...
const BASE_URL = 'https://www.stacktmarket.com';
...
async function scrapeStacktMarketEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Stackt Market events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Stackt Market URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Stackt Market URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Stackt Market page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Stackt Market event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Stackt Market in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Stackt Market events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Stackt Market event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Stackt Market event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Stackt Market event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeStacktMarketEventsClean  };
