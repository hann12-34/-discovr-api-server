...
const BASE_URL = 'https://www.coc.com';
...
async function scrapeCocEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Coc events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Coc URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Coc URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Coc page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Coc event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Coc in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Coc events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Coc event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Coc event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Coc event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Coc events`);
    return events;
  } catch (error) {
    console.error('Error scraping Coc events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeCocEventsClean  };
