...
const BASE_URL = 'https://www.torontolibrary.com';
...
async function scrapeTorontoLibraryEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Library events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Library URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Library URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Library page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Library event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Library in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Library events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Toronto Library-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Library event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Library event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Library event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeTorontoLibraryEventsClean  };
