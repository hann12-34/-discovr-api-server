...
const BASE_URL = 'https://theatrecentre.org';
...
async function scrapeTheatreCentreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Theatre Centre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Theatre Centre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Theatre Centre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Theatre Centre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Theatre Centre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Theatre Centre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Theatre Centre events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Theatre Centre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Theatre Centre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Theatre Centre event "${event.title}":`, error);
      }
    }
    ...
    console.error('Error scraping Theatre Centre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTheatreCentreEventsClean  };
