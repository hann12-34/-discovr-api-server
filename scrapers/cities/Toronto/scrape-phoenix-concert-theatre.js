...
class PhoenixConcertTheatreScraper {
    constructor() {
        this.name = 'Phoenix Concert Theatre';
        this.url = 'https://www.phoenixconcerttheatre.com/events';
        this.city = 'Toronto';
    ...
    async scrape() {
        ...
        try {
            ...
            const events = await page.evaluate(() => {
                const eventElements = document.querySelectorAll('.event, .show, .concert, [class*="event"], [class*="show"], .listing');
                const events = [];
                ...
                eventElements.forEach(element => {
                    try {
                        ...
                        if (titleEl && titleEl.textContent?.trim()) {
                            ...
                            const eventId = `phoenix-concert-theatre-${title.toLowerCase().replace(/[^a-z0-9]/g, '-')}-${new Date(startDate).getTime()}`;

                            events.push({
                                id: eventId,
                                title: title,
                                description: element.textContent?.replace(/\s+/g, ' ')?.trim()?.substring(0, 500) || `Live concert at ${title}`,
                                startDate: startDate,
                                venue: {
