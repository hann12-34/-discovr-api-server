...
const BASE_URL = 'https://www.torontocompleteantibotproduction.com';
...
const getGardinerVenue = (city) => ({
  name: 'Toronto Complete Antibot Production',
...
async function scrapeTorontoCompleteAntibotProductionEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Complete Antibot Production events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Complete Antibot Production URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Complete Antibot Production URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Complete Antibot Production page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Complete Antibot Production event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Complete Antibot Production in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Complete Antibot Production events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Toronto Complete Antibot Production-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Complete Antibot Production event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Complete Antibot Production event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Complete Antibot Production event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Toronto Complete Antibot Production events`);
    return events;
  } catch (error) {
    console.error('Error scraping Toronto Complete Antibot Production events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTorontoCompleteAntibotProductionEventsClean  };
