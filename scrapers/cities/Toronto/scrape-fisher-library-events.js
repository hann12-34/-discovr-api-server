...
const BASE_URL = 'https://www.fisherlibrary.com';
...
async function scrapeFisherLibraryEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Fisher Library events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Fisher Library URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Fisher Library URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Fisher Library page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Fisher Library event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Fisher Library in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Fisher Library events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Fisher Library event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Fisher Library event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Fisher Library event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Fisher Library events`);
    return events;
  } catch (error) {
    console.error('Error scraping Fisher Library events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeFisherLibraryEventsClean  };
