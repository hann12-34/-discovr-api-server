...
const BASE_URL = 'https://www.torontoca.com';
...
async function scrapeTorontoCaEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Ca events (clean version)...');
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Ca event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Ca in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Ca events`);
...
module.exports = { scrapeEvents: scrapeTorontoCaEventsClean  };
