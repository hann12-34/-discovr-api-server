...
const BASE_URL = 'https://www.rogerscentre.com';
...
async function scrapeRogersCentreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rogers Centre events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rogers Centre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rogers Centre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rogers Centre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rogers Centre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rogers Centre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rogers Centre events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rogers Centre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rogers Centre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rogers Centre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Rogers Centre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Rogers Centre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRogersCentreEventsClean  };
