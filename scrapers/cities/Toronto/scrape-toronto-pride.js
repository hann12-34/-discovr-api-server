...
const BASE_URL = 'https://www.torontopride.com';
...
async function scrapeTorontoPrideEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Pride events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Pride URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Pride URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Pride page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Pride event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Pride in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Pride events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Pride event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Pride event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Pride event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Toronto Pride events`);
    return events;
  } catch (error) {
    console.error('Error scraping Toronto Pride events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTorontoPrideEventsClean  };
