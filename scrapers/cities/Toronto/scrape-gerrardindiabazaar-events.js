...
async function scrapeGerrardindiabazaarEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Gerrardindiabazaar events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Gerrardindiabazaar URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Gerrardindiabazaar URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Gerrardindiabazaar page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Gerrardindiabazaar event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Gerrardindiabazaar in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Gerrardindiabazaar events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Gerrardindiabazaar-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Gerrardindiabazaar event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Gerrardindiabazaar event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Gerrardindiabazaar event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Gerrardindiabazaar events`);
    return events;
  } catch (error) {
    console.error('Error scraping Gerrardindiabazaar events:', error);
    throw error;
  } finally {
    await client.close();
  }
