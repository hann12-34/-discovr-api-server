...
async function scrapeTorontoZooEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Zoo URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Zoo URLs failed, cannot proceed');
      return [];
    }
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Zoo event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Zoo events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Zoo event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Zoo event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Zoo event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeTorontoZooEventsClean  };
