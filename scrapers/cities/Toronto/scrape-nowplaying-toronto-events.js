...
const BASE_URL = 'https://www.nowplayingtoronto.com';
...
async function scrapeNowplayingTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Nowplaying Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Nowplaying Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Nowplaying Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Nowplaying Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Nowplaying Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Nowplaying Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Nowplaying Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Nowplaying Toronto-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Nowplaying Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Nowplaying Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Nowplaying Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Nowplaying Toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping Nowplaying Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeNowplayingTorontoEventsClean  };
