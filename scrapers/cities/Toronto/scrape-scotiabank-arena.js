...
const BASE_URL = 'https://www.scotiabankarena.com';
...
async function scrapeScotiabankArenaEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Scotiabank Arena events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Scotiabank Arena URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Scotiabank Arena URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Scotiabank Arena page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Scotiabank Arena event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Scotiabank Arena in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Scotiabank Arena events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Scotiabank Arena-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Scotiabank Arena event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Scotiabank Arena event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Scotiabank Arena event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Scotiabank Arena events`);
    return events;
  } catch (error) {
    console.error('Error scraping Scotiabank Arena events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeScotiabankArenaEventsClean  };
