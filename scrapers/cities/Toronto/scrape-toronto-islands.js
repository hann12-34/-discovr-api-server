...
const BASE_URL = 'https://www.toronto.ca/explore-enjoy/parks-recreation/toronto-island-park';
...
async function scrapeTorontoIslandsEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Islands events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Islands URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Islands URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Islands page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Islands event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Islands in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Islands events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Toronto Islands-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Islands event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Islands event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Islands event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Toronto Islands events`);
    return events;
  } catch (error) {
    console.error('Error scraping Toronto Islands events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeTorontoIslandsEventsClean  };
