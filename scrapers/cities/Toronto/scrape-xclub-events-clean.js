...
// Enhanced filtering for XClub nightclub content
const isValidEvent = (title) => {
  if (!title || title.length < 5) return false;
  
  const skipPatterns = [
    /^(home|about|contact|menu|search|login|register|subscribe|follow|visit|hours|directions|book|table)$/i,
    /^(xclub|x-club|toronto|nightclub|club|bar|drink|bottle|service|party|dance|music|dj|entertainment)$/i,
...
const hasEventCharacteristics = (title, description, dateText, eventUrl) => {
  ...
  const eventIndicators = [
    ...
    /guest|artist|performer|headliner|feature|resident|opening|closing|set|stage|floor|underground/i
  ];
...
const getXClubVenue = (city) => ({
  name: 'XClub Toronto',
  address: '559 College St, Toronto, ON M6G 1A9',
  city: 'Toronto',
  state: 'ON',
  zip: 'M6G 1A9',
  latitude: 43.6567,
...
async function scrapeXClubEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying XClub Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All XClub Toronto URLs failed, cannot proceed');
      return [];
    }

    const $ = cheerio.load(response.data);
    const candidateEvents = [];
    const venue = getXClubVenue(city);

    console.log(`📊 XClub Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified XClub Toronto event: "${title}"`);
        ...
        qualityScore += /party|dj|live|special|vip|guest|underground/.test(title.toLowerCase()) ? 1 : 0;
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality XClub Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          description: event.description || `Experience ${event.title} at XClub Toronto underground nightclub.`,
          startDate: startDate || new Date(),
          endDate: endDate || startDate || new Date(),
          venue: venue,
          price: extractPrice('Cover charge applies') || 'Contact venue',
          categories: extractCategories('Nightlife, Music, Dancing, Underground, Toronto'),
          source: 'XClub Toronto-Toronto',
          city: 'Toronto',
          featured: false,
          tags: ['nightclub', 'underground', 'music', 'dancing', 'toronto'],
          createdAt: new Date(),
          updatedAt: new Date()
        };
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added XClub Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate XClub Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing XClub Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new XClub Toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping XClub Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeXClubEventsClean  };
