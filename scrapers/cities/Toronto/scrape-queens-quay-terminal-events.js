...
const BASE_URL = 'https://www.queensquayterminal.com';
...
async function scrapeQueensQuayTerminalEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Queens Quay Terminal events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Queens Quay Terminal URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Queens Quay Terminal URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Queens Quay Terminal page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Queens Quay Terminal event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Queens Quay Terminal in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Queens Quay Terminal events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Queens Quay Terminal-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Queens Quay Terminal event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Queens Quay Terminal event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Queens Quay Terminal event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeQueensQuayTerminalEventsClean  };
