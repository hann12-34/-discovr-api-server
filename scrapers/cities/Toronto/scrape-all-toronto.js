...
const BASE_URL = 'https://www.alltoronto.com';
...
async function scrapeAllTorontoEventsClean(city) {
  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying All Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All All Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 All Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified All Toronto event: "${title}"`);
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality All Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added All Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate All Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing All Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new All Toronto events`);
    return events;
  } catch (error) {
    console.error('Error scraping All Toronto events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrape: scrapeAllTorontoEventsClean };
