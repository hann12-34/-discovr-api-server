...
async function scrapeRoyalAlexandraTheatreEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Royal Alexandra Theatre events (clean version)...');

    // Anti-bot delay
    await delay(Math.floor(Math.random() * 2000) + 1000);

    const urlsToTry = [
      `${BASE_URL}/whats-on/calendar`,
      `${BASE_URL}/shows/royal-alexandra-theatre`,
      `${BASE_URL}/events/`,
      `${BASE_URL}/whats-on/`,
      `${BASE_URL}/`
    ];
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Royal Alexandra Theatre URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Royal Alexandra Theatre URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Royal Alexandra Theatre page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Royal Alexandra Theatre event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Royal Alexandra Theatre in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Royal Alexandra Theatre events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Royal Alexandra Theatre-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Royal Alexandra Theatre event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Royal Alexandra Theatre event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Royal Alexandra Theatre event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Royal Alexandra Theatre events`);
    return events;
  } catch (error) {
    console.error('Error scraping Royal Alexandra Theatre events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeRoyalAlexandraTheatreEventsClean  };
