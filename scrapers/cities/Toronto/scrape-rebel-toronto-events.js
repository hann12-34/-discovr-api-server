...
const BASE_URL = 'https://www.rebeltoronto.com';
...
async function scrapeRebelTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Rebel Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Rebel Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Rebel Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Rebel Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Rebel Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Rebel Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Rebel Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Rebel Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Rebel Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Rebel Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Rebel Toronto events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeRebelTorontoEventsClean  };
