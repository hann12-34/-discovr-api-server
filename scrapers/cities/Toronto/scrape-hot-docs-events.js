...
const BASE_URL = 'https://www.hotdocs.com';
...
async function scrapeHotDocsEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Hot Docs events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Hot Docs URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Hot Docs URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Hot Docs page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Hot Docs event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Hot Docs in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Hot Docs events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Hot Docs event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Hot Docs event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Hot Docs event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Hot Docs events`);
    return events;
  } catch (error) {
    console.error('Error scraping Hot Docs events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeHotDocsEventsClean  };
