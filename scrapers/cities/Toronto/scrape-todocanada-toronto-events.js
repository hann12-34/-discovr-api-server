...
const BASE_URL = 'https://www.todocanadatoronto.com';
...
async function scrapeTodocanadaTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Todocanada Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Todocanada Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Todocanada Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Todocanada Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Todocanada Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Todocanada Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Todocanada Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Todocanada Toronto-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Todocanada Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Todocanada Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Todocanada Toronto event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeTodocanadaTorontoEventsClean  };
