...
const BASE_URL = 'https://www.century.com';
...
async function scrapeCenturyEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Century events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Century URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Century URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Century page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Century event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Century in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Century events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Century event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Century event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Century event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Century events`);
    return events;
  } catch (error) {
    console.error('Error scraping Century events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeCenturyEventsClean  };
