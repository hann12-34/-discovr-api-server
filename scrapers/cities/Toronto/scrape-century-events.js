 ...
 * Based on events from https://wearecentury.ca/events/
 */
...
// Constants
const CENTURY_URL = 'https://wearecentury.ca/events/';
const CENTURY_VENUE = {
  name: 'Century',
...
function generateEventId(title, startDate) {
  const dataToHash = `${CENTURY_VENUE.name}-${title}-${startDate.toISOString()}`;
...
function normalizeUrl(url, baseUrl = 'https://wearecentury.ca') {
  if (!url) return '';
...
async function scrapeCenturyEvents(eventsCollection) {
  let addedEvents = 0;
  
  try {
    console.log('🔍 Fetching events from Century website...');
    
    // Fetch HTML content
    const response = await axios.get(CENTURY_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .party, .show, .listing, article, .grid-item, .event-card').each((i, el) => {
      try {
        ...
        const description = element.find('p, .description, .details, .event-description').first().text().trim() || 
                           'Join us at Century for an unforgettable night of music, dancing, and entertainment.';
...
async function scrapeCenturyEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeCenturyEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeCenturyEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeCenturyEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Century scraper completed. Added ${addedEvents} new events.`);
    })
    .catch(error => {
      console.error('❌ Error running Century scraper:', error);
      process.exit(1);
    });
}
