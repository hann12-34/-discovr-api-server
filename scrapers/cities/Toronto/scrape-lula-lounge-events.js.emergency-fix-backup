...
const LULA_URL = 'https://www.lula.ca/';
const LULA_VENUE = {
  name: 'Lula Lounge',
  address: '1585 Dundas St W, Toronto, ON M6K 1T9',
  website: 'https://www.lula.ca/',
...
async function scrapeLulaLoungeEvents(eventsCollection) {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-lula-lounge-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;

  try {
    console.log('🔍 Fetching events from Lula Lounge website...');

    // Fetch HTML content
    const response = await axios.get(LULA_URL, {
      timeout: 15000,
      headers: {
    ...
    $('.event, .event-item, .show, .performance, .listing, article, .grid-item, .event-card, .calendar-item').each((i, el) => {
      try {
        const $el = $(el);

        // Extract title
        let title = $el.find('h1, h2, h3, h4, .title, .event-title, .name, .artist').first().text().trim();
        if (!title) {
        ...
        // Extract description
        const description = $el.find('p, .description, .event-description, .summary, .bio').text().trim() ||
                          $el.text().replace(title, '').trim().substring(0, 200);

        // Extract event URL
        const eventUrl = $el.find('a').attr('href') || LULA_URL;
        const fullEventUrl = eventUrl.startsWith('http') ? eventUrl : `https://www.lula.ca${eventUrl}`;

        // Extract image
        const imageUrl = $el.find('img').attr('src') || '';
        const fullImageUrl = imageUrl && !imageUrl.startsWith('http') ?
                            `https://www.lula.ca${imageUrl}` : imageUrl;
    ...
    for (const event of events) {
      ...
      const eventId = generateEventId(LULA_VENUE.name, event.title, startDate.toISOString());
      const categories = ['Nightlife', 'Music', 'Dancing', 'Latin'];
      ...
      const eventDoc = {
        ...
        startDate,
        endDate,
        venue: { ...LULA_VENUE, city },
        categories: categories,
        price: price || "Contact venue",
        officialWebsite: LULA_VENUE.website,
        location: LULA_VENUE.location,
        sourceURL: LULA_VENUE.website,
    ...
    console.error(`❌ Error scraping Lula Lounge events: ${error.message}`);
  }
...
async function scrapeLulaLoungeEventsStandalone() {
  ...
  try {
    ...
    const addedEvents = await scrapeLulaLoungeEvents(eventsCollection);
    return addedEvents;
...
module.exports = { scrapeLulaLoungeEvents };

// Run the scraper if executed directly
if (require.main === module) {
  scrapeLulaLoungeEventsStandalone()
    .then(addedEvents => {
      console.log(`✅ Lula Lounge scraper completed. Added ${addedEvents} new events.`);
    }
    .catch(error => {
      console.error('❌ Error running Lula Lounge scraper:', error);
      process.exit(1);
    };
}
