...
const BASE_URL = 'https://www.agakhanmuseum.org';
...
async function scrapeAgaKhanMuseumEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Aga Khan Museum events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Aga Khan Museum URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Aga Khan Museum URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Aga Khan Museum page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Aga Khan Museum event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Aga Khan Museum in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Aga Khan Museum events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Aga Khan Museum event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Aga Khan Museum event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Aga Khan Museum event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeAgaKhanMuseumEventsClean  };
