...
const BASE_URL = 'https://www.miatoronto.com';
...
async function scrapeMiaTorontoEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Mia Toronto events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Mia Toronto URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Mia Toronto URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Mia Toronto page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Mia Toronto event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Mia Toronto in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Mia Toronto events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Mia Toronto event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Mia Toronto event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Mia Toronto event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Mia Toronto events`);
    return events;
  } catch (error) {
...
module.exports = { scrapeEvents: scrapeMiaTorontoEventsClean  };
