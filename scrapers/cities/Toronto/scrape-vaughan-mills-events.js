...
const BASE_URL = 'https://www.vaughanmills.com';
...
async function scrapeVaughanMillsEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Vaughan Mills events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Vaughan Mills URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Vaughan Mills URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Vaughan Mills page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Vaughan Mills event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Vaughan Mills in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Vaughan Mills events`);
    ...
    for (const event of events) {
      try {
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Vaughan Mills event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Vaughan Mills event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Vaughan Mills event "${event.title}":`, error);
      }
    }

    console.log(`✅ Successfully added ${addedEvents} new Vaughan Mills events`);
    return events;
  } catch (error) {
    console.error('Error scraping Vaughan Mills events:', error);
    throw error;
  } finally {
    await client.close();
  }
...
module.exports = { scrapeEvents: scrapeVaughanMillsEventsClean  };
