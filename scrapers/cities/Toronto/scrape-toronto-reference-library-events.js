...
async function scrapeTPLEvents() {
  const city = city;
  if (!city) {
    console.error('❌ City argument is required. e.g. node scrape-toronto-reference-library-events.js Toronto');
    process.exit(1);
  }
  let addedEvents = 0;
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          venue: { ...RegExp.venue: { ...RegExp.venue: TPL_VENUE,, city }, city },,
...
module.exports = scrapeTPLEvents;