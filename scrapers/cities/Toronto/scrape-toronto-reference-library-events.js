...
const BASE_URL = 'https://www.torontopubliclibrary.ca';
...
async function scrapeTorontoReferenceLibraryEventsClean(city) {
  ...
  try {
    ...
    console.log('🚀 Scraping Toronto Reference Library events (clean version)...');
    ...
    for (const url of urlsToTry) {
      try {
        console.log(`🔍 Trying Toronto Reference Library URL: ${url}`);
    ...
    if (!response) {
      console.log('❌ All Toronto Reference Library URLs failed, cannot proceed');
      return [];
    }
    ...
    console.log(`📊 Toronto Reference Library page loaded from ${workingUrl}, analyzing content...`);
    ...
    for (const selector of eventSelectors) {
      $(selector).each((i, el) => {
        ...
        console.log(`📝 Found qualified Toronto Reference Library event: "${title}"`);
        ...
        candidateEvents.push({
          ...
          dateText,
          description: description || `Experience ${title} at the Toronto Reference Library in Toronto.`,
          qualityScore
        });
      });
    }
    ...
    console.log(`📊 Found ${candidateEvents.length} candidates, selected ${events.length} quality Toronto Reference Library events`);
    ...
    for (const event of events) {
      try {
        ...
        const formattedEvent = {
          ...
          source: 'Toronto Reference Library-Toronto',
          city: 'Toronto',
          featured: false,
        ...
        if (!existingEvent) {
          ...
          addedEvents++;
          console.log(`✅ Added Toronto Reference Library event: ${formattedEvent.title}`);
        } else {
          console.log(`⏭️ Skipped duplicate Toronto Reference Library event: ${formattedEvent.title}`);
        }
      } catch (error) {
        console.error(`❌ Error processing Toronto Reference Library event "${event.title}":`, error);
      }
    }
...
module.exports = { scrapeEvents: scrapeTorontoReferenceLibraryEventsClean  };
