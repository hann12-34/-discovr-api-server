/**
 * Kelly's Olympian Alt Calendar Portland Events Scraper
 * URL: https://www.kellysolympian.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKellyCalendar(city = 'Portland') {
  console.log('🍻 Scraping Kelly\'s Calendar...');
  ...
  try {
    ...
    await page.goto('https://www.kellysolympian.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Kelly\'s Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kelly's Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kelly\'s Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeKellyCalendar;
