/**
 * Dante's Calendar Portland Events Scraper
 * URL: https://www.danteslive.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDanteCalendar(city = 'Portland') {
  console.log('🔥 Scraping Dante\'s Calendar...');
  ...
  try {
    ...
    await page.goto('https://www.danteslive.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Dante\'s Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Dante's Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Dante\'s Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeDanteCalendar;
