/**
 * Hawthorne Theatre Portland Scraper
...
async function scrapeHawthorneNew(city = 'Portland') {
  ...
  try {
    ...
    await page.goto('https://www.hawthornetheatre.com/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/event"], .event-item, .event-card, article, [class*="event"]').forEach(el => {
        try {
          ...
          const title = el.querySelector('h2, h3, h4, .title, .headliners')?.textContent?.trim() ||
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Hawthorne events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Hawthorne error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeHawthorneNew;
