/**
 * Hawthorne Theatre Portland Events Scraper
 * URL: https://www.hawthornetheatre.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHawthorneTheatre(city = 'Portland') {
  console.log('🎭 Scraping Hawthorne Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.hawthornetheatre.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-box', '.event', 'article', '[class*="show"]', '[class*="event"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Hawthorne Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Hawthorne Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Hawthorne Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeHawthorneTheatre;
