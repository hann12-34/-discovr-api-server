/**
 * Roseland Theater Final Portland Events Scraper
 * URL: https://roselandpdx.com/calendar/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoselandFinal(city = 'Portland') {
  console.log('🌹 Scraping Roseland Final...');
  ...
  try {
    ...
    await page.goto('https://roselandpdx.com/calendar/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Roseland Final'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Roseland Final events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Roseland Final error:', error.message);
    return [];
  }
}

module.exports = scrapeRoselandFinal;
