/**
 * Roseland Theater Portland Scraper
 * URL: https://roselandpdx.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoselandNew(city = 'Portland') {
  console.log('🎸 Scraping Roseland Theater PDX...');
  ...
  try {
    ...
    await page.goto('https://roselandpdx.com/events/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/event"], .event-item, .event-card, article, .tw-event, [class*="event"]').forEach(el => {
        try {
          ...
          const title = el.querySelector('h2, h3, h4, .title, .headliners, .tw-name, .event-name')?.textContent?.trim() ||
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Roseland Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Roseland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Roseland error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRoselandNew;
