/**
 * Keller Auditorium Portland Events Scraper
 * URL: https://www.portland5.com/keller-auditorium/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKellerAuditorium(city = 'Portland') {
  console.log('🎭 Scraping Keller Auditorium...');
  ...
  try {
    ...
    await page.goto('https://www.portland5.com/keller-auditorium/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-card', '.event', 'article', '[class*="show"]', '[class*="event"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Arts',
        source: 'Keller Auditorium'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Keller Auditorium events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Keller Auditorium error:', error.message);
    return [];
  }
}

module.exports = scrapeKellerAuditorium;
