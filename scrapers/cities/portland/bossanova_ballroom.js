/**
 * Bossanova Ballroom Portland Events Scraper
 * URL: https://www.bossanovaballroom.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBossanova(city = 'Portland') {
  console.log('💃 Scraping Bossanova Ballroom...');
  ...
  try {
    ...
    await page.goto('https://www.bossanovaballroom.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Bossanova Ballroom'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bossanova Ballroom events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bossanova Ballroom error:', error.message);
    return [];
  }
}

module.exports = scrapeBossanova;
