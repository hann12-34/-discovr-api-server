 ...
 * URL: https://cascadebrewing.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCascadeBarrel(city = 'Portland') {
  console.log('🍺 Scraping Cascade Barrel House...');
  ...
  try {
    ...
    await page.goto('https://cascadebrewing.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Cascade Barrel House'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cascade Barrel House events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cascade Barrel House error:', error.message);
    return [];
  }
}

module.exports = scrapeCascadeBarrel;
