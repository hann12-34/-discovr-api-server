/**
 * Jack London Revue Portland Events Scraper
 * URL: https://www.jacklondonrevue.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeJackLondon(city = 'Portland') {
  console.log('🎭 Scraping Jack London Revue...');
  ...
  try {
    ...
    await page.goto('https://www.jacklondonrevue.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Jack London Revue'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Jack London events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Jack London error:', error.message);
    return [];
  }
}

module.exports = scrapeJackLondon;
