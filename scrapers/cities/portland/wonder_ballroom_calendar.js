/**
 * Wonder Ballroom Alt Calendar Portland Events Scraper
 * URL: https://www.wonderballroom.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWonderCalendar(city = 'Portland') {
  console.log('✨ Scraping Wonder Calendar...');
  ...
  try {
    ...
    await page.goto('https://www.wonderballroom.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Wonder Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Wonder Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Wonder Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeWonderCalendar;
