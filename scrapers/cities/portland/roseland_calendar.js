/**
 * Roseland Theater Calendar Portland Events Scraper
 * URL: https://www.roselandpdx.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoselandCalendar(city = 'Portland') {
  console.log('🎭 Scraping Roseland Calendar...');
  ...
  try {
    ...
    await page.goto('https://www.roselandpdx.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Roseland Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Roseland Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Roseland Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeRoselandCalendar;
