/**
 * Mississippi Studios Complete Portland Events Scraper
 * URL: https://www.mississippistudios.com/upcoming-events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMississippiAll(city = 'Portland') {
  console.log('🎸 Scraping Mississippi All...');
  ...
  try {
    ...
    await page.goto('https://www.mississippistudios.com/upcoming-events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Mississippi All'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Mississippi All events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Mississippi All error:', error.message);
    return [];
  }
}

module.exports = scrapeMississippiAll;
