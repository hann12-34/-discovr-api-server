/**
 * Mississippi Studios Calendar Portland Events Scraper
 * URL: https://www.mississippistudios.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMississippiCalendar(city = 'Portland') {
  console.log('🎸 Scraping Mississippi Calendar...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Mississippi Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Mississippi Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Mississippi Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeMississippiCalendar;
