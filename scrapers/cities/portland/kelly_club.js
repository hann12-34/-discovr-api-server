/**
 * Kelly's Olympian Portland Events Scraper
 * URL: https://www.kellysolympian.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKellysOlympian(city = 'Portland') {
  console.log('🍻 Scraping Kelly\'s Olympian...');
  ...
  try {
    ...
    await page.goto('https://www.kellysolympian.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Kelly\'s Olympian'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kelly's Olympian events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kelly\'s Olympian error:', error.message);
    return [];
  }
}

module.exports = scrapeKellysOlympian;
