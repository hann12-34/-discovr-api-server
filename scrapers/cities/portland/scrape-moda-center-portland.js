/**
 * Moda Center Portland - portland Events Scraper
...
async function scrapeModaCenterPortland(city = 'portland') {
  console.log('🎭 Scraping Moda Center Portland...');
  try {
    const resp = await axios.get('https://www.rosequarter.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Moda Center Portland', address: addr || '1 N Center Ct St, Portland, OR 97227', city: 'portland' },
            latitude: 45.5316,
            longitude: -122.6669,
            city: 'portland',
            category: 'Events',
            source: 'Moda Center Portland'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.rosequarter.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.rosequarter.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Moda Center Portland', address: '1 N Center Ct St, Portland, OR 97227', city: 'portland' },
            latitude: 45.5316,
            longitude: -122.6669,
            city: 'portland',
            category: 'Events',
    ...
    console.log('  ✅ Moda Center Portland:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Moda Center Portland error:', error.message);
    return [];
  }
}

module.exports = scrapeModaCenterPortland;
