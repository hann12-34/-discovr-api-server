/**
 * Cascade Brewing Barrel House Portland Events Scraper
 * URL: https://www.cascadebrewingbarrelhouse.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCascadeBrewing(city = 'Portland') {
  console.log('🍺 Scraping Cascade Brewing...');
  ...
  try {
    ...
    await page.goto('https://www.cascadebrewingbarrelhouse.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Cascade Brewing'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cascade Brewing events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cascade Brewing error:', error.message);
    return [];
  }
}

module.exports = scrapeCascadeBrewing;
