/**
 * Analog Theater Portland Events Scraper
 * URL: https://www.analogtheater.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAnalogTheater(city = 'Portland') {
  console.log('🎬 Scraping Analog Theater...');
  ...
  try {
    ...
    await page.goto('https://www.analogtheater.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Analog Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Analog Theater events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Analog Theater error:', error.message);
    return [];
  }
}

module.exports = scrapeAnalogTheater;
