/**
 * Aladdin Theater Portland Events Scraper
 * URL: https://aladdin-theater.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAladdin(city = 'Portland') {
  console.log('🎭 Scraping Aladdin Theater...');
  ...
  try {
    ...
    await page.goto('https://aladdin-theater.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Nightlife',
        source: 'Aladdin Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Aladdin events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Aladdin error:', error.message);
    return [];
  }
}

module.exports = scrapeAladdin;
