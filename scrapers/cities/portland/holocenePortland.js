/**
 * Holocene Portland Events Scraper
 * URL: https://www.holocene.org/calendar
 * Beloved indie/electronic venue in SE Portland
...
const HolocenePortlandEvents = {
  async scrape(city = 'Portland') {
    console.log('🌲 Scraping Holocene Portland...');

    try {
      const response = await axios.get('https://www.holocene.org/calendar', {
        headers: {
      ...
      $('.eventWrapper').each((i, el) => {
        ...
        if (!url) return;
        if (!url.startsWith('http')) url = 'https://www.holocene.org' + url;
        ...
        events.push({
          ...
          city: 'Portland',
          category: 'Music',
          source: 'Holocene Portland'
        });
      });

      console.log(`  ✅ Found ${events.length} Holocene events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Holocene error: ${error.message}`);
      return [];
    }
  }
};

module.exports = HolocenePortlandEvents.scrape;
