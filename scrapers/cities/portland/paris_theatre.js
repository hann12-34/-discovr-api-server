/**
 * Paris Theatre Portland Events Scraper
 * URL: https://www.paristheatrepdx.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeParisTheatre(city = 'Portland') {
  console.log('🎭 Scraping Paris Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.paristheatrepdx.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Portland',
        category: 'Arts',
        source: 'Paris Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Paris Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Paris Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeParisTheatre;
