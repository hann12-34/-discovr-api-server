/**
 * The Venetian Las Vegas Events Scraper
 * URL: https://www.venetianlasvegas.com/entertainment.html
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVenetian(city = 'Las Vegas') {
  console.log('🏛️ Scraping The Venetian Las Vegas...');
  ...
  try {
    ...
    await page.goto('https://www.venetianlasvegas.com/entertainment.html', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="entertainment"], a[href*="shows"], .event-card, article, [class*="show"]').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Venetian events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Venetian error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeVenetian;
