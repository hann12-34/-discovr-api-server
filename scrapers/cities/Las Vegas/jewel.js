/**
 * JEWEL Nightclub Las Vegas Events Scraper
 * URL: https://jewelnightclub.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeJewel(city = 'Las Vegas') {
  console.log('💎 Scraping JEWEL Nightclub Las Vegas...');
  ...
  try {
    ...
    await page.goto('https://jewelnightclub.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'JEWEL Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} JEWEL events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ JEWEL error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeJewel;
