...
async function scrapeTMobileArena(city = 'Las Vegas') {
  console.log('🎭 Scraping T-Mobile Arena...');
  try {
    const resp = await axios.get('https://www.t-mobilearena.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'T-Mobile Arena', address: addr || '3780 S Las Vegas Blvd, Las Vegas, NV 89158', city: 'Las Vegas' },
            latitude: 36.1029,
            longitude: -115.1785,
            city: 'Las Vegas',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.t-mobilearena.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.t-mobilearena.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'T-Mobile Arena', address: '3780 S Las Vegas Blvd, Las Vegas, NV 89158', city: 'Las Vegas' },
            latitude: 36.1029,
            longitude: -115.1785,
            city: 'Las Vegas',
            category: 'Events',
    ...
    console.log('  ✅ T-Mobile Arena:', futureEvents.length, 'events');
