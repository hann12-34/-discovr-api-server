/**
 * Wynn Las Vegas Events Scraper
 * URL: https://www.wynnlasvegas.com/entertainment
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWynn(city = 'Las Vegas') {
  console.log('🎰 Scraping Wynn Las Vegas...');
  ...
  try {
    ...
    await page.goto('https://www.wynnlasvegas.com/entertainment', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="entertainment"], a[href*="shows"], .event-card, article').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Wynn events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Wynn error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeWynn;
