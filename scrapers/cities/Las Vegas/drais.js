/**
 * Drai's Nightclub Las Vegas Events Scraper
 * URL: https://draisgroup.com/lasvegas/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDrais(city = 'Las Vegas') {
  console.log('🎤 Scraping Drais Nightclub Las Vegas...');
  ...
  try {
    ...
    await page.goto('https://draisgroup.com/lasvegas/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Drais Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Drais events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Drais error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeDrais;
