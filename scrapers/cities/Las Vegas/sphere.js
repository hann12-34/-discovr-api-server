/**
 * Sphere Las Vegas Events Scraper
 * URL: https://www.thespherevegas.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSphere(city = 'Las Vegas') {
  console.log('🌐 Scraping Sphere Las Vegas...');
  ...
  try {
    ...
    await page.goto('https://www.thespherevegas.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event-card, article, [class*="event"]').forEach(el => {
        try {
          ...
          const title = el.querySelector('h1, h2, h3, h4, .title')?.textContent.trim() || linkEl?.textContent.trim();
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Sphere events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Sphere error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSphere;
