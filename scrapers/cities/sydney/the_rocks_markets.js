...
async function scrapeRocksMarkets(city = 'Sydney') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'The Rocks', address: 'George Street, The Rocks NSW 2000', city: 'Sydney' }, latitude: -33.8587, longitude: 151.2087, city: 'Sydney', category: 'Festival', source: 'The Rocks Markets' });
    }
