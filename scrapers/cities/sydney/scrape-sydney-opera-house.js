/**
 * Sydney Opera House - sydney Events Scraper
...
async function scrapeSydneyOperaHouse(city = 'sydney') {
  console.log('🎭 Scraping Sydney Opera House...');
  try {
    const resp = await axios.get('https://www.sydneyoperahouse.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Sydney Opera House', address: addr || 'Bennelong Point, Sydney NSW 2000', city: 'sydney' },
            latitude: -33.8568,
            longitude: 151.2153,
            city: 'sydney',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.sydneyoperahouse.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.sydneyoperahouse.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Sydney Opera House', address: 'Bennelong Point, Sydney NSW 2000', city: 'sydney' },
            latitude: -33.8568,
            longitude: 151.2153,
            city: 'sydney',
            category: 'Events',
    ...
    console.log('  ✅ Sydney Opera House:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Sydney Opera House error:', error.message);
    return [];
  }
}

module.exports = scrapeSydneyOperaHouse;
