...
async function scrapeNewtownFest(city = 'Sydney') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Camperdown Memorial Rest Park', address: 'Australia Street, Newtown NSW 2042', city: 'Sydney' }, latitude: -33.8975, longitude: 151.1794, city: 'Sydney', category: 'Festival', source: 'Newtown Festival' });
    }
