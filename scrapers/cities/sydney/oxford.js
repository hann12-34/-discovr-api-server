/**
 * Oxford Art Factory Sydney Events Scraper
 * Live music and club venue
 * URL: https://oxfordartfactory.com/
 */
...
async function scrapeOxfordArt(city = 'Sydney') {
  console.log('🎸 Scraping Oxford Art Factory...');
  ...
  try {
    ...
    await page.goto('https://oxfordartfactory.com/whats-on/', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventItems = document.querySelectorAll('article.event, .event-item, .show-item');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Oxford Art Factory`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Oxford Art Factory events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Oxford Art Factory error:', error.message);
    return [];
  }
}

module.exports = scrapeOxfordArt;
