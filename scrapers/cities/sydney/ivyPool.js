/**
 * Ivy Pool Club sydney Events Scraper
 * URL: https://merivale.com/venues/ivy
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeIvypool(city = 'sydney') {
  console.log('🎵 Scraping Ivy Pool Club...');
  ...
  try {
    ...
    await page.goto('https://merivale.com/venues/ivy/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Ivy Pool Club',
          address: '320-330 George Street, Sydney NSW 2000',
          city: 'sydney'
        },
        latitude: -33.867,
        longitude: 151.2074,
        city: 'sydney',
        category: 'Nightlife',
        source: 'Ivy Pool Club'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ivy Pool Club events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Ivy Pool Club error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeIvypool;
