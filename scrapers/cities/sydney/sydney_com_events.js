/**
 * Sydney.com Events Scraper
 * URL: https://www.sydney.com/events
 * Official Sydney tourism events
 */
...
async function scrapeSydneyComEvents(city = 'Sydney') {
  console.log('🌊 Scraping Sydney.com Events...');
  let browser;
  ...
  try {
    ...
    await page.goto('https://www.sydney.com/events', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(3000);
    ...
    const eventLinks = await page.evaluate(() => {
      ...
      anchors.forEach(a => {
        ...
        if (href && !href.endsWith('/events') && !href.endsWith('/events/')) {
          const fullUrl = href.startsWith('http') ? href : `https://www.sydney.com${href}`;
    ...
    for (const url of eventLinks) {
      try {
        ...
        events.push({
          ...
          venue: {
            name: eventData.location ? eventData.location.split(',')[0].trim() : 'Sydney',
            address: eventData.location || 'Sydney, NSW, Australia',
            city: 'Sydney'
          },
          latitude: -33.8688,
          longitude: 151.2093,
    ...
    console.log(`  ✅ Found ${unique.length} Sydney.com events`);
    return unique;
  } catch (error) {
    console.error(`  ⚠️ Sydney.com Events error: ${error.message}`);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeSydneyComEvents;
