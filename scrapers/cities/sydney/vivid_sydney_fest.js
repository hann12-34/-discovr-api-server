...
async function scrapeVividSydney(city = 'Sydney') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Sydney Harbour', address: 'Circular Quay, Sydney NSW 2000', city: 'Sydney' }, latitude: -33.8568, longitude: 151.2153, city: 'Sydney', category: 'Festival', source: 'Vivid Sydney' });
    }
