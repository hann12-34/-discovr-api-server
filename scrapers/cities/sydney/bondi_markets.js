...
async function scrapeBondiMarkets(city = 'Sydney') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Bondi Beach', address: 'Campbell Parade, Bondi Beach NSW 2026', city: 'Sydney' }, latitude: -33.8915, longitude: 151.2767, city: 'Sydney', category: 'Festival', source: 'Bondi Markets' });
    }
