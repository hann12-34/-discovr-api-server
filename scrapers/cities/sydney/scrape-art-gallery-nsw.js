/**
 * Art Gallery NSW - sydney Events Scraper
 * URL: https://www.artgallery.nsw.gov.au/whats-on/
 */
...
async function scrapeArtGalleryNSW(city = 'sydney') {
  console.log('🎭 Scraping Art Gallery NSW...');
  try {
    const resp = await axios.get('https://www.artgallery.nsw.gov.au/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Art Gallery NSW', address: addr || 'Art Gallery Rd, Sydney NSW 2000', city: 'sydney' },
            latitude: -33.8689,
            longitude: 151.2172,
            city: 'sydney',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.artgallery.nsw.gov.au/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.artgallery.nsw.gov.au/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Art Gallery NSW', address: 'Art Gallery Rd, Sydney NSW 2000', city: 'sydney' },
            latitude: -33.8689,
            longitude: 151.2172,
            city: 'sydney',
            category: 'Events',
    ...
    console.log('  ✅ Art Gallery NSW:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Art Gallery NSW error:', error.message);
    return [];
  }
}

module.exports = scrapeArtGalleryNSW;
