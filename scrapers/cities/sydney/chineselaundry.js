/**
 * Chinese Laundry Sydney Events Scraper
 * Nightclub venue
 * URL: https://chineselaundry.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeChineseLaundry(city = 'Sydney') {
  console.log('🎧 Scraping Chinese Laundry Sydney...');
  ...
  try {
    ...
    await page.goto('https://chineselaundry.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Sydney',
        category: 'Nightlife',
        source: 'Chinese Laundry'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Chinese Laundry events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Chinese Laundry error:', error.message);
    return [];
  }
}

module.exports = scrapeChineseLaundry;
