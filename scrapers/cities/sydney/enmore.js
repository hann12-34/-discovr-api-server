/**
 * Enmore Theatre sydney Events Scraper
 * URL: https://enmoretheatre.com.au
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEnmore(city = 'sydney') {
  console.log('🎵 Scraping Enmore Theatre...');
  ...
  try {
    ...
    await page.goto('https://enmoretheatre.com.au/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'sydney',
        category: 'Nightlife',
        source: 'Enmore Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Enmore Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Enmore Theatre error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeEnmore;
