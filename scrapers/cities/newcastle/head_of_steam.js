/**
 * Head of Steam Newcastle Events Scraper
 * URL: https://www.headofsteamnewcastle.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHeadOfSteam(city = 'Newcastle') {
  console.log('🍺 Scraping Head of Steam...');
  ...
  try {
    ...
    await page.goto('https://www.headofsteamnewcastle.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Head of Steam',
          address: '2-8 Neville St, Newcastle upon Tyne NE1 5EN',
          city: 'Newcastle'
        },
        latitude: 54.9685,
        longitude: -1.6170,
        city: 'Newcastle',
        category: 'Nightlife',
        source: 'Head of Steam'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Head of Steam events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Head of Steam error:', error.message);
    return [];
  }
}

module.exports = scrapeHeadOfSteam;
