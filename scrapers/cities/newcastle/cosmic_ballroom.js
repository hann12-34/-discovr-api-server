/**
 * Cosmic Ballroom Newcastle Events Scraper
 * URL: https://www.cosmicballroom.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCosmicBallroom(city = 'Newcastle') {
  console.log('🌌 Scraping Cosmic Ballroom...');
  ...
  try {
    ...
    await page.goto('https://www.cosmicballroom.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Cosmic Ballroom',
          address: 'Newgate St, Newcastle upon Tyne NE1 5RE',
          city: 'Newcastle'
        },
        latitude: 54.9725,
        longitude: -1.6125,
        city: 'Newcastle',
        category: 'Nightlife',
        source: 'Cosmic Ballroom'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cosmic Ballroom events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cosmic Ballroom error:', error.message);
    return [];
  }
}

module.exports = scrapeCosmicBallroom;
