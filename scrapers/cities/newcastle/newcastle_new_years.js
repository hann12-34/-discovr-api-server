/**
 * Newcastle New Year's Eve - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNYENewcastle(city = 'Newcastle') {
  console.log('🎆 Scraping New Year\'s Eve Newcastle...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T20:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Quayside', address: 'Quayside, Newcastle', city: 'Newcastle' }, latitude: 54.9685, longitude: -1.6050, city: 'Newcastle', category: 'Festival', source: 'NYE Newcastle' });
    }
    console.log(`  ✅ Found ${results.length} NYE Newcastle events`);
    return results;
  } catch (err) {
    console.error('NYE Newcastle error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeNYENewcastle;
