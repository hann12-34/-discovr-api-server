/**
 * Northern Pride Newcastle Events Scraper
 * URL: https://www.northern-pride.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNewcastlePride(city = 'Newcastle') {
  console.log('🏳️‍🌈 Scraping Northern Pride...');
  ...
  try {
    ...
    await page.goto('https://www.northern-pride.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Newcastle',
        category: 'Festival',
        source: 'Northern Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Northern Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Northern Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeNewcastlePride;
