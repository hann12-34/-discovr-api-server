/**
 * Riverside Newcastle Scraper
 * URL: https://www.riversidencl.co.uk/
...
async function scrapeRiversideV2(city = 'Newcastle') {
  const events = [];
  let browser;
  ...
  try {
    ...
    await page.goto('https://www.riversidencl.co.uk/', {
    ...
    const eventData = await page.evaluate(() => {
      const items = [];
      const eventCards = document.querySelectorAll('.event, article, [class*="event"], [class*="gig"]');
      ...
      eventCards.forEach(card => {
        ...
        if (!items.find(e => e.title === title)) {
          items.push({ title, url: href.startsWith('http') ? href : `https://www.riversidencl.co.uk${href}`, image: imgEl?.src || null, dateStr });
        }
      });
      return items;
    });
    ...
    for (const item of eventData) {
      ...
      events.push({
        ...
        venue: { name: 'Riverside', address: 'Melbourne Street, Newcastle upon Tyne NE1 2JQ', city: city },
    ...
    console.log(`Riverside Newcastle: Found ${events.length} events`);
    return events;
    
  } catch (error) {
    console.error('Riverside scraper error:', error.message);
    return events;
  } finally {
...
module.exports = scrapeRiversideV2;
