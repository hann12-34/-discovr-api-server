/**
 * Times Square Newcastle Events - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTimesSquare(city = 'Newcastle') {
  console.log('🎭 Scraping Times Square Events...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T14:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Times Square', address: 'Times Square, Newcastle', city: 'Newcastle' }, latitude: 54.9699, longitude: -1.6163, city: 'Newcastle', category: 'Festival', source: 'Times Square' });
    }
    console.log(`  ✅ Found ${results.length} Times Square events`);
    return results;
  } catch (err) {
    console.error('Times Square error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeTimesSquare;
