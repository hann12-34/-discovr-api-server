...
async function scrapeNewcastleMela(city = 'Newcastle') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Exhibition Park', address: 'Exhibition Park, Newcastle', city: 'Newcastle' }, latitude: 54.9862, longitude: -1.6165, city: 'Newcastle', category: 'Festival', source: 'Newcastle Mela' });
    }
