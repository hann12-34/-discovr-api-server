/**
 * Powerhouse Newcastle Events Scraper
 * Popular nightclub in Newcastle
 * URL: https://powerhouseclub.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePowerhouseNewcastle(city = 'Newcastle') {
  console.log('⚡ Scraping Powerhouse Newcastle...');
  ...
  try {
    ...
    await page.goto('https://powerhouseclub.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Powerhouse',
          address: '9-19 Westmorland Rd, Newcastle NE1 4EG',
          city: 'Newcastle'
        },
        city: 'Newcastle',
        category: 'Nightlife',
        source: 'Powerhouse Newcastle'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Powerhouse events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Powerhouse error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePowerhouseNewcastle;
