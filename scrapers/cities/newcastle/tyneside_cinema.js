/**
 * Tyneside Cinema Newcastle Events Scraper
 * URL: https://www.tynesidecinema.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTynesideCinema(city = 'Newcastle') {
  console.log('🎬 Scraping Tyneside Cinema...');
  ...
  try {
    ...
    await page.goto('https://www.tynesidecinema.co.uk/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.film-listing', '.event', 'article', '[class*="film"]', '[class*="event"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Tyneside Cinema events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Tyneside Cinema error:', error.message);
    return [];
  }
}

module.exports = scrapeTynesideCinema;
