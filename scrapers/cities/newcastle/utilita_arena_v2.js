...
async function scrapeUtilitaArenaV2(city = 'Newcastle') {
  ...
  try {
    ...
    const junkPatterns = [
      ...
      /returns?\s*policy/i, /harassment\s*policy/i, /^live\s*events$/i
    ];

    const eventData = await page.evaluate(() => {
      const items = [];
      const eventCards = document.querySelectorAll('.event, article, [class*="event"]');
      ...
      eventCards.forEach(card => {
        ...
        if (!items.find(e => e.title === title)) {
          items.push({ title, url: href.startsWith('http') ? href : `https://www.utilitaarena.co.uk${href}`, image: imgEl?.src || null, dateStr });
        }
      });
      return items;
    });
    ...
    for (const item of eventData) {
      ...
      events.push({
        ...
        venue: { name: 'Utilita Arena', address: 'Arena Way, Newcastle upon Tyne NE4 7NA', city: city },
