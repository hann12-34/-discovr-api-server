    ...
    for (const url of urls) {
      ...
      const pageEvents = await page.evaluate(() => {
        ...
        eventCards.forEach(card => {
          ...
          if (!items.find(e => e.title === title)) {
            items.push({ title, url: href.startsWith('http') ? href : `https://www.nxnewcastle.com${href}`, image: imgEl?.src || null, dateStr });
          }
        });
        return items;
      });
      ...
      for (const item of pageEvents) {
        ...
        events.push({
          ...
          venue: { name: 'NX Newcastle', address: 'City Road, Newcastle upon Tyne NE1 2AF', city: city },
