/**
 * Newcastle City Hall Events Scraper
 * URL: https://www.newcastlecityhall.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNewcastleCityHall(city = 'Newcastle') {
  console.log('🏛️ Scraping Newcastle City Hall...');
  ...
  try {
    ...
    await page.goto('https://www.newcastlecityhall.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Newcastle',
        category: 'Arts',
        source: 'Newcastle City Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Newcastle City Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Newcastle City Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeNewcastleCityHall;
