/**
 * Newcastle Fringe Events Scraper
 * URL: https://www.newcastlefringe.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNewcastleFringe(city = 'Newcastle') {
  console.log('🎭 Scraping Newcastle Fringe...');
  ...
  try {
    ...
    await page.goto('https://www.newcastlefringe.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Newcastle',
        category: 'Festival',
        source: 'Newcastle Fringe'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Newcastle Fringe events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Newcastle Fringe error:', error.message);
    return [];
  }
}

module.exports = scrapeNewcastleFringe;
