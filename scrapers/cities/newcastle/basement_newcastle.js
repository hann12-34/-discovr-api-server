/**
 * Basement Newcastle Events Scraper
 * Popular nightclub in Newcastle
 * URL: https://basementnewcastle.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBasementNewcastle(city = 'Newcastle') {
  console.log('🎵 Scraping Basement Newcastle...');
  ...
  try {
    ...
    await page.goto('https://basementnewcastle.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Basement',
          address: '24-26 Close, Newcastle NE1 3RF',
          city: 'Newcastle'
        },
        city: 'Newcastle',
        category: 'Nightlife',
        source: 'Basement Newcastle'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Basement events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Basement error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBasementNewcastle;
