/**
 * Forum Melbourne Events Scraper
 * Historic live music venue
 * URL: https://forummelbourne.com.au/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeForumMelbourne(city = 'Melbourne') {
  console.log('🎭 Scraping Forum Melbourne...');
  ...
  try {
    ...
    await page.goto('https://forummelbourne.com.au/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Forum Melbourne`,
        date: isoDate,
        ...
        city: 'Melbourne',
        category: 'Nightlife',
        source: 'Forum Melbourne'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Forum Melbourne events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Forum Melbourne error:', error.message);
    return [];
  }
}

module.exports = scrapeForumMelbourne;
