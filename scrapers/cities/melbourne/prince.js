/**
 * Prince Bandroom Melbourne Events Scraper
 * Live music venue in St Kilda
 * URL: https://www.princebandroom.com.au/
 */
...
async function scrapePrince(city = 'Melbourne') {
  console.log('🎸 Scraping Prince Bandroom Melbourne...');
  ...
  try {
    ...
    await page.goto('https://www.princebandroom.com.au/', {
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.event-card', 'article', '[class*="event"]', '.show', '.gig'];
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} Prince events`);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Prince Bandroom', address: '29 Fitzroy Street, St Kilda VIC 3182', city: 'Melbourne' },
        latitude: -37.8680,
        longitude: 144.9800,
        city: 'Melbourne',
        category: 'Nightlife',
        source: 'Prince Bandroom'
      });
    }
    ...
    console.error('  ⚠️  Prince error:', error.message);
    return [];
  }
}

module.exports = scrapePrince;
