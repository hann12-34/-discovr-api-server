...
async function scrapeWhiteNight(city = 'Melbourne') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Melbourne CBD', address: 'Swanston Street, Melbourne VIC 3000', city: 'Melbourne' }, latitude: -37.8136, longitude: 144.9631, city: 'Melbourne', category: 'Festival', source: 'White Night' });
    }
