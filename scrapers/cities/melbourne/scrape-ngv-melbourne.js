/**
 * NGV Melbourne - melbourne Events Scraper
 * URL: https://www.ngv.vic.gov.au/whats-on/
 */
...
async function scrapeNGVMelbourne(city = 'melbourne') {
  console.log('🎭 Scraping NGV Melbourne...');
  try {
    const resp = await axios.get('https://www.ngv.vic.gov.au/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'NGV Melbourne', address: addr || '180 St Kilda Rd, Melbourne VIC 3006', city: 'melbourne' },
            latitude: -37.8226,
            longitude: 144.9689,
            city: 'melbourne',
            category: 'Events',
            source: 'NGV Melbourne'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.ngv.vic.gov.au/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.ngv.vic.gov.au/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'NGV Melbourne', address: '180 St Kilda Rd, Melbourne VIC 3006', city: 'melbourne' },
            latitude: -37.8226,
            longitude: 144.9689,
            city: 'melbourne',
            category: 'Events',
    ...
    console.log('  ✅ NGV Melbourne:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ NGV Melbourne error:', error.message);
    return [];
  }
}

module.exports = scrapeNGVMelbourne;
