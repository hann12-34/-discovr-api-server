...
async function scrapeFedSquare(city = 'Melbourne') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Federation Square', address: 'Swanston Street, Melbourne VIC 3000', city: 'Melbourne' }, latitude: -37.8179, longitude: 144.9690, city: 'Melbourne', category: 'Festival', source: 'Federation Square' });
    }
