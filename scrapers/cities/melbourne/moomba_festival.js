...
async function scrapeMoomba(city = 'Melbourne') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Alexandra Gardens', address: 'Alexandra Avenue, Melbourne VIC 3004', city: 'Melbourne' }, latitude: -37.8262, longitude: 144.9742, city: 'Melbourne', category: 'Festival', source: 'Moomba' });
    }
