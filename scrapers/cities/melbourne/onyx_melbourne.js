/**
 * Onyx Melbourne Events Scraper
 * Popular nightclub in Melbourne
 * URL: https://onyxmelbourne.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOnyxMelbourne(city = 'Melbourne') {
  console.log('💎 Scraping Onyx Melbourne...');
  ...
  try {
    ...
    await page.goto('https://onyxmelbourne.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Melbourne',
        category: 'Nightlife',
        source: 'Onyx Melbourne'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Onyx events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Onyx error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOnyxMelbourne;
