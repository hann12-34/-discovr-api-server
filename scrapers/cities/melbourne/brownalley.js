/**
 * Brown Alley Melbourne Events Scraper
 * Nightclub venue
 * URL: https://brownalley.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBrownAlley(city = 'Melbourne') {
  console.log('🎧 Scraping Brown Alley Melbourne...');
  ...
  try {
    ...
    await page.goto('https://brownalley.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Club night at Brown Alley`,
        date: isoDate,
        ...
        venue: {
          name: 'Brown Alley',
          address: '585 Lonsdale Street, Melbourne VIC 3000',
          city: 'Melbourne'
        },
        latitude: -37.8113,
        longitude: 144.9583,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Brown Alley events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Brown Alley error:', error.message);
    return [];
  }
}

module.exports = scrapeBrownAlley;
