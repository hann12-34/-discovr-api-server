/**
 * Nightcat Melbourne Events Scraper
 * Multi-level club venue
 * URL: https://thenightcat.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNightcat(city = 'Melbourne') {
  console.log('🐱 Scraping Nightcat Melbourne...');
  ...
  try {
    ...
    await page.goto('https://thenightcat.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Club night at Nightcat`,
        date: isoDate,
        ...
        venue: {
          name: 'Nightcat',
          address: '141 Johnston Street, Fitzroy VIC 3065',
          city: 'Melbourne'
        },
        latitude: -37.8007,
        longitude: 144.9787,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Nightcat events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nightcat error:', error.message);
    return [];
  }
}

module.exports = scrapeNightcat;
