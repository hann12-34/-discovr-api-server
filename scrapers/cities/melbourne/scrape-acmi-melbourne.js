/**
 * ACMI Melbourne - melbourne Events Scraper
 * URL: https://www.acmi.net.au/whats-on/
 */
...
async function scrapeACMIMelbourne(city = 'melbourne') {
  console.log('🎭 Scraping ACMI Melbourne...');
  try {
    const resp = await axios.get('https://www.acmi.net.au/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'ACMI Melbourne', address: addr || 'Fed Square, Melbourne VIC 3000', city: 'melbourne' },
            latitude: -37.818,
            longitude: 144.9691,
            city: 'melbourne',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.acmi.net.au/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.acmi.net.au/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'ACMI Melbourne', address: 'Fed Square, Melbourne VIC 3000', city: 'melbourne' },
            latitude: -37.818,
            longitude: 144.9691,
            city: 'melbourne',
            category: 'Events',
    ...
    console.log('  ✅ ACMI Melbourne:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ ACMI Melbourne error:', error.message);
    return [];
  }
}

module.exports = scrapeACMIMelbourne;
