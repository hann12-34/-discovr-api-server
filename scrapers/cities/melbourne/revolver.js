/**
 * Revolver Upstairs melbourne Events Scraper
 * URL: https://revolverupstairs.com.au
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRevolver(city = 'melbourne') {
  console.log('🎵 Scraping Revolver Upstairs...');
  ...
  try {
    ...
    await page.goto('https://revolverupstairs.com.au/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'melbourne',
        category: 'Nightlife',
        source: 'Revolver Upstairs'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Revolver Upstairs events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Revolver Upstairs error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRevolver;
