    ...
    try {
      ...
      try {
        ...
        if (!endDate || isNaN(endDate.getTime())) {
          ...
          console.log(`⚠️ Using default end date for Canoe Races: ${endDate.toDateString()}`);
        }
        ...
        const id = `musqueam-${slugTitle}-${dateStr}`;
        ...
        events.push(event);
        console.log(`✅ Added Musqueam Canoe Races event: ${title} on ${startDate.toDateString()}`);
      } catch (eventError) {
        ...
        for (const selector of eventSelectors) {
          ...
          if (eventElements.length > 0) {
            ...
            for (const element of eventElements) {
              try {
                // Extract title
                const titleElement = await element.$('h2, h3, .tribe-events-calendar-list__event-title, .tribe-events-calendar-latest-past__event-title');
                const title = titleElement ? 
                ...
                // Extract date
                const dateElement = await element.$('.tribe-events-calendar-list__event-date-tag, .tribe-events-calendar-latest-past__event-date-tag, time');
                let dateText = dateElement ? 
                  await page.evaluate(el => el.textContent.trim(), dateElement) : null;
                  
                // Extract description
                const descriptionElement = await element.$('.tribe-events-calendar-list__event-description, .tribe-events-calendar-latest-past__event-description, p');
                const description = descriptionElement ? 
                  await page.evaluate(el => el.textContent.trim(), descriptionElement) : 
