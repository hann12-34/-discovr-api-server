/**
 * Vancouver Summer Cinema Events Scraper
 * Extracts events from Vancouver outdoor summer cinema events
 */
...
class SummerCinemaEvents {
  constructor() {
    this.name = 'Vancouver Summer Cinema Events';
    this.url = 'https://www.summercinema.ca/';
    this.baseUrl = 'https://www.summercinema.ca';
    this.venue = {
      name: 'Vancouver Summer Cinema',
      address: 'Various Outdoor Locations, Vancouver, BC',
      city: 'Vancouver',
      province: 'BC',
      country: 'Canada',
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Summer Cinema events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Summer Cinema events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Summer Cinema events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Summer Cinema website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .movie, .screening, .cinema, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .movie-title') || event;
          ...
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .movie-description, .details');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Film', 'Outdoor Cinema', 'Summer Events', 'Entertainment'],
...
module.exports = SummerCinemaEvents;
