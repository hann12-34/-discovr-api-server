/**
 * Vancouver Metropolis Events Scraper
 * Extracts events from Metropolis at Metrotown venue
 */
...
class MetropolisEvents {
  constructor() {
    this.name = 'Vancouver Metropolis Events';
    this.url = 'https://www.metropolisatmetrotown.com/';
    this.baseUrl = 'https://www.metropolisatmetrotown.com';
    this.venue = {
      ...
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2266, lng: -123.0037 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Metropolis events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Metropolis events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Metropolis events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Metropolis website
  ...
  async extractEvents(page) {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Entertainment', 'Shopping', 'Community'],
...
module.exports = MetropolisEvents;

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new MetropolisEvents();
