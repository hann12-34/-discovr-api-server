...
class MetropolisEvents {
  ...
  async scrape() {
    ...
    try {
      ...
      await page.waitForSelector('s-listing, -list, s-container', { timeout: 5000 }.catch(() => {};

      // Get all event elements
      const eventElements = await page.$$('-item, -card, -listing');
      ...
      if (eventElements.length > 0) {
        ...
        for (const eventElement of eventElements) {
          try {
            // Extract event title
            const title = await eventElement.$eval('h3, h4, -title, .title', el => el.textContent.trim())
              .catch(() => null);
  ...
  extractEventMentions(text) {
    ...
    const seasonalEvents = [
      ...
      },
      {
        ...
        description: 'Explore the las throughout the mall.',
        month: 9, // September
        durationDays: 10
      },
      {
...
module.exports.MetropolisEvents = MetropolisEvents;