    ...
    try {
      ...
      const extractedEvents = await page.evaluate(() => {
        ...
        if (eventElements.length < 3) {
          ...
          console.log(` ${eventElements.length} potential events`);
        }
        ...
        eventElements.forEach((element, index) => {
          try {
            // Extract title
            let title = '';
            const titleSelectors = ['h1', 'h2', 'h3', 'h4', '.title', '-title', '.name'];
            ...
            // Extract date
            let da = '';
            const dateSelectors = ['.date', '-date', 'time', '.datetime'];
            for (const sel of dateSelectors) {
              const dateEl = element.querySelector(sel);
              if (dateEl && dateEl.textContent.trim()) {
                da = dateEl.textContent.trim();
                break;
              }
            }
      ...
      extractedEvents.forEach((rawEvent) => {
        try {
          ...
          // Parse dates from the extracted da
          let startDate, endDate;

          try {
            if (da) {
              // Look for month abbreviations like "JUL 25", "AUG 15-20"
              const dateMatch = da.match(/(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\s+(\d{1,2}(?:-(\d{1,2}?/i);
              if (dateMatch) {
