...
class RichmondNightMarketScraper {
  ...
  isHolidayMonday(date) {
    ...
    return holidayMondays2025.includes(da);
  }
    ...
    try {
      ...
      for (const date of marketDates) {
        try {
          ...
          const dateFormatted = date.toLocaleDa('en-US', { month: 'short', day: 'numeric' };
          const dayName = date.toLocaleDa('en-US', { weekday: 'long' };
          ...
          console.error(`Error processing date ${date.toLocaleDa: ${error.message}`);
        }
      }
...
module.exports = async (city) => {
  const scraper = new RichmondNightMarketScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.RichmondNightMarketScraper = RichmondNightMarketScraper;