...
class RedRoomScraper {
  ...
  async scrape(city) {
    ...
    try {
      ...
      const eventElements = await page.$$('-list -item, .upcoming-events , .content-section article');
      ...
      for (const eventElement of eventElements) {
        try {
          ...
          const titleElement = await eventElement.$('h2, -title, .title');
          const title = titleElement ? await page.evaluate(el => el.textContent.trim(), titleElement) : 'Untitled Event';

          // Extract date info
          const dateElement = await eventElement.$('-date, .date, time');
          ...
          const descElement = await eventElement.$('-description, .description, .content');
          ...
          const linkElement = await eventElement.$('a-link, a.more-info, .title a');
          ...
          const imageElement = await eventElement.$('img-image, -img img, .image img');
          let imageUrl = null;
          if (imageElement) {
          ...
          events.push(event);
          console.log(`✅ Added event: ${title} on ${dateObj.toLocaleDa`);
