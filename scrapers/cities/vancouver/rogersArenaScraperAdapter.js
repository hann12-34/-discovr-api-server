  ...
  try {
    ...
    knownEvents.forEach(knownEvent => {
      ...
      if (!foundEquivalent) {
        ...
        const eventId = `rogers-arena-${titleSlug}-${da}`;
    ...
    const formattedEvents = allEvents.map(event => {
      ...
      if (startDate) {
        ...
        const titleSlug = properTitle
          ...
          .replace(/[^\w\s-]/g, '')
          .replace(/\s+/g, '-');
        eventId = `rogers-arena-${titleSlug}-${da}`;
      }
      ...
      return {
        ...
        startDate: startDate,
        endDate: endDate,
        date: startDate ? startDate.toLocaleDa() : 'TBA',
        image: event.image,
        venue: {
          name: "Rogers Arena",
...
module.exports = async (city) => {
    const scraper = new compatible();
    if (typeof scraper.scrape === 'function') {
        return await scraper.scrape(city);
    } else {
        throw new Error('No scrape method found in compatible');
    }
};