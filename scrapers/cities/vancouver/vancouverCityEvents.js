...
class VancouverCityEventsScraper {
  ...
  parseDateRange(da) {
    ...
    try {
      ...
      if (da.toLowerCase().includes('ongoing') || da.toLowerCase().includes('permanent')) {
      ...
      if (da.includes(' - ') || da.includes(' to ') || da.includes('–')) {
        let separator = ' - ';
        if (da.includes(' to ')) separator = ' to ';
        if (da.includes('–')) separator = '–';

        const [startPart, endPart] = da.split(separator).map(s => s.trim());
  ...
  async scrape(city) {
    ...
    try {
      ...
      await page.waitForSelector(', -listing, s-list, .calendar-event', {
        timeout: 10000
      }.catch(() => {
      ...
      const extractedEvents = await page.evaluate(() => {
        const eventElements = document.querySelectorAll(', -listing, s-list .item, .calendar-event');
        ...
        return Array.from(elements).map(element => {
          // Extract title
          const titleElement = element.querySelector('h2, h3, h4, -title, .title, a[class*="title"]');
      ...
      for (const item of extractedEvents) {
        try {
          ...
          // Generate unique ID
          const daDates.startDate ?
            eventDates.startDate.toISOString().split('T')[0] : '';
          const eventId = `vancouver-city-${slugify(item.title, { lower: true }}-${da}`;
          ...
          const event = {
            ...
            categories,
            sourceURL: this.url,
            officialWebsite: itemUrl || this.url,
            image: item.imageUrl,
...
module.exports = async (city) => {
  const scraper = new VancouverCityEventsScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.VancouverCityEventsScraper = VancouverCityEventsScraper;