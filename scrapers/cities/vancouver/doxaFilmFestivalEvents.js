...
class DoxaFilmFestivalEvents {
        ...
        if (!link.url.includes('javascript:') && !link.url.includes('mailto:')) {
          ...
          try {
            ...
            const filmInfo = await page.evaluate(() => {
              const films = [];
              const filmElements = document.querySelectorAll('.film, .movie, , .program-item, article');
  ...
  parseFestivalDateRange(da) {
    try {
      ...
      const singleMonthMatch = da.match(/(\w+)\s+(\d{1,2}(?:st|nd|rd|th)?\s*(?:to|through|–|-)\s*(\d{1,2}(?:st|nd|rd|th)?/i);
      ...
      if (singleMonthMatch) {
        ...
        const twoMonthMatch = da.match(/(\w+)\s+(\d{1,2}(?:st|nd|rd|th)?\s*(?:to|through|–|-)\s*(\w+)\s+(\d{1,2}(?:st|nd|rd|th)?/i);
...
module.exports.DoxaFilmFestivalEvents = DoxaFilmFestivalEvents;