/**
 * Vancouver DOXA Documentary Film Festival Events Scraper
 * Extracts events from DOXA Documentary Film Festival
 */
...
class DoxaFilmFestivalEvents {
  constructor() {
    this.name = 'DOXA Documentary Film Festival Events';
    this.url = 'https://doxafestival.ca/';
    this.baseUrl = 'https://doxafestival.ca';
    this.venue = {
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting DOXA Film Festival events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} DOXA Film Festival events`);

      return events;
    } catch (error) {
      console.error(`Error scraping DOXA Film Festival events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from DOXA Film Festival website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .film, .screening, '.documentary', article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          ...
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .film-description, .details');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Film', 'Documentary', 'Festival', 'Arts & Culture'],
...
module.exports = async (city) => {
  const scraper = new DoxaFilmFestivalEvents();
