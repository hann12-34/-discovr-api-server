          ...
          try {
            ...
            if (showData.showUrl) {
              ...
              const detailedInfo = await page.evaluate(() => {
                ...
                const dateElements = document.querySelectorAll('.date, .dates, .show-date, -date, time, .showtime');
        ...
        for (const link of linksToCheck) {
          try {
            ...
            const showInfo = await page.evaluate(() => {
              // Look for a title
              const titleElements = document.querySelectorAll('h1, h2, .title, .show-title, -title');
              let title = '';
              ...
              // Look for dates
              const dateElements = document.querySelectorAll('.date, .dates, .show-date, -date, time, .datetime');
              let dateText = '';
              for (const el of dateElements) {
                const text = el.textContent.trim();
...
module.exports.ImprovCentreEvents = ImprovCentreEvents;