  ...
  async scrape(city) {
    ...
    try {
      ...
      const events = await page.evaluate(() => {
        const eventElements = document.querySelectorAll('-item, -card, , .program, [class*="event"], .performance');
        const events = [];
...
module.exports = async (city) => {
  const scraper = new CoquitlamCentreForTheArtsScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.CoquitlamCentreForTheArtsScraper = CoquitlamCentreForTheArtsScraper;