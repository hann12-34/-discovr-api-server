/**
 * Vancouver Chan Centre Events Scraper
 * Extracts events from Chan Centre for the Performing Arts (UBC)
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class ChanCentreEvents {
  constructor() {
    this.name = 'Chan Centre Events';
    this.url = 'https://chancentre.com/';
    this.baseUrl = 'https://chancentre.com';
    this.venue = {
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Chan Centre events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Chan Centre events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Chan Centre events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Chan Centre website
  ...
  async extractEvents(page) {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Music', 'Theatre', 'Performing Arts', 'Concerts'],
...
module.exports = async (city) => {
  const scraper = new ChanCentreEvents();
