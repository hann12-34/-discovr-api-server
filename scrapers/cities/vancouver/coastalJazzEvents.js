/**
 * Vancouver Coastal Jazz Festival Events Scraper
 * Extracts events from Vancouver International Jazz Festival
 */
...
class CoastalJazzEvents {
  constructor() {
    this.name = 'Vancouver Coastal Jazz Festival Events';
    this.url = 'https://www.coastaljazz.ca/';
    this.baseUrl = 'https://www.coastaljazz.ca';
    this.venue = {
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Coastal Jazz events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Coastal Jazz events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Coastal Jazz events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Coastal Jazz website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .concert, .show, .performance, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .concert-title') || event;
          const title = titleElement.textContent?.trim();
          
          // Extract date information
          const dateElement = event.querySelector('.date, .event-date, .concert-date, time, [datetime]');
          const dateText = dateElement?.textContent?.trim() || dateElement?.getAttribute('datetime') || '';
          
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .concert-description, .details');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Music', 'Jazz', 'Festival', 'Concert'],
...
module.exports = CoastalJazzEvents;
