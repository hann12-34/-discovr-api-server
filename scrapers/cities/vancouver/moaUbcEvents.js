/**
 * Museum of Anthropology at UBC Events Scraper
 * Scrapes events from Museum of Anthropology website
 */
...
class MOAUBCEvents {
  constructor() {
    this.name = 'Museum of Anthropology Events';
    ...
    this.venue = {
      ...
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2695, lng: -123.2593 }
    };
  }

  /**
   * Scrape events from Museum of Anthropology
  ...
  async extractEvents(page) {
    ...
    await page.waitForSelector('-item, -card, .exhibition, .program', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo) => {
      ...
      return eventElements.map(event => {
        try {
          // Extract title
          const titleSelectors = ['h2', 'h3', 'h4', '.title', '-title', '.name'];
          let title = 'Museum of Anthropology Event';
          ...
          // Extract date
          const dateSelectors = ['.date', '-date', 'time', '.datetime', '.date-range'];
          let dateText = '';
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Museum', 'Art', 'Culture', 'Indigenous', 'Education'],
  ...
  parseDates(dateText) {
    ...
    try {
      ...
      if (rangeMatch) {
        ...
        endDate.setHours(17, 0); // 5:00 PM typical end time for exhibitions
      ...
      if (longExhibitionMatch) {
        ...
        // Default museum exhibition time
      ...
      if (monthMatch) {
        ...
        const endDate = new Date(date);
        if (timeMatch) {
          // If time is specified, event lasts 2 hours
...
module.exports = new MOAUBCEvents();

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new MOAUBCEvents();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.MOAUBCEvents = MOAUBCEvents;