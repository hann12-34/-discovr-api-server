/**
 * Vancouver Biltmore Cabaret Events Scraper
 * Extracts events from Biltmore Cabaret
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class BiltmoreCabaretEvents {
  constructor() {
    this.name = 'Biltmore Cabaret Events';
    this.url = 'https://biltmorecabaret.com/';
    this.baseUrl = 'https://biltmorecabaret.com';
    this.venue = {
      name: 'Biltmore Cabaret',
      address: '2755 Prince Edward St, Vancouver, BC V5T 1N5',
      city: 'Vancouver',
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2627, lng: -123.0943 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Biltmore Cabaret events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Biltmore Cabaret events`);
  ...
  /**
   * Extract events from Biltmore Cabaret website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, '.show', '.concert', '.gig', article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .show-title, .band-name') || event;
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Music', 'Live Music', 'Nightlife', 'Concerts'],
...
module.exports = async (city) => {
  const scraper = new BiltmoreCabaretEvents();
