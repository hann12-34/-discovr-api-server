/**
 * Vancouver Run to End Endo Events Scraper
 * Extracts events from Run to End Endo charity run and endometriosis awareness events
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class RunToEndEndoEvents {
  constructor() {
    this.name = 'Vancouver Run to End Endo Events';
    this.url = 'https://www.runtoendendo.org/';
    this.baseUrl = 'https://www.runtoendendo.org';
    this.venue = {
      name: 'Run to End Endo',
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Run to End Endo events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Run to End Endo events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Run to End Endo events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Run to End Endo website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .run, .race, .charity, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .run-title') || event;
          const title = titleElement.textContent?.trim();
          
          // Extract date information
          const dateElement = event.querySelector('.date, .event-date, .run-date, time, [datetime]');
          const dateText = dateElement?.textContent?.trim() || dateElement?.getAttribute('datetime') || '';
          
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .run-description, .details');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Sports & Fitness', 'Charity', 'Running', 'Health Awareness'],
...
module.exports = RunToEndEndoEvents;

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new RunToEndEndoEvents();
