  ...
  async scrape(city) {
    ...
    try {
        ...
        if (dateElement) {
          ...
          try {
            ...
            if (dateMatch) {
              ...
              console.log(`✅ Parsed event date: ${eventDate.toDa`);
              break;
            }
          ...
          if (match) {
            try {
              ...
              if (date && !isNaN(date.getTime())) {
                eventDate = date;
                console.log(`✅ Parsed event date from text: ${eventDate.toDa`);
                break;
              }
            } catch (dateError) {
      ...
      if (!eventDate) {
        ...
        console.log(`⚠️ Using default event date: ${eventDate.toDa`);
      }
      ...
      if (raceTypes.length > 0) {
        ...
        for (let i = 0; i < raceTypes.length; i++) {
          ...
          // Generate event ID
          const da = raceStartTime.toISOString().split('T')[0];
          const slugRaceType = slugify(raceType, { lower: true, strict: true };
          const id = `run-to-end-endo-${slugRaceType}-${da}`;
          ...
          events.push(event);
          console.log(`✅ Added ${raceType} event at ${raceStartTime.toLocaleTimeString()} on ${raceStartTime.toDa`);
        }
      } else {
        ...
        events.push(event);
        console.log(`✅ Added general Run to End Endo event on ${startTime.toDa`);
      }
