...
class VancouverArtGalleryEventsScraper {
  ...
  parseDateRange(da) {
    ...
    try {
      ...
      if (da.includes('–') || da.includes('-')) {
        const separator = da.includes('–') ? '–' : '-';
        let [startPart, endPart] = da.split(separator).map(s => s.trim());
      ...
      console.log(`Could not parse date: ${da}`);
...
module.exports = async (city) => {
  const scraper = new VancouverArtGalleryEventsScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.VancouverArtGalleryEventsScraper = VancouverArtGalleryEventsScraper;