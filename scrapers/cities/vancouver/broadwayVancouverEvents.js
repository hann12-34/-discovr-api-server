    ...
    try {
          ...
          for (const element of showElements) {
            try {
              ...
              if (!startDate || !endDate) {
                ...
                console.log(`⚠️ Using estimated show dates: ${startDate.toLocaleDa to ${endDate.toLocaleDa`);
              }

              // Generate a unique ID
              const startDa = startDate.toISOString().split('T')[0];
              const slugTitle = slugify(title, { lower: true, strict: true };
              const id = `broadway-vancouver-${slugTitle}-${startDa}`;
              ...
              for (let week = 0; week < weeksToCreate; week++) {
                ...
                if (weekStartDate <= endDate) {
                  ...
                  for (const day of performanceDays) {
                    ...
                    const eveningDa = eveningStart.toISOString().split('T')[0];
                    const eveningId = `broadway-vancouver-${slugTitle}-${eveningDa}-evening`;
                    ...
                    if (matineeDays.includes(day)) {
                      ...
                      const matineeDa = matineeStart.toISOString().split('T')[0];
                      const matineeId = `broadway-vancouver-${slugTitle}-${matineeDa}-matinee`;
        ...
        for (const selector of linkSelectors) {
          ...
          for (let i = 0; i < links.length; i++) {
            try {
              ...
              const da = openingNight.toISOString().split('T')[0];
              const slugTitle = slugify(linkData.text, { lower: true, strict: true };
              const id = `broadway-vancouver-${slugTitle}-${da}`;
              ...
              console.log(`✅ Added show from link: ${linkData.text} on ${openingNight.toLocaleDa`);
      ...
      if (events.length === 0) {
        ...
        for (let i = 0; i < popularShows.length; i++) {
          ...
          events.push(event);
          console.log(`✅ Added generic Broadway event: ${title} on ${openingNight.toLocaleDa`);
        }
      }
...
module.exports = async (city) => {
  const scraper = new BroadwayVancouverScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.BroadwayVancouverScraper = BroadwayVancouverScraper;