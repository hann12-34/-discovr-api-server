...
class BarNoneClubScraper {
  ...
  extractEvents($) {
    ...
    $('list-column-upcoming').find('list-column-event').each((i, element) => {
      try {
        ...
        if (!title) {
          ...
          title = eventBlock.find('list-title').first().text().trim();
          if (!title) {
        ...
        const dateText = eventBlock.find('list-meta-date').text().trim() ||
                         eventBlock.text().match(/([A-Za-z]+,\s+[A-Za-z]+\s+\d+)/)?.[1] || '';

        const descriptionText = eventBlock.find('list-description').text().trim() ||
        ...
        events.push(event);
        console.log(`✅ Found event: ${title} on ${startDate.toLocaleDa`);
      ...
      if (!hasRegularFridayEvent || !hasRegularSaturdayEvent) {
        ...
        for (let d = new Date(today); d <= eightWeeksFromNow; d.setDate(d.getDate() + 1)) {
          ...
          if (d.getDay() === 5 && !hasRegularFridayEvent) { // 5 is Friday
            ...
            console.log(`✅ Added recurring Friday event on ${fridayEvent.startDate.toLocaleDa`);
          }
          ...
          if (d.getDay() === 6 && !hasRegularSaturdayEvent) { // 6 is Saturday
            ...
            console.log(`✅ Added recurring Saturday event on ${saturdayEvent.startDate.toLocaleDa`);
          }
        }
      }
