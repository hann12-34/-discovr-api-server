...
class CoastalJazzFestivalScraper {
  ...
  async scrape(city) {
    ...
    try {
      ...
      $('-item, .tribe-events-list-event-container, article').each((i, element) => {
        try {
          // Extract event details (adjust selectors based on actual website structure)
          const title = $(element).find('.tribe-events-list-event-title, -title, h2').text().trim();
          const dateText = $(element).find('.tribe-event-date-start, -date, .date').text().trim();
          ...
          const description = $(element).find('.tribe-events-list-event-description, -description, .description').text().trim();
...
module.exports = async (city) => {
  const scraper = new CoastalJazzFestivalScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.CoastalJazzFestivalScraper = CoastalJazzFestivalScraper;