    ...
    try {
      ...
      for (const eventData of extractedEvents) {
        try {
          ...
          if (eventDataUrl) {
            console.log(`Visiting event page: ${eventDataUrl}`);
            await page.goto(eventDataUrl, { waitUntil: 'networkidle2' };

            // Extract detailed event info
            const eventDetails = await page.evaluate(() => {
              // Try to get a more detailed date
              const dateElement = document.querySelector('.date, time, -date, [datetime]');
              const dateText = dateElement ? dateElement.textContent.trim() : '';

              // Try to get a more detailed description
              const descElement = document.querySelector('.description, -description, article p');
              const description = descElement ? descElement.textContent.trim() : '';

              // Try to get a better image
              const imgElement = document.querySelector('-image img, .featured-image img, article img');
