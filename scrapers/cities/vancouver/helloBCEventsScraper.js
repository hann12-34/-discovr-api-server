...
class HelloBCEventsScraper {
   ...
   * @param {string} da - Date range string from website
   * @returns {Object} - Object containing start and end dates
   */
  parseDateRange(da) {
    if (!da) return { startDate: null, endDate: null };
    ...
    try {
      ...
      if (da.includes('-')) {
        const [startPart, endPart] = da.split('-').map(s => s.trim());
      ...
      console.error(`Error parsing date range: ${da}`, error);
  ...
  parseLocation(locationText) {
    ...
    const cities = [
      city, 'Victoria', 'Richmond', 'Burnaby', 'Surrey',
  ...
  async scrape(city) {
    ...
    try {
      ...
      await page.waitForSelector('.card, -card, .listing-card, .content-listing', { timeout: 15000 }.catch(() => {
      ...
      const extractedEvents = await page.evaluate(() => {
        ...
        const eventElements = document.querySelectorAll('.content-listing-item, -card, .card, .content-card');
        console.log(`Found ${eventElements.length} potential event elements on HelloBC events page`);

        const elements = eventElements.length > 0 ? eventElements : document.querySelectorAll('article, .listing-item, s-list > *');

        return Array.from(elements).map(element => {
          // Extract event details with HelloBC-specific selectors
          const titleElement = element.querySelector('h3, h2, .card-title, .content-title, -name, .title');
          const title = titleElement ? titleElement.innerText.trim() : '';

          const descriptionElement = element.querySelector('.card-text, .description, p, -description, .summary');
          const description = descriptionElement ? descriptionElement.innerText.trim() : '';

          const dateElement = element.querySelector('-date, .date-range, .date, time');
          const dateText = dateElement ? dateElement.innerText.trim() : '';

          const locationElement = element.querySelector('.location, .venue, -location, .place');
      ...
      for (const item of extractedEvents) {
        try {
          ...
          // Generate unique ID
          const daDates.startDate ? eventDates.startDate.toISOString().split('T')[0] : '';
          const eventId = `hellobc-${slugify(item.title, { lower: true }}-${da}`;

          // Determine categories
          const categories = this.determineCategories(item.title, item.description);

          // Format event URL
          const fullUrl = itemUrl ? (itemUrl.startsWith('http') ? itemUrl : `https://www.hellobc.com${itemUrl}`) : this.url;
