/**
 * Rogers Arena Vancouver - vancouver Events Scraper
 * URL: https://www.rogersarena.com/events
 */
...
async function scrapeRogersArenaVancouver(city = 'vancouver') {
  console.log('🎭 Scraping Rogers Arena Vancouver...');
  try {
    const resp = await axios.get('https://www.rogersarena.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Rogers Arena Vancouver', address: addr || '800 Griffiths Way, Vancouver, BC', city: 'vancouver' },
            latitude: 49.2778,
            longitude: -123.1089,
            city: 'vancouver',
            category: 'Events',
            source: 'Rogers Arena Vancouver'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.rogersarena.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.rogersarena.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Rogers Arena Vancouver', address: '800 Griffiths Way, Vancouver, BC', city: 'vancouver' },
            latitude: 49.2778,
            longitude: -123.1089,
            city: 'vancouver',
            category: 'Events',
    ...
    console.log('  ✅ Rogers Arena Vancouver:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Rogers Arena Vancouver error:', error.message);
    return [];
  }
}

module.exports = scrapeRogersArenaVancouver;
