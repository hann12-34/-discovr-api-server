/**
 * Vancouver Queer Arts Festival Events Scraper
 * Extracts events from Queer Arts Festival website
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class QueerArtsFestivalEvents {
  constructor() {
    this.name = 'Vancouver Queer Arts Festival Events';
    this.url = 'https://queerartsfestival.com/';
    this.baseUrl = 'https://queerartsfestival.com';
    this.venue = {
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Queer Arts Festival events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Queer Arts Festival events`);
  ...
  /**
   * Extract events from Queer Arts Festival website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .show, .performance, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          ...
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .show-description');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Arts & Culture', 'LGBTQ+', 'Performance', 'Theater'],
...
module.exports = QueerArtsFestivalEvents;

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new QueerArtsFestivalEvents();
