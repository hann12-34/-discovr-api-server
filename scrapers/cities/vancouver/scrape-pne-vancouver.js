/**
 * PNE Vancouver - vancouver Events Scraper
...
async function scrapePNEVancouver(city = 'vancouver') {
  console.log('🎭 Scraping PNE Vancouver...');
  try {
    const resp = await axios.get('https://www.pne.ca/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'PNE Vancouver', address: addr || '2901 E Hastings St, Vancouver, BC V5K 5J1', city: 'vancouver' },
            latitude: 49.2827,
            longitude: -123.0387,
            city: 'vancouver',
            category: 'Events',
            source: 'PNE Vancouver'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.pne.ca/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.pne.ca/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'PNE Vancouver', address: '2901 E Hastings St, Vancouver, BC V5K 5J1', city: 'vancouver' },
            latitude: 49.2827,
            longitude: -123.0387,
            city: 'vancouver',
            category: 'Events',
    ...
    console.log('  ✅ PNE Vancouver:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ PNE Vancouver error:', error.message);
    return [];
  }
}

module.exports = scrapePNEVancouver;
