/**
 * Vancouver Science World Events Scraper
 * Extracts events from Science World Vancouver
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class ScienceWorldEvents {
  constructor() {
    this.name = 'Science World Events';
    this.url = 'https://www.scienceworld.ca/';
    this.baseUrl = 'https://www.scienceworld.ca';
    this.venue = {
      name: 'Science World',
      ...
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2732, lng: -123.1028 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Science World events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Science World events`);
  ...
  /**
   * Extract events from Science World website
  ...
  async extractEvents(page) {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Science', 'Education', 'Family', 'Technology'],
...
module.exports = async (city) => {
  const scraper = new ScienceWorldEvents();
