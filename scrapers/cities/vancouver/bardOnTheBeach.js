    ...
    try {
      ...
      for (const url of Array.from(showUrls)) {
        try {
          ...
          // Get title from og:title, strip venue suffix
          let title = $p('meta[property="og:title"]').attr('content');
          if (!title) title = $p('h1').first().text().trim();
          if (title) title = title.replace(/\s*[|–-]\s*Bard on the Beach\s*$/i, '').trim();
          if (!title || title.length < 3 || seenTitles.has(title)) continue;
          if (['Bard on the Beach', 'What\'s On', 'Home', 'Offers & Deals', 'Season Announcement'].includes(title)) continue;
          if (/^\d{4} Season|Offer|Deal|Gift|Subscribe/i.test(title)) continue;
          ...
          if (!description || description.length < 20) {
            description = $p('.post-content p').first().text().trim();
          }
          if (description) {
