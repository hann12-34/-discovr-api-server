 ...
 * Extracts events from Stanley Park
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class StanleyParkEvents {
  constructor() {
    this.name = 'Stanley Park Events';
    this.url = 'https://vancouver.ca/parks-recreation-culture/stanley-park.aspx';
    this.baseUrl = 'https://vancouver.ca';
    this.venue = {
      name: 'Stanley Park',
      ...
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.3017, lng: -123.1442 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Stanley Park events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Stanley Park events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Stanley Park events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Stanley Park website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .activity, .program, .attraction, article', { timeout: 10000 })
      .catch(() => {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Outdoor', 'Nature', 'Parks', 'Recreation', 'Family'],
...
module.exports = async (city) => {
  const scraper = new StanleyParkEvents();
