  ...
  async scrape(city) {
    ...
    try {
      ...
      const events = await page.evaluate(() => {
        const eventElements = document.querySelectorAll('-item, -card, , .program, [class*="event"]');
        const events = [];
...
module.exports = async (city) => {
  const scraper = new WestVancouverCommunityEventsScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.WestVancouverCommunityEventsScraper = WestVancouverCommunityEventsScraper;