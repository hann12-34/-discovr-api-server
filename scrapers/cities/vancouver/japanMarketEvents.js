    ...
    try {
        ...
        if (eventSections.length > 0) {
          ...
          for (const section of eventSections) {
            try {
              ...
              const dateElements = await section.$$('time, .date, .event-date');
              for (const dateElement of dateElements) {
                ...
                if (eventDate && !isNaN(eventDate.getTime())) {
                  ...
                  const title = `Japan Market Vancouver - ${eventDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}`;
                  ...
                  const id = `japan-market-vancouver-${dateStr}`;
                  ...
                  events.push(event);
                  console.log(`✅ Added Japan Market event on: ${startDate.toDateString()}`);
                }
              }
            } catch (sectionError) {
            ...
            try {
              ...
              if (eventDate && !isNaN(eventDate.getTime()) && eventDate >= new Date()) {
                ...
                if (!foundDates.has(isoDate)) {
                  ...
                  const event = {
                    id: id,
                    title: `Japan Market Vancouver - ${startDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}`,
                    description: 'Japan Market Vancouver showcases authentic Japanese food, crafts, and culture. Explore a variety of Japanese products, enjoy delicious food, and experience Japanese cultural elements in a vibrant market atmosphere.',
                    startDate: startDate,
                    endDate: endDate,
                    venue: this.venue,
                  ...
                  events.push(event);
                  console.log(`✅ Added Japan Market event from text date: ${startDate.toDateString()}`);
                }
              }
            } catch (dateError) {
      ...
      if (events.length === 0) {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          events.push(event);
          console.log(`✅ Added projected Japan Market event on: ${startDate.toDateString()}`);
        }
      }
