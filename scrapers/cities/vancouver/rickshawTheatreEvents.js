/**
 * Vancouver Rickshaw Theatre Events Scraper
 * Extracts events from the Rickshaw Theatre
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class RickshawTheatreEvents {
  constructor() {
    this.name = 'Rickshaw Theatre Events';
    this.url = 'https://rickshawtheatre.com/';
    this.baseUrl = 'https://rickshawtheatre.com';
    this.venue = {
      ...
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2814, lng: -123.0994 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Rickshaw Theatre events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Rickshaw Theatre events`);
  ...
  /**
   * Extract events from Rickshaw Theatre website
  ...
  async extractEvents(page) {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Music', 'Live Performance', 'Theatre', 'Entertainment'],
...
module.exports = async (city) => {
  const scraper = new RickshawTheatreEvents();
