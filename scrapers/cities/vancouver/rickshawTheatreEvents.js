async scrape() {
    ...
    try {
      ...
      try {
        await page.waitForSelector(', list-event, article, -card, list-column', { timeout: 8000 };
      } catch (error) {
      ...
      const eventsData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          ', list-event, article, -card, list-column, .list-view-item'
        ));

        eventElements.forEach(element => {
          const title = element.querySelector('h2, h3, h4, list-title, .title, -title')?.textContent.trim() || '';
          if (!title) return;

          const description = element.querySelector('p, list-description, .description, .excerpt, .summary')?.textContent.trim() || '';
          const dateText = element.querySelector('list-meta-date, .date, time, -date')?.textContent.trim() || '';
          const imageUrl = element.querySelector('img')?.src || '';
          const sourceUrl = element.querySelector('a[href]')?.href || '';
          const priceElement = element.querySelector('list-meta-price, .price');
