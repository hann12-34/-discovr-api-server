/**
 * Vancouver ARC Dining Events Scraper
 * Extracts events from ARC Restaurant Vancouver
 */
...
class ARCDiningEvents {
  constructor() {
    this.name = 'Vancouver ARC Dining Events';
    this.url = 'https://www.arcvancouver.com/';
    this.baseUrl = 'https://www.arcvancouver.com';
    this.venue = {
      name: 'ARC Restaurant',
      address: '1185 Marinaside Crescent, Vancouver, BC',
      city: 'Vancouver',
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2744, lng: -123.1226 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting ARC Dining events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} ARC Dining events`);

      return events;
    } catch (error) {
      console.error(`Error scraping ARC Dining events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from ARC website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .dining, .special, .menu, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .dining-title') || event;
          const title = titleElement.textContent?.trim();
          
          // Extract date information
          const dateElement = event.querySelector('.date, .event-date, .dining-date, time, [datetime]');
          const dateText = dateElement?.textContent?.trim() || dateElement?.getAttribute('datetime') || '';
          
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description, .dining-description, .details');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Dining', 'Fine Dining', 'Restaurant', 'Special Events'],
...
module.exports = ARCDiningEvents;
