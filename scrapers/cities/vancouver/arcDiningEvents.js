...
class ArcDiningEvents {
  ...
  async scrape(city) {
    ...
    try {
      ...
      await Promise.race([
        page.waitForSelector('list-event', { timeout: 5000 }.catch(() => {},
        page.waitForSelector('-item', { timeout: 5000 }.catch(() => {},
        page.waitForSelector('s-container', { timeout: 5000 }.catch(() => {},
        page.waitForSelector('-block', { timeout: 5000 }.catch(() => {}
      ]);
      ...
      if (eventElements.length > 0) {
        ...
        for (const eventElement of eventElements) {
          try {
            // Extract event title
            const title = await eventElement.$eval('h3, h1, h2, -title, .title', el => el.textContent.trim())
              .catch(() => null);

            // Extract event date
            const dateText = await eventElement.$eval('-date, .date, time', el => el.textContent.trim())
              .catch(() => null);

            // Extract event description
            const description = await eventElement.$eval('p, -description, .description', el => el.textContent.trim())
        ...
        if (eventMentions.length > 0) {
          for (const mention of eventMentions) {
            try {
              ...
              console.log(`✅ Added event from text: ${event.title} on ${new Date(event.startDate).toLocaleDa`);
              events.push(event);
            } catch (error) {
