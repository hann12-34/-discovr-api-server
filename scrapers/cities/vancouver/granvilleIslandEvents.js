/**
 * Vancouver Granville Island Events Scraper
 * Extracts events from Granville Island
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class GranvilleIslandEvents {
  constructor() {
    this.name = 'Granville Island Events';
    this.url = 'https://granvilleisland.com/';
    this.baseUrl = 'https://granvilleisland.com';
    this.venue = {
      name: 'Granville Island',
      address: 'Granville Island, Vancouver, BC V6H 3S3',
      city: 'Vancouver',
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2719, lng: -123.1349 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Granville Island events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Granville Island events`);
  ...
  /**
   * Extract events from Granville Island website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .activity, .show, .market, article', { timeout: 10000 })
      .catch(() => {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Arts & Culture', 'Markets', 'Shopping', 'Entertainment'],
...
module.exports = async (city) => {
  const scraper = new GranvilleIslandEvents();
