...
class MalonesEvents {
  ...
  async extractMalonesEvents(page, sourceURL) {
    ...
    try {
      ...
      const eventArticles = await page.$$('.mec-event-article, .tribe-events-event-article, -list ');
          ...
          try {
            const eventData = await page.evaluate(el => {
              ...
              const dateText =
                ...
                el.querySelector('.date, .time, -date')?.textContent.trim() ||
                '';
  ...
  async extractEvents(page, sourceURL) {
    ...
    try {
      ...
      const eventSelectors = [
        '-item', '-card', '-list-item',
        '', 's-container', '.upcoming-events',
        'article', '.calendar-event', '-detail',
        '.post-event', '-post', '-entry',
          ...
          for (const element of elements) {
            try {
              const eventData = await page.evaluate(el => {
                ...
                const title = el.querySelector('h1, h2, h3, h4, h5, .title, -title')?.textContent.trim() || '';
                const dateText = el.querySelector('.date, -date, .datetime')?.textContent.trim() || '';
                const description = el.querySelector('p, .description, -description')?.textContent.trim() || '';
      ...
      if (hasSports) {
        ...
        if (sportMatches.length > 0) {
          ...
          for (const match of sportMatches) {
            ...
            // Parse the date
            const dateInfo = this.parseEventDate(da);
  ...
  parseEventDate(da) {
    ...
    try {
      ...
      const monthDayYearMatch = cleanDa.match(/(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}(?:st|nd|rd|th)?(?:,?\s+(\d{4}?/i);
      ...
      const slashMatch = cleanDa.match(/(\d{1,2}\/(\d{1,2}(?:\/(\d{4}?/);
