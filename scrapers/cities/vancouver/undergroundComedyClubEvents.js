  ...
  async extractEvents(page, sourceURL) {
    ...
    try {
      ...
      const eventContainers = await page.evaluate(() => {
        ...
        const selectors = [
          '-item', '.show-item', '-listing', '.show-listing',
          '-card', '.show-card', '', '.show',
          '-container', '.show-container',
          'article', 'article.show', '.calendar-event',
      ...
      if (eventContainers) {
        ...
        const extractedEvents = await page.evaluate((selector) => {
          ...
          containers.forEach(container => {
            ...
            const titleElement = container.querySelector('h1, h2, h3, h4, .title, -title, .show-title');
            const title = titleElement ? titleElement.textContent.trim() : '';

            // Try to extract date
            const dateElement = container.querySelector('.date, -date, .show-date, time, .datetime');
            const dateText = dateElement ? dateElement.textContent.trim() : '';

            // Try to extract description
            const descElement = container.querySelector('.description, -description, .show-description, p');
  ...
  async extractSingleShowEvent(page, sourceURL) {
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const titleSelectors = ['h1', 'h2', '.show-title', '-title', '.title',
                           'h3.card-title', '-name', '.name', '.heading',
        ...
        const dateSelectors = [
          '.date', '-date', '.show-date', '.datetime', 'time', '.show-time', '-time',
          '[itemprop="startDate"]', '[data-, '-details', '.meta-info',
          '.card-subtitle', '-datetime', '.date-and-time', '.date-display'
        ];
        let dateText = '';
        ...
        // Find description
        const descSelectors = ['.description', '.show-description', '-description', '.content', 'p'];
        let description = '';
      ...
      const dateSelectors = [
        ...
        '[itemprop="startDate"]', '[data-, '-details', '.meta-info',
        '.card-subtitle', '-datetime', '.date-and-time', '.date-display',
        'div:has(.date), div:has(-date), div:has(.show-date), div:has(.datetime), div:has(time), div:has(.show-time), div:has(-time)'
      ];
      let dateText = '';
      ...
      // Format: YYYY-MM-DD
      if (!dateObj) {
        const isoMatch = cleanDa.match(/(\d{4}-(\d{1,2}-(\d{1,2}/);
      ...
      const timeMatch = cleanDa.match(/(\d{1,2}(?::(\d{2}?\s*(am|pm)?/i);
...
module.exports.UndergroundComedyClubEvents = UndergroundComedyClubEvents;