/**
 * Vancouver Dragon Boat Festival Events Scraper
 * Extracts events from Vancouver Dragon Boat Festival
 */
...
class DragonBoatFestivalEvents {
  constructor() {
    this.name = 'Vancouver Dragon Boat Festival Events';
    this.url = 'https://www.dragonboat.bc.ca/';
    this.baseUrl = 'https://www.dragonboat.bc.ca';
    this.venue = {
      name: 'Vancouver Dragon Boat Festival',
      address: 'False Creek, Vancouver, BC',
      city: 'Vancouver',
      province: 'BC',
      country: 'Canada',
      coordinates: { lat: 49.2745, lng: -123.1336 }
    };
  }
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Dragon Boat Festival events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Dragon Boat Festival events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Dragon Boat Festival events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Dragon Boat Festival website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .race, .festival, .dragon, article', { timeout: 10000 })
      .catch(() => {
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Sports', 'Festival', 'Dragon Boat Racing', 'Cultural'],
...
module.exports = DragonBoatFestivalEvents;
