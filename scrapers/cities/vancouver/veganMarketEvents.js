/**
 * Vancouver Vegan Market Events Scraper
 * Extracts events from Vancouver Vegan Market website
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class VeganMarketEvents {
  constructor() {
    this.name = 'Vancouver Vegan Market Events';
    this.url = 'https://veganmarket.ca/';
    this.baseUrl = 'https://veganmarket.ca';
    this.venue = {
      name: 'Vancouver Vegan Market',
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Vegan Market events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Vegan Market events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Vegan Market events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Vegan Market website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .market-event, .upcoming-event, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title') || event;
          const title = titleElement.textContent?.trim();
          
          // Extract date information
          const dateElement = event.querySelector('.date, .event-date, time, [datetime]');
          const dateText = dateElement?.textContent?.trim() || dateElement?.getAttribute('datetime') || '';
          
          // Extract description
          const descElement = event.querySelector('p, .description, .event-description');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Food', 'Health & Wellness', 'Sustainable Living', 'Community'],
...
module.exports = VeganMarketEvents;

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new VeganMarketEvents();
