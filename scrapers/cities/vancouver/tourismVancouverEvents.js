...
class TourismVancouverEventsScraper {
  ...
  parseDateRange(da) {
    ...
    try {
      ...
      if (da.includes(' - ') || da.includes(' to ')) {
        const separator = da.includes(' - ') ? ' - ' : ' to ';
  ...
  async scrape(city) {
    ...
    try {
      ...
      await page.waitForSelector('-listing, .listing-item, -item', {
        timeout: 10000
      }.catch(() => {
      ...
      const extractedEvents = await page.evaluate(() => {
        const eventElements = document.querySelectorAll('-listing, .listing-item, -item, .listing');
        ...
        const elements = eventElements.length > 0 ?
          eventElements :
          document.querySelectorAll(', article, .listing');
        ...
        return Array.from(elements).map(element => {
          ...
          const dateElement = element.querySelector('.date, .dates, -date, time');
      ...
      for (const item of extractedEvents) {
        try {
          ...
          const eventId = `tourism-vancouver-${slugify(item.title, { lower: true }}-${da}`;
