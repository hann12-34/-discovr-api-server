/**
 * Vancouver Festival D'Été Events Scraper
 * Extracts events from Festival D'Été Vancouver website
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

class FestivalDEteEvents {
  constructor() {
    this.name = 'Vancouver Festival D\'Été Events';
    this.url = 'https://festivaldetevancouver.com/';
    this.baseUrl = 'https://festivaldetevancouver.com';
    this.venue = {
      name: 'Festival D\'Été Vancouver',
  ...
  async scrape() {
    ...
    try {
      ...
      console.log('Extracting Festival D\'Été events...');
      const events = await this.extractEvents(page);
      console.log(`Found ${events.length} Festival D\'Été events`);

      return events;
    } catch (error) {
      console.error(`Error scraping Festival D'Été events: ${error.message}`);
      return [];
    } finally {
      await browser.close();
    }
  ...
  /**
   * Extract events from Festival D'Été website
   * @param {Page} page - Puppeteer page object
   * @returns {Promise<Array>} - Array of event objects
   */
  async extractEvents(page) {
    // Wait for event containers to load
    await page.waitForSelector('.event, .show, .performance, .festival-event, article', { timeout: 10000 })
      .catch(() => {
    ...
    const events = await page.evaluate((venueInfo, baseUrl) => {
      ...
      return Array.from(eventElements).map((event, index) => {
        try {
          // Extract title
          const titleElement = event.querySelector('h1, h2, h3, h4, .title, .event-title, .show-title, .festival-title') || event;
          const title = titleElement.textContent?.trim();
          
          // Extract date information
          const dateElement = event.querySelector('.date, .event-date, .show-date, .festival-date, time, [datetime]');
    ...
    return Promise.all(events.map(async event => {
      ...
      return {
        ...
        image: event.image,
        venue: this.venue,
        categories: ['Arts & Culture', 'Festival', 'Music', 'Summer Festival'],
...
module.exports = FestivalDEteEvents;

// Function export for compatibility with runner/validator
module.exports = async (city) => {
  const scraper = new FestivalDEteEvents();
