  ...
  async scrape(city) {
    ...
    try {
        ...
        for (const pattern of patterns) {
          ...
          if (match) {
            try {
              ...
              console.log(`✅ Found festival dates: ${festivalStartDate.toLocaleDa to ${festivalEndDate.toLocaleDa`);
              break;
            } catch (dateError) {
      ...
      if (!festivalStartDate || !festivalEndDate) {
        ...
        console.log(`⚠️ Using estimated festival dates: ${festivalStartDate.toLocaleDa to ${festivalEndDate.toLocaleDa`);
      }
        ...
        if (eventElements.length > 0) {
          ...
          for (let i = 0; i < eventElements.length; i++) {
            try {
                ...
                try {
                  ...
                  if (dateMatch) {
                    ...
                    if (eventDate < festivalStartDate || eventDate > festivalEndDate) {
                      console.log(`⚠️ Event date ${eventDate.toLocaleDa outside festival range, adjusting...`);
              ...
              const id = `festival-d-ete-${slugTitle}-${da}`;
        ...
        for (const selector of contentSelectors) {
          ...
          for (let i = 0; i < elements.length; i++) {
            try {
              ...
              events.push(event);
              console.log(`✅ Added event from text: ${event.title} on ${eventStartDate.toLocaleDa`);
...
module.exports = async (city) => {
  const scraper = new FestivalDEteScraper();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.FestivalDEteScraper = FestivalDEteScraper;