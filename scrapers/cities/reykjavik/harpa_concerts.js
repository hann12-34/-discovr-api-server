 ...
 * URL: https://www.harpa.is/en/events
 */
...
async function scrapeHarpa(city = 'Reykjavik') {
  ...
  try {
    ...
    await page.goto('https://www.harpa.is/en/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.harpa.is/en/events', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Harpa events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Harpa error:', error.message);
    return [];
  }
}
