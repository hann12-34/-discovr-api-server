/**
 * Reykjavik Pride Festival Events Scraper
 * URL: https://www.hinsegindagar.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeReykjavikPride(city = 'Reykjavik') {
  console.log('🏳️‍🌈 Scraping Reykjavik Pride...');
  ...
  try {
    ...
    await page.goto('https://www.hinsegindagar.is/en/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Reykjavik',
        category: 'Festival',
        source: 'Reykjavik Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Reykjavik Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Reykjavik Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeReykjavikPride;
