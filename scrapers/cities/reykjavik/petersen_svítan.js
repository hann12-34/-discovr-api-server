/**
 * Petersen Svítan Reykjavik Events Scraper
 * URL: https://www.petersensvitan.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePetersenSvitan(city = 'Reykjavik') {
  console.log('🍸 Scraping Petersen Svítan...');
  ...
  try {
    ...
    await page.goto('https://www.petersensvitan.is/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Petersen Svítan',
          address: 'Ingólfsstræti 2a, Reykjavik',
          city: 'Reykjavik'
        },
        latitude: 64.1478,
        longitude: -21.9345,
        city: 'Reykjavik',
        category: 'Nightlife',
        source: 'Petersen Svítan'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Petersen Svítan events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Petersen Svítan error:', error.message);
    return [];
  }
}

module.exports = scrapePetersenSvitan;
