/**
 * Reykjavik Beer Festival Events Scraper
 * URL: https://www.visitreykjavik.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBeerFest(city = 'Reykjavik') {
  ...
  try {
    ...
    await page.goto('https://www.visitreykjavik.is/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.visitreykjavik.is', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Beer Fest events`);
