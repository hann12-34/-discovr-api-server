/**
 * DesignMarch Reykjavik Events Scraper
 * URL: https://www.designmarch.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDesignMarch(city = 'Reykjavik') {
  console.log('🎨 Scraping DesignMarch...');
  ...
  try {
    ...
    await page.goto('https://www.designmarch.is/programme', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.designmarch.is', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} DesignMarch events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  DesignMarch error:', error.message);
    return [];
  }
}

module.exports = scrapeDesignMarch;
