/**
 * Bíó Paradís Reykjavik Events Scraper
 * URL: https://www.bioparadis.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBioParadis(city = 'Reykjavik') {
  console.log('🎬 Scraping Bíó Paradís...');
  ...
  try {
    ...
    await page.goto('https://www.bioparadis.is/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Bíó Paradís',
          address: 'Hverfisgata 54, Reykjavik',
          city: 'Reykjavik'
        },
        latitude: 64.1465,
        longitude: -21.9285,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Bíó Paradís events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bíó Paradís error:', error.message);
    return [];
  }
}

module.exports = scrapeBioParadis;
