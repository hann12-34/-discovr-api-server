/**
 * Reykjavik Winter Lights Festival Events Scraper
 * URL: https://www.winterlightsfestival.is
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWinterLights(city = 'Reykjavik') {
  console.log('💡 Scraping Winter Lights Festival...');
  ...
  try {
    ...
    await page.goto('https://www.winterlightsfestival.is/en/programme', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.winterlightsfestival.is', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Reykjavik',
        category: 'Festival',
        source: 'Winter Lights'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Winter Lights events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Winter Lights error:', error.message);
    return [];
  }
}

module.exports = scrapeWinterLights;
