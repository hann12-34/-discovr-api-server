/**
 * Paloma Reykjavik Events Scraper
 * Popular nightclub in downtown Reykjavik
 * URL: https://www.facebook.com/palomaiceland/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePalomaRvk(city = 'Reykjavik') {
  console.log('🌴 Scraping Paloma Reykjavik...');
  ...
  try {
    ...
    await page.goto('https://www.facebook.com/palomaiceland/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Paloma',
          address: 'Naustin 1-3, 101 Reykjavik',
          city: 'Reykjavik'
        },
        city: 'Reykjavik',
        category: 'Nightlife',
        source: 'Paloma Reykjavik'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Paloma events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Paloma error: ${error.message}`);
    return [];
  }
}

module.exports = scrapePalomaRvk;
