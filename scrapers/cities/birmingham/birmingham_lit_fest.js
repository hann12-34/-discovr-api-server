/**
 * Birmingham Literature Festival Events Scraper
 * URL: https://www.birminghamliteraturefestival.org
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBirminghamLit(city = 'Birmingham') {
  console.log('📚 Scraping Birmingham Literature Festival...');
  ...
  try {
    ...
    await page.goto('https://www.birminghamliteraturefestival.org', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Birmingham',
        category: 'Festival',
        source: 'Literature Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Literature Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Literature Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeBirminghamLit;
