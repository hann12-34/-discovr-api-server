/**
 * Asylum Birmingham Events Scraper
 * Nightclub venue
 * URL: https://www.asylumbirmingham.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAsylum(city = 'Birmingham') {
  console.log('🎧 Scraping Asylum Birmingham...');
  ...
  try {
    ...
    await page.goto('https://www.asylumbirmingham.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Club night at Asylum`,
        date: isoDate,
        ...
        venue: {
          name: 'Asylum',
          address: '78 Allison Street, Birmingham B5 5TH',
          city: 'Birmingham'
        },
        latitude: 52.4715,
        longitude: -1.8943,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Asylum events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Asylum error:', error.message);
    return [];
  }
}

module.exports = scrapeAsylum;
