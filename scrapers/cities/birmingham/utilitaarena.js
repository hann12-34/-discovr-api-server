/**
 * Utilita Arena Birmingham Events Scraper
 * Major arena venue in Birmingham
...
async function scrapeUtilitaArena(city = 'Birmingham') {
  console.log('🏟️ Scraping Utilita Arena Birmingham...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/event"]').forEach(el => {
        ...
        const dateMatch = allText.match(/(\d{1,2})(?:st|nd|rd|th)?\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s*(\d{4})?/i);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Utilita Arena events`);
