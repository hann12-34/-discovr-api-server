/**
 * Birmingham Food Festival Events Scraper
 * URL: https://www.birminghamfoodfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBirminghamFood(city = 'Birmingham') {
  console.log('🍽️ Scraping Birmingham Food Festival...');
  ...
  try {
    ...
    await page.goto('https://www.birminghamfoodfestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Birmingham',
        category: 'Festival',
        source: 'Food Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Food Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Food Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeBirminghamFood;
