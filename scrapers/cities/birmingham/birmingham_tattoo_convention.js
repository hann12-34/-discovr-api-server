/**
 * Birmingham Tattoo Convention Events Scraper
 * URL: https://www.birminghamtattooconvention.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTattooConvention(city = 'Birmingham') {
  ...
  try {
    ...
    await page.goto('https://www.birminghamtattooconvention.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'NEC Birmingham',
          address: 'National Exhibition Centre, Birmingham',
          city: 'Birmingham'
        },
        latitude: 52.4509,
        longitude: -1.7210,
        city: 'Birmingham',
        category: 'Festival',
        source: 'Tattoo Convention'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Tattoo Convention events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Tattoo Convention error:', error.message);
    return [];
  }
}
