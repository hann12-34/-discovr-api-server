/**
 * The Mill Birmingham Events Scraper
 * Nightclub and live music venue
 * URL: https://themilldigbeth.com/
 */
...
async function scrapeTheMill(city = 'Birmingham') {
  console.log('🎧 Scraping The Mill Birmingham...');
  ...
  try {
    ...
    await page.goto('https://themilldigbeth.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Event at The Mill Birmingham`,
        date: isoDate,
        ...
        venue: {
          name: 'The Mill',
          address: '38 Lower Trinity Street, Birmingham B9 4AG',
          city: 'Birmingham'
        },
        latitude: 52.4759,
        longitude: -1.8867,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid The Mill events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  The Mill error:', error.message);
    return [];
  }
}

module.exports = scrapeTheMill;
