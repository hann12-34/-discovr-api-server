/**
 * Hare & Hounds Birmingham Events Scraper
 * Live music venue in Kings Heath
...
async function scrapeHareAndHounds(city = 'Birmingham') {
  console.log('🐰 Scraping Hare & Hounds Birmingham...');
  ...
  try {
    ...
    await page.goto('https://hareandhoundskingsheath.co.uk/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Hare & Hounds`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Hare & Hounds events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Hare & Hounds error:', error.message);
    return [];
  }
}
