/**
 * Snobs Nightclub Birmingham Events Scraper
 * Iconic Birmingham nightclub
 * URL: https://www.snobsnightclub.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSnobsBirmingham(city = 'Birmingham') {
  console.log('🎸 Scraping Snobs Birmingham...');
  ...
  try {
    ...
    await page.goto('https://www.snobsnightclub.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '52 Smallbrook Queensway, Birmingham B5 4EY',
          city: 'Birmingham'
        },
        city: 'Birmingham',
        category: 'Nightlife',
        source: 'Snobs Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Snobs Birmingham events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Snobs Birmingham error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSnobsBirmingham;
