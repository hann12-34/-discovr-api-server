...
async function scrapeSoulFest(city = 'Birmingham') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T14:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Handsworth Park', address: 'Handsworth Park, Birmingham', city: 'Birmingham' }, latitude: 52.5094, longitude: -1.9344, city: 'Birmingham', category: 'Festival', source: 'Soul Festival' });
    }
