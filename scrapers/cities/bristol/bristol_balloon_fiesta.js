/**
 * Bristol International Balloon Fiesta Events Scraper
 * URL: https://www.bristolballoonfiesta.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBalloonFiesta(city = 'Bristol') {
  console.log('🎈 Scraping Balloon Fiesta...');
  ...
  try {
    ...
    await page.goto('https://www.bristolballoonfiesta.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Bristol',
        category: 'Festival',
        source: 'Balloon Fiesta'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Balloon Fiesta events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Balloon Fiesta error:', error.message);
    return [];
  }
}

module.exports = scrapeBalloonFiesta;
