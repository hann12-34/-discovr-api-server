/**
 * Visit Bristol Events Scraper
 * URL: https://visitbristol.co.uk/whats-on
 * Official Bristol tourism events
 */
...
async function scrapeVisitBristolEvents(city = 'Bristol') {
  console.log('🎈 Scraping Visit Bristol Events...');
  let browser;
  ...
  try {
    ...
    await page.goto('https://visitbristol.co.uk/whats-on', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(3000);
    ...
    const eventLinks = await page.evaluate(() => {
      ...
      anchors.forEach(a => {
        ...
        if (href && !href.endsWith('/whats-on') && !href.endsWith('/whats-on/')) {
          const fullUrl = href.startsWith('http') ? href : `https://visitbristol.co.uk${href}`;
    ...
    for (const url of eventLinks) {
      try {
        ...
        events.push({
          ...
          venue: {
            name: eventData.location ? eventData.location.split(',')[0].trim() : 'Bristol',
            address: eventData.location || 'Bristol, UK',
            city: 'Bristol'
          },
          latitude: 51.4545,
          longitude: -2.5879,
    ...
    console.log(`  ✅ Found ${unique.length} Visit Bristol events`);
    return unique;
  } catch (error) {
    console.error(`  ⚠️ Visit Bristol Events error: ${error.message}`);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeVisitBristolEvents;
