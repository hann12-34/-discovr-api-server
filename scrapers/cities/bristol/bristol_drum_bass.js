...
async function scrapeDrumBass(city = 'Bristol') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Motion', address: 'Avon Street, Bristol', city: 'Bristol' }, latitude: 51.4484, longitude: -2.5838, city: 'Bristol', category: 'Nightlife', source: 'Drum and Bass' });
    }
