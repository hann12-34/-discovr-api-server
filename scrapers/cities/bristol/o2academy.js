/**
 * O2 Academy Bristol Events Scraper (for Birmingham folder - will move)
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academybristol/events
 */
...
async function scrapeO2AcademyBristol(city = 'Bristol') {
  console.log('🎸 Scraping O2 Academy Bristol...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academybristol/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Bristol', address: 'Frogmore Street, Bristol BS1 5NA', city: 'Bristol' },
        latitude: 51.4545,
        longitude: -2.5968,
        city: 'Bristol',
        category: 'Nightlife',
        source: 'O2 Academy Bristol'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Bristol events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Bristol error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyBristol;
