...
async function scrapeO2AcademyBristolV2(city = 'Bristol') {
  const events = [];
  let browser;
  ...
  try {
    ...
    for (const item of eventData) {
      ...
      events.push({
        ...
        location: `O2 Academy Bristol, ${city}`,
        city: city,
        ...
        imageUrl: item.image,
        category: 'Concert',
        source: 'o2_academy_bristol'
      });
    }
    
    console.log(`O2 Academy Bristol: Found ${events.length} events`);
    return events;
    
  } catch (error) {
    console.error('O2 Academy Bristol scraper error:', error.message);
    return events;
  } finally {
...
module.exports = scrapeO2AcademyBristolV2;
