/**
 * Bristol Pride Events Scraper
 * URL: https://www.bristolpride.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBristolPride(city = 'Bristol') {
  console.log('🏳️‍🌈 Scraping Bristol Pride...');
  ...
  try {
    ...
    await page.goto('https://www.bristolpride.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Bristol',
        category: 'Festival',
        source: 'Bristol Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bristol Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bristol Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeBristolPride;
