/**
 * Lakota Bristol Scraper
 * URL: https://lakota.co.uk/events/
 * Address: 6 Upper York Street, Bristol BS2 8QN
 */
...
async function scrapeLakotaV2(city = 'Bristol') {
  ...
  try {
      ...
      allLinks.forEach(link => {
        ...
        if (href.includes('event') || href.includes('lakota')) {
          ...
          if (!items.find(e => e.title === text)) {
            items.push({
              title: text,
              url: href.startsWith('http') ? href : `https://lakota.co.uk${href}`,
    ...
    console.log(`Lakota Bristol: Found ${events.length} events`);
    return events;
    
  } catch (error) {
    console.error('Lakota scraper error:', error.message);
    return events;
  } finally {
