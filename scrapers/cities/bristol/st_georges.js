...
async function scrapeStGeorges(city = 'Bristol') {
  console.log('🎵 Scraping St Georges...');
  ...
  try {
    ...
    await page.goto('https://www.stgeorgesbristol.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Bristol',
        category: 'Arts',
        source: 'St Georges Bristol'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} St Georges events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  St Georges error:', error.message);
    return [];
  }
}

module.exports = scrapeStGeorges;
