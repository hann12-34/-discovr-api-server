/**
 * Rough Trade Bristol Events Scraper
 * URL: https://www.roughtrade.com/gb/events/bristol
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoughTrade(city = 'Bristol') {
  console.log('🎵 Scraping Rough Trade...');
  ...
  try {
    ...
    await page.goto('https://www.roughtrade.com/gb/events/bristol', {
    ...
    const events = await page.evaluate(() => {
      ...
      eventElements.forEach(item => {
        try {
          ...
          if (!url || seen.has(url) || url.endsWith('/events/') || url.endsWith('/bristol')) return;
          seen.add(url);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Clifton Down Shopping Centre, Bristol BS8 3HA',
          city: 'Bristol'
        },
        latitude: 51.4650,
        longitude: -2.6150,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Rough Trade events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Rough Trade error:', error.message);
    return [];
  }
}

module.exports = scrapeRoughTrade;
