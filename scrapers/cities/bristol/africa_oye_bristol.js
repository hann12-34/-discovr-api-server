...
async function scrapeAfricaOye(city = 'Bristol') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'St Pauls', address: 'St Pauls, Bristol', city: 'Bristol' }, latitude: 51.4686, longitude: -2.5822, city: 'Bristol', category: 'Festival', source: 'Africa Oyé' });
    }
