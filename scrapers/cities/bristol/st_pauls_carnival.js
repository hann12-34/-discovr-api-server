/**
 * St Pauls Carnival Bristol Events Scraper
 * URL: https://www.stpaulscarnival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeStPaulsCarnival(city = 'Bristol') {
  console.log('🎉 Scraping St Pauls Carnival...');
  ...
  try {
    ...
    await page.goto('https://www.stpaulscarnival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Bristol',
        category: 'Festival',
        source: 'St Pauls Carnival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} St Pauls Carnival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  St Pauls Carnival error:', error.message);
    return [];
  }
}

module.exports = scrapeStPaulsCarnival;
