/**
 * Alberta Ballet Edmonton Events Scraper
 * URL: https://albertaballet.com/performances/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAlbertaBallet(city = 'Edmonton') {
  console.log('🩰 Scraping Alberta Ballet Edmonton...');
  ...
  try {
    ...
    await page.goto('https://albertaballet.com/performances/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.performance-card, .event-card, article, a[href*="performance"], [class*="event"], .card').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Alberta Ballet events`);
...
module.exports = scrapeAlbertaBallet;
