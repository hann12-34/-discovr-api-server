/**
 * Edmonton Elks CFL Events Scraper
 * URL: https://www.goelks.com/schedule
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeElks(city = 'Edmonton') {
  console.log('🏈 Scraping Edmonton Elks...');
  ...
  try {
    ...
    await page.goto('https://www.goelks.com/schedule', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="game"], [class*="schedule"], article, .event-item, tr').forEach(el => {
        try {
          const link = el.querySelector('a');
          const url = link?.href; if (!url) return;
          
          const opponent = el.querySelector('[class*="opponent"], [class*="team"]')?.textContent?.trim();
          const title = opponent ? `Elks vs ${opponent}` : 'Edmonton Elks Game';
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Elks events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Elks error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeElks;
