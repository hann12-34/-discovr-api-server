/**
 * Union Hall Edmonton Events Scraper (Alternative URL)
...
async function scrapeEdmUnion(city = 'Edmonton') {
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event-item, article, [class*="event"], .tribe-events-calendar-list__event').forEach(el => {
        try {
          ...
          const titleEl = el.querySelector('h1, h2, h3, h4, .title, [class*="title"], .tribe-events-calendar-list__event-title');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Union Hall', address: '10537 Jasper Ave NW, Edmonton, AB T5J 1Z7', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Nightlife',
        source: 'Union Hall'
      });
