/**
 * FC Edmonton Soccer Events Scraper
...
async function scrapeFcEdmonton(city = 'Edmonton') {
  console.log('⚽ Scraping FC Edmonton Schedule...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="game"], [class*="match"], article, tr, .event').forEach(el => {
        try {
          const text = el.textContent || '';
          
          const titleEl = el.querySelector('h2, h3, h4, .title, [class*="opponent"]');
          const title = titleEl?.textContent?.trim() || 'FC Edmonton Match';
          ...
          results.push({
            title: title.includes('FC Edmonton') ? title : `FC Edmonton vs ${title}`,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} FC Edmonton matches`);
...
module.exports = scrapeFcEdmonton;
