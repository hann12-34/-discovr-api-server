/**
 * Edmonton Folk Music Festival Edmonton Events Scraper
...
async function scrapeEdmontonfolkfest(city = 'Edmonton') {
  console.log('🎵 Scraping Edmonton Folk Music Festival Edmonton...');
  ...
  try {
    ...
    await page.goto('https://edmontonfolkfest.org/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edmonton',
        category: 'Festivals',
        source: 'Edmonton Folk Music Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Edmonton Folk Music Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Edmonton Folk Music Festival error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeEdmontonfolkfest;
