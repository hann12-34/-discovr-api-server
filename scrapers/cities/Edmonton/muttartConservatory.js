/**
 * Muttart Conservatory Edmonton Events Scraper
...
async function scrapeMuttartConservatory(city = 'Edmonton') {
  console.log('🌿 Scraping Muttart Conservatory Edmonton...');
  ...
  try {
    ...
    await page.goto('https://www.edmonton.ca/attractions_events/muttart-conservatory', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, .event-item, article, a[href*="event"], [class*="event"], .program').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Muttart Conservatory', address: '9626 96A St NW, Edmonton, AB T6C 4L8', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Variety',
        source: 'Muttart Conservatory'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Muttart events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Muttart error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMuttartConservatory;
