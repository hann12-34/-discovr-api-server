/**
 * Ice Castles Edmonton Edmonton Events Scraper
...
async function scrapeIcecastle(city = 'Edmonton') {
  console.log('🎵 Scraping Ice Castles Edmonton Edmonton...');
  ...
  try {
    ...
    await page.goto('https://icecastles.com/edmonton/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edmonton',
        category: 'Festivals',
        source: 'Ice Castles Edmonton'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ice Castles Edmonton events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Ice Castles Edmonton error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeIcecastle;
