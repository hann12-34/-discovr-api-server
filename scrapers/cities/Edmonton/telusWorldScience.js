/**
 * TELUS World of Science Edmonton Events Scraper
 * URL: https://telusworldofscienceedmonton.ca/explore/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTelusWorldScience(city = 'Edmonton') {
  console.log('🔬 Scraping TELUS World of Science Edmonton...');
  ...
  try {
    ...
    await page.goto('https://telusworldofscienceedmonton.ca/explore/events/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, .event-item, article, a[href*="event"], [class*="event"]').forEach(el => {
        try {
          const linkEl = el.tagName === 'A' ? el : el.querySelector('a[href]');
          const url = linkEl?.href; if (!url) return;
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'TELUS World of Science', address: '11211 142 St NW, Edmonton, AB T5M 4A1', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Variety',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} TELUS World of Science events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ TELUS World of Science error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTelusWorldScience;
