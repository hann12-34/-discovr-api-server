/**
 * Edmonton Concert Hall Events - Alberta Ballet/ESO
 * URL: https://albertaballet.com/whats-on/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEdmConcertHall(city = 'Edmonton') {
  ...
  try {
    ...
    await page.goto('https://albertaballet.com/whats-on/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="show"], .event-item, article, [class*="event"], .card, [class*="show"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Jubilee Auditorium', address: '11455 87 Ave NW, Edmonton, AB T6G 2T2', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Ballet events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Ballet error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeEdmConcertHall;
