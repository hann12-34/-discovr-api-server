/**
 * MacEwan University Events Edmonton Scraper
 * URL: https://www.macewan.ca/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMacEwanHall(city = 'Edmonton') {
  console.log('🎓 Scraping MacEwan Events Edmonton...');
  ...
  try {
    ...
    await page.goto('https://www.macewan.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'MacEwan University', address: '10700 104 Ave NW, Edmonton, AB T5J 4S2', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Variety',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} MacEwan events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ MacEwan error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMacEwanHall;
