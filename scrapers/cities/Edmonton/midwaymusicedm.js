 ...
 * URL: https://midwaymusichall.com/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMidwayMusic(city = 'Edmonton') {
  ...
  try {
    ...
    await page.goto('https://midwaymusichall.com/events/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-item, .event-card, article, a[href*="event"], .show, .tribe-events-calendar-list__event').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Midway Music Hall', address: '11414 Jasper Ave NW, Edmonton, AB T5K 0M7', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Midway Music Hall events`);
...
module.exports = scrapeMidwayMusic;
