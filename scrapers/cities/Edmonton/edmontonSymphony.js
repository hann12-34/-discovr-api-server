/**
 * Edmonton Symphony Orchestra Events Scraper
 * URL: https://www.edmontonsymphony.com/concerts-tickets/concerts/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEdmontonSymphony(city = 'Edmonton') {
  console.log('🎻 Scraping Edmonton Symphony Orchestra...');
  ...
  try {
    ...
    await page.goto('https://www.edmontonsymphony.com/concerts-tickets/concerts/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.concert-card, .event-card, article, a[href*="concert"], [class*="event"], .card').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edmonton',
        category: 'Variety',
        source: 'Edmonton Symphony'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} ESO events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ ESO error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeEdmontonSymphony;
