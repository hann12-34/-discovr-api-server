/**
 * The Mercer Tavern Edmonton Events Scraper
...
async function scrapeThemercer(city = 'Edmonton') {
  console.log('🎵 Scraping The Mercer Tavern Edmonton...');
  ...
  try {
    ...
    await page.goto('https://themercertavern.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Edmonton',
        category: 'Nightlife',
        source: 'The Mercer Tavern'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} The Mercer Tavern events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  The Mercer Tavern error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeThemercer;
