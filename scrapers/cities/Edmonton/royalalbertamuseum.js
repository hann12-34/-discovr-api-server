 ...
 * URL: https://royalalbertamuseum.ca/visit/whats-on
 */
...
async function scrapeRoyalAlbertaMuseum(city = 'Edmonton') {
  ...
  try {
    ...
    await page.goto('https://royalalbertamuseum.ca/visit/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-item, .event-card, article, a[href*="event"], .program-card, [class*="exhibit"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Royal Alberta Museum', address: '9810 103a Ave NW, Edmonton, AB T5J 0G2', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Variety',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} RAM events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ RAM error: ${error.message}`);
    return [];
  }
}
