/**
 * Edmonton Oil Kings WHL Events Scraper
 * URL: https://www.oilkings.ca/schedule
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOilKings(city = 'Edmonton') {
  console.log('🏒 Scraping Edmonton Oil Kings...');
  ...
  try {
    ...
    await page.goto('https://www.oilkings.ca/schedule', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="game"], [class*="schedule"], article, tr, .event-item').forEach(el => {
        try {
          const link = el.querySelector('a');
          const url = link?.href; if (!url) return;
          
          const opponent = el.querySelector('[class*="opponent"], [class*="team"]')?.textContent?.trim();
          const title = opponent ? `Oil Kings vs ${opponent}` : 'Edmonton Oil Kings Game';
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Oil Kings events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Oil Kings error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOilKings;
