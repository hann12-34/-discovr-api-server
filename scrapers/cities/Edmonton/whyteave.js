/**
 * Whyte Avenue / Old Strathcona Edmonton Events Scraper
 * URL: https://www.oldstrathcona.ca/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWhyteAve(city = 'Edmonton') {
  console.log('🛍️ Scraping Whyte Avenue Edmonton...');
  ...
  try {
    ...
    await page.goto('https://www.oldstrathcona.ca/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Old Strathcona / Whyte Ave', address: '82 Ave & 104 St NW, Edmonton, AB', city: 'Edmonton' },
        city: 'Edmonton',
        category: 'Festival',
        source: 'Old Strathcona'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Whyte Ave events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Whyte Ave error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeWhyteAve;
