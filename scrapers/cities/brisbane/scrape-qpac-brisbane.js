/**
 * QPAC Brisbane - brisbane Events Scraper
 * URL: https://www.qpac.com.au/whats-on/
 */
...
async function scrapeQPACBrisbane(city = 'brisbane') {
  console.log('🎭 Scraping QPAC Brisbane...');
  try {
    const resp = await axios.get('https://www.qpac.com.au/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'QPAC Brisbane', address: addr || 'Grey St, South Brisbane QLD 4101', city: 'brisbane' },
            latitude: -27.4745,
            longitude: 153.0178,
            city: 'brisbane',
            category: 'Events',
            source: 'QPAC Brisbane'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.qpac.com.au/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.qpac.com.au/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'QPAC Brisbane', address: 'Grey St, South Brisbane QLD 4101', city: 'brisbane' },
            latitude: -27.4745,
            longitude: 153.0178,
            city: 'brisbane',
            category: 'Events',
    ...
    console.log('  ✅ QPAC Brisbane:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ QPAC Brisbane error:', error.message);
    return [];
  }
}

module.exports = scrapeQPACBrisbane;
