/**
 * GOMA Brisbane - brisbane Events Scraper
 * URL: https://www.qagoma.qld.gov.au/whats-on
 */
...
async function scrapeGOMABrisbane(city = 'brisbane') {
  console.log('🎭 Scraping GOMA Brisbane...');
  try {
    const resp = await axios.get('https://www.qagoma.qld.gov.au/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'GOMA Brisbane', address: addr || 'Stanley Pl, South Brisbane QLD 4101', city: 'brisbane' },
            latitude: -27.472,
            longitude: 153.0177,
            city: 'brisbane',
            category: 'Events',
            source: 'GOMA Brisbane'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.qagoma.qld.gov.au/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.qagoma.qld.gov.au/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'GOMA Brisbane', address: 'Stanley Pl, South Brisbane QLD 4101', city: 'brisbane' },
            latitude: -27.472,
            longitude: 153.0177,
            city: 'brisbane',
            category: 'Events',
    ...
    console.log('  ✅ GOMA Brisbane:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ GOMA Brisbane error:', error.message);
    return [];
  }
}

module.exports = scrapeGOMABrisbane;
