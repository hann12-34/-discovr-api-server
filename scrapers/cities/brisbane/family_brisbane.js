/**
 * Family Brisbane Events Scraper
 * Popular nightclub in Brisbane
 * URL: https://thefamilynightclub.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFamilyBrisbane(city = 'Brisbane') {
  console.log('👨‍👩‍👧‍👦 Scraping Family Brisbane...');
  ...
  try {
    ...
    await page.goto('https://thefamilynightclub.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Family',
          address: '8 McLachlan St, Fortitude Valley QLD 4006',
          city: 'Brisbane'
        },
        city: 'Brisbane',
        category: 'Nightlife',
        source: 'Family Brisbane'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Family events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Family error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeFamilyBrisbane;
