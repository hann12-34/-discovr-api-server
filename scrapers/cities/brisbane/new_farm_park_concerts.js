...
async function scrapeNewFarmPark(city = 'Brisbane') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'New Farm Park', address: 'Brunswick Street, New Farm QLD 4005', city: 'Brisbane' }, latitude: -27.4665, longitude: 153.0534, city: 'Brisbane', category: 'Festival', source: 'New Farm Park' });
    }
