...
async function scrapeBrisbaneFestival(city = 'Brisbane') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'South Bank', address: 'South Bank Parklands, Brisbane QLD 4101', city: 'Brisbane' }, latitude: -27.4748, longitude: 153.0174, city: 'Brisbane', category: 'Festival', source: 'Brisbane Festival' });
    }
