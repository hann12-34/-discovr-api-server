/**
 * The Valley Brisbane Events Scraper
 * Fortitude Valley entertainment precinct venues
 * URL: https://thevalley.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTheValley(city = 'Brisbane') {
  console.log('🎧 Scraping The Valley Brisbane...');
  ...
  try {
    ...
    await page.goto('https://thevalley.com.au/whats-on/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Event in Fortitude Valley`,
        date: isoDate,
        ...
        venue: {
          ...
          address: 'Fortitude Valley, Brisbane QLD 4006',
          city: 'Brisbane'
        },
        latitude: -27.4570,
        longitude: 153.0350,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Valley events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  The Valley error:', error.message);
    return [];
  }
}

module.exports = scrapeTheValley;
