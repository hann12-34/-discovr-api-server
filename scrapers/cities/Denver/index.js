...
async function scrapeDenverCityEvents() {
    console.log('🏔️ Starting Denver scrapers...');
    const allEvents = [];
    ...
    for (const file of scraperFiles) {
        try {
            ...
            const events = await (typeof scraper.scrape === 'function' ? scraper.scrape() : scraper('Denver'));
            if (Array.isArray(events) && events.length > 0) allEvents.push(...events);
        } catch (error) { console.error(`❌ Denver/${file}: ${error.message}`); }
    }
    const seen = new Set();
    ...
    console.log(`✅ Denver: ${deduped.length} events`);
    return deduped;
}

module.exports = scrapeDenverCityEvents;
