/**
 * Havana Bar Wellington Events Scraper
 * Popular venue in Wellington
 * URL: https://havanabar.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHavanaBarWellington(city = 'Wellington') {
  console.log('🍹 Scraping Havana Bar Wellington...');
  ...
  try {
    ...
    await page.goto('https://havanabar.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Havana Bar',
          address: '32-34 Wigan St, Wellington 6011',
          city: 'Wellington'
        },
        city: 'Wellington',
        category: 'Nightlife',
        source: 'Havana Bar Wellington'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Havana Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Havana Bar error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeHavanaBarWellington;
