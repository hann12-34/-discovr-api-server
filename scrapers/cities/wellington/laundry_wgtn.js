/**
 * Laundry Bar Wellington Events Scraper
 * URL: https://www.laundry.bar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLaundryBar(city = 'Wellington') {
  console.log('🎉 Scraping Laundry Bar...');
  ...
  try {
    ...
    await page.goto('https://www.laundry.bar/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Laundry Bar',
          address: '240 Cuba Street, Wellington',
          city: 'Wellington'
        },
        latitude: -41.2960,
        longitude: 174.7740,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Laundry Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Laundry Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeLaundryBar;
