/**
 * Danger Danger Wellington Events Scraper
 * URL: https://www.dangerdanger.bar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDangerDanger(city = 'Wellington') {
  console.log('⚡ Scraping Danger Danger...');
  ...
  try {
    ...
    await page.goto('https://www.dangerdanger.bar/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Danger Danger',
          address: '125 Cuba Street, Wellington',
          city: 'Wellington'
        },
        latitude: -41.2925,
        longitude: 174.7755,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Danger Danger events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Danger Danger error:', error.message);
    return [];
  }
}

module.exports = scrapeDangerDanger;
