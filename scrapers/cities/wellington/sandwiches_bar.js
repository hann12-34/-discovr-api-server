/**
 * Sandwiches Bar Wellington Events Scraper
 * URL: https://www.sandwiches.bar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSandwichesBar(city = 'Wellington') {
  console.log('🎉 Scraping Sandwiches Bar...');
  ...
  try {
    ...
    await page.goto('https://www.sandwiches.bar/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Sandwiches Bar',
          address: '8 Kent Terrace, Wellington',
          city: 'Wellington'
        },
        latitude: -41.2940,
        longitude: 174.7860,
        city: 'Wellington',
        category: 'Nightlife',
        source: 'Sandwiches Bar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sandwiches Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sandwiches Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeSandwichesBar;
