/**
 * Residence Bar Wellington Events Scraper
 * URL: https://www.residencebar.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeResidence(city = 'Wellington') {
  console.log('🏠 Scraping Residence Bar...');
  ...
  try {
    ...
    await page.goto('https://www.residencebar.co.nz/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Residence Bar',
          address: '45 Tory Street, Wellington',
          city: 'Wellington'
        },
        latitude: -41.2920,
        longitude: 174.7800,
        city: 'Wellington',
        category: 'Nightlife',
        source: 'Residence Bar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Residence Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Residence Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeResidence;
