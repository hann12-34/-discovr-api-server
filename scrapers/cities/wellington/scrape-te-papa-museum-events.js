/**
 * Te Papa Museum Events - wellington Events Scraper
...
async function scrapeTePapaMuseumEvents(city = 'wellington') {
  console.log('🎭 Scraping Te Papa Museum Events...');
  try {
    const resp = await axios.get('https://www.tepapa.govt.nz/visit/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Te Papa Museum Events', address: addr || '55 Cable St, Wellington 6011', city: 'wellington' },
            latitude: -41.2903,
            longitude: 174.7819,
            city: 'wellington',
            category: 'Events',
            source: 'Te Papa Museum Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.tepapa.govt.nz/visit/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.tepapa.govt.nz/visit/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Te Papa Museum Events', address: '55 Cable St, Wellington 6011', city: 'wellington' },
            latitude: -41.2903,
            longitude: 174.7819,
            city: 'wellington',
            category: 'Events',
    ...
    console.log('  ✅ Te Papa Museum Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Te Papa Museum Events error:', error.message);
    return [];
  }
}

module.exports = scrapeTePapaMuseumEvents;
