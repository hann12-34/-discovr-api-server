...
async function scrapeWellyPlate(city = 'Wellington') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Various Wellington Venues', address: 'Wellington City', city: 'Wellington' }, latitude: -41.2865, longitude: 174.7762, city: 'Wellington', category: 'Festival', source: 'Wellington On a Plate' });
    }
