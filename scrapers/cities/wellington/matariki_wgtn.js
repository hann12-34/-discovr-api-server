...
async function scrapeMatarikiWGTN(city = 'Wellington') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Te Papa', address: 'Cable Street, Wellington', city: 'Wellington' }, latitude: -41.2901, longitude: 174.7814, city: 'Wellington', category: 'Festival', source: 'Matariki Wellington' });
    }
