/**
 * One51 Wellington Events Scraper
 * Popular nightclub in Wellington
 * URL: https://one51.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOne51Wellington(city = 'Wellington') {
  console.log('🎵 Scraping One51 Wellington...');
  ...
  try {
    ...
    await page.goto('https://one51.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'One51',
          address: '151 Cuba Street, Wellington',
          city: 'Wellington'
        },
        city: 'Wellington',
        category: 'Nightlife',
        source: 'One51 Wellington'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} One51 events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ One51 error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOne51Wellington;
