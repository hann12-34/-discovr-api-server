...
async function scrapeCubaDupa(city = 'Wellington') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Cuba Street', address: 'Cuba Street, Te Aro, Wellington', city: 'Wellington' }, latitude: -41.2939, longitude: 174.7758, city: 'Wellington', category: 'Festival', source: 'CubaDupa' });
    }
