/**
 * Foo Fighters Wellington Events Scraper
 * Popular bar in Wellington
 * URL: https://foofighters.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFooFightersWellington(city = 'Wellington') {
  console.log('🎸 Scraping Foo Fighters Wellington...');
  ...
  try {
    ...
    await page.goto('https://foofighters.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Wellington',
        category: 'Nightlife',
        source: 'Foo Fighters Wellington'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Foo Fighters events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Foo Fighters error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeFooFightersWellington;
