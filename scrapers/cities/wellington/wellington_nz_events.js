/**
 * Wellington NZ Events Scraper
 * URL: https://www.wellingtonnz.com/visit/events/
 * Official Wellington tourism events
 */
...
async function scrapeWellingtonNZEvents(city = 'Wellington') {
  console.log('🏙️ Scraping Wellington NZ Events...');
  let browser;
  ...
  try {
    ...
    await page.goto('https://www.wellingtonnz.com/visit/events/', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(3000);
    ...
    const eventLinks = await page.evaluate(() => {
      ...
      anchors.forEach(a => {
        ...
        if (href && !href.endsWith('/events/') && !href.endsWith('/events')) {
          const fullUrl = href.startsWith('http') ? href : `https://www.wellingtonnz.com${href}`;
    ...
    for (const url of eventLinks) {
      try {
        ...
        const eventData = await page.evaluate(() => {
          ...
          if (!imageUrl) {
            const imgs = document.querySelectorAll('img[src*="wellington"], img[class*="featured"], img[class*="hero"]');
            for (const img of imgs) {
              const src = img.getAttribute('src');
              if (src && !src.includes('logo') && !src.includes('icon')) {
                imageUrl = src.startsWith('http') ? src : `https://www.wellingtonnz.com${src}`;
                break;
              }
            }
          }
        ...
        events.push({
          ...
          venue: {
            name: eventData.location ? eventData.location.split(',')[0].trim() : 'Wellington',
            address: eventData.location || 'Wellington, New Zealand',
            city: 'Wellington'
          },
          latitude: -41.2866,
          longitude: 174.7756,
    ...
    console.log(`  ✅ Found ${unique.length} Wellington NZ events`);
    return unique;
