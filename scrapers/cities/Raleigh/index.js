...
async function scrapeRaleighCityEvents() {
    console.log('🌲 Starting Raleigh scrapers...');
    const allEvents = [];
    ...
    for (const file of scraperFiles) {
        try {
            ...
            const events = await (typeof scraper.scrape === 'function' ? scraper.scrape() : scraper('Raleigh'));
            if (Array.isArray(events) && events.length > 0) allEvents.push(...events);
        } catch (error) { console.error(`❌ Raleigh/${file}: ${error.message}`); }
    }
    const seen = new Set();
    ...
    console.log(`✅ Raleigh: ${deduped.length} events`);
    return deduped;
}

module.exports = scrapeRaleighCityEvents;
