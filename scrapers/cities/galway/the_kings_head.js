/**
 * The Kings Head Galway Events Scraper
 * URL: https://www.kingsheadgalway.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKingsHead(city = 'Galway') {
  console.log('👑 Scraping The Kings Head...');
  ...
  try {
    ...
    await page.goto('https://www.kingsheadgalway.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Galway',
        category: 'Nightlife',
        source: 'The Kings Head'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kings Head events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kings Head error:', error.message);
    return [];
  }
}

module.exports = scrapeKingsHead;
