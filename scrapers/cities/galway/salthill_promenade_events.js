/**
 * Salthill Promenade Galway Events - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSalthill(city = 'Galway') {
  console.log('🌊 Scraping Salthill Events...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T14:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Salthill Promenade', address: 'Salthill, Galway', city: 'Galway' }, latitude: 53.2588, longitude: -9.0816, city: 'Galway', category: 'Festival', source: 'Salthill' });
    }
    console.log(`  ✅ Found ${results.length} Salthill events`);
    return results;
  } catch (err) {
    console.error('Salthill error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeSalthill;
