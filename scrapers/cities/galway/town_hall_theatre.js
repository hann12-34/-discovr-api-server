/**
 * Town Hall Theatre Galway Events Scraper
 * URL: https://www.tht.ie/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTownHall(city = 'Galway') {
  console.log('🎭 Scraping Town Hall Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.tht.ie/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Galway',
        category: 'Arts',
        source: 'Town Hall Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Town Hall Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Town Hall Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeTownHall;
