/**
 * DNA Galway Events Scraper
 * Popular nightclub in Galway
 * URL: https://dnagalway.ie/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDnaGalway(city = 'Galway') {
  console.log('🧬 Scraping DNA Galway...');
  ...
  try {
    ...
    await page.goto('https://dnagalway.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'DNA',
          address: 'Eglinton Street, Galway',
          city: 'Galway'
        },
        city: 'Galway',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} DNA events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ DNA error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeDnaGalway;
