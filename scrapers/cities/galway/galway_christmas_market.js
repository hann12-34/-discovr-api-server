/**
 * Galway Continental Christmas Market - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeChristmasMarket(city = 'Galway') {
  ...
  try {
    ...
    await page.goto('https://www.galwaytourism.ie/christmas-market/', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T10:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Eyre Square', address: 'Eyre Square, Galway', city: 'Galway' }, latitude: 53.2744, longitude: -9.0493, city: 'Galway', category: 'Festival', source: 'Christmas Market' });
    }
