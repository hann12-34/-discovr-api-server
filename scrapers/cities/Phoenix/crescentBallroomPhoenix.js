/**
 * Crescent Ballroom Phoenix Events Scraper
 * URL: https://www.crescentphx.com/calendar
 * Live music venue in downtown Phoenix
...
const CrescentBallroomEvents = {
  async scrape(city = 'Phoenix') {
    console.log('🎸 Scraping Crescent Ballroom Phoenix...');

    try {
      const response = await axios.get('https://www.crescentphx.com/calendar', {
        headers: {
      ...
      for (let i = 0; i < allNames.length; i++) {
        ...
        if (!url) continue;
        if (!url.startsWith('http')) url = 'https://www.crescentphx.com' + url;
        ...
        events.push({
          ...
          city: 'Phoenix',
          category: 'Music',
          source: 'Crescent Ballroom Phoenix'
        });
      }

      console.log(`  ✅ Found ${events.length} Crescent Ballroom events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Crescent Ballroom error: ${error.message}`);
      return [];
    }
  }
};

module.exports = CrescentBallroomEvents.scrape;
