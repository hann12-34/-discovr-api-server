...
async function scrapePhoenixCityEvents() {
    console.log('🌵 Starting Phoenix scrapers...');
    const allEvents = [];
    ...
    for (const file of scraperFiles) {
        try {
            ...
            const events = await (typeof scraper.scrape === 'function' ? scraper.scrape() : scraper('Phoenix'));
            if (Array.isArray(events) && events.length > 0) allEvents.push(...events);
        } catch (error) { console.error(`❌ Phoenix/${file}: ${error.message}`); }
    }
    const seen = new Set();
    ...
    console.log(`✅ Phoenix: ${deduped.length} events`);
    return deduped;
}

module.exports = scrapePhoenixCityEvents;
