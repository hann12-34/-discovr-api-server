...
async function scrapeIlluminate(city = 'Adelaide') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Adelaide CBD', address: 'North Terrace, Adelaide SA 5000', city: 'Adelaide' }, latitude: -34.9205, longitude: 138.6000, city: 'Adelaide', category: 'Festival', source: 'Illuminate Adelaide' });
    }
