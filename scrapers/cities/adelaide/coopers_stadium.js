/**
 * Coopers Stadium Adelaide Events Scraper
 * URL: https://www.coopersstadium.com.au/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCoopersStadium(city = 'Adelaide') {
  console.log('🏟️ Scraping Coopers Stadium...');
  ...
  try {
    ...
    await page.goto('https://www.coopersstadium.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Hindmarsh, Adelaide SA 5007',
          city: 'Adelaide'
        },
        latitude: -34.9070,
        longitude: 138.5650,
        city: 'Adelaide',
        category: 'Arts',
        source: 'Coopers Stadium'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Coopers Stadium events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Coopers Stadium error:', error.message);
    return [];
  }
}

module.exports = scrapeCoopersStadium;
