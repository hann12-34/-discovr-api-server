...
async function scrapeRoyalCroquet(city = 'Adelaide') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Pinky Flat', address: 'Pinky Flat, Adelaide SA 5000', city: 'Adelaide' }, latitude: -34.9167, longitude: 138.5941, city: 'Adelaide', category: 'Festival', source: 'Royal Croquet Club' });
    }
