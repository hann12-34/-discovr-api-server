/**
 * HQ Complex Adelaide Events Scraper
 * Multi-room nightclub complex on Hindley Street
 * URL: https://hqcomplex.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHQComplex(city = 'Adelaide') {
  console.log('🎧 Scraping HQ Complex Adelaide...');
  ...
  try {
    ...
    await page.goto('https://hqcomplex.com.au/', {
    ...
    const eventsPageUrl = await page.evaluate(() => {
      ...
      for (const link of links) {
        ...
        if (href.includes('event') || href.includes('whats-on')) {
          return link.href;
        }
      }
      return null;
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.eventlist-event', 'article', '[class*="event"]', '.show'];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'HQ Complex',
          address: '1 North Terrace, Adelaide, SA 5000',
          city: 'Adelaide'
        },
        latitude: -34.9218,
        longitude: 138.5948,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} HQ Complex events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ HQ Complex error:', error.message);
    return [];
  }
}

module.exports = scrapeHQComplex;
