/**
 * Electric Circus Adelaide Events Scraper
 * Industrial-chic nightclub known for EDM and electronic music
 * URL: https://www.electriccircus.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeElectricCircus(city = 'Adelaide') {
  console.log('⚡ Scraping Electric Circus Adelaide...');
  ...
  try {
    ...
    await page.goto('https://www.electriccircus.com.au/', {
    ...
    // Look for events page
    const eventsPageUrl = await page.evaluate(() => {
      const links = Array.from(document.querySelectorAll('a[href]'));
      for (const link of links) {
        const href = link.href.toLowerCase();
        if (href.includes('event') || href.includes('whats-on') || href.includes('lineup')) {
          return link.href;
        }
      }
      return null;
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.eventlist-event', 'article', '[class*="event"]', '.show', '.lineup-item'];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '35 Morphett Street, Adelaide, SA 5000',
          city: 'Adelaide'
        },
        latitude: -34.9275,
        longitude: 138.5960,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Electric Circus events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ Electric Circus error:', error.message);
    return [];
  }
}

module.exports = scrapeElectricCircus;
