/**
 * BASS Adelaide Events - adelaide Events Scraper
 * URL: https://www.bass.net.au/events
 */
...
async function scrapeBASSAdelaideEvents(city = 'adelaide') {
  console.log('🎭 Scraping BASS Adelaide Events...');
  try {
    const resp = await axios.get('https://www.bass.net.au/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'BASS Adelaide Events', address: addr || 'Adelaide, SA', city: 'adelaide' },
            latitude: -34.9285,
            longitude: 138.6007,
            city: 'adelaide',
            category: 'Events',
            source: 'BASS Adelaide Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.bass.net.au/events');
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'BASS Adelaide Events', address: 'Adelaide, SA', city: 'adelaide' },
            latitude: -34.9285,
            longitude: 138.6007,
            city: 'adelaide',
            category: 'Events',
    ...
    console.log('  ✅ BASS Adelaide Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ BASS Adelaide Events error:', error.message);
    return [];
  }
}

module.exports = scrapeBASSAdelaideEvents;
