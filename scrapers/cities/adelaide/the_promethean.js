/**
 * The Promethean Adelaide Events Scraper
 * URL: https://www.thepromethean.com.au/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePromethean(city = 'Adelaide') {
  console.log('🔥 Scraping The Promethean...');
  ...
  try {
    ...
    await page.goto('https://www.thepromethean.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Promethean',
          address: '127 Waymouth St, Adelaide SA 5000',
          city: 'Adelaide'
        },
        latitude: -34.9270,
        longitude: 138.5960,
        city: 'Adelaide',
        category: 'Nightlife',
        source: 'The Promethean'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Promethean events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Promethean error:', error.message);
    return [];
  }
}

module.exports = scrapePromethean;
