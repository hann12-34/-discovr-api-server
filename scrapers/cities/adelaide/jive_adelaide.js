/**
 * Jive Adelaide Events Scraper
 * URL: https://www.jiveadelaide.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeJiveAdelaide(city = 'Adelaide') {
  console.log('🎵 Scraping Jive Adelaide...');
  ...
  try {
    ...
    await page.goto('https://www.jiveadelaide.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Jive',
          address: '181 Hindley St, Adelaide SA 5000',
          city: 'Adelaide'
        },
        latitude: -34.9260,
        longitude: 138.5930,
        city: 'Adelaide',
        category: 'Nightlife',
        source: 'Jive Adelaide'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Jive Adelaide events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Jive Adelaide error:', error.message);
    return [];
  }
}

module.exports = scrapeJiveAdelaide;
