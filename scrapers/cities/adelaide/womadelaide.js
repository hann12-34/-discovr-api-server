...
async function scrapeWOMADelaide(city = 'Adelaide') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'WOMADelaide', address: 'Botanic Park, Adelaide SA 5000', city: 'Adelaide' }, latitude: -34.9168, longitude: 138.6094, city: 'Adelaide', category: 'Festival', source: 'WOMADelaide' });
    }
