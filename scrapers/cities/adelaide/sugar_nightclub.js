/**
 * Sugar Nightclub Adelaide Events Scraper
 * URL: https://sugarnightclub.com.au/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSugarNightclub(city = 'Adelaide') {
  console.log('🎉 Scraping Sugar Nightclub...');
  ...
  try {
    ...
    await page.goto('https://sugarnightclub.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '12-16 Synagogue Place, Adelaide SA 5000',
          city: 'Adelaide'
        },
        latitude: -34.9280,
        longitude: 138.6005,
        city: 'Adelaide',
        category: 'Nightlife',
        source: 'Sugar Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sugar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sugar error:', error.message);
    return [];
  }
}

module.exports = scrapeSugarNightclub;
