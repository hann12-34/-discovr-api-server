/**
 * Adelaide Entertainment Centre Events Scraper
 * URL: https://www.aec.net.au/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAdelaideEntertainment(city = 'Adelaide') {
  console.log('🏟️ Scraping Adelaide Entertainment Centre...');
  ...
  try {
    ...
    await page.goto('https://www.aec.net.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '98 Port Road, Hindmarsh SA 5007',
          city: 'Adelaide'
        },
        latitude: -34.9050,
        longitude: 138.5643,
        city: 'Adelaide',
        category: 'Festival',
        source: 'Adelaide Entertainment Centre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} AEC events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  AEC error:', error.message);
    return [];
  }
}

module.exports = scrapeAdelaideEntertainment;
