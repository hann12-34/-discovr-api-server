...
async function scrapeAtlantaCityEvents() {
    console.log('🍑 Starting Atlanta scrapers...');
    const allEvents = [];
    ...
    for (const file of scraperFiles) {
        try {
            ...
            const events = await (typeof scraper.scrape === 'function' ? scraper.scrape() : scraper('Atlanta'));
            if (Array.isArray(events) && events.length > 0) allEvents.push(...events);
        } catch (error) { console.error(`❌ Atlanta/${file}: ${error.message}`); }
    }
    const seen = new Set();
    ...
    console.log(`✅ Atlanta: ${deduped.length} events`);
    return deduped;
}

module.exports = scrapeAtlantaCityEvents;
