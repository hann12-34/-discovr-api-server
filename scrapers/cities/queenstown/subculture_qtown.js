/**
 * Subculture Queenstown Events Scraper
 * Popular nightclub in Queenstown
 * URL: https://subculture.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSubcultureQtown(city = 'Queenstown') {
  console.log('🎵 Scraping Subculture Queenstown...');
  ...
  try {
    ...
    await page.goto('https://subculture.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Queenstown',
        category: 'Nightlife',
        source: 'Subculture Queenstown'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Subculture events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Subculture error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSubcultureQtown;
