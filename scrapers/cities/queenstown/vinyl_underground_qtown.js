/**
 * Vinyl Underground Queenstown Events Scraper
 * Popular bar in Queenstown
 * URL: https://vinylunderground.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVinylUndergroundQtown(city = 'Queenstown') {
  console.log('💿 Scraping Vinyl Underground Queenstown...');
  ...
  try {
    ...
    await page.goto('https://vinylunderground.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '16 Church St, Queenstown 9300',
          city: 'Queenstown'
        },
        city: 'Queenstown',
        category: 'Nightlife',
        source: 'Vinyl Underground'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Vinyl Underground events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Vinyl Underground error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeVinylUndergroundQtown;
