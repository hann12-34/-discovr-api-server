...
async function scrapeGlenorchyRaces(city = 'Queenstown') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Glenorchy Racecourse', address: 'Glenorchy, Otago', city: 'Queenstown' }, latitude: -44.8500, longitude: 168.3769, city: 'Queenstown', category: 'Festival', source: 'Glenorchy Races' });
    }
