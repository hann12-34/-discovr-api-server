...
async function scrapeWarbirds(city = 'Queenstown') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Wanaka Airport', address: 'Wanaka, Otago', city: 'Queenstown' }, latitude: -44.7225, longitude: 169.2461, city: 'Queenstown', category: 'Festival', source: 'Warbirds' });
    }
