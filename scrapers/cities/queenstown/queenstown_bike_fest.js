/**
 * Queenstown Bike Festival Events Scraper
 * URL: https://www.queenstownbikefestival.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBikeFest(city = 'Queenstown') {
  console.log('🚴 Scraping Bike Fest...');
  ...
  try {
    ...
    await page.goto('https://www.queenstownbikefestival.co.nz/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.queenstownbikefestival.co.nz', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Bike Fest events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bike Fest error:', error.message);
    return [];
  }
}
