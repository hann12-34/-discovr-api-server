/**
 * Little Blackwood Queenstown Events Scraper
 * URL: https://www.littleblackwood.co.nz/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLittleBlackwood(city = 'Queenstown') {
  console.log('🎵 Scraping Little Blackwood...');
  ...
  try {
    ...
    await page.goto('https://www.littleblackwood.co.nz/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Little Blackwood',
          address: 'O\'Connells Mall, Queenstown 9300',
          city: 'Queenstown'
        },
        latitude: -45.0310,
        longitude: 168.6595,
        city: 'Queenstown',
        category: 'Nightlife',
        source: 'Little Blackwood'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Little Blackwood events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Little Blackwood error:', error.message);
    return [];
  }
}

module.exports = scrapeLittleBlackwood;
