/**
 * Atlas Beer Cafe Queenstown Events Scraper
 * URL: https://www.atlasbeercafe.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAtlasBeer(city = 'Queenstown') {
  console.log('🍺 Scraping Atlas Beer Cafe...');
  ...
  try {
    ...
    await page.goto('https://www.atlasbeercafe.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Atlas Beer Cafe',
          address: 'Steamer Wharf, Beach St, Queenstown 9300',
          city: 'Queenstown'
        },
        latitude: -45.0315,
        longitude: 168.6595,
        city: 'Queenstown',
        category: 'Nightlife',
        source: 'Atlas Beer Cafe'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Atlas Beer Cafe events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Atlas Beer Cafe error:', error.message);
    return [];
  }
}

module.exports = scrapeAtlasBeer;
