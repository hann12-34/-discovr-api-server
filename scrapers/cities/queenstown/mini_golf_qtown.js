/**
 * Mini Golf Bar Queenstown Events Scraper
 * URL: https://www.minigolfbar.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMiniGolfBar(city = 'Queenstown') {
  console.log('⛳ Scraping Mini Golf Bar...');
  ...
  try {
    ...
    await page.goto('https://www.minigolfbar.co.nz/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Queenstown',
        category: 'Nightlife',
        source: 'Mini Golf Bar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Mini Golf Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Mini Golf Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeMiniGolfBar;
