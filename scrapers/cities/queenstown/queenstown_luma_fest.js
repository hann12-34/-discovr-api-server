/**
 * Luma Light Festival Queenstown Events Scraper
 * URL: https://www.luma.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLuma(city = 'Queenstown') {
  console.log('💡 Scraping Luma Fest...');
  ...
  try {
    ...
    await page.goto('https://www.luma.nz/programme', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.luma.nz', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Luma events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Luma error:', error.message);
    return [];
  }
}
