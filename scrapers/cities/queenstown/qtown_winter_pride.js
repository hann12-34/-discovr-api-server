/**
 * Queenstown Winter Pride Events Scraper
 * URL: https://www.winterpride.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWinterPride(city = 'Queenstown') {
  console.log('🏳️‍🌈 Scraping Winter Pride...');
  ...
  try {
    ...
    await page.goto('https://www.winterpride.co.nz/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.winterpride.co.nz', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Winter Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Winter Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeWinterPride;
