...
async function scrapeStLouisCityEvents() {
    console.log('⚾ Starting St Louis scrapers...');
    const allEvents = [];
    ...
    for (const file of scraperFiles) {
        try {
            ...
            const events = await (typeof scraper.scrape === 'function' ? scraper.scrape() : scraper('St Louis'));
            if (Array.isArray(events) && events.length > 0) allEvents.push(...events);
        } catch (error) { console.error(`❌ St Louis/${file}: ${error.message}`); }
    }
    const seen = new Set();
    ...
    console.log(`✅ St Louis: ${deduped.length} events`);
    return deduped;
}

module.exports = scrapeStLouisCityEvents;
