/**
 * The Pourhouse Minneapolis Events Scraper
 * URL: https://www.thepourhousempls.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePourhouse(city = 'Minneapolis') {
  console.log('🍺 Scraping The Pourhouse...');
  ...
  try {
    ...
    await page.goto('https://www.thepourhousempls.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Pourhouse',
          address: '1230 Hennepin Ave, Minneapolis, MN 55403',
          city: 'Minneapolis'
        },
        latitude: 44.9680,
        longitude: -93.2790,
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'The Pourhouse'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Pourhouse events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Pourhouse error:', error.message);
    return [];
  }
}

module.exports = scrapePourhouse;
