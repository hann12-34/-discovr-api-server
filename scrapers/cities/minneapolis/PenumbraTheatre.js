/**
 * Penumbra Theatre Minneapolis Events Scraper
 * URL: https://penumbratheatre.org/season
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePenumbra(city = 'Minneapolis') {
  console.log('🎭 Scraping Penumbra Theatre...');
  ...
  try {
    ...
    await page.goto('https://penumbratheatre.org/season', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Penumbra Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Penumbra Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Penumbra Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapePenumbra;
