/**
 * Parkway Theater Minneapolis Events Scraper
 * URL: https://theparkwaytheater.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeParkwayTheater(city = 'Minneapolis') {
  console.log('🎭 Scraping Parkway Theater...');
  ...
  try {
    ...
    await page.goto('https://theparkwaytheater.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Parkway Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Parkway Theater events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Parkway Theater error:', error.message);
    return [];
  }
}

module.exports = scrapeParkwayTheater;
