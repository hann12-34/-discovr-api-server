/**
 * Hexagon Bar Minneapolis Events Scraper
 * URL: https://www.hexagonbar.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHexagonBar(city = 'Minneapolis') {
  console.log('⬢ Scraping Hexagon Bar...');
  ...
  try {
    ...
    await page.goto('https://www.hexagonbar.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-listing', '.event', 'article', '[class*="show"]', '[class*="event"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Hexagon Bar',
          address: '2600 27th Ave S, Minneapolis, MN 55406',
          city: 'Minneapolis'
        },
        latitude: 44.9540,
        longitude: -93.2325,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Hexagon Bar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Hexagon Bar error:', error.message);
    return [];
  }
}

module.exports = scrapeHexagonBar;
