/**
 * Can Can Wonderland Minneapolis Events Scraper
 * URL: https://www.cancanwonderland.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCanCan(city = 'Minneapolis') {
  console.log('🎡 Scraping Can Can Wonderland...');
  ...
  try {
    ...
    await page.goto('https://www.cancanwonderland.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Can Can Wonderland'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Can Can Wonderland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Can Can Wonderland error:', error.message);
    return [];
  }
}

module.exports = scrapeCanCan;
