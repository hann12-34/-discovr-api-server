/**
 * White Squirrel Bar Minneapolis Events Scraper
 * URL: https://whitesquirrelbar.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWhiteSquirrel(city = 'Minneapolis') {
  console.log('🐿️ Scraping White Squirrel...');
  ...
  try {
    ...
    await page.goto('https://whitesquirrelbar.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '974 W 7th St, St Paul, MN 55102',
          city: 'Minneapolis'
        },
        latitude: 44.9419,
        longitude: -93.1224,
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'White Squirrel'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} White Squirrel events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  White Squirrel error:', error.message);
    return [];
  }
}

module.exports = scrapeWhiteSquirrel;
