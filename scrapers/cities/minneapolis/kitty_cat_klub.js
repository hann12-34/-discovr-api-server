/**
 * Kitty Cat Klub Minneapolis Events Scraper
 * URL: https://www.kittycatklub.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKittyCatKlub(city = 'Minneapolis') {
  console.log('🐱 Scraping Kitty Cat Klub...');
  ...
  try {
    ...
    await page.goto('https://www.kittycatklub.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Kitty Cat Klub',
          address: '315 14th Ave SE, Minneapolis, MN 55414',
          city: 'Minneapolis'
        },
        latitude: 44.9815,
        longitude: -93.2325,
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Kitty Cat Klub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kitty Cat Klub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kitty Cat Klub error:', error.message);
    return [];
  }
}

module.exports = scrapeKittyCatKlub;
