/**
 * Mixed Blood Theatre Minneapolis Events Scraper
 * URL: https://mixedblood.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMixedBlood(city = 'Minneapolis') {
  console.log('🎭 Scraping Mixed Blood Theatre...');
  ...
  try {
    ...
    await page.goto('https://mixedblood.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Mixed Blood Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Mixed Blood Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Mixed Blood Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeMixedBlood;
