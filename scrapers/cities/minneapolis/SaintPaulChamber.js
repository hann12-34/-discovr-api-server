/**
 * Ordway Center Minneapolis Events Scraper
...
async function scrapeOrdway(city = 'Minneapolis') {
  console.log('🎭 Scraping Ordway Center...');
  ...
  try {
    ...
    await page.goto('https://ordway.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Ordway Center'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ordway Center events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Ordway Center error:', error.message);
    return [];
  }
}

module.exports = scrapeOrdway;
