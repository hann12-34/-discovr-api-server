/**
 * Target Center Minneapolis - minneapolis Events Scraper
...
async function scrapeTargetCenterMinneapolis(city = 'minneapolis') {
  console.log('🎭 Scraping Target Center Minneapolis...');
  try {
    const resp = await axios.get('https://www.targetcenter.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Target Center Minneapolis', address: addr || '600 1st Ave N, Minneapolis, MN 55403', city: 'minneapolis' },
            latitude: 44.9795,
            longitude: -93.276,
            city: 'minneapolis',
            category: 'Events',
            source: 'Target Center Minneapolis'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.targetcenter.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.targetcenter.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Target Center Minneapolis', address: '600 1st Ave N, Minneapolis, MN 55403', city: 'minneapolis' },
            latitude: 44.9795,
            longitude: -93.276,
            city: 'minneapolis',
            category: 'Events',
    ...
    console.log('  ✅ Target Center Minneapolis:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Target Center Minneapolis error:', error.message);
    return [];
  }
}

module.exports = scrapeTargetCenterMinneapolis;
