/**
 * Nectar Lounge Minneapolis Events Scraper  
 * URL: https://www.fillmorempls.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFillmoreMpls(city = 'Minneapolis') {
  console.log('🎸 Scraping Fillmore Mpls...');
  ...
  try {
    ...
    await page.goto('https://www.fillmorempls.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Fillmore Mpls'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Fillmore Mpls events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Fillmore Mpls error:', error.message);
    return [];
  }
}

module.exports = scrapeFillmoreMpls;
