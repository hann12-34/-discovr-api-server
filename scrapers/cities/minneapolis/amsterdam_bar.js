/**
 * Amsterdam Bar & Hall Minneapolis Events Scraper
 * URL: https://www.amsterdambarandhall.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAmsterdamBar(city = 'Minneapolis') {
  console.log('🍺 Scraping Amsterdam Bar & Hall...');
  ...
  try {
    ...
    await page.goto('https://www.amsterdambarandhall.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '6th St N & Hennepin Ave, Minneapolis, MN 55403',
          city: 'Minneapolis'
        },
        latitude: 44.9820,
        longitude: -93.2750,
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Amsterdam Bar & Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Amsterdam Bar & Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Amsterdam Bar & Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeAmsterdamBar;
