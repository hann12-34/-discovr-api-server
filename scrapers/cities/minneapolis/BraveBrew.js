/**
 * Brave New Workshop Minneapolis Events Scraper
 * URL: https://www.bravenewworkshop.com/shows
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBraveNew(city = 'Minneapolis') {
  console.log('😂 Scraping Brave New Workshop...');
  ...
  try {
    ...
    await page.goto('https://www.bravenewworkshop.com/shows', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Brave New Workshop'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Brave New Workshop events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Brave New Workshop error:', error.message);
    return [];
  }
}

module.exports = scrapeBraveNew;
