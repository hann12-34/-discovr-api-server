/**
 * The Capri Theater Minneapolis Events Scraper
 * URL: https://thecapritheater.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCapriTheater(city = 'Minneapolis') {
  console.log('🎬 Scraping Capri Theater...');
  ...
  try {
    ...
    await page.goto('https://thecapritheater.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Capri Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Capri Theater events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Capri Theater error:', error.message);
    return [];
  }
}

module.exports = scrapeCapriTheater;
