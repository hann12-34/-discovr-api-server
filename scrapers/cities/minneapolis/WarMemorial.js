...
async function scrapeCedarCultural(city = 'Minneapolis') {
  console.log('🎵 Scraping Cedar Cultural...');
  ...
  try {
    ...
    await page.goto('https://thecedar.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Minneapolis',
        category: 'Nightlife',
        source: 'Cedar Cultural'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cedar Cultural events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cedar Cultural error:', error.message);
    return [];
  }
}
