/**
 * The Cabooze Minneapolis Events Scraper
 * URL: https://www.cabooze.com/events
 * Beloved Cedar Avenue music venue
...
const CaboozeMinneapolisEvents = {
  async scrape(city = 'Minneapolis') {
    console.log('🎵 Scraping The Cabooze Minneapolis...');

    try {
      const response = await axios.get('https://www.cabooze.com/events', {
        headers: {
      ...
      $('.eventWrapper').each((i, el) => {
        ...
        if (!url) return;
        if (!url.startsWith('http')) url = 'https://cabooze.com' + url;
        ...
        events.push({
          ...
          venue: {
            name: 'The Cabooze',
            address: '917 Cedar Ave S, Minneapolis, MN 55404',
            city: 'Minneapolis'
          },
          city: 'Minneapolis',
          category: 'Music',
          source: 'The Cabooze Minneapolis'
        });
      });

      console.log(`  ✅ Found ${events.length} Cabooze events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Cabooze error: ${error.message}`);
      return [];
    }
  }
};

module.exports = CaboozeMinneapolisEvents.scrape;
