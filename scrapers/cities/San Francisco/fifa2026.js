...
module.exports = async function scrape(city) {
  console.log('⚽ Scraping FIFA World Cup 2026 matches for San Francisco...');
  const matches = await getFifa2026Matches('San Francisco');
  const filtered = filterEvents(matches);
  console.log('  ✅ ' + filtered.length + ' FIFA 2026 San Francisco matches');
  return filtered;
};
