/**
 * Noise Pop SF - San Francisco Events Scraper
 * URL: https://www.noisepop.com/
 */
...
async function scrapeNoisePopSF(city = 'San Francisco') {
  console.log('🎭 Scraping Noise Pop SF...');
  try {
    const resp = await axios.get('https://www.noisepop.com/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Noise Pop SF', address: addr || 'San Francisco, CA', city: 'San Francisco' },
            latitude: 37.7749,
            longitude: -122.4194,
            city: 'San Francisco',
            category: 'Music',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.noisepop.com/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.noisepop.com/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Noise Pop SF', address: 'San Francisco, CA', city: 'San Francisco' },
            latitude: 37.7749,
            longitude: -122.4194,
            city: 'San Francisco',
            category: 'Music',
    ...
    console.log('  ✅ Noise Pop SF:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Noise Pop SF error:', error.message);
    return [];
  }
}

module.exports = scrapeNoisePopSF;
