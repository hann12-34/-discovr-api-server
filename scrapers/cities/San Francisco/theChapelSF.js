 ...
 * Intimate venue in the Mission District
...
const TheChapelSFEvents = {
  async scrape(city = 'San Francisco') {
    console.log('🌉 Scraping The Chapel San Francisco...');

    try {
      const response = await axios.get('https://www.thechapelsf.com/calendar', {
        headers: {
      ...
      $('.seetickets-list-event-container').each((i, el) => {
        ...
        events.push({
          ...
          city: 'San Francisco',
          category: 'Music',
          source: 'The Chapel SF'
        });
      });

      console.log(`  ✅ Found ${events.length} Chapel SF events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Chapel SF error: ${error.message}`);
      return [];
    }
  }
};

module.exports = TheChapelSFEvents.scrape;
