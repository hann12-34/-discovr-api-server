/**
 * DNA Lounge San Francisco Events Scraper
 * Legendary nightclub
...
async function scrapeDNALounge(city = 'San Francisco') {
  console.log('🎧 Scraping DNA Lounge San Francisco...');
  ...
  try {
    ...
    await page.goto('https://www.dnalounge.com/calendar/', {
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.event-card', 'article', '[class*="event"]', '.show', 'tr'];

      for (const selector of selectors) {
        document.querySelectorAll(selector).forEach(el => {
          try {
            const titleEl = el.querySelector('h1, h2, h3, h4, .title, [class*="title"], a');
            ...
            if (!dateStr) {
              const dateEl = el.querySelector('.date, [class*="date"], time, td');
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} DNA Lounge events`);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'DNA Lounge', address: '375 11th Street, San Francisco CA 94103', city: 'San Francisco' },
        latitude: 37.7702,
        longitude: -122.4130,
        city: 'San Francisco',
        category: 'Nightlife',
    ...
    console.error('  ⚠️  DNA Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeDNALounge;
