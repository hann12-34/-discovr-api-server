/**
 * Cal Academy of Sciences - San Francisco Events Scraper
...
async function scrapeCalAcademyofSciences(city = 'San Francisco') {
  console.log('🎭 Scraping Cal Academy of Sciences...');
  try {
    const resp = await axios.get('https://www.calacademy.org/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Cal Academy of Sciences', address: addr || '55 Music Concourse Dr, San Francisco, CA 94118', city: 'San Francisco' },
            latitude: 37.7699,
            longitude: -122.4661,
            city: 'San Francisco',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.calacademy.org/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.calacademy.org/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Cal Academy of Sciences', address: '55 Music Concourse Dr, San Francisco, CA 94118', city: 'San Francisco' },
            latitude: 37.7699,
            longitude: -122.4661,
            city: 'San Francisco',
            category: 'Events',
    ...
    console.log('  ✅ Cal Academy of Sciences:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Cal Academy of Sciences error:', error.message);
    return [];
  }
}

module.exports = scrapeCalAcademyofSciences;
