...
async function scrapeFillmore(city = 'San Francisco') {
  console.log('🎸 Scraping The Fillmore SF...');
  ...
  try {
    ...
    await page.goto('https://www.livenation.com/venue/KovZpZAEkAeA/the-fillmore-events', {
    ...
    const events = await page.evaluate(() => {
      ...
      eventItems.forEach(item => {
        try {
          const titleEl = item.querySelector('h2, h3, h4, .title, .event-title, .event-name');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at The Fillmore`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Fillmore events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Fillmore error:', error.message);
    return [];
  }
}

module.exports = scrapeFillmore;
