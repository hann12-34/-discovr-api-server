/**
 * Stubb's Austin Events Scraper v2
 * URL: https://stubbsaustin.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeStubbs2(city = 'Austin') {
  console.log('🎸 Scraping Stubb\'s Austin...');
  ...
  try {
    ...
    await page.goto('https://stubbsaustin.com/calendar', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="event"], article, .card, a[href*="event"], .show').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Stubb's events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Stubb\'s error:', error.message);
    return [];
  }
}

module.exports = scrapeStubbs2;
