/**
 * Bass Concert Hall Austin Events Scraper
 * URL: https://texasperformingarts.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBassHall(city = 'Austin') {
  console.log('🎭 Scraping Bass Concert Hall...');
  ...
  try {
    ...
    await page.goto('https://texasperformingarts.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Festival',
        source: 'Bass Concert Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bass Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bass Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeBassHall;
