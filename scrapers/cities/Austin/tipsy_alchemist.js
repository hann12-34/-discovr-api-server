/**
 * Tipsy Alchemist Austin Events Scraper
 * Bar and nightclub on Rainey Street
 * URL: https://tipsyalchemist.com/austin/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTipsyAlchemist(city = 'Austin') {
  console.log('🧪 Scraping Tipsy Alchemist Austin...');
  ...
  try {
    ...
    await page.goto('https://tipsyalchemist.com/austin/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '70 Rainey St #200, Austin, TX 78701',
          city: 'Austin'
        },
        city: 'Austin',
        category: 'Nightlife',
        source: 'Tipsy Alchemist'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Tipsy Alchemist events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Tipsy Alchemist error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTipsyAlchemist;
