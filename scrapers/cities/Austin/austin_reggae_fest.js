/**
 * Austin Reggae Festival Events Scraper
 * URL: https://www.austinreggaefest.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAustinReggaeFest(city = 'Austin') {
  console.log('🎶 Scraping Austin Reggae Fest...');
  ...
  try {
    ...
    await page.goto('https://www.austinreggaefest.com', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event-card, article, .event-item, .show, a[href*="event"]').forEach(el => {
        ...
        const href = linkEl?.href || 'https://www.austinreggaefest.com';
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Festival',
        source: 'Austin Reggae Fest'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Reggae Fest events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Reggae Fest error:', error.message);
    return [];
  }
}

module.exports = scrapeAustinReggaeFest;
