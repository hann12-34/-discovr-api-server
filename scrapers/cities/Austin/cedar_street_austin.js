/**
 * Cedar Street Courtyard Austin Events Scraper
 * Popular venue in Austin
 * URL: https://cedarstreetaustin.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCedarStreetAustin(city = 'Austin') {
  console.log('🌳 Scraping Cedar Street Austin...');
  ...
  try {
    ...
    await page.goto('https://cedarstreetaustin.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Nightlife',
        source: 'Cedar Street Austin'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cedar Street events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Cedar Street error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCedarStreetAustin;
