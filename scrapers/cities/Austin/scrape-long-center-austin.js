/**
 * Long Center Austin - Austin Events Scraper
...
async function scrapeLongCenterAustin(city = 'Austin') {
  console.log('🎭 Scraping Long Center Austin...');
  try {
    const resp = await axios.get('https://thelongcenter.org/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Long Center Austin', address: addr || '701 W Riverside Dr, Austin, TX 78704', city: 'Austin' },
            latitude: 30.2586,
            longitude: -97.7509,
            city: 'Austin',
            category: 'Events',
            source: 'Long Center Austin'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://thelongcenter.org/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://thelongcenter.org/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Long Center Austin', address: '701 W Riverside Dr, Austin, TX 78704', city: 'Austin' },
            latitude: 30.2586,
            longitude: -97.7509,
            city: 'Austin',
            category: 'Events',
    ...
    console.log('  ✅ Long Center Austin:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Long Center Austin error:', error.message);
    return [];
  }
}

module.exports = scrapeLongCenterAustin;
