/**
 * Visit Austin Events - Austin Events Scraper
 * URL: https://www.austintexas.org/events/
 */
...
async function scrapeVisitAustinEvents(city = 'Austin') {
  console.log('🎭 Scraping Visit Austin Events...');
  try {
    const resp = await axios.get('https://www.austintexas.org/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Visit Austin Events', address: addr || 'Austin, TX', city: 'Austin' },
            latitude: 30.2672,
            longitude: -97.7431,
            city: 'Austin',
            category: 'Events',
            source: 'Visit Austin Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.austintexas.org/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.austintexas.org/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Visit Austin Events', address: 'Austin, TX', city: 'Austin' },
            latitude: 30.2672,
            longitude: -97.7431,
            city: 'Austin',
            category: 'Events',
    ...
    console.log('  ✅ Visit Austin Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Visit Austin Events error:', error.message);
    return [];
  }
}

module.exports = scrapeVisitAustinEvents;
