/**
 * Continental Club Austin Events Scraper
 * URL: https://continentalclub.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeContinentalclub(city = 'Austin') {
  console.log('🎵 Scraping Continental Club...');
  ...
  try {
    ...
    await page.goto('https://continentalclub.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '1315 S Congress Avenue, Austin, TX 78704',
          city: 'Austin'
        },
        latitude: 30.2495,
        longitude: -97.7494,
        city: 'Austin',
        category: 'Nightlife',
        source: 'Continental Club'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Continental Club events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Continental Club error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeContinentalclub;
