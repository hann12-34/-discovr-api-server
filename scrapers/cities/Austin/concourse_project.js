/**
 * The Concourse Project Austin Events Scraper
 * Large nightclub and event venue
 * URL: https://theconcourseproject.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeConcourseProject(city = 'Austin') {
  console.log('🎪 Scraping The Concourse Project Austin...');
  ...
  try {
    ...
    await page.goto('https://theconcourseproject.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Nightlife',
        source: 'The Concourse Project'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Concourse Project events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Concourse Project error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeConcourseProject;
