/**
 * The Rose Room Austin Events Scraper
 * Nightclub at Rock Rose Ave
 * URL: https://www.roseroomaustin.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoseroomAustin(city = 'Austin') {
  console.log('🌹 Scraping The Rose Room Austin...');
  ...
  try {
    ...
    await page.goto('https://www.roseroomaustin.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Nightlife',
        source: 'The Rose Room'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Rose Room events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Rose Room error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRoseroomAustin;
