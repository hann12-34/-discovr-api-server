 ...
 * URL: https://www.stubbsaustin.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeStubbsAustin(city = 'Austin') {
  console.log('🎸 Scraping Stubbs BBQ Austin...');
  ...
  try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Stubbs events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Stubbs error:', error.message);
    return [];
  }
}

module.exports = scrapeStubbsAustin;
