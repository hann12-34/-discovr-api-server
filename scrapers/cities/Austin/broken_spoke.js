/**
 * Broken Spoke Austin Events Scraper
 * Classic country dance hall
 * URL: https://www.brokenspokeaustintx.net/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBrokenSpoke(city = 'Austin') {
  console.log('🤠 Scraping Broken Spoke Austin...');
  ...
  try {
    ...
    await page.goto('https://www.brokenspokeaustintx.net/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event, .show, article, .calendar-item, .wix-events__event').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Broken Spoke events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Broken Spoke error:', error.message);
    return [];
  }
}

module.exports = scrapeBrokenSpoke;
