/**
 * Empire Control Room Austin Events Scraper
 * Live music venue
...
async function scrapeEmpire(city = 'Austin') {
  console.log('🎸 Scraping Empire Control Room Austin...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Empire Control Room`,
        date: isoDate,
        ...
        venue: {
          ...
          address: '606 E 7th Street, Austin TX 78701',
          city: 'Austin'
        },
        latitude: 30.2676,
        longitude: -97.7366,
        city: 'Austin',
        category: 'Nightlife',
        source: 'Empire Control Room'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Empire events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Empire error:', error.message);
    return [];
  }
}

module.exports = scrapeEmpire;
