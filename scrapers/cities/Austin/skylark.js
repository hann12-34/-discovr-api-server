/**
 * Skylark Lounge Austin Events Scraper
 * URL: https://skylarkaustin.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSkylark(city = 'Austin') {
  console.log('🎸 Scraping Skylark Lounge Austin...');
  ...
  try {
    ...
    await page.goto('https://skylarkaustin.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Nightlife',
        source: 'Skylark Lounge'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Skylark Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Skylark Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeSkylark;
