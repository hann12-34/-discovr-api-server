/**
 * Rainey Street Bars Austin Events Scraper
 * URL: https://www.raineystreetbars.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRaineyStreet(city = 'Austin') {
  console.log('🍺 Scraping Rainey Street Austin...');
  ...
  try {
    ...
    await page.goto('https://www.raineystreetbars.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Austin',
        category: 'Nightlife',
        source: 'Rainey Street'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Rainey Street events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Rainey Street error:', error.message);
    return [];
  }
}

module.exports = scrapeRaineyStreet;
