/**
 * OVO Hydro Glasgow - glasgow Events Scraper
 * URL: https://www.ovohydro.com/events
 */
...
async function scrapeOVOHydroGlasgow(city = 'glasgow') {
  console.log('🎭 Scraping OVO Hydro Glasgow...');
  try {
    const resp = await axios.get('https://www.ovohydro.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'OVO Hydro Glasgow', address: addr || 'Exhibition Way, Glasgow G3 8YW', city: 'glasgow' },
            latitude: 55.8601,
            longitude: -4.2865,
            city: 'glasgow',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.ovohydro.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.ovohydro.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'OVO Hydro Glasgow', address: 'Exhibition Way, Glasgow G3 8YW', city: 'glasgow' },
            latitude: 55.8601,
            longitude: -4.2865,
            city: 'glasgow',
            category: 'Events',
    ...
    console.log('  ✅ OVO Hydro Glasgow:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ OVO Hydro Glasgow error:', error.message);
    return [];
  }
}

module.exports = scrapeOVOHydroGlasgow;
