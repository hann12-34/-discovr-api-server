/**
 * Glasgow Pride Events Scraper
 * URL: https://www.glasgowpride.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeGlasgowPride(city = 'Glasgow') {
  console.log('🏳️‍🌈 Scraping Glasgow Pride...');
  ...
  try {
    ...
    await page.goto('https://www.glasgowpride.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Glasgow',
        category: 'Festival',
        source: 'Glasgow Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Glasgow Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Glasgow Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeGlasgowPride;
