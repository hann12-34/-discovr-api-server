/**
 * Sub Club Glasgow Events Scraper
...
async function scrapeSubClub(city = 'Glasgow') {
  console.log('🎧 Scraping Sub Club Glasgow...');
  ...
  try {
    ...
    await page.goto('https://subclub.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Club night at Sub Club`,
        date: isoDate,
        ...
        venue: {
          name: 'Sub Club',
          address: '22 Jamaica Street, Glasgow G1 4QD',
          city: 'Glasgow'
        },
        latitude: 55.8570,
        longitude: -4.2585,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Sub Club events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sub Club error:', error.message);
    return [];
  }
}

module.exports = scrapeSubClub;
