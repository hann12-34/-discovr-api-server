/**
 * West End Festival Glasgow Events Scraper
 * URL: https://www.westendfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWestEndFest(city = 'Glasgow') {
  console.log('🎪 Scraping West End Festival...');
  ...
  try {
    ...
    await page.goto('https://www.westendfestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Glasgow',
        category: 'Festival',
        source: 'West End Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} West End Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  West End Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeWestEndFest;
