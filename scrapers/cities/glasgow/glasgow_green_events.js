...
async function scrapeGlasgowGreen(city = 'Glasgow') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Glasgow Green', address: 'Greendyke Street, Glasgow', city: 'Glasgow' }, latitude: 55.8486, longitude: -4.2338, city: 'Glasgow', category: 'Festival', source: 'Glasgow Green' });
    }
