/**
 * Barrowland Ballroom Glasgow Events Scraper
 * Legendary live music venue
 * URL: https://www.barrowland-ballroom.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBarrowland(city = 'Glasgow') {
  console.log('🎸 Scraping Barrowland Ballroom Glasgow...');
  ...
  try {
    ...
    await page.goto('https://www.barrowland-ballroom.co.uk/', {
    ...
    const events = await page.evaluate(() => {
      const results = [];
      const eventItems = document.querySelectorAll('.event-card, .event, [class*="event"], article, .gig, .show');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Barrowland Ballroom Glasgow`,
        date: isoDate,
        ...
        city: 'Glasgow',
        category: 'Nightlife',
        source: 'Barrowland Ballroom'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Barrowland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Barrowland error:', error.message);
    return [];
  }
}

module.exports = scrapeBarrowland;
