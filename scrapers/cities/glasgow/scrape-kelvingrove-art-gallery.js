/**
 * Kelvingrove Art Gallery - glasgow Events Scraper
...
async function scrapeKelvingroveArtGallery(city = 'glasgow') {
  console.log('🎭 Scraping Kelvingrove Art Gallery...');
  try {
    const resp = await axios.get('https://www.glasgowlife.org.uk/museums/venues/kelvingrove-art-gallery-and-museum', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Kelvingrove Art Gallery', address: addr || 'Argyle St, Glasgow G3 8AG', city: 'glasgow' },
            latitude: 55.8686,
            longitude: -4.2897,
            city: 'glasgow',
            category: 'Events',
            source: 'Kelvingrove Art Gallery'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.glasgowlife.org.uk/museums/venues/kelvingrove-art-gallery-and-museum');
          ...
          let img = imgEl.attr('src') || imgEl.attr('data-src') || null;
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.glasgowlife.org.uk/museums/venues/kelvingrove-art-gallery-and-museum').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Kelvingrove Art Gallery', address: 'Argyle St, Glasgow G3 8AG', city: 'glasgow' },
            latitude: 55.8686,
            longitude: -4.2897,
            city: 'glasgow',
            category: 'Events',
    ...
    console.log('  ✅ Kelvingrove Art Gallery:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Kelvingrove Art Gallery error:', error.message);
    return [];
  }
}

module.exports = scrapeKelvingroveArtGallery;
