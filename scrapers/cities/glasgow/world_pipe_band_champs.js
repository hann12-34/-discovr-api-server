/**
 * World Pipe Band Championships Glasgow Events Scraper
 * URL: https://www.theworlds.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWorldPipeBand(city = 'Glasgow') {
  console.log('🎵 Scraping World Pipe Band Championships...');
  ...
  try {
    ...
    await page.goto('https://www.theworlds.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Glasgow Green',
          address: 'Greendyke Street, Glasgow',
          city: 'Glasgow'
        },
        latitude: 55.8486,
        longitude: -4.2338,
        city: 'Glasgow',
        category: 'Festival',
        source: 'World Pipe Band'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} World Pipe Band events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  World Pipe Band error:', error.message);
    return [];
  }
}

module.exports = scrapeWorldPipeBand;
