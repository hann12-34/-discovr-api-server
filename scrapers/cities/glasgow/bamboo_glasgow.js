/**
 * Bamboo Glasgow Events Scraper
 * Popular nightclub in Glasgow
 * URL: https://bamboonightclub.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBambooGlasgow(city = 'Glasgow') {
  console.log('🎋 Scraping Bamboo Glasgow...');
  ...
  try {
    ...
    await page.goto('https://bamboonightclub.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Bamboo',
          address: '51A West Regent St, Glasgow G2 2AE',
          city: 'Glasgow'
        },
        city: 'Glasgow',
        category: 'Nightlife',
        source: 'Bamboo Glasgow'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bamboo events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Bamboo error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBambooGlasgow;
