  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      allContainers.forEach(container => {
        ...
        // CRITICAL: Get event URL - find closest link
        const linkEl = container.querySelector('a[href*="dice.fm"], a[href*="event"], a[href*="clubspace"]');
        const eventUrl = linkEl?.href || null;
        
        // Skip if no URL - we MUST have a URL per rules
        if (!eventUrl) return;
        ...
        if (!seen.has(key)) {
          seen.add(key);
          results.push({ title, date: isoDate, venue, imageUrl, eventUrl });
        }
      });
    ...
    // Filter out events without URLs (per rules: URL is required)
    const validEvents = events.filter(e => e.eventUrl);
    console.log(`  📋 ${validEvents.length} events with valid URLs`);
