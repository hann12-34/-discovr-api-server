/**
 * Miami and Beaches Events - Miami Events Scraper
...
async function scrapeMiamiandBeachesEvents(city = 'Miami') {
  console.log('🎭 Scraping Miami and Beaches Events...');
  try {
    const resp = await axios.get('https://www.miamiandbeaches.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Miami and Beaches Events', address: addr || 'Miami, FL', city: 'Miami' },
            latitude: 25.7617,
            longitude: -80.1918,
            city: 'Miami',
            category: 'Events',
            source: 'Miami and Beaches Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.miamiandbeaches.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.miamiandbeaches.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Miami and Beaches Events', address: 'Miami, FL', city: 'Miami' },
            latitude: 25.7617,
            longitude: -80.1918,
            city: 'Miami',
            category: 'Events',
    ...
    console.log('  ✅ Miami and Beaches Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Miami and Beaches Events error:', error.message);
    return [];
  }
}

module.exports = scrapeMiamiandBeachesEvents;
