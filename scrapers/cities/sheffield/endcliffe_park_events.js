...
async function scrapeEndcliffePark(city = 'Sheffield') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Endcliffe Park', address: 'Endcliffe Vale Road, Sheffield', city: 'Sheffield' }, latitude: 53.3699, longitude: -1.5112, city: 'Sheffield', category: 'Festival', source: 'Endcliffe Park' });
    }
