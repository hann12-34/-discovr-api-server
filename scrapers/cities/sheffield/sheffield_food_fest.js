...
async function scrapeSheffieldFood(city = 'Sheffield') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Peace Gardens', address: 'Peace Gardens, Sheffield', city: 'Sheffield' }, latitude: 53.3797, longitude: -1.4714, city: 'Sheffield', category: 'Festival', source: 'Sheffield Food' });
    }
