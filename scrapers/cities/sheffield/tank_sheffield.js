/**
 * Tank Sheffield Events Scraper
 * Popular nightclub in Sheffield
 * URL: https://tanksheffield.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTankSheffield(city = 'Sheffield') {
  console.log('🎧 Scraping Tank Sheffield...');
  ...
  try {
    ...
    await page.goto('https://tanksheffield.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Tank',
          address: '5 Eyre Street, Sheffield S1 4QW',
          city: 'Sheffield'
        },
        city: 'Sheffield',
        category: 'Nightlife',
        source: 'Tank Sheffield'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Tank events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Tank error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeTankSheffield;
