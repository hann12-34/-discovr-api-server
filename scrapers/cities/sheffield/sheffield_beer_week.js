/**
 * Sheffield Beer Week Events Scraper
 * URL: https://www.sheffieldbeerweek.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBeerWeek(city = 'Sheffield') {
  console.log('🍺 Scraping Beer Week...');
  ...
  try {
    ...
    await page.goto('https://www.sheffieldbeerweek.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.sheffieldbeerweek.co.uk', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Beer Week events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Beer Week error:', error.message);
    return [];
  }
}
