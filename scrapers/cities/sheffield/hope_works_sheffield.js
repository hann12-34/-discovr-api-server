/**
 * Hope Works Sheffield Events Scraper
 * Popular nightclub in Sheffield
 * URL: https://hopeworksbar.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHopeWorksSheffield(city = 'Sheffield') {
  console.log('🏭 Scraping Hope Works Sheffield...');
  ...
  try {
    ...
    await page.goto('https://hopeworksbar.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Hope Works',
          address: '115 Eyre St, Sheffield S1 4QW',
          city: 'Sheffield'
        },
        city: 'Sheffield',
        category: 'Nightlife',
        source: 'Hope Works Sheffield'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Hope Works events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Hope Works error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeHopeWorksSheffield;
