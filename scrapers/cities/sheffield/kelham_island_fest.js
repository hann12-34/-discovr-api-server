/**
 * Kelham Island Festival Sheffield Events Scraper
 * URL: https://www.kelhamisland.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeKelhamIsland(city = 'Sheffield') {
  console.log('🍺 Scraping Kelham Island Festival...');
  ...
  try {
    ...
    await page.goto('https://www.kelhamisland.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Sheffield',
        category: 'Festival',
        source: 'Kelham Island'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Kelham Island events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Kelham Island error:', error.message);
    return [];
  }
}

module.exports = scrapeKelhamIsland;
