/**
 * Yellow Arch Studios Sheffield Events Scraper
 * URL: https://www.yellowarchstudios.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeYellowArch(city = 'Sheffield') {
  console.log('🎨 Scraping Yellow Arch Studios...');
  ...
  try {
    ...
    await page.goto('https://www.yellowarchstudios.com/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '30-36 Burton Rd, Sheffield S3 8BX',
          city: 'Sheffield'
        },
        latitude: 53.3900,
        longitude: -1.4795,
        city: 'Sheffield',
        category: 'Arts',
        source: 'Yellow Arch Studios'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Yellow Arch Studios events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Yellow Arch Studios error:', error.message);
    return [];
  }
}

module.exports = scrapeYellowArch;
