/**
 * O2 Academy Sheffield Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academysheffield/events
 */
...
async function scrapeO2AcademySheffield(city = 'Sheffield') {
  console.log('🎸 Scraping O2 Academy Sheffield...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academysheffield/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Sheffield', address: 'Arundel Gate, Sheffield S1 2PN', city: 'Sheffield' },
        latitude: 53.3781,
        longitude: -1.4697,
        city: 'Sheffield',
        category: 'Nightlife',
        source: 'O2 Academy Sheffield'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Sheffield events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Sheffield error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademySheffield;
