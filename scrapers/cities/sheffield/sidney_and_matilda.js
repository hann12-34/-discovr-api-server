/**
 * Sidney & Matilda Sheffield Events Scraper
 * URL: https://www.sidneyandmatilda.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSidneyAndMatilda(city = 'Sheffield') {
  console.log('🎵 Scraping Sidney & Matilda...');
  ...
  try {
    ...
    await page.goto('https://www.sidneyandmatilda.com/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Sidney & Matilda',
          address: '44 Charles St, Sheffield S1 2HP',
          city: 'Sheffield'
        },
        latitude: 53.3800,
        longitude: -1.4675,
        city: 'Sheffield',
        category: 'Nightlife',
        source: 'Sidney & Matilda'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sidney & Matilda events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sidney & Matilda error:', error.message);
    return [];
  }
}

module.exports = scrapeSidneyAndMatilda;
