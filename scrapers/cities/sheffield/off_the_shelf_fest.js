/**
 * Off The Shelf Festival of Words Sheffield Events Scraper
 * URL: https://www.offtheshelf.org.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOffTheShelf(city = 'Sheffield') {
  console.log('📚 Scraping Off The Shelf...');
  ...
  try {
    ...
    await page.goto('https://www.offtheshelf.org.uk/programme', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.offtheshelf.org.uk', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Off The Shelf events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Off The Shelf error:', error.message);
    return [];
  }
}

module.exports = scrapeOffTheShelf;
