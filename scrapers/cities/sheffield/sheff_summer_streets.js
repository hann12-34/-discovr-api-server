/**
 * Sheffield Summer Streets Events Scraper
...
async function scrapeSummerStreets(city = 'Sheffield') {
  console.log('☀️ Scraping Summer Streets...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.welcometosheffield.co.uk', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Summer Streets events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Summer Streets error:', error.message);
    return [];
  }
}

module.exports = scrapeSummerStreets;
