/**
 * Code Nightclub Sheffield Events Scraper
 * URL: https://www.code-club.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCode(city = 'Sheffield') {
  ...
  try {
    ...
    await page.goto('https://www.code-club.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Code',
          address: 'Eyre St, Sheffield S1 4QZ',
          city: 'Sheffield'
        },
        latitude: 53.3785,
        longitude: -1.4710,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Code events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Code error:', error.message);
    return [];
  }
}
