/**
 * Peddler Night Market Sheffield Events Scraper
 * URL: https://www.peddlermarket.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePeddler(city = 'Sheffield') {
  console.log('🍴 Scraping Peddler Market...');
  ...
  try {
    ...
    await page.goto('https://www.peddlermarket.com', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.peddlermarket.com', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Peace Gardens, Sheffield',
          city: 'Sheffield'
        },
        latitude: 53.3797,
        longitude: -1.4714,
        city: 'Sheffield',
        category: 'Festival',
        source: 'Peddler Market'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Peddler Market events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Peddler Market error:', error.message);
    return [];
  }
}

module.exports = scrapePeddler;
