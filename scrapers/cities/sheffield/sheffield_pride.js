/**
 * Sheffield Pride Events Scraper
 * URL: https://www.sheffieldpride.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSheffieldPride(city = 'Sheffield') {
  console.log('🏳️‍🌈 Scraping Sheffield Pride...');
  ...
  try {
    ...
    await page.goto('https://www.sheffieldpride.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Sheffield',
        category: 'Festival',
        source: 'Sheffield Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sheffield Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sheffield Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeSheffieldPride;
