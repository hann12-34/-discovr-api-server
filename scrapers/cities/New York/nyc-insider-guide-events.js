/**
 * NYC Insider Guide Events Scraper
 *
 * Scrapes events from NYC Insider Guide
 * URL: https://www.nycinsiderguide.com/nyc-events
 */

const axios = require('axios');
const cheerio = require('cheerio');

class NYCInsiderGuideEvents {
    constructor() {
        this.venueName = 'NYC Insider Guide';
        this.venueLocation = 'New York City, NY';
        this.baseUrl = 'https://www.nycinsiderguide.com';
        this.eventsUrl = 'https://www.nycinsiderguide.com/nyc-events';
        this.category = 'Local Insider Events';
    }

    /**
     * Scrape events from NYC Insider Guide
    ...
    async scrape() {
        ...
        try {
            ...
            // Look for event listing containers
            const eventSelectors = [
                '.event-item', '.event-card', '.event', '.insider-event',
                '[class*="event"]', '.listing', '.article', 'article',
                '.card', '.content-card', '.post', '.entry', '.guide-item'
            ];

            eventSelectors.forEach(selector => {
                $(selector).each((index, element) => {
                    const $el = $(element);
                    let title = $el.find('h1, h2, h3, h4, .title, .event-title, .name, .headline, .entry-title').first().text().trim();
                    ...
                    if (title && this.isValidEvent(title)) {
                        ...
                        const dateSelectors = [
                            ...
                            '.calendar', '.day', '.meta', '.publish-date'
                        ];
                        ...
                        let description = '';
                        const descSelectors = ['.description', '.excerpt', '.summary', '.details', '.content', '.entry-content'];
                        ...
                        // Look for insider tips or recommendations
                        let insiderTip = '';
                        const tipSelectors = ['.tip', '.insider', '.recommendation', '.note', '[class*="tip"]'];
                        for (const tipSelector of tipSelectors) {
                            const tipElement = $el.find(tipSelector).first();
                            if (tipElement.length > 0) {
                                insiderTip = tipElement.text().trim();
                                if (insiderTip && insiderTip.length > 10 && insiderTip.length < 200) break;
                            }
                        }
    ...
    isValidEvent(title) {
        ...
        const invalidKeywords = [
            ...
            'sponsored', 'insider guide', 'see more', 'show more'
        ];
...
module.exports = async (city) => {
  const scraper = new NYCInsiderGuideEvents();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.NYCInsiderGuideEvents = NYCInsiderGuideEvents;