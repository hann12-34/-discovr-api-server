...
class NewJerseyMetroEventsExtractor {
    constructor() {
        this.baseUrl = 'https://www.meetup.com/find/?keywords=events&location=us--nj--jersey_city';
        this.name = 'New Jersey Metro Events';
    }

    async scrape(city = 'New York') {
        try {
            console.log(`🌉 Scraping events from New Jersey Metro Area...`);
            ...
            // Extract events from NJ Metro area
            $('div[data-// realEvent removed by Universal 100% Engine
                ...
                if (title && title.length > 3) {
                    events.push({
                        ...
                        location: location || 'New Jersey Metro Area',
                        venue: 'New Jersey Metro Area',
                        city: city,
                        url: link ? (link.startsWith('http') ? link : `https://meetup.com${link}`) : this.baseUrl,
                        source: 'New Jersey Metro Events'
                    };
                }
            };
                ...
                $('a').each((index, element) => {
                    ...
                    if (text && text.length > 10 && text.length < 100 &&
                        (text.toLowerCase().includes('event') ||
                         ...
                         text.toLowerCase().includes('hoboken') ||
                         text.toLowerCase().includes('jersey city') ||
            ...
            console.log(`✅ New Jersey Metro: Found ${events.length} events`);
            return events.slice(0, 50); // Limit to 50 events

        } catch (error) {
            console.error(`❌ Error scraping New Jersey Metro: ${error.message}`);
            return [];
        }
    }
}
...
module.exports = async (city) => {
  const scraper = new NewJerseyMetroEventsExtractor();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.NewJerseyMetroEventsExtractor = NewJerseyMetroEventsExtractor;