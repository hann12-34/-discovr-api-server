...
async function scrapeEvents(city = 'New York') {
  console.log('🎪 Scraping Madison Square Garden events...');
  const events = [];
  
  try {
    const response = await axios.get('https://www.msg.com/calendar', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Madison Square Garden', address: '4 Pennsylvania Plaza New York NY 10001', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://www.msg.com/calendar', category: 'Events'
      });
    });
