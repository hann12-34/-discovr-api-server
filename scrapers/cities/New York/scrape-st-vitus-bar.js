...
async function scrapeEvents(city = 'New York') {
  console.log('🎪 Scraping Saint Vitus Bar events...');
  let events = [];
  
  try {
    const response = await axios.get('https://saintvitusbar.com/calendar/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Saint Vitus Bar', address: '1120 Manhattan Ave Brooklyn NY 11222', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://saintvitusbar.com/calendar/', category: 'Events'
      });
    });
