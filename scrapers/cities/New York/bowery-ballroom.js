...
class BoweryBallroom {
    ...
    async fetchEvents() {
        try {
            ...
            // Parse Bowery Ballroom events - target Ticketmaster links with real dates
            $('a[href*="ticketmaster"], a[href*="tm-event"]').each((index, element) => {
                try {
                    const $el = $(element);
                    ...
                    // Extract date from Ticketmaster URL - MUST have real date or skip!
                    let dateText = null;
                    
                    if (href) {
                        // Extract date from Ticketmaster URLs (format: MM-DD-YYYY)
                        ...
                        if (dateMatch) {
                            ...
                            // Try other date patterns (YYYY-MM-DD)
                    ...
                    if (!dateText) {
                        console.log(`   ❌ No date found in URL, skipping event`);
