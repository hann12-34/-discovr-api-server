...
async function scrapeEvents(city = 'New York') {
  console.log('🎨 Scraping SummerStage events...');
  let events = [];
  
  try {
    const response = await axios.get('https://www.summerstage.org/', {
      timeout: 15000,
    ...
    Array.from(containers).forEach((el) => {
      ...
      events.push({
        ...
        venue: { name: 'Central Park', address: 'Central Park, New York, NY 10024', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://www.summerstage.org/', category: 'Events'
      });
    });
