/**
 * Prospect Park Alliance Events Scraper
 *
 * Scrapes events from Prospect Park Alliance in Brooklyn
...
class ProspectParkEvents {
    constructor() {
        this.venueName = 'Prospect Park';
        this.venueLocation = 'Prospect Park, Brooklyn, NY 11215';
        this.baseUrl = 'https://www.prospectpark.org';
        this.eventsUrl = 'https://www.prospectpark.org/events';
        this.category = 'Brooklyn Park Events';
    }

    /**
     * Scrape events from Prospect Park Alliance
    ...
    async scrape() {
        ...
        try {
            ...
            eventSelectors.forEach(selector => {
                $(selector).each((index, element) => {
                    ...
                    if (title && this.isValidEvent(title)) {
                        ...
                        const locationSelectors = ['.location', '.where', '.area', '.field', '[class*="location"]'];
                        for (const locSelector of locationSelectors) {
                            ...
                            if (locElement.length > 0) {
                                ...
                                if (locText && locText.length > 0) {
                                    parkLocation = `${locText}, Prospect Park, Brooklyn, NY`;
                                    break;
                                }
                            }
                        }
                        ...
                        // Look for organizer/presenter
                        let organizer = '';
                        const organizerSelectors = ['.organizer', '.presenter', '.host', '[class*="organizer"]'];
            ...
            // Look for general park activities and concerts
            $('div, section, article, p').each((index, element) => {
                ...
                if (text.length > 25 && text.length < 400) {
                    const hasParkKeywords = text.match(/\b(park|prospect|brooklyn|outdoor|nature|concert|festival|bandshell|lake|zoo|carousel)\b/i);
    ...
    isValidEvent(title) {
        ...
        const validKeywords = [
            'park', 'outdoor', 'nature', 'family', 'kids', 'exercise',
            'walk', 'tour', 'concert', 'festival', 'activity', 'program',
            'class', 'workshop', 'bandshell', 'zoo', 'lake'
        ];
...
module.exports = async (city) => {
  const scraper = new ProspectParkEvents();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.ProspectParkEvents = ProspectParkEvents;