...
async function scrapeEvents(city = 'New York') {
  ...
  try {
    const response = await axios.get('https://www.apollotheater.org/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Apollo Theater', address: '253 W 125th St New York NY 10027', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://www.apollotheater.org/', category: 'Events'
      });
    });
