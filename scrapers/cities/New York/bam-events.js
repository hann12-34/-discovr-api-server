 ...
 *
 * Scrapes events from BAM's official website
...
class BAMEvents {
    constructor() {
        this.venueName = 'Brooklyn Academy of Music (BAM)';
        this.venueLocation = '30 Lafayette Ave, Brooklyn, NY 11217';
        this.baseUrl = 'https://www.bam.org';
        this.eventsUrl = 'https://www.bam.org/calendar';
        this.category = 'Theater & Performing Arts';
    }

    /**
     * Scrape events from BAM
    ...
    async scrape() {
        ...
        try {
            ...
            const eventSelectors = [
                ...
                '[class*="event"]', '.listing', '.performance', '.show',
                '.card', '.content-card', '.bam-event', '.production'
            ];

            eventSelectors.forEach(selector => {
                $(selector).each((index, element) => {
                    const $el = $(element);
                    let title = $el.find('h1, h2, h3, h4, .title, .event-title, .name, .headline, .show-title').first().text().trim();
                    ...
                    if (title && this.isValidEvent(title)) {
                        ...
                        const dateSelectors = [
                            ...
                            '.calendar', '.day', '.show-dates', '.performance-date'
                        ];
                        ...
                        // Look for artist/performer
                        let artist = '';
                        const artistSelectors = ['.artist', '.performer', '.by', '.creator', '[class*="artist"]'];
                        ...
                        // Look for venue within BAM (Harvey Theater, Opera House, etc.)
                        let specificVenue = this.venueName;
                        const venueSelectors = ['.venue', '.theater', '.hall', '[class*="venue"]', '.location'];
                        for (const venueSelector of venueSelectors) {
                            ...
                            if (venueElement.length > 0) {
                                ...
                                if (venueText && venueText.length > 0) {
                                    specificVenue = `${this.venueName} - ${venueText}`;
                                    break;
                                }
                            }
                        }
    ...
    isValidEvent(title) {
        ...
        const invalidKeywords = [
            ...
            'directions', 'parking', 'accessibility', 'donate'
        ];
...
module.exports = async (city) => {
  const scraper = new BAMEvents();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.BAMEvents = BAMEvents;