...
async function scrapeEvents(city = 'New York') {
  ...
  try {
    const response = await axios.get('https://nationalsawdust.org/events/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'National Sawdust', address: '80 N 6th St Brooklyn NY 11249', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://nationalsawdust.org/events/', category: 'Events'
      });
    });
