...
async function scrapeEvents(city = 'New York') {
  ...
  try {
    const response = await axios.get('https://bryantpark.org/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Bryant Park', address: 'Bryant Park New York NY 10018', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://bryantpark.org/', category: 'Events'
      });
    });
