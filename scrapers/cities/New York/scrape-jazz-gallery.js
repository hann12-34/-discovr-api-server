...
async function scrapeEvents(city = 'New York') {
  console.log('🎵 Scraping The Jazz Gallery events...');
  
  let events = [];
  
  try {
    const response = await axios.get('https://www.jazzgallery.org/', {
      timeout: 15000,
      headers: {
    ...
    Array.from(containers).forEach((el) => {
      ...
      events.push({
        ...
        venue: { name: 'The Jazz Gallery', address: '1160 Broadway New York NY 10001', city: 'New York' },
        location: 'New York, NY',
        description: title,
        url: 'https://www.jazzgallery.org/',
        category: 'Events'
      });
    });
