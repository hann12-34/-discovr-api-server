...
class QueensEventsExtractor {
    constructor() {
        this.baseUrl = 'https://www.meetup.com/find/?keywords=events&location=us--ny--queens';
        this.name = 'Queens Events';
    }

    async scrape() {
        try {
            console.log(`👑 Scraping events from Queens, NY...`);
                ...
                $('a').each((index, element) => {
                    ...
                    if (text && text.length > 10 && text.length < 100 &&
                        (text.toLowerCase().includes('event') ||
                         ...
                         text.toLowerCase().includes('queens') ||
                         text.toLowerCase().includes('flushing') ||
                         text.toLowerCase().includes('astoria') ||
            ...
            console.log(`✅ Queens: Found ${events.length} events`);
            return events.slice(0, 50); // Limit to 50 events

        } catch (error) {
            console.error(`❌ Error scraping Queens: ${error.message}`);
            return [];
        }
    }
}
...
module.exports = async (city) => {
  const scraper = new QueensEventsExtractor();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.QueensEventsExtractor = QueensEventsExtractor;