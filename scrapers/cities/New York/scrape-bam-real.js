...
async function scrapeEvents(city = 'New York') {
  console.log('🎪 Scraping Brooklyn Academy of Music events...');
  const events = [];
  
  try {
    const response = await axios.get('https://www.bam.org/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Brooklyn Academy of Music', address: '30 Lafayette Ave Brooklyn NY 11217', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://www.bam.org/', category: 'Events'
      });
    });
