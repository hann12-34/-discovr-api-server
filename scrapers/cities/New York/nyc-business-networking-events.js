/**
 * NYC Business Networking Events Scraper
 *
 * Scrapes events from NYC business networking and professional events
 * URL: https://www.eventbrite.com/d/ny--new-york/business-networking/
 */

const axios = require('axios');
const cheerio = require('cheerio');

class NYCBusinessNetworkingEvents {
    constructor() {
        this.venueName = 'NYC Business Networking';
        this.venueLocation = 'Various NYC Business Venues';
        this.baseUrl = 'https://www.eventbrite.com';
        this.eventsUrl = 'https://www.eventbrite.com/d/ny--new-york/business-networking/';
        this.category = 'Business & Networking';
    }

    /**
     * Scrape events from NYC Business Networking
    ...
    async scrape() {
        ...
        try {
            ...
            // Look for business networking-specific event containers
            const eventSelectors = [
                '.business-event', '.networking-event', '.professional-event',
                '.event-item', '.event-card', '.event', '.meetup-event',
                '[class*="business"]', '[class*="networking"]', '[class*="event"]',
                '.card', '.content-card', '.corporate-event', '.conference'
            ];

            eventSelectors.forEach(selector => {
                $(selector).each((index, element) => {
                    const $el = $(element);
                    let title = $el.find('h1, h2, h3, h4, .title, .event-title, .business-title, .name, .headline').first().text().trim();
                    ...
                    if (title && this.isValidEvent(title)) {
                        ...
                        // Look for speaker/host
                        let speaker = '';
                        const speakerSelectors = ['.speaker', '.host', '.presenter', '[class*="speaker"]'];
                        ...
                        let organization = '';
                        const orgSelectors = ['.organization', '.company', '.organizer', '[class*="organization"]'];
                        ...
                        // Look for attendance/capacity
                        let attendance = '';
                        const attendanceSelectors = ['.attendance', '.capacity', '.spots', '[class*="attendance"]'];
                        ...
                        const event = {
                            ...
                            date: dateTime || 'Check website for networking event schedule',
                            category: eventType,
                            industry: industry,
                            speaker: speaker,
                            ...
                            source: 'NYCBusinessNetworkingEvents'
                        };
            ...
            // Look for general business networking information
            $('div, section, article, p').each((index, element) => {
                ...
                if (text.length > 30 && text.length < 400) {
                    const hasBusinessKeywords = text.match(/\b(business|networking|professional|entrepreneur|startup|corporate|conference|summit|workshop)\b/i);
                    const hasEventPattern = text.match(/\b(event|meetup|gathering|session|seminar|panel|presentation)\b/i);
                    ...
                    if (hasBusinessKeywords && hasEventPattern) {
                        ...
                        if (title && this.isValidEvent(title) && title.length > 20) {
                            const event = {
                                ...
                                location: this.venueLocation,
                                date: 'Check website for networking event schedule',
    ...
    isValidEvent(title) {
        ...
        // Check for valid business networking keywords
        const validKeywords = [
            'business', 'networking', 'professional', 'entrepreneur', 'startup',
            'corporate', 'conference', 'summit', 'workshop', 'seminar',
            'panel', 'presentation', 'industry', 'career', 'leadership'
        ];
...
module.exports = async (city) => {
  const scraper = new NYCBusinessNetworkingEvents();
  return await scraper.scrape(city);
};

// Also export the class for backward compatibility
module.exports.NYCBusinessNetworkingEvents = NYCBusinessNetworkingEvents;