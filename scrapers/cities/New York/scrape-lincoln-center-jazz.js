...
async function scrapeEvents(city = 'New York') {
  ...
  try {
    const response = await axios.get('https://www.jazz.org/events/', {
      timeout: 15000,
    ...
    $('.event, [class*="event"], article, .show, .performance, [data-event]').each((i, el) => {
      ...
      events.push({
        ...
        venue: { name: 'Jazz at Lincoln Center', address: '10 Columbus Circle New York NY 10019', city: 'New York' },
        location: 'New York, NY', description: title, url: 'https://www.jazz.org/events/', category: 'Events'
      });
    });
