...
async function scrapeEvents(city = 'New York') {
  console.log('🎪 Scraping Brooklyn Mirage (Avant Gardner) events...');
  ...
  try {
    ...
    await page.goto('https://www.avant-gardner.com/shows', {
    ...
    const events = await page.evaluate(() => {
      ...
      // Find event links
      const links = document.querySelectorAll('a[href*="/event/"], a[href*="/shows/"]');
      ...
      links.forEach(link => {
        ...
        let title = '';
        const h3 = link.querySelector('h3, h2, h4');
        if (h3) {
        ...
        if (/^(buy|ticket|view|shows|more info)/i.test(title)) return;
        if (title.includes('<img') || title.startsWith('<')) return;
        ...
        if (dateMatch) {
          ...
          return; // Skip events without a parseable date
        }
        ...
        results.push({
          ...
          date: formattedDate,
          url: href.startsWith('http') ? href : 'https://www.avant-gardner.com' + href
        });
      });
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} Brooklyn Mirage events`);
    ...
    console.error('  ⚠️ Brooklyn Mirage error:', error.message);
    return [];
  }
}
