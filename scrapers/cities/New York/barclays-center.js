  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      links.forEach(link => {
        ...
        if (/^(buy|ticket|view|more info)/i.test(title)) return;
        ...
        results.push({
          ...
          date: formattedDate,
          url: href.startsWith('http') ? href : 'https://www.barclayscenter.com' + href
        });
      });
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} Barclays Center events`);
    ...
    console.error('  ⚠️ Barclays Center error:', error.message);
    return [];
  }
}
