/**
 * The Chapel SF Alt Events Scraper
 * URL: https://www.slimspresents.com/the-chapel/calendar/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeChapelAlt(city = 'San Francisco') {
  console.log('⛪ Scraping Chapel Calendar...');
  ...
  try {
    ...
    await page.goto('https://www.slimspresents.com/the-chapel/calendar/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Francisco',
        category: 'Nightlife',
        source: 'Chapel Calendar'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Chapel Calendar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Chapel Calendar error:', error.message);
    return [];
  }
}

module.exports = scrapeChapelAlt;
