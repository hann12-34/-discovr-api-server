/**
 * UC Theatre Berkeley Events Scraper
 * URL: https://theuctheatre.org/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeUCTheatre(city = 'San Francisco') {
  console.log('🎭 Scraping UC Theatre Berkeley...');
  ...
  try {
    ...
    await page.goto('https://theuctheatre.org/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Francisco',
        category: 'Nightlife',
        source: 'UC Theatre Berkeley'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} UC Theatre Berkeley events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  UC Theatre Berkeley error:', error.message);
    return [];
  }
}

module.exports = scrapeUCTheatre;
