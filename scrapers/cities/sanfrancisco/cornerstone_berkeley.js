/**
 * Cornerstone Berkeley Events Scraper
 * URL: https://cornerstoneberkeley.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCornerstone(city = 'San Francisco') {
  console.log('🎵 Scraping Cornerstone Berkeley...');
  ...
  try {
    ...
    await page.goto('https://cornerstoneberkeley.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Francisco',
        category: 'Nightlife',
        source: 'Cornerstone Berkeley'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cornerstone events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cornerstone error:', error.message);
    return [];
  }
}

module.exports = scrapeCornerstone;
