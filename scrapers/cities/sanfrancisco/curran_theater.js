/**
 * Curran SF Events Scraper
 * URL: https://sfcurran.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCurran(city = 'San Francisco') {
  console.log('🎭 Scraping Curran SF...');
  ...
  try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Curran SF events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Curran SF error:', error.message);
    return [];
  }
}

module.exports = scrapeCurran;
