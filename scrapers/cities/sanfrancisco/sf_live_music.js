/**
 * SF Live Music Guide Events Scraper
 * URL: https://www.sflivemusicguide.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSFLiveMusic(city = 'San Francisco') {
  console.log('🎵 Scraping SF Live Music...');
  ...
  try {
    ...
    await page.goto('https://www.sflivemusicguide.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Francisco',
        category: 'Nightlife',
        source: 'SF Live Music'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} SF Live Music events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  SF Live Music error:', error.message);
    return [];
  }
}

module.exports = scrapeSFLiveMusic;
