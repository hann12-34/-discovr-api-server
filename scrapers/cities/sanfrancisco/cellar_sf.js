/**
 * The Cellar SF Events Scraper
 * URL: https://www.thecellarsf.com/calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCellarSF(city = 'San Francisco') {
  console.log('🍷 Scraping The Cellar SF...');
  ...
  try {
    ...
    await page.goto('https://www.thecellarsf.com/calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'San Francisco',
        category: 'Nightlife',
        source: 'The Cellar SF'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cellar SF events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cellar SF error:', error.message);
    return [];
  }
}

module.exports = scrapeCellarSF;
