/**
 * Impala Auckland Events Scraper
 * Popular nightclub in Auckland CBD
 * URL: https://impala.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeImpalaAuckland(city = 'Auckland') {
  console.log('🦌 Scraping Impala Auckland...');
  ...
  try {
    ...
    await page.goto('https://impala.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Impala',
          address: '7 Shortland Street, Auckland CBD',
          city: 'Auckland'
        },
        city: 'Auckland',
        category: 'Nightlife',
        source: 'Impala Auckland'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Impala Auckland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Impala Auckland error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeImpalaAuckland;
