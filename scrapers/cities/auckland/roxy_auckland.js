/**
 * Roxy Auckland Events Scraper
 * Nightclub in Auckland CBD
 * URL: https://theroxy.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoxyAuckland(city = 'Auckland') {
  console.log('🎭 Scraping Roxy Auckland...');
  ...
  try {
    ...
    await page.goto('https://theroxy.co.nz/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Roxy',
          address: '3 Fort Lane, Auckland CBD',
          city: 'Auckland'
        },
        city: 'Auckland',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Roxy Auckland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Roxy Auckland error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeRoxyAuckland;
