/**
 * Basement Theatre Auckland Events Scraper
 * URL: https://www.basementtheatre.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBasementTheatre(city = 'Auckland') {
  console.log('🎭 Scraping Basement Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.basementtheatre.co.nz/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Lower Greys Avenue, Auckland',
          city: 'Auckland'
        },
        latitude: -36.8510,
        longitude: 174.7620,
        city: 'Auckland',
        category: 'Nightlife',
        source: 'Basement Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Basement Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Basement Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeBasementTheatre;
