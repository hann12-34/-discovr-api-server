/**
 * Sweat Shop Brew Auckland Events Scraper
 * URL: https://www.sweatshopbrew.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSweatShopBrew(city = 'Auckland') {
  console.log('🍺 Scraping Sweat Shop Brew...');
  ...
  try {
    ...
    await page.goto('https://www.sweatshopbrew.co.nz/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Sweat Shop Brew',
          address: 'K Road, Auckland',
          city: 'Auckland'
        },
        latitude: -36.8575,
        longitude: 174.7560,
        city: 'Auckland',
        category: 'Nightlife',
        source: 'Sweat Shop Brew'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sweat Shop events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sweat Shop error:', error.message);
    return [];
  }
}

module.exports = scrapeSweatShopBrew;
