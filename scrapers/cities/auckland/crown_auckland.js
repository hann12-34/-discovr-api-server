/**
 * Crown Cocktail Lounge & Nightclub Auckland Events Scraper
 * Premium nightclub on Queen Street
 * URL: https://crownnz.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCrownAuckland(city = 'Auckland') {
  console.log('👑 Scraping Crown Auckland...');
  ...
  try {
    ...
    await page.goto('https://crownnz.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Crown Cocktail Lounge & Nightclub',
          address: '450 Queen Street, Auckland CBD',
          city: 'Auckland'
        },
        city: 'Auckland',
        category: 'Nightlife',
        source: 'Crown Auckland'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Crown Auckland events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Crown Auckland error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCrownAuckland;
