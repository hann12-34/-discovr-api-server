...
async function scrapeAucklandPride(city = 'Auckland') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Ponsonby Road', address: 'Ponsonby, Auckland', city: 'Auckland' }, latitude: -36.8578, longitude: 174.7488, city: 'Auckland', category: 'Festival', source: 'Auckland Pride' });
    }
