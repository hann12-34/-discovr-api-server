...
async function scrapeACL(city = 'Auckland') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Western Springs Park', address: 'Western Springs, Auckland', city: 'Auckland' }, latitude: -36.8671, longitude: 174.7222, city: 'Auckland', category: 'Festival', source: 'Auckland City Limits' });
    }
