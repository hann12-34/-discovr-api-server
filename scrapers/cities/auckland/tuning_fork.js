/**
 * Tuning Fork Auckland Events Scraper
 * Live music venue at Spark Arena precinct
 * URL: https://www.tuningfork.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTuningFork(city = 'Auckland') {
  console.log('🎵 Scraping Tuning Fork Auckland...');
  ...
  try {
    ...
    await page.goto('https://www.tuningfork.co.nz/', {
    ...
    // Look for events page link
    const eventsPageUrl = await page.evaluate(() => {
      const links = Array.from(document.querySelectorAll('a[href]'));
      for (const link of links) {
        const href = link.href.toLowerCase();
        if (href.includes('event') || href.includes('whats-on') || href.includes('gig') || href.includes('show')) {
          return link.href;
        }
      }
      return null;
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.show', 'article', '[class*="event"]', 'a[href*="event"]'];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Tuning Fork events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ Tuning Fork error:', error.message);
    return [];
  }
}

module.exports = scrapeTuningFork;
