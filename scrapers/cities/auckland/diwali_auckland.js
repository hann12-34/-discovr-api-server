...
async function scrapeDiwaliAKL(city = 'Auckland') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Aotea Square', address: 'Queen Street, Auckland CBD', city: 'Auckland' }, latitude: -36.8522, longitude: 174.7643, city: 'Auckland', category: 'Festival', source: 'Diwali Auckland' });
    }
