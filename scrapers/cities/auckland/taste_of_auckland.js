...
async function scrapeTasteAKL(city = 'Auckland') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Victoria Park', address: 'Victoria Street West, Auckland', city: 'Auckland' }, latitude: -36.8493, longitude: 174.7571, city: 'Auckland', category: 'Festival', source: 'Taste of Auckland' });
    }
