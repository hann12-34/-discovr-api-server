/**
 * Auckland Arts Festival Events Scraper
 * URL: https://www.aucklandfestival.co.nz
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAucklandArts(city = 'Auckland') {
  console.log('🎭 Scraping Auckland Arts Festival...');
  ...
  try {
    ...
    await page.goto('https://www.aucklandfestival.co.nz/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Auckland',
        category: 'Festival',
        source: 'Auckland Arts Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Auckland Arts Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Auckland Arts error:', error.message);
    return [];
  }
}

module.exports = scrapeAucklandArts;
