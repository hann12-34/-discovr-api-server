/**
 * 1885 Britomart Auckland Events Scraper
 * Jazz bar and nightclub in Britomart
 * URL: https://www.1885.co.nz/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrape1885Britomart(city = 'Auckland') {
  console.log('🎺 Scraping 1885 Britomart Auckland...');
  ...
  try {
    ...
    await page.goto('https://www.1885.co.nz/', {
    ...
    const eventsPageUrl = await page.evaluate(() => {
      ...
      for (const link of links) {
        ...
        if (href.includes('event') || href.includes('whats-on') || href.includes('gig') || href.includes('live')) {
          return link.href;
        }
      }
      return null;
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: '1885 Britomart',
          address: '27 Galway Street, Britomart, Auckland 1010',
          city: 'Auckland'
        },
        latitude: -36.8435,
        longitude: 174.7665,
        city: 'Auckland',
        category: 'Nightlife',
        source: '1885 Britomart'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} 1885 Britomart events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️ 1885 Britomart error:', error.message);
    return [];
  }
}

module.exports = scrape1885Britomart;
