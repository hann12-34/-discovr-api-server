/**
 * The Majestic Madison Events Scraper
 * URL: https://www.majesticmadison.com/calendar
 * Historic live music venue in downtown Madison, WI
 * Indie, rock, electronic, folk, comedy
 */
...
const MajesticMadisonEvents = {
  async scrape(city = 'Madison') {
    console.log('🎵 Scraping Majestic Madison...');

    try {
      const response = await axios.get('https://www.majesticmadison.com/calendar', {
        headers: {
      ...
      for (let i = 0; i < allTitles.length; i++) {
        ...
        let url = allUrls[i];
        if (!url) continue;
        if (!url.startsWith('http')) url = 'https://majesticmadison.com' + url;
        ...
        events.push({
          ...
          city: 'Madison',
          category: 'Music',
          source: 'Majestic Madison'
        });
      }

      console.log(`  ✅ Found ${events.length} Majestic Madison events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Majestic Madison error: ${error.message}`);
      return [];
    }
  }
};

module.exports = MajesticMadisonEvents.scrape;
