/**
 * New Speak MTL Events Scraper
 * Scrapes events from New Speak MTL in Montreal
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

const NewSpeakMTLEvents = {
  name: 'New Speak MTL',
  ...
  _parseSingleDate(daeventDateText) {
    ...
    if (dayMonthDayYearMatch) {
      ...
      if (monthNum !== undefined) {
        ...
        let hours = 21; // Default to 9 PM for nightclub events
        let minutes = 0;
    ...
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          '.event, .event-item, .upcoming, article, .card, .post, .evenement'
        ));
...
module.exports = NewSpeakMTLEvents;