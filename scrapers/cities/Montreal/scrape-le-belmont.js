/**
 * Le Belmont Events Scraper
 * Scrapes events from Le Belmont in Montreal
 */

const puppeteer = require('puppeteer');
const slugify = require('slugify');

const LeBelmontEvents = {
  name: 'Le Belmont',
  url: 'https://www.lebelmont.com/evenements',
  enabled: false, // Domain no longer active (for sale)
  ...
  generateEventId(title, startDate) {
    ...
    return `le-belmont-${slug}-${dateStr}`;
  },
  
  createEventObject(id, title, description, startDate, endDate, imageUrl, sourceUrl) {
    let categories = ['nightlife', 'bar', 'social', 'plateau'];
    
    const categoryKeywords = {
      'dj': ['dj', 'disc jockey', 'musique'],
      'dance': ['dance', 'dancing', 'danse'],
      'karaoke': ['karaoke', 'sing'],
      'trivia': ['trivia', 'quiz'],
      'theme': ['theme', 'themed', 'thématique'],
      'party': ['party', 'soirée', 'fête'],
      'live': ['live', 'band', 'concert']
    };
    ...
    return {
      ...
      venue: {
        name: this.name,
        address: '4483 Boulevard Saint-Laurent',
        city: 'Montreal',
        province: 'QC',
        country: 'Canada',
        postalCode: 'H2W 1Z8',
        website: this.url,
        googleMapsUrl: 'https://goo.gl/maps/lebelmontexample'
      },
      categories: [...new Set(categories)],
      isFallback: false,
      lastUpdated: new Date(),
      sourceIdentifier: 'le-belmont'
    };
  },
    ...
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          '.event, .evenement, .soiree, article, .card, .listing'
        ));
...
module.exports = LeBelmontEvents;
