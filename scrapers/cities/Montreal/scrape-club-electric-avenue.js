...
const ClubElectricAvenueEvents = {
  name: 'Club Electric Avenue',
  url: 'https://clubelectricavenue.ca/events/',
  enabled: true,
  ...
  generateEventId(title, startDate) {
    ...
    return `club-electric-avenue-${slug}-${dateStr}`;
  },
  ...
  createEventObject(id, title, description, startDate, endDate, imageUrl, sourceUrl) {
    ...
    const categoryKeywords = {
      ...
      'electronic': ['electronic', 'edm', 'rave'],
      'dj': ['dj', 'guest dj', 'resident'],
      'special': ['special', 'party', 'celebration', 'launch']
    };
    ...
    return {
      ...
      venue: { ...RegExp.venue: {
        ...
        postalCode: 'H3G 2B4',
        website: this.url,
        googleMapsUrl: 'https://goo.gl/maps/electricavenueexample'
      }, city },,
      categories: [...new Set(categories)],
      isFallback: false,
      lastUpdated: new Date(),
      sourceIdentifier: 'club-electric-avenue'
    };
  },
    ...
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          '.event, .event-item, .party, .show, article, .card, .upcoming-event'
        ));
...
module.exports = ClubElectricAvenueEvents;