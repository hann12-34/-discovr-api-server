...
const SalsathequeEvents = {
  name: 'Salsatheque',
  url: 'https://www.salsatheque.ca/parties.html',
  enabled: true,
  ...
  _parseSingleDate(dateString) {
    ...
    if (dayMonthDayYearMatch) {
      ...
      if (monthNum !== undefined) {
        ...
        let hours = 20; // Default to 8 PM for dance parties
        let minutes = 0;
  ...
  generateEventId(title, startDate) {
    ...
    return `salsatheque-${slug}-${dateStr}`;
  },
  
  createEventObject(id, title, description, startDate, endDate, imageUrl, sourceUrl) {
    let categories = ['dance', 'salsa', 'latin', 'social dance'];
    ...
    const categoryKeywords = {
      ...
      'merengue': ['merengue'],
      'tango': ['tango'],
      'lesson': ['lesson', 'class', 'cours', 'workshop'],
      'party': ['party', 'fiesta', 'soirée', 'social'],
      'live': ['live', 'band', 'orchestra', 'orchestre']
    };
    ...
    return {
      ...
      venue: {
        name: this.name,
        address: '4848 Boulevard Saint-Laurent',
        city: 'Montreal',
        province: 'QC',
        country: 'Canada',
        postalCode: 'H2T 1R5',
        website: this.url,
        googleMapsUrl: 'https://goo.gl/maps/salsathequeexample'
      },
      categories: [...new Set(categories)],
      isFallback: false,
      lastUpdated: new Date(),
      sourceIdentifier: 'salsatheque'
    };
  },
    ...
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          '.event, .party, .dance, '.class, article, .card, '.listing, tr'
        ));
        
        eventElements.forEach(element => {
          let title = '';
          const titleElement = element.querySelector('h1, h2, h3, h4, .title, .name, td:first-child');
          if (titleElement) {
          ...
          let description = '';
          const descElement = element.querySelector('p, .description, .details, .info, td:nth-child(2)');
          if (descElement) {
            description = descElement.textContent.trim();
          }
          
          let dateText = '';
          const dateElement = element.querySelector('.date, .dates, time, .when, td:last-child');
          if (dateElement) {
...
module.exports = SalsathequeEvents;
