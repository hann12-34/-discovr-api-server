...
const DiesOnzeEvents = {
  name: 'Dies Onze',
  url: 'http://www.dieseonze.com',
  enabled: false, // Site blocks automated requests
  ...
  _parseSingleDate(daeventDateText) {
    ...
    if (dayMonthDayYearMatch) {
      ...
      const months = {
        ...
        november: 10, nov: 10, december: 11, dec: 11, decembre: 11
      };
  ...
  generateEventId(title, startDate) {
    ...
    return `dies-onze-${slug}-${dateStr}`;
  },

  createEventObject(id, title, description, startDate, endDate, imageUrl, sourceUrl) {
    let categories = ['nightlife', 'nightclub', 'electronic', 'dance'];
    ...
    const categoryKeywords = {
      ...
      'rave': ['rave', 'underground', 'after hours'],
    ...
    return {
      ...
      venue: { ...RegExp.venue: {
        ...
        postalCode: 'H3G 2B1',
        website: this.url,
        googleMapsUrl: 'https://goo.gl/maps/diesonzeexample'
      }, city },,
      categories: [...new Set(categories)],
      isFallback: false,
      lastUpdated: new Date(),
      sourceIdentifier: 'dies-onze'
    };
  },
    ...
    try {
      ...
      const eventData = await page.evaluate(() => {
        ...
        const eventElements = Array.from(document.querySelectorAll(
          '.event, .party, .show, .night, article, .card, .listing'
        ));
