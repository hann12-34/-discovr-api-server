/**
 * Wire Leeds Events Scraper
 * Popular nightclub in Leeds
 * URL: https://www.wireclub.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWireLeeds(city = 'Leeds') {
  console.log('⚡ Scraping Wire Leeds...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Wire',
          address: '2-8 Call Lane, Leeds LS1 6DN',
          city: 'Leeds'
        },
        city: 'Leeds',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Wire events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Wire error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeWireLeeds;
