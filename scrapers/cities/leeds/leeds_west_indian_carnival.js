/**
 * Leeds West Indian Carnival Events Scraper
 * URL: https://www.leedswestindiancarnival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWestIndianCarnival(city = 'Leeds') {
  console.log('🎉 Scraping West Indian Carnival...');
  ...
  try {
    ...
    await page.goto('https://www.leedswestindiancarnival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Potternewton Park, Leeds',
          city: 'Leeds'
        },
        latitude: 53.8217,
        longitude: -1.5351,
        city: 'Leeds',
        category: 'Festival',
        source: 'West Indian Carnival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} West Indian Carnival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  West Indian Carnival error:', error.message);
    return [];
  }
}

module.exports = scrapeWestIndianCarnival;
