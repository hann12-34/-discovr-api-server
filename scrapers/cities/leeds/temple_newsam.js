/**
 * Temple Newsam Leeds Events Scraper
 * URL: https://www.templenewsamfestival.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTempleNewsam(city = 'Leeds') {
  console.log('🏰 Scraping Temple Newsam...');
  ...
  try {
    ...
    await page.goto('https://www.templenewsamfestival.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.festival-event', '.event', 'article', '[class*="event"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Temple Newsam events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Temple Newsam error:', error.message);
    return [];
  }
}

module.exports = scrapeTempleNewsam;
