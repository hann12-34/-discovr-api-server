/**
 * Headrow House Leeds Events Scraper
 * URL: https://headrowhouse.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHeadrowHouse(city = 'Leeds') {
  console.log('🏠 Scraping Headrow House...');
  ...
  try {
    ...
    await page.goto('https://headrowhouse.com/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Leeds',
        category: 'Nightlife',
        source: 'Headrow House'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Headrow House events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Headrow House error:', error.message);
    return [];
  }
}

module.exports = scrapeHeadrowHouse;
