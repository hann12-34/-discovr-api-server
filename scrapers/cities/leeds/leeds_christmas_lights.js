...
async function scrapeChristmasLights(city = 'Leeds') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Millennium Square', address: 'Millennium Square, Leeds', city: 'Leeds' }, latitude: 53.8008, longitude: -1.5491, city: 'Leeds', category: 'Festival', source: 'Christmas Lights' });
    }
