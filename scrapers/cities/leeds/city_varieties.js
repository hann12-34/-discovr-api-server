/**
 * City Varieties Leeds Events Scraper
 * URL: https://cityvarieties.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCityVarieties(city = 'Leeds') {
  console.log('🎪 Scraping City Varieties...');
  ...
  try {
    ...
    await page.goto('https://cityvarieties.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Leeds',
        category: 'Arts',
        source: 'City Varieties'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} City Varieties events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  City Varieties error:', error.message);
    return [];
  }
}

module.exports = scrapeCityVarieties;
