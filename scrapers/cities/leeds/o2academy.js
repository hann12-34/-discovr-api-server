/**
 * O2 Academy Leeds Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academyleeds/events
 */
...
async function scrapeO2AcademyLeeds(city = 'Leeds') {
  console.log('🎸 Scraping O2 Academy Leeds...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academyleeds/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Leeds', address: '55 Cookridge Street, Leeds LS2 3AW', city: 'Leeds' },
        latitude: 53.8008,
        longitude: -1.5491,
        city: 'Leeds',
        category: 'Nightlife',
        source: 'O2 Academy Leeds'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Leeds events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Leeds error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyLeeds;
