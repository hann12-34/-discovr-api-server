...
async function scrapeRoundhayPark(city = 'Leeds') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Roundhay Park', address: 'Roundhay Park, Leeds', city: 'Leeds' }, latitude: 53.8371, longitude: -1.4988, city: 'Leeds', category: 'Festival', source: 'Roundhay Park' });
    }
