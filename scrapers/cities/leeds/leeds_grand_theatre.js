/**
 * Leeds Grand Theatre Events Scraper
 * URL: https://leedsgrandtheatre.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLeedsGrandTheatre(city = 'Leeds') {
  console.log('🎭 Scraping Leeds Grand Theatre...');
  ...
  try {
    ...
    await page.goto('https://leedsgrandtheatre.com/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Leeds',
        category: 'Arts',
        source: 'Leeds Grand Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Leeds Grand Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Leeds Grand Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeLeedsGrandTheatre;
