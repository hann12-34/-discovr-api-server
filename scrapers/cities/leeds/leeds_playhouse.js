/**
 * Leeds Playhouse Events Scraper
 * URL: https://leedsplayhouse.org.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLeedsPlayhouse(city = 'Leeds') {
  console.log('🎭 Scraping Leeds Playhouse...');
  ...
  try {
    ...
    await page.goto('https://leedsplayhouse.org.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Leeds',
        category: 'Arts',
        source: 'Leeds Playhouse'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Leeds Playhouse events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Leeds Playhouse error:', error.message);
    return [];
  }
}

module.exports = scrapeLeedsPlayhouse;
