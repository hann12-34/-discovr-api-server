/**
 * Bournemouth Pride Events Scraper
 * URL: https://www.bournemouthpride.org
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBournemouthPride(city = 'Bournemouth') {
  console.log('🏳️‍🌈 Scraping Bournemouth Pride...');
  ...
  try {
    ...
    await page.goto('https://www.bournemouthpride.org', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Pier Approach',
          address: 'Pier Approach, Bournemouth',
          city: 'Bournemouth'
        },
        latitude: 50.7164,
        longitude: -1.8769,
        city: 'Bournemouth',
        category: 'Festival',
        source: 'Bournemouth Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bournemouth Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Bournemouth Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeBournemouthPride;
