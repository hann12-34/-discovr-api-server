...
async function scrapeNYEBournemouth(city = 'Bournemouth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Bournemouth Beach', address: 'Bournemouth Beach', city: 'Bournemouth' }, latitude: 50.7192, longitude: -1.8808, city: 'Bournemouth', category: 'Festival', source: 'NYE Bournemouth' });
    }
