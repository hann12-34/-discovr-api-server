/**
 * Bournemouth New Year Eve - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNYEBournemouth(city = 'Bournemouth') {
  console.log('🎆 Scraping New Year Eve Bournemouth...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T20:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Bournemouth Beach', address: 'Bournemouth Beach', city: 'Bournemouth' }, latitude: 50.7192, longitude: -1.8808, city: 'Bournemouth', category: 'Festival', source: 'NYE Bournemouth' });
    }
    console.log(`  ✅ Found ${results.length} NYE Bournemouth events`);
    return results;
  } catch (err) {
    console.error('NYE Bournemouth error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeNYEBournemouth;
