...
async function scrapeBonfire(city = 'Bournemouth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Kings Park', address: 'Kings Park, Bournemouth', city: 'Bournemouth' }, latitude: 50.7334, longitude: -1.8472, city: 'Bournemouth', category: 'Festival', source: 'Bonfire Night' });
    }
