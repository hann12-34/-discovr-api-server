/**
 * BIC (Bournemouth International Centre) Events Scraper
 * URL: https://www.bic.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBIC(city = 'Bournemouth') {
  console.log('🏛️ Scraping BIC...');
  ...
  try {
    ...
    await page.goto('https://www.bic.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'BIC (Bournemouth International Centre)',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} BIC events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  BIC error:', error.message);
    return [];
  }
}

module.exports = scrapeBIC;
