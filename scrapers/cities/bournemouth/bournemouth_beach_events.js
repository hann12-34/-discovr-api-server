/**
 * Bournemouth Beach Summer Events - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBeachEvents(city = 'Bournemouth') {
  console.log('🏖️ Scraping Beach Events...');
  let browser;
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T14:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Bournemouth Beach', address: 'Bournemouth Beach', city: 'Bournemouth' }, latitude: 50.7192, longitude: -1.8808, city: 'Bournemouth', category: 'Festival', source: 'Beach Events' });
    }
    console.log(`  ✅ Found ${results.length} Beach events`);
    return results;
  } catch (err) {
    console.error('Beach Events error:', err.message);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeBeachEvents;
