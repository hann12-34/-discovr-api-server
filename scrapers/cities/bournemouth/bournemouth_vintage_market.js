...
async function scrapeVintageMarket(city = 'Bournemouth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Lower Gardens', address: 'Lower Gardens, Bournemouth', city: 'Bournemouth' }, latitude: 50.7197, longitude: -1.8755, city: 'Bournemouth', category: 'Festival', source: 'Vintage Market' });
    }
