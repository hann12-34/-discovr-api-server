/**
 * O2 Academy Bournemouth Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2academybournemouth/events
 */
...
async function scrapeO2AcademyBournemouth(city = 'Bournemouth') {
  console.log('🎸 Scraping O2 Academy Bournemouth...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2academybournemouth/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Academy Bournemouth', address: '570 Christchurch Road, Bournemouth BH1 4BH', city: 'Bournemouth' },
        latitude: 50.7325,
        longitude: -1.8413,
        city: 'Bournemouth',
        category: 'Nightlife',
        source: 'O2 Academy Bournemouth'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Academy Bournemouth events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Academy Bournemouth error:', error.message);
    return [];
  }
}

module.exports = scrapeO2AcademyBournemouth;
