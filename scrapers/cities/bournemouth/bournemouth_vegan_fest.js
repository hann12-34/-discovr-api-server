...
async function scrapeVeganFest(city = 'Bournemouth') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'BH2', address: 'Exeter Road, Bournemouth', city: 'Bournemouth' }, latitude: 50.7189, longitude: -1.8791, city: 'Bournemouth', category: 'Festival', source: 'Vegan Festival' });
    }
