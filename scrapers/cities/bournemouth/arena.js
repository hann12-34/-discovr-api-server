/**
 * Arena Bournemouth Events Scraper
 * URL: https://www.arenabournemouth.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeArena(city = 'Bournemouth') {
  console.log('🏟️ Scraping Arena...');
  ...
  try {
    ...
    await page.goto('https://www.arenabournemouth.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Arena',
          address: 'Holdenhurst Rd, Bournemouth BH8 8EB',
          city: 'Bournemouth'
        },
        latitude: 50.7300,
        longitude: -1.8615,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Arena events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Arena error:', error.message);
    return [];
  }
}
