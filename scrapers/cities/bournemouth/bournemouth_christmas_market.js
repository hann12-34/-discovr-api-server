/**
 * Bournemouth Christmas Market - Real event scraper
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeChristmasMarket(city = 'Bournemouth') {
  ...
  try {
    ...
    await page.goto('https://www.bournemouth.co.uk/whats-on/christmas', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T10:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Lower Gardens', address: 'Lower Gardens, Bournemouth', city: 'Bournemouth' }, latitude: 50.7197, longitude: -1.8755, city: 'Bournemouth', category: 'Festival', source: 'Christmas Market' });
    }
