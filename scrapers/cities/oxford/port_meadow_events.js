...
async function scrapePortMeadow(city = 'Oxford') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Port Meadow', address: 'Walton Well Road, Oxford', city: 'Oxford' }, latitude: 51.7668, longitude: -1.2781, city: 'Oxford', category: 'Festival', source: 'Port Meadow' });
    }
