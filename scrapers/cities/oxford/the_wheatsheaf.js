/**
 * The Wheatsheaf Oxford Events Scraper
 * URL: https://www.wheatsheafoxford.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWheatsheaf(city = 'Oxford') {
  console.log('🌾 Scraping The Wheatsheaf...');
  ...
  try {
    ...
    await page.goto('https://www.wheatsheafoxford.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Wheatsheaf',
          address: '129 High St, Oxford OX1 4DF',
          city: 'Oxford'
        },
        latitude: 51.7515,
        longitude: -1.2515,
        city: 'Oxford',
        category: 'Nightlife',
        source: 'The Wheatsheaf'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Wheatsheaf events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Wheatsheaf error:', error.message);
    return [];
  }
}

module.exports = scrapeWheatsheaf;
