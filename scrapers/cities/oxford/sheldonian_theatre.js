/**
 * Sheldonian Theatre Oxford Events Scraper
 * URL: https://www.sheldonian.ox.ac.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSheldonian(city = 'Oxford') {
  console.log('🏛️ Scraping Sheldonian Theatre...');
  ...
  try {
    ...
    await page.goto('https://www.sheldonian.ox.ac.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Oxford',
        category: 'Arts',
        source: 'Sheldonian Theatre'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sheldonian Theatre events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sheldonian Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeSheldonian;
