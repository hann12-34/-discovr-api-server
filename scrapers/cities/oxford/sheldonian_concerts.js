...
async function scrapeSheldonian(city = 'Oxford') {
  console.log('🎵 Scraping Sheldonian Theatre...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.sheldonian.ox.ac.uk/whats-on', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Sheldonian events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sheldonian error:', error.message);
    return [];
  }
}
