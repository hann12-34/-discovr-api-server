/**
 * Bridge Oxford Events Scraper
 * Popular nightclub in Oxford
 * URL: https://thebridgeoxford.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBridgeOxford(city = 'Oxford') {
  console.log('🌉 Scraping Bridge Oxford...');
  ...
  try {
    ...
    await page.goto('https://thebridgeoxford.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Oxford',
        category: 'Nightlife',
        source: 'The Bridge Oxford'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Bridge Oxford events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Bridge Oxford error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeBridgeOxford;
