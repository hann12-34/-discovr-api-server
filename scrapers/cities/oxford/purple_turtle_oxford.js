/**
 * Purple Turtle Oxford Events Scraper
 * URL: https://www.purpleturtlebar.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePurpleTurtle(city = 'Oxford') {
  console.log('🐢 Scraping Purple Turtle...');
  ...
  try {
    ...
    await page.goto('https://www.purpleturtlebar.com/oxford/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Oxford',
        category: 'Nightlife',
        source: 'Purple Turtle'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Purple Turtle events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Purple Turtle error:', error.message);
    return [];
  }
}

module.exports = scrapePurpleTurtle;
