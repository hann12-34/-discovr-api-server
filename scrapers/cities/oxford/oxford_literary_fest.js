...
async function scrapeOxfordLiterary(city = 'Oxford') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Christ Church College', address: 'St Aldates, Oxford', city: 'Oxford' }, latitude: 51.7502, longitude: -1.2556, city: 'Oxford', category: 'Festival', source: 'Oxford Literary' });
    }
