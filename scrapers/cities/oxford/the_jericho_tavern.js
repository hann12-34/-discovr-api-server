/**
 * The Jericho Tavern Oxford Events Scraper
 * URL: https://www.thejerichotavern.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeJerichoTavern(city = 'Oxford') {
  console.log('🍺 Scraping The Jericho Tavern...');
  ...
  try {
    ...
    await page.goto('https://www.thejerichotavern.co.uk/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.gig-item', '.event', 'article', '[class*="gig"]', '[class*="event"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Oxford',
        category: 'Nightlife',
        source: 'The Jericho Tavern'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Jericho Tavern events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Jericho Tavern error:', error.message);
    return [];
  }
}

module.exports = scrapeJerichoTavern;
