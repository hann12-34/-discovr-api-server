...
async function scrapeOxfordComedy(city = 'Oxford') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Oxford Playhouse', address: 'Beaumont Street, Oxford', city: 'Oxford' }, latitude: 51.7557, longitude: -1.2608, city: 'Oxford', category: 'Festival', source: 'Oxford Comedy' });
    }
