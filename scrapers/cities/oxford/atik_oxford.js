/**
 * Atik Oxford Events Scraper
 * Popular nightclub in Oxford
 * URL: https://atikoxford.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAtikOxford(city = 'Oxford') {
  console.log('🎵 Scraping Atik Oxford...');
  ...
  try {
    ...
    await page.goto('https://atikoxford.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Atik',
          address: '1 Park End St, Oxford OX1 1HH',
          city: 'Oxford'
        },
        city: 'Oxford',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Atik events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Atik error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeAtikOxford;
