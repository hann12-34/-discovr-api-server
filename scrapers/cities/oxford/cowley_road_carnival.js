/**
 * Cowley Road Carnival Oxford Events Scraper
 * URL: https://www.cowleyroadcarnival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCowleyCarnival(city = 'Oxford') {
  console.log('🎪 Scraping Cowley Road Carnival...');
  ...
  try {
    ...
    await page.goto('https://www.cowleyroadcarnival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Oxford',
        category: 'Festival',
        source: 'Cowley Carnival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cowley Carnival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cowley Carnival error:', error.message);
    return [];
  }
}

module.exports = scrapeCowleyCarnival;
