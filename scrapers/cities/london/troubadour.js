/**
 * Troubadour London london Events Scraper
 * URL: https://troubadourlondon.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTroubadour(city = 'london') {
  console.log('🎵 Scraping Troubadour London...');
  ...
  try {
    ...
    await page.goto('https://troubadourlondon.com/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'london',
        category: 'Nightlife',
        source: 'Troubadour London'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Troubadour London events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️  Troubadour London error: ${error.message}`);
    return [];
  }
}
