/**
 * Wigmore Hall - london Events Scraper
...
async function scrapeWigmoreHall(city = 'london') {
  console.log('🎭 Scraping Wigmore Hall...');
  try {
    const resp = await axios.get('https://www.wigmore-hall.org.uk/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Wigmore Hall', address: addr || '36 Wigmore St, London W1U 2BP', city: 'london' },
            latitude: 51.5175,
            longitude: -0.1509,
            city: 'london',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.wigmore-hall.org.uk/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.wigmore-hall.org.uk/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Wigmore Hall', address: '36 Wigmore St, London W1U 2BP', city: 'london' },
            latitude: 51.5175,
            longitude: -0.1509,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Wigmore Hall:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Wigmore Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeWigmoreHall;
