/**
 * Night Tales London Events Scraper
 * Open-air venue in Hackney
 * URL: https://www.nighttales.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNightTales(city = 'London') {
  console.log('🎧 Scraping Night Tales...');
  ...
  try {
    ...
    await page.goto('https://www.nighttales.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Event at Night Tales`,
        date: isoDate,
        ...
        venue: {
          name: 'Night Tales',
          address: '14-17 Bohemia Place, London E8 1DU',
          city: 'London'
        },
        latitude: 51.5436,
        longitude: -0.0558,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Night Tales events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Night Tales error:', error.message);
    return [];
  }
}

module.exports = scrapeNightTales;
