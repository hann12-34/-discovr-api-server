/**
 * Studio Spaces London Events Scraper
 * URL: https://www.studio-spaces.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeStudioSpaces(city = 'London') {
  console.log('🎵 Scraping Studio Spaces London...');
  ...
  try {
    ...
    await page.goto('https://www.studio-spaces.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Nightlife',
        source: 'Studio Spaces'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Studio Spaces events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Studio Spaces error:', error.message);
    return [];
  }
}

module.exports = scrapeStudioSpaces;
