/**
 * The Nest Dalston London Events Scraper
 * URL: https://www.thenestdalston.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNest(city = 'London') {
  console.log('🎸 Scraping The Nest Dalston...');
  ...
  try {
    ...
    await page.goto('https://www.thenestdalston.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.thenestdalston.com/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Nest events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nest error:', error.message);
    return [];
  }
}

module.exports = scrapeNest;
