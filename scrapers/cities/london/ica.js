...
async function scrapeICA(city = 'London') {
  console.log('🎨 Scraping ICA London...');
  ...
  try {
    ...
    await page.goto('https://www.ica.art/whats-on', {
    ...
    const events = await page.evaluate(() => {
      const results = [];
      const eventItems = document.querySelectorAll('.event-card, .event, [class*="event"], article, .programme-item');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Event at ICA London`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid ICA events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  ICA error:', error.message);
    return [];
  }
}
