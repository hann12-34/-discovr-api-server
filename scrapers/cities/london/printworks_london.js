/**
 * Printworks London Events Scraper
 * URL: https://printworkslondon.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePrintworksLondon(city = 'London') {
  console.log('🎵 Scraping Printworks London...');
  ...
  try {
    ...
    await page.goto('https://printworkslondon.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://printworkslondon.co.uk/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Printworks events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Printworks error:', error.message);
    return [];
  }
}

module.exports = scrapePrintworksLondon;
