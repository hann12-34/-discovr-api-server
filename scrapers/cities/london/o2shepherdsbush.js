/**
 * O2 Shepherd's Bush Empire Events Scraper
...
async function scrapeO2ShepherdsBush(city = 'London') {
  console.log('🎸 Scraping O2 Shepherds Bush Empire...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        url: event.url,
        imageUrl: null,
        venue: { name: "O2 Shepherd's Bush Empire", address: "Shepherds Bush Green, London W12 8TT", city: 'London' },
        latitude: 51.5039,
        longitude: -0.2263,
        city: 'London',
        category: 'Nightlife',
    ...
    console.log(`  ✅ Found ${formattedEvents.length} O2 Shepherds Bush events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Shepherds Bush error:', error.message);
    return [];
  }
}
