/**
 * The Forum Kentish Town London Events Scraper
 * URL: https://www.kentishtown.forum/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeForum(city = 'London') {
  console.log('🎸 Scraping The Forum Kentish Town...');
  ...
  try {
    ...
    await page.goto('https://www.kentishtown.forum/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.kentishtown.forum/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Forum events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Forum error:', error.message);
    return [];
  }
}
