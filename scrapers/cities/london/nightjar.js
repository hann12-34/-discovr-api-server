/**
 * Nightjar London Events Scraper
 * URL: https://barnightjar.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeNightjar(city = 'London') {
  console.log('🍸 Scraping Nightjar London...');
  ...
  try {
    ...
    await page.goto('https://barnightjar.com/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('[class*="event"], article, .card, a[href*="event"], a[href*="whats-on"]').forEach(el => {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Nightjar events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Nightjar error:', error.message);
    return [];
  }
}

module.exports = scrapeNightjar;
