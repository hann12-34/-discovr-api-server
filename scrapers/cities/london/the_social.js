/**
 * The Social London Events Scraper
 * URL: https://www.thesocial.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSocial(city = 'London') {
  console.log('🎸 Scraping The Social...');
  ...
  try {
    ...
    await page.goto('https://www.thesocial.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.thesocial.com/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Social events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Social error:', error.message);
    return [];
  }
}
