/**
 * The Old Blue Last London Events Scraper
 * URL: https://www.theoldbluelast.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOldBlueLast(city = 'London') {
  console.log('🎵 Scraping The Old Blue Last London...');
  ...
  try {
    ...
    await page.goto('https://www.theoldbluelast.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Nightlife',
        source: 'The Old Blue Last'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Old Blue Last events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Old Blue Last error:', error.message);
    return [];
  }
}

module.exports = scrapeOldBlueLast;
