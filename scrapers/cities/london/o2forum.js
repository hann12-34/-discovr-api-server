/**
 * O2 Forum Kentish Town Events Scraper
 * Historic live music venue
 * URL: https://www.theo2.co.uk/venues/o2-forum-kentish-town
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeO2Forum(city = 'London') {
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      const results = [];
      const eventItems = document.querySelectorAll('.event-card, .event, [class*="event"], article, .show-card');
      
      eventItems.forEach(item => {
        try {
          const titleEl = item.querySelector('h2, h3, h4, .title, .event-title, .show-title');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at O2 Forum Kentish Town`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid O2 Forum events`);
