/**
 * Royal Albert Hall London Events Scraper
 * URL: https://www.royalalberthall.com/tickets/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRoyalAlbertHall2(city = 'London') {
  console.log('🎭 Scraping Royal Albert Hall...');
  ...
  try {
    ...
    await page.goto('https://www.royalalberthall.com/tickets/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/tickets/"], .event, article').forEach(el => {
        const link = el.tagName === 'A' ? el : el.querySelector('a');
        if (!link) return;
        const href = link.href;
        if (seen.has(href) || href === 'https://www.royalalberthall.com/tickets/') return;
        seen.add(href);
...
module.exports = scrapeRoyalAlbertHall2;
