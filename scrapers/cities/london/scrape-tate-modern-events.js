/**
 * Tate Modern Events - london Events Scraper
 * URL: https://www.tate.org.uk/whats-on?gallery=tate-modern
 */
...
async function scrapeTateModernEvents(city = 'london') {
  console.log('🎭 Scraping Tate Modern Events...');
  try {
    const resp = await axios.get('https://www.tate.org.uk/whats-on?gallery=tate-modern', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Tate Modern Events', address: addr || 'Bankside, London SE1 9TG', city: 'london' },
            latitude: 51.5076,
            longitude: -0.0994,
            city: 'london',
            category: 'Events',
            source: 'Tate Modern Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.tate.org.uk/whats-on?gallery=tate-modern');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.tate.org.uk/whats-on?gallery=tate-modern').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Tate Modern Events', address: 'Bankside, London SE1 9TG', city: 'london' },
            latitude: 51.5076,
            longitude: -0.0994,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Tate Modern Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Tate Modern Events error:', error.message);
    return [];
  }
}

module.exports = scrapeTateModernEvents;
