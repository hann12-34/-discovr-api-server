/**
 * The Hope and Anchor London Events Scraper
 * URL: https://www.thehopeanch.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHopeAndAnchor(city = 'London') {
  console.log('🎸 Scraping The Hope and Anchor...');
  ...
  try {
    ...
    await page.goto('https://www.thehopeanch.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.thehopeanch.co.uk/events') return;
        seen.add(href);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Nightlife',
        source: 'The Hope and Anchor'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Hope and Anchor events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Hope and Anchor error:', error.message);
    return [];
  }
}

module.exports = scrapeHopeAndAnchor;
