/**
 * National Theatre London - london Events Scraper
...
async function scrapeNationalTheatreLondon(city = 'london') {
  console.log('🎭 Scraping National Theatre London...');
  try {
    const resp = await axios.get('https://www.nationaltheatre.org.uk/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'National Theatre London', address: addr || 'Upper Ground, London SE1 9PX', city: 'london' },
            latitude: 51.5071,
            longitude: -0.1138,
            city: 'london',
            category: 'Events',
            source: 'National Theatre London'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.nationaltheatre.org.uk/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.nationaltheatre.org.uk/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'National Theatre London', address: 'Upper Ground, London SE1 9PX', city: 'london' },
            latitude: 51.5071,
            longitude: -0.1138,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ National Theatre London:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ National Theatre London error:', error.message);
    return [];
  }
}

module.exports = scrapeNationalTheatreLondon;
