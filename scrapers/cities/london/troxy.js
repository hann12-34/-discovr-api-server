/**
 * Troxy London Events Scraper
 * URL: https://www.troxy.co.uk/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeTroxy(city = 'London') {
  console.log('🎵 Scraping Troxy London...');
  ...
  try {
    ...
    await page.goto('https://www.troxy.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.troxy.co.uk/events') return;
        seen.add(href);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Troxy',
          address: '490 Commercial Rd, London E1 0HX',
          city: 'London'
        },
        latitude: 51.5137,
        longitude: -0.0434,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Troxy events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Troxy error:', error.message);
    return [];
  }
}

module.exports = scrapeTroxy;
