...
async function scrapeOsloHackney(city = 'London') {
  console.log('🎵 Scraping Oslo Hackney...');
  ...
  try {
    ...
    await page.goto('https://www.oslohackney.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.oslohackney.com/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Oslo Hackney events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Oslo Hackney error:', error.message);
    return [];
  }
}
