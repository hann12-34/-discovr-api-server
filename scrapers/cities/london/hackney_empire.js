/**
 * Hackney Empire London Events Scraper
 * URL: https://hackneyempire.co.uk/whats-on/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHackneyEmpire(city = 'London') {
  console.log('🎭 Scraping Hackney Empire...');
  ...
  try {
    ...
    await page.goto('https://hackneyempire.co.uk/whats-on/', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/whats-on/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/whats-on/') || href === 'https://hackneyempire.co.uk/whats-on/') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Hackney Empire events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Hackney Empire error:', error.message);
    return [];
  }
}

module.exports = scrapeHackneyEmpire;
