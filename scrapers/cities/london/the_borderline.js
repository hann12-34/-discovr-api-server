/**
 * The Borderline London Events Scraper
 * URL: https://www.theborderlinelondon.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBorderline(city = 'London') {
  console.log('🎸 Scraping The Borderline...');
  ...
  try {
    ...
    await page.goto('https://www.theborderlinelondon.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.theborderlinelondon.com/events') return;
        seen.add(href);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Nightlife',
        source: 'The Borderline'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Borderline events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Borderline error:', error.message);
    return [];
  }
}

module.exports = scrapeBorderline;
