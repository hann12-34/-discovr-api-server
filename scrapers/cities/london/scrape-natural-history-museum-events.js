/**
 * Natural History Museum Events - london Events Scraper
 * URL: https://www.nhm.ac.uk/events.html
 */
...
async function scrapeNaturalHistoryMuseumEvents(city = 'london') {
  console.log('🎭 Scraping Natural History Museum Events...');
  try {
    const resp = await axios.get('https://www.nhm.ac.uk/events.html', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Natural History Museum Events', address: addr || 'Cromwell Rd, London SW7 5BD', city: 'london' },
            latitude: 51.4967,
            longitude: -0.1764,
            city: 'london',
            category: 'Events',
            source: 'Natural History Museum Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.nhm.ac.uk/events.html');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.nhm.ac.uk/events.html').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Natural History Museum Events', address: 'Cromwell Rd, London SW7 5BD', city: 'london' },
            latitude: 51.4967,
            longitude: -0.1764,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Natural History Museum Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Natural History Museum Events error:', error.message);
    return [];
  }
}

module.exports = scrapeNaturalHistoryMuseumEvents;
