/**
 * Camden Assembly London Events Scraper
 * URL: https://www.camdenassembly.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCamdenAssembly(city = 'London') {
  ...
  try {
    ...
    await page.goto('https://www.camdenassembly.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.camdenassembly.com/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Camden Assembly events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Camden Assembly error:', error.message);
    return [];
  }
}

module.exports = scrapeCamdenAssembly;
