/**
 * Somerset House - london Events Scraper
 * URL: https://www.somersethouse.org.uk/whats-on
 */
...
async function scrapeSomersetHouse(city = 'london') {
  console.log('🎭 Scraping Somerset House...');
  try {
    const resp = await axios.get('https://www.somersethouse.org.uk/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Somerset House', address: addr || 'Strand, London WC2R 1LA', city: 'london' },
            latitude: 51.5113,
            longitude: -0.1174,
            city: 'london',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.somersethouse.org.uk/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.somersethouse.org.uk/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Somerset House', address: 'Strand, London WC2R 1LA', city: 'london' },
            latitude: 51.5113,
            longitude: -0.1174,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Somerset House:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Somerset House error:', error.message);
    return [];
  }
}

module.exports = scrapeSomersetHouse;
