...
async function scrapeO2ForumKentish(city = 'London') {
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Nightlife',
        source: 'O2 Forum Kentish Town'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Forum Kentish events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Forum Kentish error:', error.message);
    return [];
  }
}

module.exports = scrapeO2ForumKentish;
