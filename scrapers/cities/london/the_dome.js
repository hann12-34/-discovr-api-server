/**
 * The Dome Tufnell Park London Events Scraper
 * URL: https://www.thedometufnellpark.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDome(city = 'London') {
  console.log('🎸 Scraping The Dome Tufnell Park...');
  ...
  try {
    ...
    await page.goto('https://www.thedometufnellpark.com/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.thedometufnellpark.com/events') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Dome events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Dome error:', error.message);
    return [];
  }
}

module.exports = scrapeDome;
