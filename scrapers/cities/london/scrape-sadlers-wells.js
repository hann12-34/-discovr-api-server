/**
 * Sadlers Wells - london Events Scraper
...
async function scrapeSadlersWells(city = 'london') {
  console.log('🎭 Scraping Sadlers Wells...');
  try {
    const resp = await axios.get('https://www.sadlerswells.com/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Sadlers Wells', address: addr || 'Rosebery Ave, London EC1R 4TN', city: 'london' },
            latitude: 51.5293,
            longitude: -0.106,
            city: 'london',
            category: 'Events',
            source: 'Sadlers Wells'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.sadlerswells.com/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.sadlerswells.com/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Sadlers Wells', address: 'Rosebery Ave, London EC1R 4TN', city: 'london' },
            latitude: 51.5293,
            longitude: -0.106,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Sadlers Wells:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Sadlers Wells error:', error.message);
    return [];
  }
}

module.exports = scrapeSadlersWells;
