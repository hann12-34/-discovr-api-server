/**
 * Southbank Centre Events - london Events Scraper
...
async function scrapeSouthbankCentreEvents(city = 'london') {
  console.log('🎭 Scraping Southbank Centre Events...');
  try {
    const resp = await axios.get('https://www.southbankcentre.co.uk/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Southbank Centre Events', address: addr || 'Belvedere Rd, London SE1 8XX', city: 'london' },
            latitude: 51.5071,
            longitude: -0.1163,
            city: 'london',
            category: 'Events',
            source: 'Southbank Centre Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.southbankcentre.co.uk/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.southbankcentre.co.uk/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Southbank Centre Events', address: 'Belvedere Rd, London SE1 8XX', city: 'london' },
            latitude: 51.5071,
            longitude: -0.1163,
            city: 'london',
            category: 'Events',
    ...
    console.log('  ✅ Southbank Centre Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Southbank Centre Events error:', error.message);
    return [];
  }
}

module.exports = scrapeSouthbankCentreEvents;
