/**
 * O2 Shepherd's Bush Empire London Events Scraper
...
async function scrapeO2ShepherdsBush(city = 'London') {
  console.log('🎵 Scraping O2 Shepherd\'s Bush Empire...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/events/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/events/') || href === 'https://www.academymusicgroup.com/o2shepherdsbushempire/events') return;
        seen.add(href);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'London',
        category: 'Concert',
        source: 'O2 Shepherd\'s Bush Empire'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Shepherd's Bush events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Shepherd\'s Bush error:', error.message);
    return [];
  }
}

module.exports = scrapeO2ShepherdsBush;
