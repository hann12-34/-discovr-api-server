/**
 * Southbank Centre London Events Scraper
...
async function scrapeSouthbank2(city = 'London') {
  ...
  try {
    ...
    await page.goto('https://www.southbankcentre.co.uk/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/whats-on/"], .event, article').forEach(el => {
        const link = el.tagName === 'A' ? el : el.querySelector('a');
        if (!link) return;
        const href = link.href;
        if (seen.has(href) || href === 'https://www.southbankcentre.co.uk/whats-on') return;
        seen.add(href);
...
module.exports = scrapeSouthbank2;
