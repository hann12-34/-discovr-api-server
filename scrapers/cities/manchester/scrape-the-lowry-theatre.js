/**
 * The Lowry Theatre - manchester Events Scraper
...
async function scrapeTheLowryTheatre(city = 'manchester') {
  console.log('🎭 Scraping The Lowry Theatre...');
  try {
    const resp = await axios.get('https://thelowry.com/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'The Lowry Theatre', address: addr || 'Pier 8, Salford Quays, Manchester M50 3AZ', city: 'manchester' },
            latitude: 53.4707,
            longitude: -2.2916,
            city: 'manchester',
            category: 'Events',
            source: 'The Lowry Theatre'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://thelowry.com/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://thelowry.com/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'The Lowry Theatre', address: 'Pier 8, Salford Quays, Manchester M50 3AZ', city: 'manchester' },
            latitude: 53.4707,
            longitude: -2.2916,
            city: 'manchester',
            category: 'Events',
    ...
    console.log('  ✅ The Lowry Theatre:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ The Lowry Theatre error:', error.message);
    return [];
  }
}

module.exports = scrapeTheLowryTheatre;
