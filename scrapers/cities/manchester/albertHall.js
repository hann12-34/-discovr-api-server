/**
 * Albert Hall Manchester Events Scraper
 * Historic live music venue in a converted church
 * URL: https://www.alberthallmanchester.com/whats-on/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAlbertHall(city = 'Manchester') {
  console.log('🏛️ Scraping Albert Hall Manchester...');
  ...
  try {
    ...
    await page.goto('https://www.alberthallmanchester.com/whats-on/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Albert Hall Manchester`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Albert Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Albert Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeAlbertHall;
