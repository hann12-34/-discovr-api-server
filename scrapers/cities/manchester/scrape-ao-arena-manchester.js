/**
 * AO Arena Manchester - manchester Events Scraper
...
async function scrapeAOArenaManchester(city = 'manchester') {
  console.log('🎭 Scraping AO Arena Manchester...');
  try {
    const resp = await axios.get('https://www.ao-arena.com/events', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'AO Arena Manchester', address: addr || 'Victoria Station Approach, Manchester M3 1AR', city: 'manchester' },
            latitude: 53.4881,
            longitude: -2.2444,
            city: 'manchester',
            category: 'Events',
            source: 'AO Arena Manchester'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.ao-arena.com/events');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.ao-arena.com/events').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'AO Arena Manchester', address: 'Victoria Station Approach, Manchester M3 1AR', city: 'manchester' },
            latitude: 53.4881,
            longitude: -2.2444,
            city: 'manchester',
            category: 'Events',
    ...
    console.log('  ✅ AO Arena Manchester:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ AO Arena Manchester error:', error.message);
    return [];
  }
}

module.exports = scrapeAOArenaManchester;
