...
async function scrapeManchesterFest(city = 'Manchester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Various Manchester Venues', address: 'Manchester City Centre, Manchester M1', city: 'Manchester' }, latitude: 53.4808, longitude: -2.2426, city: 'Manchester', category: 'Festival', source: 'MIF' });
    }
