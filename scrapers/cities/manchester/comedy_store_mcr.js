...
async function scrapeComedyStoreMCR(city = 'Manchester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Comedy Store Manchester', address: 'Deansgate Locks, Manchester', city: 'Manchester' }, latitude: 53.4717, longitude: -2.2516, city: 'Manchester', category: 'Nightlife', source: 'Comedy Store' });
    }
