/**
 * Parklife Festival Manchester Events Scraper
 * Major music festival
 * URL: https://parklife.uk.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeParklife(city = 'Manchester') {
  console.log('🎪 Scraping Parklife Festival...');
  ...
  try {
    ...
    await page.goto('https://parklife.uk.com/', {
    ...
    const events = await page.evaluate(() => {
      const results = [];
      const eventItems = document.querySelectorAll('.event-card, .event, [class*="event"], article, .lineup-item');
      
      eventItems.forEach(item => {
        try {
          const titleEl = item.querySelector('h1, h2, h3, h4, .title, .artist-name');
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Parklife Festival Manchester`,
        date: isoDate,
        ...
        city: 'Manchester',
        category: 'Nightlife',
        source: 'Parklife Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Parklife events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Parklife error:', error.message);
    return [];
  }
}

module.exports = scrapeParklife;
