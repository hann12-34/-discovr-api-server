/**
 * Mint Lounge Manchester - Nightclub
 * URL: https://www.mintlounge.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMintLounge(city = 'Manchester') {
  console.log('🎵 Scraping Mint Lounge...');
  ...
  try {
    ...
    await page.goto('https://www.mintlounge.co.uk/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('.event, article, [class*="event"], .show').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Mint Lounge',
          address: '46 Spring Gardens, Manchester M2 1EN',
          city: 'Manchester'
        },
        latitude: 53.4804,
        longitude: -2.2401,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Mint Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Mint Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeMintLounge;
