...
async function scrapeChinatownFest(city = 'Manchester') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Chinatown', address: 'Faulkner Street, Manchester', city: 'Manchester' }, latitude: 53.4770, longitude: -2.2382, city: 'Manchester', category: 'Festival', source: 'Chinatown Festival' });
    }
