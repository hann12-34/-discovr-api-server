...
async function scrapeJoshuaBrooks(city = 'Manchester') {
  console.log('🎵 Scraping Joshua Brooks...');
  ...
  try {
    ...
    await page.goto('https://www.joshuabrooks.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Manchester',
        category: 'Nightlife',
        source: 'Joshua Brooks'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Joshua Brooks events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Joshua Brooks error:', error.message);
    return [];
  }
}

module.exports = scrapeJoshuaBrooks;
