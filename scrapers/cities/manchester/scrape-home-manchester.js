/**
 * HOME Manchester - manchester Events Scraper
...
async function scrapeHOMEManchester(city = 'manchester') {
  ...
  try {
    const resp = await axios.get('https://homemcr.org/whats-on/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'HOME Manchester', address: addr || '2 Tony Wilson Pl, Manchester M15 4FN', city: 'manchester' },
            latitude: 53.4737,
            longitude: -2.2472,
            city: 'manchester',
            category: 'Events',
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://homemcr.org/whats-on/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://homemcr.org/whats-on/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'HOME Manchester', address: '2 Tony Wilson Pl, Manchester M15 4FN', city: 'manchester' },
            latitude: 53.4737,
            longitude: -2.2472,
            city: 'manchester',
            category: 'Events',
    ...
    console.log('  ✅ HOME Manchester:', futureEvents.length, 'events');
