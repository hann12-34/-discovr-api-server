/**
 * Bridgewater Hall - manchester Events Scraper
 * URL: https://www.bridgewater-hall.co.uk/whats-on
 */
...
async function scrapeBridgewaterHall(city = 'manchester') {
  console.log('🎭 Scraping Bridgewater Hall...');
  try {
    const resp = await axios.get('https://www.bridgewater-hall.co.uk/whats-on', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Bridgewater Hall', address: addr || 'Lower Mosley St, Manchester M2 3WS', city: 'manchester' },
            latitude: 53.476,
            longitude: -2.2444,
            city: 'manchester',
            category: 'Events',
            source: 'Bridgewater Hall'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.bridgewater-hall.co.uk/whats-on');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.bridgewater-hall.co.uk/whats-on').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Bridgewater Hall', address: 'Lower Mosley St, Manchester M2 3WS', city: 'manchester' },
            latitude: 53.476,
            longitude: -2.2444,
            city: 'manchester',
            category: 'Events',
    ...
    console.log('  ✅ Bridgewater Hall:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Bridgewater Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeBridgewaterHall;
