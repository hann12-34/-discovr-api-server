/**
 * Royal Northern College of Music Manchester Events Scraper
...
async function scrapeRNCM(city = 'Manchester') {
  console.log('🎼 Scraping Royal Northern College of Music...');
  ...
  try {
    ...
    await page.goto('https://www.rncm.ac.uk/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="/whats-on/"]').forEach(el => {
        const href = el.href;
        if (seen.has(href) || href.endsWith('/whats-on/') || href === 'https://www.rncm.ac.uk/whats-on') return;
        seen.add(href);
    ...
    console.log(`  ✅ Found ${formattedEvents.length} RNCM events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  RNCM error:', error.message);
    return [];
  }
}

module.exports = scrapeRNCM;
