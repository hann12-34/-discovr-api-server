/**
 * O2 Apollo Manchester Events Scraper
 * Major live music venue - Academy Music Group site
 * URL: https://www.academymusicgroup.com/o2apollomanchester/events
 */
...
async function scrapeO2ApolloManchester(city = 'Manchester') {
  console.log('🎸 Scraping O2 Apollo Manchester...');
  ...
  try {
    ...
    await page.goto('https://www.academymusicgroup.com/o2apollomanchester/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'O2 Apollo Manchester', address: 'Stockport Road, Ardwick Green, Manchester M12 6AP', city: 'Manchester' },
        latitude: 53.4697,
        longitude: -2.2189,
        city: 'Manchester',
        category: 'Nightlife',
        source: 'O2 Apollo Manchester'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} O2 Apollo Manchester events`);
    return filterEvents(formattedEvents);

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  O2 Apollo Manchester error:', error.message);
    return [];
  }
}

module.exports = scrapeO2ApolloManchester;
