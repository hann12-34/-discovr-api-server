...
async function scrapeJazzFest(city = 'Chicago') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Chicago Jazz Festival', address: 'Millennium Park, Chicago IL 60601', city: 'Chicago' }, latitude: 41.8826, longitude: -87.6226, city: 'Chicago', category: 'Festival', source: 'Chicago Jazz Festival' });
    }
