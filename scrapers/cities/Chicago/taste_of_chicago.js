...
async function scrapeTasteOfChicago(city = 'Chicago') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Taste of Chicago', address: 'Grant Park, Chicago IL 60605', city: 'Chicago' }, latitude: 41.8756, longitude: -87.6244, city: 'Chicago', category: 'Festival', source: 'Taste of Chicago' });
    }
