...
async function scrapePitchfork(city = 'Chicago') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Pitchfork Music Festival', address: 'Union Park, Chicago IL 60607', city: 'Chicago' }, latitude: 41.8799, longitude: -87.6733, city: 'Chicago', category: 'Festival', source: 'Pitchfork Festival' });
    }
