/**
 * Choose Chicago Events - Chicago Events Scraper
...
async function scrapeChooseChicagoEvents(city = 'Chicago') {
  console.log('🎭 Scraping Choose Chicago Events...');
  try {
    const resp = await axios.get('https://www.choosechicago.com/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Choose Chicago Events', address: addr || 'Chicago, IL', city: 'Chicago' },
            latitude: 41.8781,
            longitude: -87.6298,
            city: 'Chicago',
            category: 'Events',
            source: 'Choose Chicago Events'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.choosechicago.com/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.choosechicago.com/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Choose Chicago Events', address: 'Chicago, IL', city: 'Chicago' },
            latitude: 41.8781,
            longitude: -87.6298,
            city: 'Chicago',
            category: 'Events',
    ...
    console.log('  ✅ Choose Chicago Events:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Choose Chicago Events error:', error.message);
    return [];
  }
}

module.exports = scrapeChooseChicagoEvents;
