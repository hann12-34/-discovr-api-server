...
async function scrapeRiotFest(city = 'Chicago') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Riot Fest', address: 'Douglass Park, Chicago IL 60623', city: 'Chicago' }, latitude: 41.8603, longitude: -87.7028, city: 'Chicago', category: 'Festival', source: 'Riot Fest' });
    }
