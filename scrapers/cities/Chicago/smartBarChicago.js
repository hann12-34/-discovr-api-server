 ...
 * URL: https://www.smartbarchicago.com/events
 * Iconic underground club in Wrigleyville
...
const SmartBarChicagoEvents = {
  async scrape(city = 'Chicago') {
    console.log('🌬️ Scraping Smart Bar Chicago...');

    try {
      const response = await axios.get('https://www.smartbarchicago.com/events', {
        headers: {
      ...
      $('.eventWrapper').each((i, el) => {
        ...
        if (!url) return;
        if (!url.startsWith('http')) url = 'https://smartbarchicago.com' + url;
        ...
        events.push({
          ...
          city: 'Chicago',
          category: 'Music',
          source: 'Smart Bar Chicago'
        });
      });

      console.log(`  ✅ Found ${events.length} Smart Bar events`);
      return events;
...
module.exports = SmartBarChicagoEvents.scrape;
