/**
 * Concord Music Hall Chicago Events Scraper
 * Major live music venue
 * URL: https://concordmusichall.com/
 */
...
async function scrapeConcord(city = 'Chicago') {
  console.log('🎸 Scraping Concord Music Hall Chicago...');
  ...
  try {
    ...
    await page.goto('https://concordmusichall.com/', {
    ...
    const events = await page.evaluate(() => {
      ...
      const selectors = ['.event', '.event-card', 'article', '[class*="event"]', '.show', '.tw-plugin-event'];

      for (const selector of selectors) {
        document.querySelectorAll(selector).forEach(el => {
          try {
            const titleEl = el.querySelector('h1, h2, h3, h4, .title, [class*="title"], .headliner');
    ...
    await browser.close();
    console.log(`  ✅ Found ${events.length} Concord events`);
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: { name: 'Concord Music Hall', address: '2047 N Milwaukee Ave, Chicago IL 60647', city: 'Chicago' },
        latitude: 41.9189,
        longitude: -87.6876,
        city: 'Chicago',
        category: 'Nightlife',
        source: 'Concord Music Hall'
      });
    }
    ...
    console.error('  ⚠️  Concord error:', error.message);
    return [];
  }
}

module.exports = scrapeConcord;
