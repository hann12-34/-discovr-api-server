...
async function scrapeAirWaterShow(city = 'Chicago') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Chicago Air and Water Show', address: 'North Avenue Beach, Chicago IL 60614', city: 'Chicago' }, latitude: 41.9165, longitude: -87.6269, city: 'Chicago', category: 'Festival', source: 'Chicago Air Water Show' });
    }
