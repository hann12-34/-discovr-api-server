/**
 * Aragon Ballroom Chicago Events Scraper
 * URL: https://aragonballroom.org/events/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAragon(city = 'Chicago') {
  console.log('🎸 Scraping Aragon Ballroom Chicago...');
  ...
  try {
    ...
    await page.goto('https://aragonballroom.org/events/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Chicago',
        category: 'Concert',
        source: 'Aragon Ballroom'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Aragon events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Aragon error:', error.message);
    return [];
  }
}

module.exports = scrapeAragon;
