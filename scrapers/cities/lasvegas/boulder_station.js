/**
 * Boulder Station Las Vegas Entertainment Scraper
 * URL: https://www.boulderstation.sclv.com/entertainment
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeBoulderStation(city = 'Las Vegas') {
  console.log('🏔️ Scraping Boulder Station...');
  ...
  try {
    ...
    await page.goto('https://www.boulderstation.sclv.com/entertainment', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Boulder Station'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Boulder Station events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Boulder Station error:', error.message);
    return [];
  }
}

module.exports = scrapeBoulderStation;
