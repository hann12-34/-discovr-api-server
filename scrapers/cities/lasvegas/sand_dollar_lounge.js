/**
 * Sand Dollar Lounge Las Vegas Events Scraper
 * URL: https://www.sanddollarlounge.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSandDollar(city = 'Las Vegas') {
  console.log('🎵 Scraping Sand Dollar Lounge...');
  ...
  try {
    ...
    await page.goto('https://www.sanddollarlounge.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Sand Dollar Lounge'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Sand Dollar Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sand Dollar Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeSandDollar;
