/**
 * Downtown Grand Las Vegas Entertainment Scraper
 * URL: https://www.downtowngrand.com/entertainment
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDowntownGrand(city = 'Las Vegas') {
  console.log('🏙️ Scraping Downtown Grand...');
  ...
  try {
    ...
    await page.goto('https://www.downtowngrand.com/entertainment', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Downtown Grand'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Downtown Grand events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Downtown Grand error:', error.message);
    return [];
  }
}

module.exports = scrapeDowntownGrand;
