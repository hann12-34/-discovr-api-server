/**
 * Palace Station Las Vegas Entertainment Scraper
 * URL: https://www.palacestation.sclv.com/entertainment
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePalaceStation(city = 'Las Vegas') {
  console.log('🏰 Scraping Palace Station...');
  ...
  try {
    ...
    await page.goto('https://www.palacestation.sclv.com/entertainment', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Palace Station'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Palace Station events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Palace Station error:', error.message);
    return [];
  }
}

module.exports = scrapePalaceStation;
