/**
 * Downtown Container Park Las Vegas Events Scraper
 * URL: https://downtowncontainerpark.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeContainerPark(city = 'Las Vegas') {
  console.log('📦 Scraping Container Park...');
  ...
  try {
    ...
    await page.goto('https://downtowncontainerpark.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Festival',
        source: 'Container Park'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Container Park events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Container Park error:', error.message);
    return [];
  }
}

module.exports = scrapeContainerPark;
