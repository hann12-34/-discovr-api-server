/**
 * Orleans Arena Las Vegas Events Scraper
 * URL: https://www.orleansarena.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOrleansArena(city = 'Las Vegas') {
  console.log('🏟️ Scraping Orleans Arena...');
  ...
  try {
    ...
    await page.goto('https://www.orleansarena.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Festival',
        source: 'Orleans Arena'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Orleans Arena events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Orleans Arena error:', error.message);
    return [];
  }
}

module.exports = scrapeOrleansArena;
