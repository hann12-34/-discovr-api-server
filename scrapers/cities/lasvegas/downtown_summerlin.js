/**
 * Downtown Summerlin Events Scraper
 * URL: https://www.summerlin.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSummerlin(city = 'Las Vegas') {
  console.log('🏘️ Scraping Downtown Summerlin...');
  ...
  try {
    ...
    await page.goto('https://www.summerlin.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Festival',
        source: 'Downtown Summerlin'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Downtown Summerlin events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Downtown Summerlin error:', error.message);
    return [];
  }
}

module.exports = scrapeSummerlin;
