/**
 * Zappos Theater at Planet Hollywood Events Scraper
 * URL: https://www.caesars.com/planet-hollywood/shows
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeZapposTheater(city = 'Las Vegas') {
  console.log('👞 Scraping Zappos Theater...');
  ...
  try {
    ...
    await page.goto('https://www.caesars.com/planet-hollywood/shows', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Zappos Theater'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Zappos Theater events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Zappos Theater error:', error.message);
    return [];
  }
}

module.exports = scrapeZapposTheater;
