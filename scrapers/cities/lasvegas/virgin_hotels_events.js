/**
 * Virgin Hotels Las Vegas Complete Events Calendar Scraper
 * URL: https://www.virginhotelslv.com/events-calendar
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVirginEvents(city = 'Las Vegas') {
  console.log('🎉 Scraping Virgin Hotels Events...');
  ...
  try {
    ...
    await page.goto('https://www.virginhotelslv.com/events-calendar', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Virgin Hotels Events'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Virgin Hotels Events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Virgin Hotels Events error:', error.message);
    return [];
  }
}

module.exports = scrapeVirginEvents;
