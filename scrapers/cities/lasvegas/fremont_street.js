/**
 * Fremont Street Experience Las Vegas Events Scraper
 * URL: https://vegasexperience.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFremont(city = 'Las Vegas') {
  console.log('🎸 Scraping Fremont Street...');
  ...
  try {
    ...
    await page.goto('https://vegasexperience.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Festival',
        source: 'Fremont Street'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Fremont Street events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Fremont Street error:', error.message);
    return [];
  }
}

module.exports = scrapeFremont;
