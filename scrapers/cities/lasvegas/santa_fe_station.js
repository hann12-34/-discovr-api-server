/**
 * Santa Fe Station Las Vegas Entertainment Scraper
 * URL: https://www.santafestation.sclv.com/entertainment
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSantaFe(city = 'Las Vegas') {
  console.log('🏜️ Scraping Santa Fe Station...');
  ...
  try {
    ...
    await page.goto('https://www.santafestation.sclv.com/entertainment', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Santa Fe Station'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Santa Fe Station events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Santa Fe Station error:', error.message);
    return [];
  }
}

module.exports = scrapeSantaFe;
