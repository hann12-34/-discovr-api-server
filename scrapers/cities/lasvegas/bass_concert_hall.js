/**
 * Reynolds Hall at The Smith Center Las Vegas Events Scraper
 * URL: https://www.thesmithcenter.com/events/reynolds-hall
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeReynoldsHall(city = 'Las Vegas') {
  console.log('🎭 Scraping Reynolds Hall...');
  ...
  try {
    ...
    await page.goto('https://www.thesmithcenter.com/events/reynolds-hall', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Reynolds Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Reynolds Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Reynolds Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeReynoldsHall;
