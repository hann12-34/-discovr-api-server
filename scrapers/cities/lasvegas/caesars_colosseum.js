/**
 * The Colosseum at Caesars Palace Events Scraper
 * URL: https://www.caesars.com/caesars-palace/shows
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCaesarsColosseum(city = 'Las Vegas') {
  console.log('🏛️ Scraping Colosseum at Caesars Palace...');
  ...
  try {
    ...
    await page.goto('https://www.caesars.com/caesars-palace/shows', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="shows"], .event-card, article, [class*="show"]').forEach(el => {
        try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Caesars Colosseum'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Colosseum events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Colosseum error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCaesarsColosseum;
