/**
 * Virgin Hotels Las Vegas Events Scraper
 * URL: https://virginhotelslv.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVirginHotels(city = 'Las Vegas') {
  console.log('🎸 Scraping Virgin Hotels LV...');
  ...
  try {
    ...
    await page.goto('https://virginhotelslv.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Las Vegas',
        category: 'Nightlife',
        source: 'Virgin Hotels LV'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Virgin Hotels LV events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Virgin Hotels LV error:', error.message);
    return [];
  }
}

module.exports = scrapeVirginHotels;
