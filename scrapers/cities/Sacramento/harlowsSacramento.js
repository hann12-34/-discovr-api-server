/**
 * Harlow's Sacramento Events Scraper
 * URL: https://www.harlows.com/calendar
 * Live music venue in East Sacramento
 * Rock, indie, hip-hop, jazz, electronic
...
const HarlowsSacramentoEvents = {
  async scrape(city = 'Sacramento') {
    console.log('🎸 Scraping Harlow\'s Sacramento...');

    try {
      const response = await axios.get('https://www.harlows.com/calendar', {
        headers: {
      ...
      for (let i = 0; i < allTitles.length; i++) {
        ...
        let url = allUrls[i] || '';
        if (!url) continue;
        if (!url.startsWith('http')) url = 'https://www.harlows.com' + url;
        ...
        events.push({
          ...
          city: 'Sacramento',
          category: 'Music',
          source: "Harlow's Sacramento"
        });
      }

      console.log(`  ✅ Found ${events.length} Harlow's events`);
      return events;

    } catch (error) {
      console.error(`  ⚠️ Harlow's error: ${error.message}`);
      return [];
    }
  }
};

module.exports = HarlowsSacramentoEvents.scrape;
