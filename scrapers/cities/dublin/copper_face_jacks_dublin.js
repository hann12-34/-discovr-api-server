/**
 * Copper Face Jacks Dublin Events Scraper
 * Iconic nightclub in Dublin
 * URL: https://copperfacejacks.ie/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCopperFaceJacksDublin(city = 'Dublin') {
  console.log('🍀 Scraping Copper Face Jacks Dublin...');
  ...
  try {
    ...
    await page.goto('https://copperfacejacks.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: '29-30 Harcourt St, Dublin 2, D02 XR29',
          city: 'Dublin'
        },
        city: 'Dublin',
        category: 'Nightlife',
        source: 'Copper Face Jacks'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Copper Face Jacks events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Copper Face Jacks error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCopperFaceJacksDublin;
