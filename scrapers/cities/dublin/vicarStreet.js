/**
 * Vicar Street Dublin Events Scraper
 * Major live music venue
 * URL: https://www.vicarstreet.ie/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVicarStreet(city = 'Dublin') {
  console.log('🎸 Scraping Vicar Street Dublin...');
  ...
  try {
    ...
    await page.goto('https://www.vicarstreet.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Vicar Street Dublin`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Vicar Street events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Vicar Street error:', error.message);
    return [];
  }
}

module.exports = scrapeVicarStreet;
