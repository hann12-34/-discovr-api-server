/**
 * Academy Dublin Events Scraper
 * Live music venue on Middle Abbey Street
 * URL: https://www.theacademydublin.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAcademyDublin(city = 'Dublin') {
  console.log('🎸 Scraping Academy Dublin...');
  ...
  try {
    ...
    await page.goto('https://www.theacademydublin.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at Academy Dublin`,
        date: isoDate,
        ...
        venue: {
          name: 'Academy Dublin',
          address: '57 Middle Abbey Street, Dublin D01 X002',
          city: 'Dublin'
        },
        latitude: 53.3487,
        longitude: -6.2617,
        city: 'Dublin',
        category: 'Nightlife',
        source: 'Academy Dublin'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Academy Dublin events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Academy Dublin error:', error.message);
    return [];
  }
}

module.exports = scrapeAcademyDublin;
