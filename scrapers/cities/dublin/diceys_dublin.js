/**
 * Diceys Dublin Events Scraper
 * Popular nightclub in Dublin
 * URL: https://diceys.ie/
 */
...
async function scrapeDiceysDublin(city = 'Dublin') {
  console.log('🎲 Scraping Diceys Dublin...');
  ...
  try {
    ...
    await page.goto('https://diceys.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Diceys',
          address: '21-25 Harcourt St, Saint Kevin\'s, Dublin',
          city: 'Dublin'
        },
        city: 'Dublin',
        category: 'Nightlife',
        source: 'Diceys Dublin'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Diceys events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Diceys error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeDiceysDublin;
