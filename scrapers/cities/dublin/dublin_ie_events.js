/**
 * Dublin.ie Events Scraper
 * URL: https://dublin.ie/whats-on/
 * Official Dublin tourism events
 */
...
async function scrapeDublinIeEvents(city = 'Dublin') {
  console.log('🍀 Scraping Dublin.ie Events...');
  let browser;
  ...
  try {
    ...
    await page.goto('https://dublin.ie/whats-on/', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(3000);
    ...
    const eventLinks = await page.evaluate(() => {
      ...
      anchors.forEach(a => {
        ...
        if (href && !href.endsWith('/whats-on/') && !href.endsWith('/whats-on')) {
          const fullUrl = href.startsWith('http') ? href : `https://dublin.ie${href}`;
    ...
    // Limit to 40 links to prevent hanging
    const limitedLinks = eventLinks.slice(0, 40);
    console.log(`  Processing ${limitedLinks.length} links...`);
    ...
    for (const url of limitedLinks) {
      try {
        ...
        events.push({
          ...
          venue: {
            name: eventData.location ? eventData.location.split(',')[0].trim() : 'Dublin',
            address: eventData.location || 'Dublin, Ireland',
            city: 'Dublin'
          },
          latitude: 53.3498,
          longitude: -6.2603,
    ...
    console.log(`  ✅ Found ${unique.length} Dublin.ie events`);
    return unique;
  } catch (error) {
    console.error(`  ⚠️ Dublin.ie Events error: ${error.message}`);
    if (browser) await browser.close();
    return [];
  }
}

module.exports = scrapeDublinIeEvents;
