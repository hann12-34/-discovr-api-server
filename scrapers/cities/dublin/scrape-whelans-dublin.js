/**
 * Whelans Dublin - dublin Events Scraper
 * URL: https://www.whelanslive.com/events/
 */
...
async function scrapeWhelansDublin(city = 'dublin') {
  console.log('🎭 Scraping Whelans Dublin...');
  try {
    const resp = await axios.get('https://www.whelanslive.com/events/', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: 'Whelans Dublin', address: addr || '25 Wexford St, Dublin 2', city: 'dublin' },
            latitude: 53.3365,
            longitude: -6.2642,
            city: 'dublin',
            category: 'Events',
            source: 'Whelans Dublin'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (eventUrl && !eventUrl.startsWith('http')) {
            const base = new URL('https://www.whelanslive.com/events/');
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, 'https://www.whelanslive.com/events/').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: 'Whelans Dublin', address: '25 Wexford St, Dublin 2', city: 'dublin' },
            latitude: 53.3365,
            longitude: -6.2642,
            city: 'dublin',
            category: 'Events',
    ...
    console.log('  ✅ Whelans Dublin:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ Whelans Dublin error:', error.message);
    return [];
  }
}

module.exports = scrapeWhelansDublin;
