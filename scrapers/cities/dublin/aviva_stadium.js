/**
 * Aviva Stadium Dublin Events Scraper
 * URL: https://avivastadium.ie/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAvivaStadium(city = 'Dublin') {
  console.log('🏟️ Scraping Aviva Stadium...');
  ...
  try {
    ...
    await page.goto('https://avivastadium.ie/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Dublin',
        category: 'Arts',
        source: 'Aviva Stadium'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Aviva Stadium events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Aviva Stadium error:', error.message);
    return [];
  }
}

module.exports = scrapeAvivaStadium;
