/**
 * Factory function to generate a standalone venue scraper.
 * Returns the JS source code string for a scraper file.
 * 
 * Two modes: 'axios' (default) and 'puppeteer' (for JS-rendered sites)
 */
function generateScraperCode(venue, mode = 'axios') {
  const safeName = venue.name.replace(/[^a-zA-Z0-9]/g, '');
  const fnName = `scrape${safeName}`;

  if (mode === 'puppeteer') return generatePuppeteerScraper(venue, safeName, fnName);

  return `/**
 * ${venue.name} - ${venue.city} Events Scraper
 * URL: ${venue.url}
 */
...
async function ${fnName}(city = '${venue.city}') {
  console.log('🎭 Scraping ${venue.name.replace(/'/g, "\\'")}...');
  try {
    const resp = await axios.get('${venue.url}', {
      ...
      try {
        ...
        for (const item of items) {
          ...
          events.push({
            ...
            imageUrl: img,
            venue: { name: '${venue.name.replace(/'/g, "\\'")}', address: addr || '${venue.address.replace(/'/g, "\\'")}', city: '${venue.city}' },
            ${venue.lat ? `latitude: ${venue.lat},` : ''}
            ${venue.lng ? `longitude: ${venue.lng},` : ''}
            city: '${venue.city}',
            category: '${venue.category || 'Events'}',
            source: '${venue.name.replace(/'/g, "\\'")}'
          });
        }
      } catch (e) {}
    });
      ...
      $events.each((_, el) => {
        try {
          ...
          if (img && !img.startsWith('http')) {
            try { img = new URL(img, '${venue.url}').href; } catch(e) { img = null; }
          }
          ...
          events.push({
            ...
            url: eventUrl,
            imageUrl: img,
            venue: { name: '${venue.name.replace(/'/g, "\\'")}', address: '${venue.address.replace(/'/g, "\\'")}', city: '${venue.city}' },
            ${venue.lat ? `latitude: ${venue.lat},` : ''}
            ${venue.lng ? `longitude: ${venue.lng},` : ''}
            city: '${venue.city}',
            category: '${venue.category || 'Events'}',
    ...
    console.log('  ✅ ${venue.name.replace(/'/g, "\\'")}:', futureEvents.length, 'events');
    return filterEvents(futureEvents);
  } catch (error) {
    console.error('  ⚠️ ${venue.name.replace(/'/g, "\\'")} error:', error.message);
    return [];
  }
}
...
/**
 * Generate a Puppeteer-based scraper for JS-rendered sites
 */
function generatePuppeteerScraper(venue, safeName, fnName) {
  return `/**
 * ${venue.name} - ${venue.city} Events Scraper (Puppeteer)
 * URL: ${venue.url}
 */
...
async function ${fnName}(city = '${venue.city}') {
  console.log('🎭 Scraping ${venue.name.replace(/'/g, "\\'")} (Puppeteer)...');
  let browser;
  try {
    ...
    const rawEvents = await page.evaluate(() => {
      ...
      eventEls.forEach(el => {
        try {
          ...
          if (img && /logo|icon|placeholder|default\\./i.test(img)) img = '';
    ...
    for (const raw of rawEvents) {
      ...
      if (eventUrl && !eventUrl.startsWith('http')) {
        try { eventUrl = new URL(eventUrl, '${venue.url}').href; } catch(e) { eventUrl = ''; }
      }
      ...
      events.push({
        ...
        url: eventUrl,
        imageUrl: img,
        venue: { name: '${venue.name.replace(/'/g, "\\'")}', address: raw.address || '${venue.address.replace(/'/g, "\\'")}', city: '${venue.city}' },
        ${venue.lat ? `latitude: ${venue.lat},` : ''}
        ${venue.lng ? `longitude: ${venue.lng},` : ''}
        city: '${venue.city}',
        category: '${venue.category || 'Events'}',
...
module.exports = { generateScraperCode };
