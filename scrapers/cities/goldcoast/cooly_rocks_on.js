/**
 * Cooly Rocks On Festival Gold Coast Events Scraper
...
async function scrapeCoolyRocks(city = 'Goldcoast') {
  console.log('🎸 Scraping Cooly Rocks On...');
  ...
  try {
    ...
    await page.goto('https://www.coolyrockson.com/program', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.coolyrockson.com', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Coolangatta',
          address: 'Marine Parade, Coolangatta QLD',
          city: 'Goldcoast'
        },
        latitude: -28.1677,
        longitude: 153.5367,
        city: 'Goldcoast',
        category: 'Festival',
        source: 'Cooly Rocks On'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cooly Rocks On events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cooly Rocks On error:', error.message);
    return [];
  }
}

module.exports = scrapeCoolyRocks;
