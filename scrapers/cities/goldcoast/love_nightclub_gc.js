/**
 * Love Nightclub Gold Coast Events Scraper
 * Popular nightclub in Surfers Paradise
 * URL: https://lovenightclub.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLoveNightclubGc(city = 'Gold Coast') {
  console.log('💕 Scraping Love Nightclub Gold Coast...');
  ...
  try {
    ...
    await page.goto('https://lovenightclub.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Gold Coast',
        category: 'Nightlife',
        source: 'Love Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Love Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Love Nightclub error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeLoveNightclubGc;
