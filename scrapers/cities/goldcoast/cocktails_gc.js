/**
 * Cocktails Gold Coast Events Scraper
 * Popular nightclub in Gold Coast
 * URL: https://cocktailsnightclub.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCocktailsGC(city = 'Gold Coast') {
  console.log('🍸 Scraping Cocktails Gold Coast...');
  ...
  try {
    ...
    await page.goto('https://cocktailsnightclub.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Cocktails',
          address: '2 Orchid Ave, Surfers Paradise QLD 4217',
          city: 'Gold Coast'
        },
        city: 'Gold Coast',
        category: 'Nightlife',
        source: 'Cocktails Gold Coast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cocktails events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Cocktails error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCocktailsGC;
