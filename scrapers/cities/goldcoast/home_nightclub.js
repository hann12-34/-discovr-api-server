/**
 * Home Nightclub Gold Coast Events Scraper
 * URL: https://www.homegoldcoast.com.au/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeHomeNightclub(city = 'Gold Coast') {
  console.log('🏠 Scraping Home Nightclub...');
  ...
  try {
    ...
    await page.goto('https://www.homegoldcoast.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Gold Coast',
        category: 'Nightlife',
        source: 'Home Nightclub'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Home Nightclub events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Home Nightclub error:', error.message);
    return [];
  }
}

module.exports = scrapeHomeNightclub;
