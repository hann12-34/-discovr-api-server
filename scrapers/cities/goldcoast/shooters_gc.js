/**
 * Shooters Saloon Gold Coast Events Scraper
 * URL: https://www.shooterssaloon.com.au
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeShootersGC(city = 'Goldcoast') {
  console.log('🤠 Scraping Shooters GC...');
  ...
  try {
    ...
    await page.goto('https://www.shooterssaloon.com.au/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Shooters Saloon',
          address: 'Orchid Avenue, Surfers Paradise QLD',
          city: 'Goldcoast'
        },
        latitude: -28.0010,
        longitude: 153.4290,
        city: 'Goldcoast',
        category: 'Nightlife',
        source: 'Shooters Saloon'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Shooters events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Shooters error:', error.message);
    return [];
  }
}

module.exports = scrapeShootersGC;
