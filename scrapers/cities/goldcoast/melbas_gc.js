/**
 * Melbas Gold Coast Events Scraper
 * Popular nightclub in Gold Coast
 * URL: https://melbas.com.au/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeMelbasGC(city = 'Gold Coast') {
  console.log('🎵 Scraping Melbas Gold Coast...');
  ...
  try {
    ...
    await page.goto('https://melbas.com.au/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Melbas',
          address: '46 Cavill Ave, Surfers Paradise QLD 4217',
          city: 'Gold Coast'
        },
        city: 'Gold Coast',
        category: 'Nightlife',
        source: 'Melbas Gold Coast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Melbas events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Melbas error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeMelbasGC;
