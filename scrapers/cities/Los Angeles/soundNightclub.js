      ...
      links.forEach(link => {
        ...
        if (dateMatch) {
          ...
          if (title && title.length > 2) {
            ...
            if (!seen.has(key)) {
              seen.add(key);
              const container = link.closest('article, [class*="event"], [class*="show"], li') || link.parentElement;
              const imgEl = container ? container.querySelector('img[src]:not([src*="logo"]):not([src*="icon"])') : null;
