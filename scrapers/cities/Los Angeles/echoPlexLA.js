...
async function scrapeEchoPlex(city = 'Los Angeles') {
  ...
  try {
        ...
        if (title && dateMatch) {
          ...
          if (month) {
            ...
            if (!seen.has(title + isoDate)) {
              ...
              const linkEl = item.querySelector('a[href*="spaceland"], a[href*="/event"], a[href]');
              const href = linkEl ? linkEl.href : '';
              const imgEl = item.querySelector('img[src]:not([src*="logo"]):not([src*="icon"])');
              const imageUrl = imgEl ? imgEl.src : null;
              results.push({ title: title.substring(0, 100), date: isoDate, venue, url: href, imageUrl });
            }
          }
        }
      });
    ...
    const formattedEvents = events.map(event => ({
      ...
      url: event.url || 'https://www.spacelandpresents.com',
