...
async function scrapeResidentAdvisorLA(city = 'Los Angeles') {
  ...
  try {
        ...
        if (title && dateMatch) {
          ...
          if (month) {
            ...
            if (!seen.has(title + isoDate)) {
              ...
              const linkEl = item.querySelector('a[href*="ra.co"], a[href*="/events/"], a[href]');
              const href = linkEl ? linkEl.href : '';
              const imgEl = item.querySelector('img[src]:not([src*="logo"]):not([src*="icon"])');
              const imageUrl = imgEl ? imgEl.src : null;
              results.push({ title: title.substring(0, 100), date: isoDate, venue: venue.substring(0, 80), url: href, imageUrl });
            }
          }
        }
      });
    ...
    const formattedEvents = allEvents.map(event => ({
      ...
      url: event.url || 'https://ra.co/events/us/losangeles',
