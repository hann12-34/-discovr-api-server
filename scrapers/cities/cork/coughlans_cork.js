/**
 * Coughlan's Live Cork Events Scraper
 * URL: https://www.coughlans.ie
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCoughlans(city = 'Cork') {
  console.log('🎵 Scraping Coughlans...');
  ...
  try {
    ...
    await page.goto('https://www.coughlans.ie/live', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"], .gig').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Coughlans events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Coughlans error:', error.message);
    return [];
  }
}

module.exports = scrapeCoughlans;
