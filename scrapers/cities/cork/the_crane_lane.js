/**
 * The Crane Lane Theatre Cork Events Scraper
 * URL: https://www.thecranelane.ie/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCraneLane(city = 'Cork') {
  console.log('🎭 Scraping The Crane Lane...');
  ...
  try {
    ...
    await page.goto('https://www.thecranelane.ie/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-item', '.event', 'article', '[class*="show"]', '[class*="event"]', 'a[href*="/event"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Cork',
        category: 'Arts',
        source: 'The Crane Lane'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Crane Lane events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Crane Lane error:', error.message);
    return [];
  }
}

module.exports = scrapeCraneLane;
