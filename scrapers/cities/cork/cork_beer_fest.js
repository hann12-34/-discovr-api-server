/**
 * Cork Oktoberfest Events Scraper
 * URL: https://www.corkevents.ie
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCorkBeer(city = 'Cork') {
  console.log('🍺 Scraping Cork Beer Fest...');
  ...
  try {
    ...
    await page.goto('https://www.corkevents.ie', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.corkevents.ie', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.error('  ⚠️  Cork Beer error:', error.message);
    return [];
  }
}

module.exports = scrapeCorkBeer;
