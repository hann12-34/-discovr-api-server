/**
 * Live at St Luke's Cork Events Scraper
 * URL: https://www.liveatstlukes.ie/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeStLukes(city = 'Cork') {
  console.log('⛪ Scraping Live at St Luke\'s...');
  ...
  try {
    ...
    await page.goto('https://www.liveatstlukes.ie/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Live at St Luke\'s',
          address: 'St Luke\'s, Douglas St, Centre, Cork',
          city: 'Cork'
        },
        latitude: 51.8965,
        longitude: -8.4680,
        city: 'Cork',
        category: 'Arts',
        source: 'Live at St Luke\'s'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} St Luke's events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  St Luke\'s error:', error.message);
    return [];
  }
}

module.exports = scrapeStLukes;
