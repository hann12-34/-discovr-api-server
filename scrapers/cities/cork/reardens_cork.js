/**
 * Reardens Cork Events Scraper
 * Popular nightclub in Cork
 * URL: https://reardens.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeReardensCork(city = 'Cork') {
  console.log('🍀 Scraping Reardens Cork...');
  ...
  try {
    ...
    await page.goto('https://reardens.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Reardens',
          address: '26 Washington St, Cork, T12 EW26',
          city: 'Cork'
        },
        city: 'Cork',
        category: 'Nightlife',
        source: 'Reardens Cork'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Reardens events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Reardens error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeReardensCork;
