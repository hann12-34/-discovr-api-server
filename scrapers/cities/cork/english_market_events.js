/**
 * English Market Cork Events Scraper
 * URL: https://www.englishmarket.ie
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEnglishMarket(city = 'Cork') {
  console.log('🛍️ Scraping English Market...');
  ...
  try {
    ...
    await page.goto('https://www.englishmarket.ie/events', {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], .event, article, [class*="event"]').forEach(el => {
        try {
          ...
          results.push({ title, url: url || 'https://www.englishmarket.ie', dateStr, imageUrl });
        } catch (e) {}
      });
    ...
    console.log(`  ✅ Found ${formattedEvents.length} English Market events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  English Market error:', error.message);
    return [];
  }
}

module.exports = scrapeEnglishMarket;
