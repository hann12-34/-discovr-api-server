/**
 * Cypress Avenue Nightclub Cork Events Scraper
 * URL: https://www.cypressavenue.ie
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCypressAvenueClub(city = 'Cork') {
  console.log('🎉 Scraping Cypress Avenue...');
  ...
  try {
    ...
    await page.goto('https://www.cypressavenue.ie/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Cork',
        category: 'Nightlife',
        source: 'Cypress Avenue'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cypress Avenue events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cypress Avenue error:', error.message);
    return [];
  }
}

module.exports = scrapeCypressAvenueClub;
