...
async function scrapeMarquee(city = 'Cork') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Live at the Marquee', address: 'Marina Park, Cork', city: 'Cork' }, latitude: 51.8959, longitude: -8.4425, city: 'Cork', category: 'Festival', source: 'Live at the Marquee' });
    }
