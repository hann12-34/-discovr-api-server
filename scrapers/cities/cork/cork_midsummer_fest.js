/**
 * Cork Midsummer Festival Events Scraper
 * URL: https://www.corkmidsummer.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCorkMidsummer(city = 'Cork') {
  console.log('☀️ Scraping Cork Midsummer Festival...');
  ...
  try {
    ...
    await page.goto('https://www.corkmidsummer.com/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Cork',
        category: 'Festival',
        source: 'Cork Midsummer'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cork Midsummer events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cork Midsummer error:', error.message);
    return [];
  }
}

module.exports = scrapeCorkMidsummer;
