/**
 * The Pavilion Cork Events Scraper
 * Popular nightclub in Cork
 * URL: https://thepavilion.ie/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeThePavilionCork(city = 'Cork') {
  console.log('🎪 Scraping The Pavilion Cork...');
  ...
  try {
    ...
    await page.goto('https://thepavilion.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'The Pavilion',
          address: 'Carey\'s Ln, Cork, T12 W8HY',
          city: 'Cork'
        },
        city: 'Cork',
        category: 'Nightlife',
        source: 'The Pavilion Cork'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} The Pavilion events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ The Pavilion error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeThePavilionCork;
