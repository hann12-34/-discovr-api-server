/**
 * Liquid Lounge Cork Events Scraper
 * URL: https://www.liquidlive.ie
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLiquidLounge(city = 'Cork') {
  console.log('🎉 Scraping Liquid Lounge...');
  ...
  try {
    ...
    await page.goto('https://www.liquidlive.ie/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Cork',
        category: 'Nightlife',
        source: 'Liquid Lounge'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Liquid Lounge events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Liquid Lounge error:', error.message);
    return [];
  }
}

module.exports = scrapeLiquidLounge;
