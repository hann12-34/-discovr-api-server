/**
 * Cork Harbour Festival Events Scraper
 * URL: https://www.corkharbourfestival.com
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCorkHarbour(city = 'Cork') {
  console.log('⚓ Scraping Cork Harbour Festival...');
  ...
  try {
    ...
    await page.goto('https://www.corkharbourfestival.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Cork',
        category: 'Festival',
        source: 'Cork Harbour Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cork Harbour events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cork Harbour error:', error.message);
    return [];
  }
}

module.exports = scrapeCorkHarbour;
