/**
 * Secret Garden Cork Events Scraper
 * Popular venue in Cork
 * URL: https://secretgardencork.ie/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSecretGardenCork(city = 'Cork') {
  console.log('🌿 Scraping Secret Garden Cork...');
  ...
  try {
    ...
    await page.goto('https://secretgardencork.ie/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Secret Garden',
          address: '6 Tuckey St, Cork, T12 KV9Y',
          city: 'Cork'
        },
        city: 'Cork',
        category: 'Nightlife',
        source: 'Secret Garden Cork'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Secret Garden events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Secret Garden error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeSecretGardenCork;
