/**
 * The Dirty Onion Belfast Events Scraper
 * URL: https://www.thedirtyonion.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeDirtyOnion(city = 'Belfast') {
  console.log('🧅 Scraping The Dirty Onion...');
  ...
  try {
    ...
    await page.goto('https://www.thedirtyonion.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Belfast',
        category: 'Nightlife',
        source: 'The Dirty Onion'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Dirty Onion events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Dirty Onion error:', error.message);
    return [];
  }
}

module.exports = scrapeDirtyOnion;
