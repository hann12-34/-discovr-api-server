/**
 * Auntie Annies Belfast Events Scraper
 * URL: https://www.auntieannies.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAuntieAnnies(city = 'Belfast') {
  console.log('🎸 Scraping Auntie Annies...');
  ...
  try {
    ...
    await page.goto('https://www.auntieannies.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Belfast',
        category: 'Nightlife',
        source: 'Auntie Annies'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Auntie Annies events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Auntie Annies error:', error.message);
    return [];
  }
}

module.exports = scrapeAuntieAnnies;
