/**
 * Belsonic Belfast Concerts - Real event scraper
 */
...
async function scrapeBelsonic(city = 'Belfast') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: null, date: isoDate, startDate: new Date(isoDate + 'T18:00:00'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Custom House Square', address: 'Custom House Square, Belfast', city: 'Belfast' }, latitude: 54.5988, longitude: -5.9260, city: 'Belfast', category: 'Festival', source: 'Belsonic' });
    }
    console.log(`  ✅ Found ${results.length} Belsonic events`);
    return results;
  } catch (err) {
    console.error('Belsonic error:', err.message);
