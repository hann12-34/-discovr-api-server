/**
 * Thompsons Belfast Events Scraper
 * Popular nightclub in Belfast
 * URL: https://thompsonsbelfast.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeThompsonsBelfast(city = 'Belfast') {
  console.log('🎉 Scraping Thompsons Belfast...');
  ...
  try {
    ...
    await page.goto('https://thompsonsbelfast.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Thompsons',
          address: '3 Patterson Pl, Belfast BT1 4FF',
          city: 'Belfast'
        },
        city: 'Belfast',
        category: 'Nightlife',
        source: 'Thompsons Belfast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Thompsons events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Thompsons error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeThompsonsBelfast;
