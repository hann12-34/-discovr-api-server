/**
 * Voodoo Belfast Events Scraper
 * URL: https://www.voodoobelfast.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeVoodooBelfast(city = 'Belfast') {
  console.log('🎸 Scraping Voodoo Belfast...');
  ...
  try {
    ...
    await page.goto('https://www.voodoobelfast.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Voodoo',
          address: '11 Fountain St, Belfast BT1 5EA',
          city: 'Belfast'
        },
        latitude: 54.5980,
        longitude: -5.9310,
        city: 'Belfast',
        category: 'Nightlife',
        source: 'Voodoo Belfast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Voodoo Belfast events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Voodoo Belfast error:', error.message);
    return [];
  }
}

module.exports = scrapeVoodooBelfast;
