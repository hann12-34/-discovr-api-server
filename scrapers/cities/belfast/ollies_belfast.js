/**
 * Ollie's Belfast Nightclub Events Scraper
 * Popular nightclub in Belfast
 * URL: https://www.olliesbelfast.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeOlliesBelfast(city = 'Belfast') {
  console.log('🎉 Scraping Ollie\'s Belfast...');
  ...
  try {
    ...
    await page.goto('https://www.glistrr.com/events/lf/ollies', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: "Ollie's Belfast",
          address: '3 Skipper Street, Belfast BT1 2DZ',
          city: 'Belfast'
        },
        city: 'Belfast',
        category: 'Nightlife',
        source: "Ollie's Belfast"
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Ollie's Belfast events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Ollie's Belfast error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeOlliesBelfast;
