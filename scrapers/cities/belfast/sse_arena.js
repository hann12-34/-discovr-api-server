/**
 * SSE Arena Belfast Events Scraper
 * URL: https://ssearenabelfast.com/events
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeSSEArena(city = 'Belfast') {
  console.log('🏟️ Scraping SSE Arena Belfast...');
  ...
  try {
    ...
    await page.goto('https://ssearenabelfast.com/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Belfast',
        category: 'Arts',
        source: 'SSE Arena Belfast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} SSE Arena events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  SSE Arena error:', error.message);
    return [];
  }
}

module.exports = scrapeSSEArena;
