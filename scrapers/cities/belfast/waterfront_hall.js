/**
 * Waterfront Hall Belfast Events Scraper
 * URL: https://www.waterfront.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeWaterfrontHall(city = 'Belfast') {
  console.log('🌊 Scraping Waterfront Hall...');
  ...
  try {
    ...
    await page.goto('https://www.waterfront.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Belfast',
        category: 'Arts',
        source: 'Waterfront Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Waterfront Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Waterfront Hall error:', error.message);
    return [];
  }
}

module.exports = scrapeWaterfrontHall;
