...
async function scrapeCultureNightBelfast(city = 'Belfast') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Belfast City Centre', address: 'Various Belfast Venues', city: 'Belfast' }, latitude: 54.5973, longitude: -5.9301, city: 'Belfast', category: 'Festival', source: 'Culture Night' });
    }
