...
async function scrapeBotanicConcerts(city = 'Belfast') {
  ...
  try {
    ...
    for (const evt of events) {
      ...
      seenKeys.add(key);
      results.push({ id: uuidv4(), title: evt.title, description: '', date: isoDate, startDate: new Date(isoDate + 'T00:00:00.000Z'), url: evt.url, imageUrl: evt.image && evt.image.startsWith('http') ? evt.image : null, venue: { name: 'Botanic Gardens', address: 'Botanic Avenue, Belfast', city: 'Belfast' }, latitude: 54.5833, longitude: -5.9333, city: 'Belfast', category: 'Festival', source: 'Botanic Gardens' });
    }
