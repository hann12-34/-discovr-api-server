/**
 * Alibi Belfast Nightclub Events Scraper
 * Popular nightclub in Belfast
 * URL: https://www.alibibelfast.com/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeAlibiBelfast(city = 'Belfast') {
  console.log('🔥 Scraping Alibi Belfast...');
  ...
  try {
    ...
    await page.goto('https://www.alibibelfast.com/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Alibi Belfast',
          address: '30-32 Bradbury Place, Belfast BT7 1RS',
          city: 'Belfast'
        },
        city: 'Belfast',
        category: 'Nightlife',
        source: 'Alibi Belfast'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Alibi Belfast events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Alibi Belfast error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeAlibiBelfast;
