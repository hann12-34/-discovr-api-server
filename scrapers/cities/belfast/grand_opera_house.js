/**
 * Grand Opera House Belfast Events Scraper
 * URL: https://www.goh.co.uk/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeGrandOperaHouse(city = 'Belfast') {
  console.log('🎭 Scraping Grand Opera House...');
  ...
  try {
    ...
    await page.goto('https://www.goh.co.uk/whats-on', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Belfast',
        category: 'Arts',
        source: 'Grand Opera House'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Grand Opera House events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Grand Opera House error:', error.message);
    return [];
  }
}

module.exports = scrapeGrandOperaHouse;
