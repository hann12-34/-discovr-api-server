/**
 * Cargo Liverpool Events Scraper
 * Popular nightclub in Liverpool
 * URL: https://cargoliverpool.co.uk/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCargoLiverpool(city = 'Liverpool') {
  console.log('📦 Scraping Cargo Liverpool...');
  ...
  try {
    ...
    await page.goto('https://cargoliverpool.co.uk/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Cargo',
          address: '18 Fleet St, Liverpool L1 4AN',
          city: 'Liverpool'
        },
        city: 'Liverpool',
        category: 'Nightlife',
        source: 'Cargo Liverpool'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Cargo events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error(`  ⚠️ Cargo error: ${error.message}`);
    return [];
  }
}

module.exports = scrapeCargoLiverpool;
