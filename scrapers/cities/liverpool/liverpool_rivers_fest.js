/**
 * Liverpool Rivers Festival Events Scraper
 * URL: https://www.liverpoolriversfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeRiversFest(city = 'Liverpool') {
  console.log('⚓ Scraping Rivers Festival...');
  ...
  try {
    ...
    await page.goto('https://www.liverpoolriversfestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Liverpool',
        category: 'Festival',
        source: 'Rivers Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Rivers Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Rivers Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeRiversFest;
