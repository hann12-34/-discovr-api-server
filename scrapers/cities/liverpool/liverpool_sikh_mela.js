/**
 * Liverpool Sikh Mela Events Scraper
...
async function scrapeSikhMela(city = 'Liverpool') {
  console.log('🎉 Scraping Liverpool Sikh Mela...');
  ...
  try {
    ...
    const events = await page.evaluate(() => {
      ...
      document.querySelectorAll('a[href*="event"], a[href*="mela"], .event, article, [class*="event"]').forEach(el => {
        try {
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Sikh Mela events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Sikh Mela error:', error.message);
    return [];
  }
}

module.exports = scrapeSikhMela;
