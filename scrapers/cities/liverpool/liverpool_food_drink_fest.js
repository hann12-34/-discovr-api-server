/**
 * Liverpool Food & Drink Festival Events Scraper
 * URL: https://www.liverpoolfoodfestival.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeFoodDrinkFest(city = 'Liverpool') {
  ...
  try {
    ...
    await page.goto('https://www.liverpoolfoodfestival.co.uk', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Liverpool',
        category: 'Festival',
        source: 'Food & Drink Festival'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Food & Drink Festival events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Food & Drink Festival error:', error.message);
    return [];
  }
}

module.exports = scrapeFoodDrinkFest;
