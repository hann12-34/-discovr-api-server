/**
 * Liverpool Pride Events Scraper
 * URL: https://www.liverpoolpride.co.uk
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLiverpoolPride(city = 'Liverpool') {
  console.log('🏳️‍🌈 Scraping Liverpool Pride...');
  ...
  try {
    ...
    await page.goto('https://www.liverpoolpride.co.uk/events', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Liverpool',
        category: 'Festival',
        source: 'Liverpool Pride'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Liverpool Pride events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Liverpool Pride error:', error.message);
    return [];
  }
}

module.exports = scrapeLiverpoolPride;
