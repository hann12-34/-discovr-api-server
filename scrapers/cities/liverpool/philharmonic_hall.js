/**
 * Liverpool Philharmonic Hall Events Scraper
 * URL: https://liverpoolphil.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapePhilharmonicHall(city = 'Liverpool') {
  console.log('🎻 Scraping Philharmonic Hall...');
  ...
  try {
    ...
    await page.goto('https://liverpoolphil.com/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.concert-item', '.event', 'article', '[class*="event"]', '[class*="concert"]', 'a[href*="/whats-on/"]'];
      let eventElements = [];
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          ...
          address: 'Hope St, Liverpool L1 9BP',
          city: 'Liverpool'
        },
        latitude: 53.3980,
        longitude: -2.9720,
        city: 'Liverpool',
        category: 'Arts',
        source: 'Philharmonic Hall'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Philharmonic Hall events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Philharmonic Hall error:', error.message);
    return [];
  }
}

module.exports = scrapePhilharmonicHall;
