/**
 * Invisible Wind Factory Liverpool Events Scraper
 * Warehouse venue for electronic music
 * URL: https://www.intothenightlife.co.uk/invisible-wind-factory
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeInvisibleWind(city = 'Liverpool') {
  console.log('💨 Scraping Invisible Wind Factory...');
  ...
  try {
    ...
    await page.goto('https://www.intothenightlife.co.uk/invisible-wind-factory', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        city: 'Liverpool',
        category: 'Nightlife',
        source: 'Invisible Wind Factory'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} valid Invisible Wind events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Invisible Wind error:', error.message);
    return [];
  }
}

module.exports = scrapeInvisibleWind;
