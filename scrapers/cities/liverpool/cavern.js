/**
 * The Cavern Club Liverpool Events Scraper
 * Legendary music venue - birthplace of The Beatles
 * URL: https://www.cavernclub.com/whats-on/
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeCavernClub(city = 'Liverpool') {
  console.log('🎸 Scraping The Cavern Club Liverpool...');
  ...
  try {
    ...
    await page.goto('https://www.cavernclub.com/whats-on/', {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        id: uuidv4(),
        title: event.title,
        description: event.description || `Live at The Cavern Club Liverpool`,
        date: isoDate,
    ...
    console.log(`  ✅ Found ${formattedEvents.length} valid Cavern Club events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Cavern Club error:', error.message);
    return [];
  }
}

module.exports = scrapeCavernClub;
