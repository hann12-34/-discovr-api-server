/**
 * Eventim Olympia Liverpool Events Scraper
 * URL: https://olympialiverpool.com/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeEventimOlympia(city = 'Liverpool') {
  console.log('🎪 Scraping Eventim Olympia...');
  ...
  try {
    ...
    for (const event of events) {
      ...
      formattedEvents.push({
        ...
        venue: {
          name: 'Eventim Olympia',
          address: 'West Derby Rd, Liverpool L6 4BX',
          city: 'Liverpool'
        },
        latitude: 53.4230,
        longitude: -2.9650,
        city: 'Liverpool',
        category: 'Arts',
        source: 'Eventim Olympia'
      });
    }

    console.log(`  ✅ Found ${formattedEvents.length} Eventim Olympia events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Eventim Olympia error:', error.message);
    return [];
  }
}

module.exports = scrapeEventimOlympia;
