/**
 * Liverpool Empire Theatre Events Scraper
 * URL: https://www.atgtickets.com/venues/liverpool-empire-theatre/whats-on
 */

const puppeteer = require('puppeteer');
const { v4: uuidv4 } = require('uuid');

async function scrapeLiverpoolEmpire(city = 'Liverpool') {
  console.log('🎭 Scraping Liverpool Empire...');
  ...
  try {
    ...
    await page.goto('https://www.atgtickets.com/venues/liverpool-empire-theatre/whats-on', {
    ...
    const events = await page.evaluate(() => {
      ...
      const eventSelectors = ['.show-card', '.event', 'article', '[class*="show"]', '[class*="event"]', 'a[href*="/show"]'];
      let eventElements = [];
    ...
    console.log(`  ✅ Found ${formattedEvents.length} Liverpool Empire events`);
    return formattedEvents;

  } catch (error) {
    if (browser) await browser.close();
    console.error('  ⚠️  Liverpool Empire error:', error.message);
    return [];
  }
}

module.exports = scrapeLiverpoolEmpire;
