...
/**
 * Dies Onze Events Scraper
 * Scrapes events from Dies Onze in Montreal
 */
class DiesOnzeEvents {
    constructor() {
        this.name = 'Dies Onze';
        this.eventsUrl = 'http://www.dieseonze.com';
        this.source = 'dies-onze';
        this.city = 'Montreal';
    ...
    async scrapeEvents() {
        try {
            ...
            $('.event, .show, .concert, .party, article, .card, .listing, .event-item, .restaurant-event, .bar-event, .spectacle, .evenement, [data-event], .wp-block-group, .entry, .post-item, .calendar-event, h1, h2, h3, h4, .wp-block-heading, .event-card, p').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Dies Onze`,
...
async function scrapeEvents() {
  const scraper = new DiesOnzeEvents();
