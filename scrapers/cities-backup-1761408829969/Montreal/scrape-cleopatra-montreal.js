 ...
 */
class CleopatraMontrealEvents {
    constructor() {
        this.name = 'Cleopatra Montreal';
        this.eventsUrl = 'https://www.cleopatramontreal.com';
        this.source = 'cleopatra-montreal';
        this.city = 'Montreal';
            ...
            $('.event, .event-item, .event-card, .show, .party, .listing, .card, article').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Cleopatra Montreal`,
                        date: this.parseDate(dateText) || new Date(),
                        venue: {
                            name: 'Cleopatra Montreal',
                            address: '1426 Rue Stanley, Montreal, QC H3A 1P7',
                            city: this.city,
...
async function scrapeEvents() {
  const scraper = new CleopatraMontrealEvents();
