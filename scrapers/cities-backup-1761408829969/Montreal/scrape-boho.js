...
/**
 * Boho Events Scraper
 * Scrapes events from Boho in Montreal
 */
class BohoEvents {
    constructor() {
        this.name = 'Boho';
        this.eventsUrl = 'https://bohomanor.com/bohoclub/';
        this.source = 'boho';
        this.city = 'Montreal';
            ...
            $('.event, .event-item, .event-card, .show, .party, .listing, .card').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Boho`,
