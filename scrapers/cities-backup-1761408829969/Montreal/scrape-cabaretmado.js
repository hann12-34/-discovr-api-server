...
/**
 * Cabaret Mado Events Scraper
 * Scrapes events from Cabaret Mado in Montreal
 */
class CabaretMadoEvents {
    constructor() {
        this.name = 'Cabaret Mado';
        this.eventsUrl = 'https://cabaret-mado.ca/spectacles';
        this.source = 'cabaret-mado';
        this.city = 'Montreal';
            ...
            $('.event, .event-item, .event-card, .show, .party, .listing, .card').each((index, element) => {
                try {
                    ...
                    const eventData = {
                        ...
                        name: title,
                        title: title,
                        description: description && description.length > 20 ? description : `${title} - ${title} at Cabaret Mado`,
                        date: this.parseDate(dateText) || new Date(),
                        venue: {
                            name: 'Cabaret Mado',
                            address: '115 Boulevard Saint-Laurent, Montreal, QC H2X 2S6',
                            city: this.city,
...
async function scrapeEvents() {
  const scraper = new CabaretMadoEvents();
