...
class BoweryBallroom {
    ...
    async fetchEvents() {
        try {
            ...
            // Parse Bowery Ballroom events using event links extraction (from breakthrough analysis)
            // Target real concert events from event-related links
            $('a[href*="event"], a[href*="show"], a[href*="concert"]').each((index, element) => {
                try {
                    const $el = $(element);
                    const href = $el.attr('href');
                    let title = $el.text().trim();
                    
                    console.log(`🔍 Processing Bowery Ballroom event link[${index}]: "${title}"`);
                    ...
                    // Skip obvious non-events based on analysis findings
                    const skipTerms = [
                        'buy tickets', 'private events', 'newsletter', 'search', 'menu', 
                        'navigation', 'footer', 'header', 'filter', 'today\'s shows'
                    ];
                    ...
                    // Extract real artist/event names (based on analysis findings)
                    // Real events typically have proper capitalization and artist/band names
                    const looksLikeArtist = title.match(/^[A-Z][a-z]+/) || // Starts with capitalized word
                                           title.includes('Tour') || 
                                           title.includes('–') || 
                                           title.includes(':') ||
                                           title.match(/\b(tribute|night|live|concert|show)\b/i);
                    
                    if (!looksLikeArtist) {
                        console.log(`   ❌ Skipped: doesn't appear to be artist/event name`);
                        return;
                    }
                    
                    // Extract date from the URL or ticketmaster link patterns
                    let dateText = '';
                    
                    if (href) {
                        // Extract date from Ticketmaster URLs (format: YYYY-MM-DD)
                        const dateMatch = href.match(/(\d{2})-(\d{2})-(\d{4})/);
                        if (dateMatch) {
                            const [_, month, day, year] = dateMatch;
                            dateText = `${month}/${day}/${year}`;
                        } else {
                            // Try other date patterns in URL
                            const altDateMatch = href.match(/(\d{4})-(\d{2})-(\d{2})/);
                            if (altDateMatch) {
                                const [_, year, month, day] = altDateMatch;
                                dateText = `${month}/${day}/${year}`;
                            }
                        }
                    }
                    
                    // If no date found in URL, use placeholder
                    if (!dateText) {
                        console.log(`   📅 No date found in URL, using placeholder`);
                        dateText = 'Date TBD';
                    } else {
                        console.log(`   📅 Date extracted from URL: "${dateText}"`);
                    }
                    ...
                    const event = {
                        ...
                        startDate: startDate,
                        endDate: null,
                        venue: { name: this.venueName, address: '6 Delancey Street, New York, NY 10002', city: 'New York' },
                        city: this.city,
                        state: this.state,
...
async function scrapeBoweryBallroom() {
    const scraper = new BoweryBallroom();
    return await scraper.scrape();
}

module.exports = scrapeBoweryBallroom;
