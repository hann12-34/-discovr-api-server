...
const EVENTS_URL = 'https://gardinermuseum.on.ca';
const VENUE_NAME = 'Gardiner Museum';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🏺 Scraping Gardiner Museum events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .exhibition, .workshop, .program').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Ceramic arts and cultural events at the Gardiner Museum, Toronto\'s premier ceramics museum.';
            ...
            const event = {
                ...
                venue: {
                    name: VENUE_NAME,
                    address: '111 Queen\'s Park, Toronto, ON, Canada'
                },
                location: 'Toronto, ON, Canada',
                city: 'Toronto',
                categories: extractCategories(title, description, ['museums', 'ceramics', 'art', 'exhibitions']),
                price: price,
                tags: ['museums', 'ceramics', 'art', 'exhibitions', 'cultural', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Gardiner Museum`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Gardiner Museum events: ${error.message}`);
        return [];
    }
}
