...
const EVENTS_URL = 'https://babygtoronto.com';
const VENUE_NAME = 'Baby G';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`👶 Scraping Baby G events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .party').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience Baby G, Toronto\'s trendy restaurant and bar with vibrant nightlife.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['dining', 'bars', 'nightlife', 'restaurant']),
                price: price,
                tags: ['dining', 'bars', 'nightlife', 'restaurant', 'trendy', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Baby G`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Baby G events: ${error.message}`);
