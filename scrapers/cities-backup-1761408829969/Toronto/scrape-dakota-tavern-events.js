...
const EVENTS_URL = 'https://thedakotatavern.com';
const VENUE_NAME = 'Dakota Tavern';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🤠 Scraping Dakota Tavern events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Country, folk, and roots music at Dakota Tavern, Toronto\'s authentic honky-tonk venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['country', 'folk', 'live music', 'bars']),
                price: price,
                tags: ['country', 'folk', 'live music', 'bars', 'honky-tonk', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Dakota Tavern`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Dakota Tavern events: ${error.message}`);
        return [];
    }
}
