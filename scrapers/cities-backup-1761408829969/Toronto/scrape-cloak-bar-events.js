...
const EVENTS_URL = 'https://cloakbar.com';
const VENUE_NAME = 'The Cloak Bar';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎭 Scraping The Cloak Bar events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Exclusive cocktail experiences at The Cloak Bar, Toronto\'s mysterious speakeasy-style venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['cocktails', 'speakeasy', 'bars', 'nightlife']),
                price: price,
                tags: ['cocktails', 'speakeasy', 'bars', 'nightlife', 'exclusive', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Cloak Bar`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Cloak Bar events: ${error.message}`);
        return [];
    }
}
