...
const EVENTS_URL = 'https://rivoli.ca';
const VENUE_NAME = 'Rivoli';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎭 Scraping Rivoli events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live music, comedy, and events at Rivoli, Toronto\'s legendary Queen Street entertainment venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'comedy', 'entertainment', 'bars']),
                price: price,
                tags: ['live music', 'comedy', 'entertainment', 'bars', 'queen street', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Rivoli`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Rivoli events: ${error.message}`);
        return [];
    }
}
