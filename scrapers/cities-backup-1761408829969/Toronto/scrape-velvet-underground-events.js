...
const EVENTS_URL = 'https://thevelvet.ca';
const VENUE_NAME = 'The Velvet Underground';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎸 Scraping The Velvet Underground events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live music and underground performances at The Velvet Underground, Toronto\'s iconic alternative venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['live music', 'alternative', 'underground', 'concerts']),
                price: price,
                tags: ['live music', 'alternative', 'underground', 'concerts', 'indie', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Velvet Underground`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Velvet Underground events: ${error.message}`);
