...
const EVENTS_URL = 'https://therecroom.com';
const VENUE_NAME = 'The Rec Room';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎮 Scraping The Rec Room events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .gaming').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Gaming, dining, and entertainment at The Rec Room, Toronto\'s ultimate entertainment destination.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['gaming', 'entertainment', 'dining', 'arcade']),
                price: price,
                tags: ['gaming', 'entertainment', 'dining', 'arcade', 'family', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Rec Room`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Rec Room events: ${error.message}`);
        return [];
    }
}
