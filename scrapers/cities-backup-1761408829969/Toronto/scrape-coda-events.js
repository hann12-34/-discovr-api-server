...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎧 Scraping Coda events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Electronic dance music events and nightlife at Coda, Toronto\'s premier electronic music venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['electronic', 'edm', 'nightlife', 'dance']),
                price: price,
                tags: ['electronic', 'edm', 'nightlife', 'dance', 'club', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Coda`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Coda events: ${error.message}`);
        return [];
    }
}
