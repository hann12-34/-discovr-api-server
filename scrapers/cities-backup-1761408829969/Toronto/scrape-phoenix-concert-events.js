...
const EVENTS_URL = 'https://phoenixconcerttheatre.com';
const VENUE_NAME = 'Phoenix Concert Theatre';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live concerts and music performances at Phoenix Concert Theatre, Toronto\'s legendary music venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['concerts', 'live music', 'rock', 'alternative']),
                price: price,
                tags: ['concerts', 'live music', 'rock', 'alternative', 'legendary', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Phoenix Concert Theatre`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Phoenix Concert Theatre events: ${error.message}`);
        return [];
    }
}
