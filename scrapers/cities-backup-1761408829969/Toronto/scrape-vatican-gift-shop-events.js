...
const EVENTS_URL = 'https://vaticanshop.ca';
const VENUE_NAME = 'Vatican Gift Shop';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`⛪ Scraping Vatican Gift Shop events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .news-item').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience Vatican Gift Shop - Toronto\'s unique themed cocktail bar.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['cocktails', 'bars', 'nightlife', 'themed']),
                price: price,
                tags: ['cocktails', 'bar', 'nightlife', 'themed', 'unique', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Vatican Gift Shop`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Vatican Gift Shop events: ${error.message}`);
