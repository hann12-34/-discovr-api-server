...
const EVENTS_URL = 'https://thepaintedlady.ca';
const VENUE_NAME = 'The Painted Lady';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🎨 Scraping The Painted Lady events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .party').each((index, element) => {
            const $event = $(element);
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Experience The Painted Lady, Toronto\'s vibrant bar and entertainment venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['bars', 'entertainment', 'nightlife', 'art']),
                price: price,
                tags: ['bars', 'entertainment', 'nightlife', 'art', 'vibrant', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Painted Lady`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Painted Lady events: ${error.message}`);
        return [];
    }
}
