...
const EVENTS_URL = 'https://revivalbar.ca';
const VENUE_NAME = 'Revival Bar';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🍸 Scraping Revival Bar events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .party, .night').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Cocktails, events, and nightlife at Revival Bar, Toronto\'s stylish cocktail destination.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['cocktails', 'bars', 'nightlife', 'events']),
                price: price,
                tags: ['cocktails', 'bars', 'nightlife', 'events', 'stylish', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from Revival Bar`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping Revival Bar events: ${error.message}`);
        return [];
    }
}
