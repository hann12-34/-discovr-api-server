...
const EVENTS_URL = 'https://thephoenixconcerttheatre.com';
const VENUE_NAME = 'The Phoenix Concert Theatre';
...
async function scrapeEvents(city = 'Toronto') {
    ...
    console.log(`🔥 Scraping The Phoenix Concert Theatre events for ${city}...`);
    ...
    try {
        ...
        $('.event, .event-item, .events-item, [class*="event"], article, .post, .show, .concert').each((index, element) => {
            ...
            const description = $event.find('p, .description, .summary, .excerpt, .content').first().text().trim() || 'Live concerts and music events at The Phoenix Concert Theatre, Toronto\'s premier concert venue.';
            ...
            const event = {
                ...
                city: 'Toronto',
                categories: extractCategories(title, description, ['concerts', 'live music', 'rock', 'indie']),
                price: price,
                tags: ['concerts', 'live music', 'rock', 'indie', 'venue', 'toronto'],
                url: eventUrl,
                source: EVENTS_URL,
                image: image || '',
        ...
        console.log(`✅ Scraped ${events.length} events from The Phoenix Concert Theatre`);
        return filterEvents(events);
        
    } catch (error) {
        console.log(`❌ Error scraping The Phoenix Concert Theatre events: ${error.message}`);
        return [];
    }
}
