/**
 * Calgary Event Scrapers Master Import Script
 * 
 * This script automatically detects and runs all available Calgary event scrapers
 * and imports their events into the MongoDB database.
 * 
 * Following the official workflow for the Discovr API system
 * 
 * Usage: node import-all-calgary-events.js
 */
    ...
    for (const event of events) {
      try {
        ...
        const normalizedEvent = {
          ...
          city: event.city || 'Calgary',
          province: event.province || 'Alberta',
...
async function importAllEvents() {
  ...
  console.log('🚀 Starting Calgary events import process...'.cyan.bold);
  ...
  try {
    ...
    // Get Calgary scrapers directory
    const scrapersDir = path.join(__dirname, '..', 'scrapers', 'cities', 'Calgary');
    
    if (!fs.existsSync(scrapersDir)) {
      throw new Error(`Calgary scrapers directory not found: ${scrapersDir}`);
    }
    ...
    console.log(`\n🔍 Found ${scraperFiles.length} valid Calgary scrapers`.cyan);
    ...
    const calgaryEvents = await Event.countDocuments({ city: 'Calgary' });
    
    // Print summary
    console.log('\n' + '='.repeat(60));
    console.log('📊 CALGARY EVENTS IMPORT SUMMARY'.cyan.bold);
    console.log('='.repeat(60));
    console.log(`🎯 Total scrapers processed: ${scraperFiles.length}`.white);
    console.log(`✅ Total events added: ${totalAdded}`.green);
    console.log(`📈 Total Calgary events in database: ${calgaryEvents}`.blue);
    ...
    console.log('\n🎉 Calgary events import completed successfully!'.green.bold);
